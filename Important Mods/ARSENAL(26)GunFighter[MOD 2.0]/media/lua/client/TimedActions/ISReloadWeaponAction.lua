...
ISReloadWeaponAction.ReloadBestMagazine = function(playerObj, gun)
	...
	if not magazine then
		-------------------------------------------------------------------------
		-- CHECK AUTO MAG-TYPE OPTION - DONT INSERT EMPTY
		-------------------------------------------------------------------------
	...
	-------------------------------------------------------------------------
	-- CHECK AUTO MAG-TYPE OPTION - DONT TOP UP EMPTY
	-------------------------------------------------------------------------
	if GUNFIGHTER.OPTIONS.options.dropdown132 <= 1 then
...
-------------------------------------------------------------------------
		if 	(magazine ~= nil) and (gun:getMagazineType() == Clip) and (gun:isContainsClip() or Mag == nil) and (not playerObj:isRunning()) then
--	DEBUG		playerObj:addLineChatElement("Using Clip...")
			gun:setContainsClip(false)					-- MUST BE REMOVED ??
			gun:getModData().isStripping = true				-- WORKAROUND FOR CANCELLED LOST MAG
...
-------------------------------------------------------------------------
		elseif	(gun:isContainsClip()) and gun:getMagazineType() ~= Clip then	-- NOT EMPTY SO REMOVE THEN INSERT
			...
			------------------------------------------------------------------
			--	STOP FOR AUTO MAG-TYPE OPTION by Arsenal[26]
			------------------------------------------------------------------
	--		if (GUNFIGHTER.OPTIONS.options.dropdown132 > 1) then
	--			return
	--		end
...
-------------------------------------------------------------------------
		if 	gun:getMagazineType() and gun:getMagazineType() ~= Clip then
			...
			if magazine then
				...
				if magazine then
					playerObj:addLineChatElement(getText("ContextMenu_ALTLoad_Empty"))
					ISInventoryPaneContextMenu.transferIfNeeded(playerObj, magazine)
					ISTimedActionQueue.add(ISInsertMagazine:new(playerObj, gun, magazine))
					return
				end
...
ISReloadWeaponAction.attackHook = function(character, chargeDelta, weapon)
	...
	if weapon:isRanged() and not character:isDoShove() then
		...
		------------------------------------------------------------------
			...
			----------------------------------
			-- A26 REMOVE SOUND RADIUS NERF --
			-- AND PREVENT MUZZLE FLASH BY  --
			-- FOR CERTAIN CASES		  --
			----------------------------------
		...
		--	end
			...
			----------------------------------
			-- A26 CONTROL MUZZLE FLASH	  --
			----------------------------------
			if (not isSuppressed(weapon)) and (not isBow(weapon)) and (not isBBGun(weapon)) and (not isFlamer(weapon)) then
...
ISReloadWeaponAction.onShoot = function(player, weapon)
	...
	if weapon:isRackAfterShoot() then
		--------------------------------------------------
		--	ARSENAL[26]									--
		if weapon:getWeaponReloadType() == "boltaction" then
			player:getModData().DelayAction = 60
		else	player:getModData().DelayAction = 60
		end
