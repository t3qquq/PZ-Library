...
local width = math.floor(getTextManager():getFontHeight(UIFont.Small) / 2 + 3)
local spacing = getCore():getOptionFontSizeReal()

local r, g, b, a = 0.274, 0.51, 1, 0.5
...
function ISSkillProgressBar:renderPerkRect()
	...
	local x = self.level * (width + spacing)
	local y = 0
	...
	self:drawTextureScaled(self.UnlockedSkill, x + math.ceil(width * (self.xp / self.xpForLvl)), y, width * missed_exp_progress, width, a, r, g, b)
	...
	for level = self.level + 2, 10 do
		...
		self:drawTextureScaled(self.UnlockedSkill, x, y, width * missed_exp_progress, width, a, r, g, b)
