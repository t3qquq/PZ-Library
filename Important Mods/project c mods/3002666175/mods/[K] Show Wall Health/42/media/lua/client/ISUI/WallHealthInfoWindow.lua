...
function Kamer_WallHealthInfoWindow:setObject(object)
	...
	local objName = object:getName() or object:getProperties():get("CustomName")
	if objName then
		local prepareName = "ContextMenu_" .. string.gsub(objName, " ", "_")
		objName = getText(prepareName)
		if objName == prepareName then
			objName = object:getProperties():get("CustomName") or object:getName() or "Unknown Object (Unnamed)"
		end
	else
