...
local SpawnCFG = {}
--print("WQSDEBUG: Loading WQS_ExtractionEvent.lua")
...
function StartStorm(h_duration)
    ...
    if isClient() then --multiplayer
        ...
        print("WQS Starting STORM request from a client stage=" .. tostring(args["stage"]) .. " hours=" ..
            args["duration"])
        WQS_Shared.SendClientCommand(pl, "WQS_ServerSideModule", "WQS_DoStartStorm", args)
        return
    else
...
local function FindSpawnLocation()
    ...
    if (use_preferred_spawn_loc) and not (PreferredSpawnPointsList[mapcurrent] == nil) then
        ...
        if PrefSpawn.x then
            local p = cPlayer
            if not (PrefSpawn.z == p:getZ()) then --se lo spawn preferito è su un piano diverso lo ritorno direttamente
                print("##### PreferredSpawnPoint on another level (" ..
                    WQS.getMapPreferredSpawnPointPerc() ..
                    "%) -> " .. PrefSpawn.x .. "," .. PrefSpawn.y .. " z=" .. PrefSpawn.z)
                return PrefSpawn
            end
            --controllo che il player non veda lo spawn
    ...
    if (#failsafe_spawn_loc > 15) then
        -- Returning a failsafe location directly because failsafe_spawn_loc already has many elements.
        local rr = ZombRand(#failsafe_spawn_loc) + 1
        local sp = failsafe_spawn_loc[rr]
        if sp.x then
            print("Picking from failsafe that have already " ..
                tostring(#failsafe_spawn_loc) ..
                " elements x: " .. tostring(sp.x) .. " y: " .. tostring(sp.y) .. " z:" .. sp.z)
            return sp
        end
    end
    ...
    SpawnCFG.spawn_min_dist_from_player = (SpawnCFG.spawn_min_dist_from_player - (math.floor(pLocation:getZ() * 1.8))) or
        12

    if SpawnCFG.spawn_min_dist_from_player < 12 then
        SpawnCFG.spawn_min_dist_from_player = 12
    end
...
local function SpawnZed(x, y, z, howmany, is_smart)
    ...
    if x then
        if isClient() then
            ...
            WQS_Shared.SendClientCommand(pl, "WQS_ServerSideModule", "WQS_DoServerSpawn", args)
            return
        else
            ...
            -- if zed ~= nil and zed.getClass then
            --     local className = zed:getClass():getName() -- Ottieni la classe dell'oggetto
            --     print("zed class name: " .. className)

            --     if string.find(className, "ArrayList") then
            --         print("zed è un ArrayList o una sua sottoclasse.")
            --         local zombie = zed:get(0)
            --         if zombie ~= nil and type(zombie.DoZombieStats) == "function" then
            --             zombie:DoZombieStats()
            --             zombie:makeInactive(true)
            --             zombie:makeInactive(false)
            --         else
            --             print("Error: zed:get(0) invalid or DoZombieStats method not foud")
            --         end
            --     else
            --         print("zed non è un ArrayList.")
            --     end
...
local function SpawnManager()
    ...
    local pl = WQS.GetCurrentPlayer()
    local zed = nil
    local currentStage = GetCurrentExtractionStage()
    ...
    if (already_spawned < SpawnCFG.total_to_spawn) then
        ...
        if not (WQS_Shared.TableIsEmptyOrNil(spawn_location)) and not (spawn_location.x == nil) then
            local zxspawn = 2
            --se currentStage=2 il 50% delle volte zxspawn=3
            --se currentStage=3 il 100% delle volte zxspawn=3
            if (currentStage == 3) or (currentStage == 2 and ZombRand(2) == 0) then
                zxspawn = 3
            end


            SpawnZed(spawn_location.x, spawn_location.y, spawn_location.z, zxspawn, true)
            already_spawned = already_spawned + zxspawn --era +1
            print("---Stage:" .. currentStage .. " - Spawned: (" ..
                zxspawn .. ") " .. already_spawned * 1 .. "/" .. SpawnCFG.total_to_spawn * 1 ..
        ...
        local maxAdditionalZeds = WQS_EXTRACTION_DIFFICULTY_MULTIPLIER / 2
        local additionalDiffProb = math.ceil(WQS_EXTRACTION_DIFFICULTY_MULTIPLIER * 3)

        if WQS_EXTRACTION_DIFFICULTY_MULTIPLIER >= 2 then
            maxAdditionalZeds = math.floor((WQS_EXTRACTION_DIFFICULTY_MULTIPLIER / 1.4) + 0.5)
            additionalDiffProb = math.ceil(WQS_EXTRACTION_DIFFICULTY_MULTIPLIER * 5)
        end

        local additional_prob = (currentStage * 10) + additionalDiffProb
        local total_prob = math.min(baseprob + additional_prob, 98) -- Limita il valore massimo a 98c

        if WQS_Shared.RandomPerc(total_prob) then
            ...
            if not (WQS_Shared.TableIsEmptyOrNil(additional_spawn_loc)) and not (additional_spawn_loc.x == nil) then
                ...
                local nz = 1 + ZombRand(math.floor(currentStage + maxAdditionalZeds))
...
function WQS_PlaySound(soundnamez)
    if isClient() then --multiplayer
        ...
        WQS_Shared.SendClientCommand(pl, "WQS_ServerSideModule", "WQS_PlaySound", args)
    else
...
function WQS_OnClientCommand(WQS_module, WQS_command, WQS_player, WQS_args)
    -- print("WQSDEBUG: WQS Environment - isClient()=" ..
    --     tostring(isClient()) ..
    --     " isServer()=" ..
    --     tostring(isServer()) ..
    --     " module: " .. tostring(WQS_module) .. " command: " .. tostring(WQS_command) .. " args: " .. tostring(WQS_args))

    -- This function should only run on the server side OR in singleplayer
    -- In singleplayer: isClient() = false, isServer() = false
    -- In multiplayer server: isClient() = false, isServer() = true
    -- In multiplayer client: isClient() = true, isServer() = false
    if isClient() then
        print("*** WQS OnClientCommand - Running on multiplayer CLIENT, skipping (server will handle)")
        return
    end

    --print("*** WQS OnClientCommand - Processing command (server or singleplayer)")

    if WQS_module ~= "WQS_ServerSideModule" then
        --print("*** WQS OnClientCommand - Module is not WQS_ServerSideModule, skipping")
        return
    end
    ...
    -- Handler for adding extraction map to player inventory
    -- Called from WQS_Recipe.DrawExtractionMap
    if WQS_command == "WQS_AddExtractionMap" then
        local inv = WQS_player:getInventory()
        local extraction_map = instanceItem(WQS_args["mapId"])
        if extraction_map then
            extraction_map:setName(WQS_args["mapLabel"] .. " " .. getText("IGUI_WQS_ExtrMapLabel"))
            inv:AddItem(extraction_map)
            sendAddItemToContainer(inv, extraction_map)
            print("*** WQS Server added extraction map: " ..
                WQS_args["mapId"] .. " to player " .. WQS_player:getDisplayName())
        else
            print("*** WQS ERROR: Failed to create extraction map for player " .. WQS_player:getDisplayName())
        end
    end

    -- Handler for adding repeater map to player inventory
    -- Called from WQS_Recipe.GenerateTargetRepeater
    -- if WQS_command == "WQS_AddRepeaterMap" then
    --     local inv = WQS_player:getInventory()
    --     local repeater_map = instanceItem(WQS_args["mapId"])
    --     if repeater_map then
    --         inv:AddItem(repeater_map)
    --         sendAddItemToContainer(inv, repeater_map)
    --         print("*** WQS Server added repeater map: " ..
    --             WQS_args["mapId"] .. " to player " .. WQS_player:getDisplayName())
    --     else
    --         print("*** WQS ERROR: Failed to create repeater map for player " .. WQS_player:getDisplayName())
    --     end
    -- end



    -- Handler for giving first quest map
    -- Called from WQS.GiveFirstQuest
    if WQS_command == "WQS_GiveFirstQuest" then
        print("### WQS GiveFirstQuest handler started")
        --print("### WQS GiveFirstQuest mapId=" .. tostring(WQS_args["mapId"]))

        local inv = WQS_player:getInventory()
        --print("### WQS GiveFirstQuest inv=" .. tostring(inv))

        -- Use instanceItem to create the item first (Build 42 compatible)
        -- This creates the item without adding it to inventory yet
        local extraction_map = instanceItem(WQS_args["mapId"])

        --print("### WQS GiveFirstQuest instanceItem result=" .. tostring(extraction_map))

        if extraction_map then
            -- Now add the created item to the inventory
            inv:AddItem(extraction_map)
            -- Synchronize the item to all clients
            sendAddItemToContainer(inv, extraction_map)
            print("*** WQS Server gave first quest map " ..
                WQS_args["mapId"] .. " to player " .. WQS_player:getDisplayName())
        else
            print("*** WQS ERROR: Failed to create first quest map " ..
                WQS_args["mapId"] .. " for player " .. WQS_player:getDisplayName())
            print("*** WQS ERROR: instanceItem returned nil - item script not found or invalid")
        end
    end

    -- Handler for adding start item to fresh spawn
    -- Called from WQS.OnStart (fresh spawn)
    if WQS_command == "WQS_AddStartItem" then
        local inv = WQS_player:getInventory()
        local start_item = instanceItem(WQS_args["itemId"])

        if start_item then
            inv:AddItem(start_item)
            sendAddItemToContainer(inv, start_item)
            print("*** WQS Server added start item: " ..
                WQS_args["itemId"] .. " to player " .. WQS_player:getDisplayName())
        else
            print("*** WQS ERROR: Failed to create start item for player " .. WQS_player:getDisplayName())
        end
    end
end

Events.OnClientCommand.Remove(WQS_OnClientCommand)
print("WQSDEBUG: Registering WQS_OnClientCommand")
