    ...
    for pkey, pdata in pairs(WQS_ExtractionPointsData) do
        ...
        if pdata and pdata.MapCenterAreaX then
            for rkey, repeater in pairs(WQS_RepeaterData) do
                local dist = WQS_Shared.getDistance(repeater.x, repeater.y, pdata.MapCenterAreaX, pdata.MapCenterAreaY)
                if dist < 130 then
    ...
    if OtherRepeaterList and not (WQS_Shared.TableIsEmptyOrNil(OtherRepeaterList)) then
        ...
        while not (isOk) do
            ...
            for key, rep in pairs(OtherRepeaterList) do
                local dist = WQS_Shared.getDistance(pick.x, pick.y, rep.x, rep.y)
                --print("dist "..dist)
...
--     local dist = WQS_Shared.getDistance(x,y,Rep.x,Rep.y);
--     --print(dist)
...
WQSAntenna.AdminTeleportToTargetRepeater = function(TargetRepArrayPos)
    ...
    if TargetRepeaterList and not (WQS_Shared.TableIsEmptyOrNil(TargetRepeaterList)) then
        ...
        if Rep then
            ...
            print("Teleport -> Rep.x: " .. Rep.x .. ", Rep.y: " .. Rep.y)
            playerObj:setX(Rep.x)
            playerObj:setY(Rep.y)
            playerObj:setZ(0.0)
            playerObj:setLastX(Rep.x) -- b42
            playerObj:setLastY(Rep.y) -- b42
        end
    end
end
...
WQSAntenna.getTargetRepeatersStatsTxt = function()
    ...
    if not (TargetRepeaterList) or WQS_Shared.TableIsEmptyOrNil(TargetRepeaterList) then
        ...
        for key, Rep in pairs(TargetRepeaterList) do
            ...
            local dist = WQS_Shared.getDistance(pl:getX(), pl:getY(), Rep.x, Rep.y);
            ...
            if dist >= WQS_Shared.getRepeaterMaxActivationDistance() then
                distTxt = WQS_COLSUBTIT .. dist .. "m " .. dir .. WQS_COLWHITE
...
WQSAntenna.CanAddMoreTargetRepeaters = function(player)
    print("WQSAntenna.CanAddMoreTargetRepeaters player=" .. tostring(player))
    local TargetRepeaterList = WQS_Shared.getWQSPlayerModData("TargetRepeaterList", player)
    if not TargetRepeaterList then
        TargetRepeaterList = {}
    end
...
WQSAntenna.GenerateTargetRepeaterList = function(ForceAdd, EraseExistingRepeaters, NumTargetRepeaters, player)
    ...
    if EraseExistingRepeaters then
        WQS_Shared.deleteWQSPlayerModData("TargetRepeaterList", player)
    end
    ...
    if not (TargetRepeaterList) or WQS_Shared.TableIsEmptyOrNil(TargetRepeaterList) or (ForceAdd) then
        for i = 1, NumTargetRepeaters, 1 do
            TargetRepeaterList = WQS_Shared.getWQSPlayerModData("TargetRepeaterList", player)
            if not TargetRepeaterList then
                TargetRepeaterList = {}
            end
            ...
            if WQSAntenna.CanAddMoreTargetRepeaters(player) then
                ...
                WQS_Shared.setWQSPlayerModData("TargetRepeaterList", TargetRepeaterList, player)
