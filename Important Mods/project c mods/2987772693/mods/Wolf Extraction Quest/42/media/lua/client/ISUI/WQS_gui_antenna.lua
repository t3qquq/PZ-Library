...
function WQS_AntennaGui:OnInfoListMouseDown(item)
    ...
    if (item and MapData and not (WQS_Shared.TableIsEmptyOrNil(MapData))) then
        ...
        local distance = -1
        if pl then
            distance = WQS_Shared.getDistance(pl:getX(), pl:getY(), MapData.MapCenterAreaX, MapData.MapCenterAreaY);
        end
...
function WQS_AntennaGui:populateInfoList(_name)
    ...
    elseif _name == "RepeaterMode" then
        ...
        local ActiveRepeaterListCount = WQS_Shared.CountTableItems(ActiveRepeaterList) or 0

        local rnep = getText("IGUI_WQS_AntennaRepeaterMode")
        local repLabel = getText("IGUI_WQS_Zone") or "Zone"
        local repActiveLabelDef = getText("IGUI_WQS_AntennaRepeaterActive") or "Repeater Currently Active"

        local repActiveLabel = repActiveLabelDef .. ": " .. tostring(ActiveRepeaterListCount)
