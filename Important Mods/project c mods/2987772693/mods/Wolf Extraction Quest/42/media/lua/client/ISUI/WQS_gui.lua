...
local function WQS_GPSWindowUpdate()
	...
	--quest WEQA hook setQuestTextOnGuiUpdate
	if WQS.isActiveAndStartedAdventureSystem() then
		...
		if currentActiveCustomQuest and type(currentActiveCustomQuest.setQuestTextOnGuiUpdate) == "function" then
			GuiTxt = currentActiveCustomQuest:setQuestTextOnGuiUpdate()
			WQS_GPSWindow.nvnGPSBody.text = GuiTxt;
			WQS_GPSWindow.nvnGPSBody:paginate();
			local myh = WQS_GPSWindow.nvnGPSBody.height
			local FullContentHeight = (myh + WQS_GPSWindow.dim.btnHgt + WQS_GPSWindow.dim.MainPadding * 4) + 10
			WQS_GPSWindow:setHeight(FullContentHeight)
			return
		end
	end
	-- end quest WEQA hook setQuestTextOnGuiUpdate
