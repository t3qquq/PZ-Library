...
--- Gives the first quest to the player by adding the extraction map
...
WQS.GiveFirstQuest = function()
    ...
    -- Send command to server to add the map item
    local args = {
        mapId = "WQS_item_list.wqs_map_dynamic"
    }
    WQS_Shared.SendClientCommand(pl, "WQS_ServerSideModule", "WQS_GiveFirstQuest", args)
...
WQS.CheckIfNewSpawn = function()
    --recupero il giocatore
    local pl = WQS.GetCurrentPlayer();
    if not (pl) then
        return nil
    end
    --if (not pl:getUsername()) or (not WQS_NetPipe.checkIfServerIsReady()) then

    --if not WQS_NetPipe.checkIfServerIsReady() then
    --if (not pl:getUsername()) or (not WQS_NetPipe.checkIfServerIsReady()) then

    -- In SP mode we don't need net pipe ready check
    local isNetReady = true
    if isClient() and WQS_NetPipe then
        isNetReady = WQS_NetPipe.checkIfServerIsReady()
    end

    if (not pl:getUsername()) or (not isNetReady) then
        --if not pl:getUsername() then
        print("WQSDEBUG: CheckIfNewSpawn - Waiting for initialization...")
        --WQS_NetPipe.initConnection()
        return nil
    end

    --controllo se esiste la key PlayerAlreadySpawned tramite WQS_Shared.getWQSPlayerModData
    local PlayerAlreadySpawned = WQS_Shared.getWQSPlayerModData("PlayerAlreadySpawned")
    --print(" ### WQS CheckIfNewSpawn PlayerAlreadySpawned= " .. tostring(PlayerAlreadySpawned))
    --se esiste allora non faccio nulla ed esco
    if PlayerAlreadySpawned then
        return nil
    end

    --se non esiste setto il valore PlayerAlreadySpawned a true tramite WQS_Shared.setWQSPlayerModData e faccio le cose che vanno fatte la prima volta sul player (Dargli la prima quest e gli oggetti iniziali nell'inventario)
    WQS_Shared.setWQSPlayerModData("PlayerAlreadySpawned", true)

    print(" ### WQS CheckIfNewSpawn Is first spawn, giving start item and quest")
    setSandboxVars();
    if (SandboxVars.WQS_StartWithExtractionItem_opt == true) then
        -- Use sendClientCommand to add start item with proper synchronization
        local args = {
            itemId = WQS_Shared.getExtractionItemId(false)
        }
        print("WQSDEBUG: Sending WQS_AddStartItem to server. Player: " .. tostring(pl))
        WQS_Shared.SendClientCommand(pl, "WQS_ServerSideModule", "WQS_AddStartItem", args)
        --WQS_NetPipe.WSendClientCommand(pl, "WQS_ServerSideModule", "WQS_AddStartItem", args)
        print(" ### WQS Requesting start item " ..
            args.itemId .. " for player  " .. tostring(pl:getFullName()))
    end
    if WQS.GameModeIs("CLASSIC_QUICK_MODE") then
        WQS.GiveFirstQuest();
        --WQS_QuestSys.QuestManagerInstance:startQuest("OldClassicStart")
    end
end
...
WQS.MainStatusCheck = function()
    ...
    --quest WEQA hook whileIsActive
    -- if WQS.isActiveAndStartedAdventureSystem() then
    --     --local currentActiveCustomQuest = WEQA.QuestManagerInstance:getActiveQuest()
    --     local currentActiveCustomQuest = WEQA.QuestLinesInstance:getCurrentQuest()
    --     if currentActiveCustomQuest and type(currentActiveCustomQuest.getIfQuestIsCompleted) == "function" then
    --         if currentActiveCustomQuest:getIfQuestIsCompleted() then
    --             WEQA.QuestLinesInstance:completeQuest()
    --         elseif currentActiveCustomQuest and type(currentActiveCustomQuest.whileIsActive) == "function" then
    --             currentActiveCustomQuest:whileIsActive()
    --         end
    --     end
    --     if currentActiveCustomQuest and type(currentActiveCustomQuest.whileIsActive) == "function" then
    --         currentActiveCustomQuest:whileIsActive()
    --     end
    -- end
    --end quest WEQA hook whileIsActive
...
-- [v42.20 Update] Prevent recursive ISWorldMap.render wrapping on player respawn or Lua re-initialization
WQS.AddExtractionPointToWorldMap = function()
    if not WQS.original_ISWorldMap_render then
        WQS.original_ISWorldMap_render = ISWorldMap.render  
        ISWorldMap.render = function(self)
            WQS.original_ISWorldMap_render(self)
...
WQS.isActiveAdventureSystem = function()
    if WEQA and type(WEQA.isActive) == "function" then
        return WEQA.isActive()
    end
    return false
end

WQS.isActiveAndStartedAdventureSystem = function()
    if WQS.isActiveAdventureSystem() then
        if WEQA.isStarted() then
            return true
        end
    end
    return false
...
WQS.OnStart = function()
    ...
    -- end
    --end hook WEQA onStart

    -- WQS_QuestSys.start()
    -- local aq = WQS_QuestSys.QuestManagerInstance:getActiveQuest()
    -- if aq then
    --     print(" ### WQS quest resume old active quest " .. aq:getId())
    --     WQS_QuestSys.QuestManagerInstance:setActiveQuest(aq:getId()) --resume old active quest
    -- else
    --     print(" ### WQS quest fresh start new quest")
    --     -- WQS_QuestSys.QuestManagerInstance:startQuest("quest1") --fresh start new quest
    -- end
    ...
    if (pl) and (pl:getHoursSurvived() < 0.2) then
        ...
        if (SandboxVars.WQS_StartWithExtractionItem_opt == true) then
            ...
            WQS_Shared.SendClientCommand(pl, "WQS_ServerSideModule", "WQS_AddStartItem", args)
            print(" ### WQS Requesting start item " .. WQS_Shared.getExtractionItemId(true))
        end
        if WQS.GameModeIs("CLASSIC_QUICK_MODE") then
            WQS.GiveFirstQuest();
            --WQS_QuestSys.QuestManagerInstance:startQuest("OldClassicStart")
