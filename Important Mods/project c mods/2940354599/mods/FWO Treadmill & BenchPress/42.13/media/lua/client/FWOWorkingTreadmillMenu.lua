	...
	for _,object in ipairs(worldobjects) do
		...
		for i=1,square:getObjects():size() do
			...
			if thisObject:getSprite() then
				...
				if properties:has("GroupName") or properties:has("CustomName") then
...
FWOWorkingTreadmillMenu2.onUseTreadmill = function(worldobjects, player, machine, actionType, length)
	...
	if FWOWorkingTreadmillMenu2.walkToFront(player, machine) then
		...
		ISTimedActionQueue.add(ISFitnessAction:new(player, actionType, length , ISFitnessUI:new(0,0, 600, 350, player) , FitnessExercises.exercisesType.treadmill.type ))
	end
end
