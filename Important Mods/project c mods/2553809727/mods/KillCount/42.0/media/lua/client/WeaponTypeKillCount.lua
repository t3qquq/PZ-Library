...
lcl.hw_getWeaponCategories = lcl.hw_base.getWeaponCategories
...
function KillCountWeaponType.GetWeaponCategoryFromWeapon(weapon)--if there is one category, return it. if there is more, return the first met that is not Improvised, if there is none, return Neither
    ...
    local categories = lcl.hw_getWeaponCategories(weapon)
    for i=0, lcl.ArrayList_size(categories)-1 do
        local wpnCat = lcl.ArrayList_get(categories,i)
        chosenCategory = wpnCat:toString()--Exception, WeaponCategories have not been set available to lua
