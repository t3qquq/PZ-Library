...
function isBow(weapon)
	if	(weapon:getAmmoType() == "Base.Bolt_Bear") or
		...
		(weapon:getAmmoType() == "Base.WoodenBolt") then
		return true
	else	return false
	end
end
...
function isBBGun(weapon)
	if	(weapon:getAmmoType() == "Base.BB177") then
		return true
	else	return false
	end
end
...
function getROF(weapon)
	...
	elseif	ammo == "Base.Bullets38" then			value = 6	-- Vanilla says 5
	elseif	ammo == "Base.Bullets9mm" then		value = 6	-- Vanilla says 5 Pistol1
	elseif	ammo == "Base.Bullets45" then			value = 5	-- Vanilla says 9 RevMed 7 1911
	elseif	ammo == "Base.Bullets357" then		value = 5
	elseif	ammo == "Base.Bullets45LC" then		value = 4
	elseif	ammo == "Base.Bullets44" then			value = 4	-- Vanilla says 12 RevLong 9 DE
	elseif	ammo == "Base.Bullets50MAG" then		value = 4
	elseif	ammo == "Base.Bullets4570" then		value = 4
	elseif	ammo == "Base.223Bullets" then		value = 5	-- Vanilla says 15 for Varmint
	elseif	ammo == "Base.556Bullets" then		value = 5	-- Vanilla says 2 for Assault2, 20 Hunting
	elseif	ammo == "Base.545x39Bullets" then		value = 5
	elseif	ammo == "Base.762x39Bullets" then		value = 4
	elseif	ammo == "Base.308Bullets" then		value = 3	-- Vanilla says 2 for Assault2
	...
	elseif	ammo == "Base.ShotgunShells" then		value = 2	-- Vanilla says 20/40 for Double
...
local function WeaponLightToggle(keyNum)
	...
	if (keyNum == getCore():getKey("WEAPON_LIGHT")) then
		...
		if (stock) and (string.find(stock:getType(), "Light") or string.find(stock:getType(), "Laser")) then
...
local function WeaponLightBeam()
	...
	if (stock) and (string.find(stock:getType(), "Light") or string.find(stock:getType(), "Laser")) and weapon:getModData().LightOn == true then
		if attacker:isAiming() then
			...
			elseif (string.find(stock:getType(), "Medium")) then
			...
			elseif (string.find(stock:getType(), "PEQ")) then		-- SMALL LESS THAN DVAL
				weapon:setLightDistance(10)
				weapon:setLightStrength(1.5)
			elseif (string.find(stock:getType(), "DVAL")) then
				weapon:setLightDistance(15)
				weapon:setLightStrength(1.25)
			end
			...
			elseif (string.find(stock:getType(), "PEQ")) then		-- SMALL MORE THAN DVAL
				weapon:setLightDistance(2.5)
				weapon:setLightStrength(0.5)
			elseif (string.find(stock:getType(), "DVAL")) then
				weapon:setLightDistance(2.0)
...
function AutoTransform()
	...
	if (weapon) and (instanceof(weapon,"HandWeapon")) and (weapon:isAimedFirearm()) then
		...
		elseif	(not weapon:isAimedHandWeapon()) and (not isShotgun(weapon)) and (not isBow(weapon)) then
			...
			if attacker:isAiming() then
				if	(mode) and (mode == "[2]Burst" or mode == "[3]Burst" or mode == "[H2]Burst" or mode == "[H3]Burst") then
					local BurstStop = 0
					local BurstShots = 0
					...
					elseif	(mode == "[3]Burst" or mode == "[H3]Burst") then
						BurstShots = 3
						BurstStop = current - BurstShots
						if	attacker:getModData().BurstStop == current or attacker:getModData().BurstStop == nil then
							attacker:getModData().BurstStop = BurstStop	-- END OF BURST DELAY
