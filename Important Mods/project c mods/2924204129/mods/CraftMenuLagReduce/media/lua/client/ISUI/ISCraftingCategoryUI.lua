...
function ISCraftingCategoryUI:prerender()
    if self.craftingUI.panel and self.craftingUI.beforeCategory ~= self.craftingUI.panel.activeView.view.category then
...
function ISCraftingCategoryUI:addToFavorite(fromKeyboard)
    ...
    local favoriteList = self.craftingUI.recipesList[self.craftingUI.recipesListH[1]]
    if selectedItem.favorite then
        table.insert(favoriteList, selectedItem);
    else
        for k, v in pairs(favoriteList) do
            if v == selectedItem then
                table.remove(favoriteList, k);
            end
        end
    end
    self.craftingUI:refresh();
