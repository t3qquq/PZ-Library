...
function ISCraftingUI:refresh()
    self.beforeCategory = self.panel.activeView.view.category
    ...
    self:getContainers();
    self:MNpopulateRecipesList();

    local num = self.panel:getActiveViewIndex()
    local categoryUI = self.categories[num];
    categoryUI:filter();
...
function ISCraftingUI:populateRecipesList()
    ...
    for i=0,allRecipes:size()-1 do
        ...
        if not recipe:isHidden() and (not recipe:needToBeLearn() or (self.character and self.character:isRecipeKnown(recipe))) then
            ...
            table.insert(self.MNhideAllRecipesList, recipe);
        end
    end
                ...
                for y=0,evorecipe:size() - 1 do
                   local evo = evorecipe:get(y);
                    ...
                    if (not evo:isHidden() or baseItem:getType() ~= evo:getBaseItem()) then
                        newRecipe = {};
                        if not doneRecipes[evo] then
                            doneRecipes[evo] = true;
                            doneItems = {};
							...
							if resultItem then 
								...
								for l=0, itemCanBeUse:size()-1 do
									...
									if not doneItems[newItem:getFullType()] then
										doneItems[newItem:getFullType()] = true;
                                        ...
                                        itemInList.MNitem = newItem:getFullType()
...
function ISCraftingUI:sortList()
    -- nothing
end
...
function ISCraftingUI:onKeyRelease(key)
    ...
    if ui.beforeCategory ~= ui.panel.activeView.view.category then
        ui:refresh()
        ui.beforeCategory = ui.panel.activeView.view.category
    end
end
...
function ISCraftingUI:MNpopulateRecipesList()
    local categoryID = self.panel.activeView.view.category
    local MNTempHideAllRecipesList = {}
    self:getContainers();

    --제작목록에 없는 레시피 추가
    for i=1, #self.MNhideAllRecipesList do
        local newItem = {};
        local recipe = self.MNhideAllRecipesList[i];
        if not recipe:isHidden() and (not recipe:needToBeLearn() or (self.character and self.character:isRecipeKnown(recipe))) then
            if recipe:getCategory() then
                newItem.category = recipe:getCategory();
            else
            ...
            table.insert(MNTempHideAllRecipesList, recipe);
        end
    end

    self.MNhideAllRecipesList = MNTempHideAllRecipesList

    -- 제작가능한 레시피 available 변경
    if self.character then
        --가능한 Evolevd 찾기
        local availableEvolved = {}
        for i=0,self.containerList:size()-1 do
            local container = self.containerList:get(i);
            local isPlayerInventory = container == self.character:getInventory()
            ...
            for x=0,container:getItems():size() - 1 do
                ...
                if evorecipe and evorecipe:size() > 0 then
                    for y=0,evorecipe:size() - 1 do
                        local evo = evorecipe:get(y);
                        local priority = 0
                        if baseItem:getExtraItems() and baseItem:getType() ~= evo:getBaseItem() then
                            priority = baseItem:getExtraItems():size()
                        end

                        local apply = false
                        if priority ~= evo:getMaxItems() then
                            if availableEvolved[evo] then   --이전 값이 있다면
                                if availableEvolved[evo].isPlayerInventory == isPlayerInventory then    --인벤토리 우선순위가 같다면
                                    apply = availableEvolved[evo].priority <= priority   --집어넣은 재료가 더 많은쪽을 선택
                                else    --인벤토리 우선순위가 같지 않다면
                                    apply = isPlayerInventory   --인벤토리 우선순위가 높은쪽을 선택
                                end
                            else   --이전 값이 없다면
                                apply = true
                            end
                        ...
                        if apply then
                            availableEvolved[evo] = {}
                            availableEvolved[evo].baseItem = baseItem
                            availableEvolved[evo].isPlayerInventory = isPlayerInventory
                            availableEvolved[evo].priority = priority
                        end
                    end
                end
            end
        ...
        --카테고리의 모든 레시피
        for _, v in pairs(self.recipesList[categoryID]) do
            if v.evolved then   --식기 사용 레시피
                -- qqqq = v --디버그용

                if availableEvolved[v.recipe] then  --제작 가능한 식기 사용 레시피
                    v.available = true
                    v.baseItem = availableEvolved[v.recipe].baseItem

                    --엑스트라 아이템 업데이트
                    v.extraItems = {}
                    if v.baseItem and v.baseItem:getExtraItems() then
                        for u=0,v.baseItem:getExtraItems():size()-1 do
                        local extraItem = self:GetItemInstance(v.baseItem:getExtraItems():get(u));
                            if extraItem then
                                table.insert(v.extraItems, extraItem:getTex());
                            end
                        end
                    end

                    --커스텀이름 업데이트
                    if #v.extraItems == 0 then
                        v.customRecipeName = nil
                    else
                        v.customRecipeName = getText("IGUI_CraftUI_FromBaseItem", v.baseItem:getDisplayName());
                    end

                    --가방에 존재하는 재료 목록
                    local itemCanBeUseID = {}
                    local itemCanBeUse = v.recipe:getItemsCanBeUse(self.character, v.baseItem, self.containerList)
                    for i=0,itemCanBeUse:size()-1 do
                        itemCanBeUseID[itemCanBeUse:get(i):getFullType()] = itemCanBeUse:get(i)
                    end

                    --재료 제작가능여부 업데이트
                    local isAvailable = false
                    for _, v2 in pairs(v.items) do
                        if itemCanBeUseID[v2.MNitem] then
                            isAvailable = true

                            v2.available = true
                            v2.fullType = itemCanBeUseID[v2.MNitem]:getFullType()
                            v2.itemToAdd = itemCanBeUseID[v2.MNitem]
                            v2.poison = self.character:isKnownPoison(itemCanBeUseID[v2.MNitem])
                        else
                            v2.available = false
                            v2.fullType = nil
                            v2.itemToAdd = nil
                ...
                else    --제작 불가능한 식기 사용 레시피
                    v.available = false
                    v.baseItem = self:GetItemInstance(v.recipe:getModule():getName() .. "." .. v.recipe:getBaseItem()) or self:GetItemInstance(v.recipe:getBaseItem())
                    v.extraItems = {}
                    v.customRecipeName = nil
                    for _, v2 in pairs(v.items) do
                        v2.available = false
                ...
                v.available = RecipeManager.IsRecipeValid(v.recipe, self.character, nil, self.containerList);
            end
        end
    end
end
