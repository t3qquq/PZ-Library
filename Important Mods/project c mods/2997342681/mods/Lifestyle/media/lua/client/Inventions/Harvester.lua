...
local function isHarvesterEmpty(item)
	...
	return string.find(itemName, "Empty")
end
]]--
...
InvHarvester.doInventoryMenu = function(player, context, items, harvester)
	-- Conditions
	if not LSUtil.isValidInvItem(harvester) or harvester:isBroken() then return; end
	...
	if LSUtil.isCharSitting(character, character:getModData()) then return; end --!
	-- Main Option
...
InvHarvester.onAction = function(worldobjects, character, item)
	-- Check conditions again
	if not LSUtil.isValidInvItem(item) or item:isBroken() then return; end
