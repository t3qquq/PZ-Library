...
local function doSkill(_player, _amount, _name, _perk)
    ...
    --if curXp>0 then
        --amount = (2 / curXp)*50;
    --end
    if _player:HasTrait("CouchPotato") then
        amount = 75*_amount;
		--_player:Say("XP 1.5")
    elseif _player:HasTrait("Disciplined") then
        amount = 25*_amount;
		--_player:Say("XP 0.5")
    end
	...
	if _player:getModData() then
		_player:getModData().xpRI = true
	end
...
local function applyBoredom(_player, _amount, _isSet)
    ...
    if bodyDamage~=nil then
		...
		elseif _player:HasTrait("CouchPotato") then
            local am = _amount*10;
            val = val+am;
			--_player:Say("B 2")
...
--Lifestyle
Interactions.DNC = function(_player, _amount) doSkill(_player, _amount, getText("IGUI_perks_Dancing"), Perks.Dancing); end   --dancing
Interactions.ART = function(_player, _amount) doSkill(_player, _amount, getText("IGUI_perks_Art"), Perks.Art); end   --art
