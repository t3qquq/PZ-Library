---credit for this genius calorie workaround goes to Chuckleberry Finn
---modified for Calorie Fix by DreadLordNaf
...
local function CF_updateCalories(player)
    ...
    --inventory impact [disabled for now so as to replicate vanilla values instead, was in Chuckleberry's original but not needed for this mod]
    ...
    ---Apply sandbox option [Is suppose to allow custom sandbox option on game creation but is broken on my mod. Have disabled for now until I can get better at modding and fix it]
...
Events.OnPlayerUpdate.Add(CF_updateCalories)