...
ISImageMod = ISPanel:derive("ISImageMod");
...
function ISImageMod:initialise()
	ISPanel.initialise(self);
end

function ISImageMod:getTexture()
	return self.texture;
end

function ISImageMod:setMouseOverText(text)
	self.mouseovertext = text;
end

function ISImageMod:setColor(r,g,b)
	self.backgroundColor.r = r;
	self.backgroundColor.g = g;
	self.backgroundColor.b = b;
end

function ISImageMod:onMouseMoveOutside(dx, dy)
	self.mouseover = false;
end

function ISImageMod:onMouseMove(dx, dy)
	self.mouseover = self:isMouseOver(); -- handle windows in front
end

function ISImageMod:onMouseUp(x,y)
...
function ISImageMod:updateTooltip()
	if self.mouseover and self.mouseovertext then
		...
		self.tooltipUI:setY(self:getAbsoluteY() + self:getHeight() - 10)
	else
...
--**
--************************************************************************--
function ISImageMod:new (x, y, width, height, texture)
	local o = {};
