...
HitmanEventMarker = ISUIElement:derive("HitmanEventMarker")

HitmanEventMarker.iconSize = 96
HitmanEventMarker.clickableSize = 45
HitmanEventMarker.maxRange = 500

function HitmanEventMarker:initialise()
...
function HitmanEventMarker:onMouseDoubleClick(x, y)
    return self:setDuration(0)
end


function HitmanEventMarker:onMouseUp(x, y)
...
function HitmanEventMarker:onMouseUpOutside(x, y)
    if not self.moveWithMouse then
        return
    end
    if not self:getIsVisible() then
...
function HitmanEventMarker:onMouseDown(x, y)
    if not self.moveWithMouse then
        return true
    end
    if not self:getIsVisible() then
...
function HitmanEventMarker:onMouseMoveOutside(dx, dy)
    if not self.moveWithMouse then
        return
    end
    ...
    if self.moving then
        ...
        if p then
            p:getModData()["HitmanEventMarkerPlacement"] = {self.x, self.y}
        end
    end
end

function HitmanEventMarker:onMouseMove(dx, dy)
    if not self.moveWithMouse then
        return
    end
    self.mouseOver = true
...
function HitmanEventMarker:setDistance(dist)
    self.distanceToPoint = dist
end
function HitmanEventMarker:setAngleFromPoint(posX,posY)
...
function HitmanEventMarker:setAngle(value)
    self.angle = value
end

function HitmanEventMarker:setDuration(value)
    self.duration = value
    if value <= 0 then
        self:setVisible(false)
    end
...
function HitmanEventMarker:getDuration()
...
function HitmanEventMarker:render()
    if self.visible and self.duration > 0 then--and self.distanceToPoint>4 then
        ...
        self:drawTexture(self.textureBG, centerX-(HitmanEventMarker.iconSize/2), centerY-(HitmanEventMarker.iconSize/2), 1, _color.r, _color.g, _color.b)
        if self.desc then
        ...
        if distanceOverRadius <= (8/HitmanEventMarker.maxRange) then
            textureForPoint = self.texturePointClose
        elseif distanceOverRadius <= (125/HitmanEventMarker.maxRange) then
            --no change
        elseif distanceOverRadius <= (375/HitmanEventMarker.maxRange) then
        ...
        self:drawTexture(self.textureIcon, centerX-(HitmanEventMarker.iconSize/2), centerY-(HitmanEventMarker.iconSize/2), 1, 1, 1, 1)

        if self.player and getNumActivePlayers()>1 then
            self:drawTexture(self.textureCoopNum[self.player:getPlayerNum()+1], centerX-(HitmanEventMarker.iconSize/2), centerY-(HitmanEventMarker.iconSize/2), 1, 1, 1, 1)
        end
...
function HitmanEventMarker:setEnabled(value)
    self.enabled = value
end

function HitmanEventMarker:getEnabled()
    return self.enabled
end

function HitmanEventMarker:prerender()
end

function HitmanEventMarker:refresh()
    self.opacity = 0
    self.opacityGain = 2
end
...
function HitmanEventMarker:getPlayer()
    return self.player
end


function HitmanEventMarker:new(eventID, icon, duration, posX, posY, player, screenX, screenY, color, desc)
    ...
    o.posX = posX or 0
    o.posY = posY or 0
    o.width = HitmanEventMarker.clickableSize
    o.height = HitmanEventMarker.clickableSize
    o.angle = 0
    o.opacity = 255
    o.opacityGain = 2
    ...
    o.visible = true
    o.title = ""
    o.distanceToPoint = HitmanEventMarker.maxRange
    o.radius = nil
    o.mouseOver = false
    o.tooltip = nil
    o.center = false
...
function HitmanEventMarker:update(posX,posY)
    ...
    if not self.radius then
        self.radius = HitmanEventMarker.maxRange*0.83
    end
    ...
    self.radius = math.max(HitmanEventMarker.maxRange/3, math.min(self.radius,HitmanEventMarker.maxRange))
