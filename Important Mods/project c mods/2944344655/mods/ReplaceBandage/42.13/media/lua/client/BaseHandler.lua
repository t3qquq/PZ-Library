...
-- Copied BaseHandler Class from ISHealthPanel.lua because it's fucking private. --
-- IndieStone please make this public!!! ---

local BaseHandler = ISBaseObject:derive("BaseHandler")

function BaseHandler:new(panel, bodyPart, playerId)
    local o = {}
    setmetatable(o, self)
    self.__index = self
    o.panel = panel
...
function BaseHandler:getDoctor()
    return self.playerId or self.panel.otherPlayer or self.panel.character
end

function BaseHandler:getPatient()
    return self.playerId or self.panel.character
end
