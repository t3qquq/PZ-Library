    ...
    public long playVocals(String file) {
        ...
        if (voice == null) {
            if (DebugLog.isEnabled(DebugType.Sound)) {
                DebugType.Sound.debugln("Playing sound: %s for %s", file, this.character.getClass().getSimpleName());
            }

            return this.vocals.playSoundImpl(file, false, null);
        } else {
            if (DebugLog.isEnabled(DebugType.Sound)) {
                DebugType.Sound.debugln("Playing sound: %s for %s", voice.sound, this.character.getClass().getSimpleName());
            }
