...
public class BaseAnimalBehavior {
    ...
    public float hutchPathTimer = -1.0F;
    ...
    public void wanderIdle() {
        ...
        if (!this.isDoingBehavior
            ...
            && this.parent.getLastSoundRespondedTo() == null
    ...
    private void updateGoingToHutch() {
        if (this.isDoingBehavior && BehaviorAction.ENTERHUTCH == this.behaviorAction) {
            ...
            if (this.hutchPathTimer > -1.0F && hutchToEnter != null) {
                this.hutchPathTimer = this.hutchPathTimer - GameTime.getInstance().getMultiplier();
                if (this.hutchPathTimer <= 0.0F && hutchToEnter != null) {
                    this.parent.stopAllMovementNow();
                    this.parent
                        .pathToLocation(
    ...
    public boolean callToHutch(IsoHutch hutch, boolean force) {
        if (this.enterHutchTimerAfterDestroy > 0.0F) {
            this.enterHutchTimerAfterDestroy = this.enterHutchTimerAfterDestroy - GameTime.getInstance().getMultiplier();
            if (this.enterHutchTimerAfterDestroy < 0.0F) {
                this.enterHutchTimerAfterDestroy = 0.0F;
            }
    ...
    public boolean canGoToHutch(IsoHutch hutch, boolean force) {
        if (hutch.getAnimalInside().size() < hutch.getMaxAnimals()) {
            ...
            if (hutch.isOpen()) {
                if (this.hutchPathTimer <= -1.0F) {
                    if (this.parent.isBaby()) {
                        this.hutchPathTimer = Rand.Next(10.0F, 30.0F);
                    } else if (this.parent.isFemale()) {
                        this.hutchPathTimer = Rand.Next(350.0F, 500.0F);
                    } else {
                        this.hutchPathTimer = Rand.Next(950.0F, 1000.0F);
                    }
                }
    ...
    private boolean tryToEatFromMom(boolean callOtherBabies) {
        if (this.parent.mother != null
            ...
            this.setDoingBehavior(true);
            if (callOtherBabies && this.parent.mother != null && this.parent.mother.getBabies() != null && !this.parent.mother.getBabies().isEmpty()) {
        ...
        if (this.parent.adef.eatTypeTrough != null && trough.getContainer() != null) {
            for (int i = 0; i < trough.getContainer().getItems().size(); i++) {
                ...
                if (!(item instanceof Food food && food.isRotten())) {
                    ...
                    if (item instanceof Food food && this.parent.adef.eatTypeTrough.contains(food.getFoodType())) {
                        return true;
                    }
                }
            }
    ...
    private void followChr() {
        if (!this.blockMovement && this.parent.eatFromTrough == null && this.parent.eatFromGround == null) {
            if (this.parent.getData().getAttachedPlayer() != null) {
                if (this.followChrTimer > 0.0F) {
                    this.followChrTimer = this.followChrTimer - GameTime.getInstance().getMultiplier();
                    return;
                }

                this.followChrTimer = 150.0F;
    ...
    private void fleeFromAttacker() {
        if (this.parent.getAttackedBy() == null) {
            this.removeAttacked();
        } else {
            ...
            if (timeNow - this.parent.attackedTimer <= 1100000L && !(this.parent.DistTo(this.parent.fightingOpponent) > 40.0F)) {
                ...
                if (this.parent.adef.attackBack && this.parent.getAttackedBy() instanceof IsoPlayer isoPlayer) {
                    if (!this.parent.getAttackedBy().isInvisible() && !isoPlayer.isGhostMode()) {
                        this.parent.fightingOpponent = this.parent.getAttackedBy();
                    } else {
                        this.parent.fightingOpponent = null;
                    }
                } else {
                    ...
                    tempVector2.x = tempVector2.x - this.parent.getAttackedBy().getX();
                    tempVector2.y = tempVector2.y - this.parent.getAttackedBy().getY();
                    ...
                    if (this.parent.fightingOpponent != this.parent.spottedChr) {
                        ...
                        if (!craven && !this.parent.isWild() && this.parent.spottedChr instanceof IsoPlayer spottedPlayer && !this.parent.adef.alwaysFleeHumans
                            )
                         {
                            ...
                            int baseChance = 0;
                            baseChance += (int)(this.parent.stressLevel / 2.2F);
                                ...
                                if (!(otherCharacter instanceof IsoZombie) || this.parent.adef.fleeZombies) {
                                    if (this.parent.isWild() && dist < 3.0F) {
                                        ...
                                        if (this.parent.isWild() && otherCharacter instanceof IsoPlayer isoPlayer && otherCharacter.isPlayerMoving()) {
                                            ...
                                            int rand = Rand.Next(Rand.AdjustForFramerate(25000));
                                            if (rand > (int)baseChance) {
                                                if (otherCharacter.isSneaking()
                                                    ...
                                                    if (GameServer.server) {
                                                        if (otherCharacter.isPlayerMoving() && Rand.NextBool(Rand.AdjustForFramerate(200))) {
                                                            GameServer.addXp(isoPlayer, PerkFactory.Perks.Tracking, (int)Rand.Next(1.0F, 3.0F));
                                                        } else if (!otherCharacter.isPlayerMoving() && Rand.NextBool(Rand.AdjustForFramerate(200))) {
                                                            GameServer.addXp(isoPlayer, PerkFactory.Perks.Tracking, (int)Rand.Next(1.0F, 3.0F));
                                                        }

                                                        if (otherCharacter.isSneaking()) {
                                                            if (otherCharacter.isPlayerMoving() && Rand.NextBool(Rand.AdjustForFramerate(385))) {
                                            ...
                                            this.parent
                                                .setDebugStress(
                                                    this.parent.getStress()
                                                        + Rand.Next(5.0E-5F, 9.0E-5F) * GameTime.getInstance().getMultiplier() / (14.0F - dist)
                                                );
