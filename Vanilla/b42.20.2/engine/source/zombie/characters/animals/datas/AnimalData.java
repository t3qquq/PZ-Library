...
public class AnimalData {
    ...
    public static final float HUNGER_PER_DRAINABLE_USE = 0.1F;
    private static final long TIME_TO_LOSE_MILK = 1814400000L;
    ...
    private void checkPregnancy() {
        if (this.pregnant && this.pregnantTime > 0) {
            ...
            if (this.pregnantTime >= this.getPregnantPeriod()) {
                ...
                if (healthDelta < 0.5) {
                    DebugType.Animal.trace("Mother health was too low, reducing nb of babies");
                    babyNbr *= (int)healthDelta;
                }

                DebugType.Animal.trace("Should pop " + babyNbr + " babies (" + this.parent.adef.minBaby + "-" + this.parent.adef.maxBaby + ")");
    ...
    private void updateMilk() {
        if (this.parent.adef.udder) {
            if (this.canHaveMilk) {
                ...
                if (!this.isPregnant()
                    && GameTime.getInstance().getCalender().getTimeInMillis() - this.lastMilkTimer > 1814400000L
                    && (this.parent.getBabies() == null || this.parent.getBabies().isEmpty())) {
    ...
    private static boolean doesItemSatisfyHunger(float itemHungerValue, float animalHunger) {
        return Math.abs(itemHungerValue) <= animalHunger;
    }
    ...
    public void eatItem(InventoryItem item, boolean onground) {
        ...
        if (item instanceof Food food) {
            float hungReduce = stats.get(CharacterStat.HUNGER) / this.parent.adef.hungerBoost;
            float hungOnFood = food.getHungerChange();
            if (doesItemSatisfyHunger(hungOnFood, hungReduce)) {
                stats.set(CharacterStat.HUNGER, stats.get(CharacterStat.HUNGER) + hungOnFood * this.parent.adef.hungerBoost);
                stats.add(CharacterStat.THIRST, food.getThirstChange());
                if (onground) {
                ...
                float thirstLeft = Math.abs(food.getThirstChange()) - stats.get(CharacterStat.THIRST) / this.parent.adef.thirstBoost;
                stats.reset(CharacterStat.HUNGER);
                stats.add(CharacterStat.THIRST, food.getThirstChange() * this.parent.adef.thirstBoost);
                food.setHungChange(food.getHungChange() * ((hungOnFood + hungReduce) / hungOnFood));
        ...
        if (item instanceof DrainableComboItem food) {
            float hungReduce = stats.get(CharacterStat.HUNGER);
            float hungOnFood = food.getCurrentUses() * 0.1F;
            if (doesItemSatisfyHunger(hungOnFood, hungReduce)) {
                ...
                if (onground) {
                    ...
                    if (food.getReplaceOnDeplete() != null) {
                        InventoryItem newItem = item.getContainer().AddItem(food.getReplaceOnDeplete());
                        assert newItem != null;
                        newItem.copyConditionStatesFrom(item);
                        if (GameServer.server) {
                            GameServer.sendAddItemToContainer(item.getContainer(), newItem);
                ...
                float usingDelta = Math.round(hungReduce / 0.1F);
    ...
    public void grow(String newtype) {
        ...
        AnimalDefinitions animalDef = AnimalDefinitions.getDef(newtype);
        AnimalBreed newBreed = animalDef.getBreedByName(this.breed.getName());
        IsoAnimal newAnimal = new IsoAnimal(this.parent.getCell(), this.parent.getXi(), this.parent.getYi(), this.parent.getZi(), newtype, newBreed);
    ...
    public void setAttachedPlayer(IsoPlayer chr) {
        ...
        if (this.attachedPlayer != chr) {
            INetworkPacket.sendToRelative(
                ...
                this.parent,
                this.attachedPlayer != null ? this.attachedPlayer : chr,
                null,
                chr == null
            );
        }
        ...
        if (this.parent.getBehavior().isDoingBehavior) {
            ...
            if (this.parent.getBehavior().behaviorObject != null) {
                if (this.parent.getBehavior().behaviorObject instanceof IsoFeedingTrough isoFeedingTrough) {
                    ...
                    if (this.parent.getBehavior().hutchPathTimer > -1.0F) {
                        result = result
