...
public class HitReactionNetworkAI {
    ...
    public void reset() {
        ...
        this.finalPosition.set(0.0F, 0.0F);
        this.finalPositionZ = 0;
        this.startDirection.set(0.0F, 0.0F);
        this.startAngle = 0.0F;
        this.finalDirection.set(0.0F, 0.0F);
    ...
    public void setup(float dropPositionX, float dropPositionY, byte dropPositionZ, Float angle) {
        ...
        this.character.getForwardDirection(this.startDirection);
        ...
        if (angle != null) {
            ...
            this.finalDirection.set(this.startDirection);
            this.finalAngle = this.finalDirection.getDirection();
        }
    ...
    private void moveInternal(float posX, float posY, float dirX, float dirY) {
        ...
        if (!this.character.isAnimal() && !PZMath.equal(dirX, 0.0F) && !PZMath.equal(dirY, 0.0F)) {
        ...
        if (GameClient.client) {
            if (wielder.isLocal()) {
                ...
                if (target.isOnFloor()) {
                    ...
                    target.calcHitDir(wielder, weapon, hitPos);
                    if (target instanceof IsoAnimal) {
                        ...
                        hitPos.x = (hitPos.x + target.getX() + networkAi.targetX) * 0.5F;
                        hitPos.y = (hitPos.y + target.getY() + networkAi.targetY) * 0.5F;
                    } else {
                        hitPos.x = hitPos.x + target.getX();
                        hitPos.y = hitPos.y + target.getY();
                    }
