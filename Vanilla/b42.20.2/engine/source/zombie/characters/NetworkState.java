...
public class NetworkState {
    ...
    public void processExitSubState() {
        if (!this.exitSubStatePackets.isEmpty()) {
            this.exitSubStatePackets.removeFirst().apply();
        }
    }

    public void processExitState() {
        if (!this.exitStatePackets.isEmpty()) {
            this.exitStatePackets.removeFirst().apply();
        }
    }
    ...
    public StatePacket removeLastExitState() {
        return this.exitStatePackets.isEmpty() ? null : this.exitStatePackets.removeLast();
    }

    public StatePacket removeLastExitSubState() {
        return this.exitSubStatePackets.isEmpty() ? null : this.exitSubStatePackets.removeLast();
    }
    ...
    public void updateExitState(StatePacket packet) {
        if (!tryCancelPendingEnterPair(this.enterStatePackets, packet)) {
            this.updateState(this.exitStatePackets, packet);
        }
    }

    public void updateExitSubState(StatePacket packet) {
        if (!tryCancelPendingEnterPair(this.enterSubStatePackets, packet)) {
            this.updateState(this.exitSubStatePackets, packet);
        }
    }

    private static boolean tryCancelPendingEnterPair(LinkedList<StatePacket> enterQueue, StatePacket exitPacket) {
        if (exitPacket != null && !enterQueue.isEmpty()) {
            State exitState = exitPacket.getState();
            if (exitState == null) {
                return false;
            }

            StatePacket matchingEnter = null;

            for (StatePacket pending : enterQueue) {
                if (pending.getState().getName().equals(exitState.getName())) {
                    matchingEnter = pending;
                    break;
                }
            }
            ...
            enterQueue.remove(matchingEnter);
            return true;
        } else {
            return false;
        }
    ...
    private void sync(LinkedList<StatePacket> packets, IConnection connection) {
        if (!packets.isEmpty()) {
    ...
    private boolean timeout(LinkedList<StatePacket> packets) {
        ...
        if (packet != null && System.currentTimeMillis() - packet.timestamp > 5000L) {
            ...
            DebugType.Multiplayer.noise("State %s timeout", packet.getState().getName());
            return true;
        } else {
            return false;
        }
