...
public class RandomizedVehicleStoryBase extends RandomizedWorldBase {
    ...
    public static boolean doRandomStory(Zone zone, IsoChunk chunk, boolean force) {
        ...
        if (roll <= baseChance) {
            ...
            RandomizedVehicleStoryBase rvs = getRandomStory();
            if (rvs == null) {
                return false;
            }
    ...
    public boolean isValid(Zone zone, IsoChunk chunk, boolean force) {
        ...
        if (this.needsPavement) {
            ...
            IsoObject floor = centerSq.getFloor();
            String floorName = floor.getSprite().getName();
        ...
        if (this.needsRegion) {
            ...
            String region = ItemPickerJava.getSquareRegion(centerSq);
        ...
        if (this.needsFarmland) {
            ...
            if (centerSq != null) {
                ...
                for (int i = -15; i < 15 && fail; i++) {
                    for (int j = -15; j < 15; j++) {
                        if (fail) {
                            IsoGridSquare sq = getSq(centerX + i * 10, centerY + j * 10, 0);
                            if (sq != null
                                && sq.getZoneType() != null
                                && (
