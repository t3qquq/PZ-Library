...
public class RZSForestCampEaten extends RandomizedZoneStoryBase {
    ...
    public void randomizeZoneStory(Zone zone) {
        ...
        for (int i = 0; i < randCorpse; i++) {
            IsoDeadBody body = createRandomDeadBody(this.getRandomExtraFreeSquare(this, zone), null, Rand.Next(5, 10), 0, outfit);
            if (body != null) {
                this.addBloodSplat(body.getSquare(), 10);
            }
        }

        IsoDeadBody body = createRandomDeadBody(getSq(midX, midY + 3, zone.z), null, Rand.Next(5, 10), 0, outfit);
        if (body != null) {
            this.addBloodSplat(body.getSquare(), 10);
            if (eatingZed != null) {
