...
public class RandomizedZoneStoryBase extends RandomizedWorldBase {
    ...
    public IsoGridSquare getRandomFreeSquare(RandomizedZoneStoryBase rzs, Zone zone, IsoGridSquare notSquare) {
        for (int index = 0; index < 1000; index++) {
            ...
            IsoGridSquare sq = getSq(randX, randY, zone.z);
        ...
        for (int xDel = x; xDel < x2; xDel++) {
            for (int yDel = y; yDel < y2; yDel++) {
                ...
                if (sq != null) {
                    ...
                    for (int i = sq.getSpecialObjects().size() - 1; i >= 0; i--) {
                        IsoObject obj = sq.getSpecialObjects().get(i);
                        sq.RemoveTileObject(obj);
                    }

                    for (int i = sq.getStaticMovingObjects().size() - 1; i >= 0; i--) {
                        if (sq.getStaticMovingObjects().get(i) instanceof IsoDeadBody deadBody) {
