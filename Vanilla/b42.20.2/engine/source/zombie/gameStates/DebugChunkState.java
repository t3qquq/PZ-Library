...
public final class DebugChunkState extends GameState {
    ...
    public GameStateMachine.StateAction updateScene() {
        ...
        if (GameKeyboard.isKeyDown(19)) {
            if (!keyQpressed) {
                ...
                keyQpressed = true;
                DebugType.General.debugln("IsoCell.newRender = %s", DebugOptions.instance.terrain.renderTiles.newRender.getValue());
            }
        } else {
            keyQpressed = false;
        }
        ...
        if (GameKeyboard.isKeyDown(31)) {
            if (!keyQpressed) {
                ...
                keyQpressed = true;
                DebugType.General.debugln("ParticlesFire.reloadShader");
            }
        } else {
            keyQpressed = false;
        }
    ...
    private void drawCursor() {
        ...
        SpriteRenderer.instance
            .renderPoly(sx, sy, sx + 32 * scale, sy - 16 * scale, sx + 64 * scale, sy, sx + 32 * scale, sy + 16 * scale, 0.0F, 0.0F, 1.0F, 0.5F);
    ...
    private void drawZones() {
        ...
        for (int i = 0; i < zones.size(); i++) {
            Zone zone = zones.get(i);
            if (zone != null && zone.isPolyline() && !zone.points.isEmpty()) {
                ...
                if (zone.polylineOutlinePoints != null) {
