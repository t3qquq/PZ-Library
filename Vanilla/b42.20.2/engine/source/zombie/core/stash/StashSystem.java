                                ...
                                for (int z = -32; z < 31; z++) {
                                    ...
                                    if (sq != null) {
                                        ...
                                        for (int o = 0; o < sq.getObjects().size(); o++) {
                                            ...
                                            if (container != null
                                                && room != null
                                                && room.getBuilding().getDef() == def
                                                && room.getName() != null
                                                && buildingLoot != null
                                                && buildingLoot.containers.containsKey(container.getType())) {
                                                ItemPickerJava.ItemPickerRoom originalLoot = ItemPickerJava.rooms.get(room.getName());
                                                boolean clearIt = false;
                                                if (originalLoot == null || !originalLoot.containers.containsKey(container.getType())) {
                                                    container.clear();
                                                    clearIt = true;
                                                }

                                                ItemPickerJava.fillContainerType(buildingLoot, container, "", null);
                                            ...
                                            BarricadeAble barricadeAble = Type.tryCastTo(obj, IsoWindow.class);
                                            if (obj instanceof IsoDoor isoDoor) {
                                                barricadeAble = isoDoor;
                                            }
            ...
            for (int i = 0; i < stash.containers.size(); i++) {
                ...
                if (sq != null) {
                    if (stashCont.containerItem != null && !stashCont.containerItem.isEmpty()) {
                        ...
                        if (item == null) {
                            DebugType.General.error("Item " + stashCont.containerItem + " Doesn't exist.");
                            return;
                        }

                        ItemPickerJava.ItemPickerContainer containerDist = spawnTable.containers.get(item.getType());
                        if (containerDist == null) {
                            containerDist = spawnTable.containers.get(stashCont.containerType);
                        }
