...
public class Bullet {
    ...
    private static final long BREAK_IMPULSE_IGNORE_MS = 2000L;
    private static final float BREAK_IMPULSE_UPRIGHT = 1500.0F;
    private static final float BREAK_IMPULSE_TILTED = 500.0F;
    public static final HashMap<String, Integer> physicsShapeNameToIndex = new HashMap<>();

    public static void init() {
        if (isInitialised) {
            DebugType.General.warn("Bullet already initialised.");
        } else {
            isInitialised = true;
            String libSuffix = "";
            if ("1".equals(System.getProperty("zomboid.debuglibs.bullet"))) {
                DebugType.General.debugln("***** Loading debug version of PZBullet");
                libSuffix = "d";
            }
            ...
            DebugType.General.debugln("Initializing logging...");
            initPZBullet();
        }
    }

    private static void loadLibrary(String libName) {
        DebugType.General.debugln("Loading library: %s", libName);
    ...
    public static void initPhysicsMeshes() {
        ...
        for (int i = 0; i < vehicleScripts.size(); i++) {
            VehicleScript vehicleScript = vehicleScripts.get(i);
    ...
    public static void onVehicleConstraintImpulse(int constraintID, int vidA, int vidB, float impulse) {
        if (!GameServer.server) {
            BaseVehicle vehicleA = VehicleManager.instance.getVehicleByID((short)vidA);
            BaseVehicle vehicleB = VehicleManager.instance.getVehicleByID((short)vidB);
            if (vehicleA != null && vehicleB != null) {
                if (!GameClient.client || vehicleA.isLocalPhysicSim() && vehicleB.isLocalPhysicSim()) {
                    float breakImpulse = vehicleA.getUpVectorDot() > 0.8F && vehicleB.getUpVectorDot() > 0.8F ? 1500.0F : 500.0F;
                    if (!(impulse < breakImpulse)) {
                        boolean recentlyConnected = vehicleA.constraintChangedTime + 2000L > System.currentTimeMillis();
                        if (vehicleA.constraintTowing == constraintID && !recentlyConnected) {
                            if (!GameClient.client || vehicleA.breakConstraintOnServer()) {
                                vehicleA.breakConstraint(true, false);
                                vehicleA.getEmitter().playSoundImpl(SoundKey.BREAK_METAL_ITEM.getSoundName(), (IsoObject)null);
                            }
                        }
                    }
                }
    ...
    public static native int setOwnVehiclePhysics(int var0, float[] var1, boolean var2);
    ...
    public static native void setConstraintERP(int var0, float var1, int var2);
    ...
    public static native void initializeRagdollPose(int var0, float[] var1, float var2, float var3, float var4, float var5);
    ...
    public static void addRagdoll(int id, org.lwjgl.util.vector.Vector3f worldPosition, Quaternion worldRotation) {
        addRagdoll(id, worldPosition.x, worldPosition.y, worldPosition.z, worldRotation.x, worldRotation.y, worldRotation.z, worldRotation.w);
    }
    ...
    public static void updateRagdoll(int id, org.lwjgl.util.vector.Vector3f worldPosition, Quaternion worldRotation) {
        updateRagdoll(id, worldPosition.x, worldPosition.y, worldPosition.z, worldRotation.x, worldRotation.y, worldRotation.z, worldRotation.w);
    }
