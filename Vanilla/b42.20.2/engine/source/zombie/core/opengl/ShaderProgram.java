...
public final class ShaderProgram {
    ...
    public void compile() {
        ...
        if (DebugLog.isEnabled(DebugType.Shader)) {
            DebugType.Shader.debugln(name + (this.isStatic ? "(Static)" : "") + (this.isInstanced ? "(Instanced)" : ""));
        }

        this.shaderId = GL20.glCreateProgram();
        if (this.shaderId == 0) {
            DebugType.Shader.error("Failed to create Shader: " + name + " could not create new Shader Program ID.");
        } else {
            ...
            if (!this.compileAllShaderUnits()) {
                ...
                if (GL20.glGetProgrami(this.shaderId, 35714) == 0) {
                    this.compileFailed = true;
                    DebugType.Shader.error("Failed to link new Shader Program:" + name + " bStatic:" + this.isStatic + " bInstanced:" + this.isInstanced);
                    DebugType.Shader.error(getLogInfo(this.shaderId));
                    this.destroy();
                } else {
                    GL20.glValidateProgram(this.shaderId);
                    if (GL20.glGetProgrami(this.shaderId, 35715) == 0) {
                        this.compileFailed = true;
                        DebugType.Shader.error("Failed to validate Shader Program:" + name + " bStatic:" + this.isStatic + " bInstanced:" + this.isInstanced);
                        DebugType.Shader.error(getLogInfo(this.shaderId));
                        this.destroy();
                    } else {
        ...
        if (this.isCompiled()) {
            ...
            for (int i = 0; i < c; i++) {
                ...
                if (loc != -1) {
                    ...
                    u.type = type;
                    if (DebugLog.isEnabled(DebugType.Shader)) {
                        DebugType.Shader.debugln(str + ", Loc: " + loc + ", Type: " + type + ", Size: " + size);
                    }
    ...
    private boolean compileAllShaderUnits() {
        if (ShaderUnit.combineShaderSources) {
            ...
            if (first != null && !first.isCompiled() && !first.compile()) {
                DebugType.Shader.error("Failed to create Shader: " + this.getName() + " Shader unit failed to compile: " + first.getFileName());
                return false;
            } else {
            ...
            for (ShaderUnit unit : this.getShaderUnits()) {
                if (!unit.isCompiled() && !unit.compile()) {
                    DebugType.Shader.error("Failed to create Shader: " + this.getName() + " Shader unit failed to compile: " + unit.getFileName());
                    return false;
                }
            }
    ...
    private boolean attachAllShaderUnits() {
        if (ShaderUnit.combineShaderSources) {
            ...
            for (ShaderUnit unit : this.getShaderUnits()) {
                if (!unit.attach()) {
                    DebugType.Shader.error("Failed to create Shader: " + this.getName() + " Shader unit failed to attach: " + unit.getFileName());
                    return false;
                }
            }
    ...
    public void destroy() {
        if (this.shaderId == 0) {
            ...
            try {
                DebugType.Shader.debugln(this.getName());
    ...
    public void setVector2(String loc, float valX, float valY) {
        ShaderProgram.Uniform u = this.getUniform(loc, 35664);
        if (u != null) {
            this.setVector2(u.loc, valX, valY);
        }
    }

    public void setVector3(String loc, float valX, float valY, float valZ) {
        ShaderProgram.Uniform u = this.getUniform(loc, 35665);
        if (u != null) {
            this.setVector3(u.loc, valX, valY, valZ);
        }
    }

    public void setVector4(String loc, float valX, float valY, float valZ, float valW) {
        ShaderProgram.Uniform u = this.getUniform(loc, 35666);
        if (u != null) {
            this.setVector4(u.loc, valX, valY, valZ, valW);
        }
    }
    ...
    public ShaderProgram.Uniform getUniform(String loc, int type, boolean bWarn) {
        ...
        if (u == null) {
            if (bWarn) {
                DebugType.Shader.warn(loc + " doesn't exist in shader");
            }

            return null;
        } else if (u.type != type) {
            DebugType.Shader.warn(loc + " isn't of type: " + type + ", it is of type: " + u.type);
            return null;
        } else {
            return u;
        }
    ...
    public void setValue(String loc, Texture tex, int samplerUnit) {
        ...
        if (u != null && tex != null) {
            ...
            PZGLUtil.checkGLErrorThrowTexture("Shader.setValue<Texture> Loc: %s, Tex: %s, samplerUnit: %d", loc, tex, samplerUnit);
        }
    }

    private void setUVScale(int samplerUnit, float x, float y) {
        if (samplerUnit < 0) {
            DebugType.Shader.error("SamplerUnit out of range: " + samplerUnit);
