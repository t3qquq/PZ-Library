...
public final class ModelInstanceTextureCreator extends TextureDraw.GenericDrawer {
    ...
    private static final ObjectPool<ModelInstanceTextureCreator> pool = new ObjectPool<>(ModelInstanceTextureCreator::new, "ModelInstanceTextureCreator.pool");
    ...
    public void init(AnimalVisual animalVisual, ModelInstance chrModelInstance) {
        ...
        if (!this.chrData.mask.isAllVisible() && !this.chrData.mask.isNothingVisible()) {
            ...
            Consumer<CharacterMask.Part> consumer = Lambda.consumer(maskFolder, this.texturesNotReady, (part, lMaskFolder, lTexturesNotReady) -> {
                Texture tex1 = Texture.getSharedTexture(lMaskFolder + "/" + part + ".png");
    ...
    public void init(HumanVisual humanVisual, ItemVisuals itemVisuals, ModelInstance chrModelInstance) {
        ...
        for (int i = itemVisuals.size() - 1; i >= 0; i--) {
            ...
            if (clothingItem == null) {
                ...
                for (int j = 0; j < clothingItem.masks.size(); j++) {
        ...
        for (int i = 0; i < BloodBodyPartType.MAX.index(); i++) {
            ...
            this.chrData.blood[i] = humanVisual.getBlood(bpt);
            this.chrData.dirt[i] = humanVisual.getDirt(bpt);
        }
        ...
        if (!this.chrData.mask.isAllVisible() && !this.chrData.mask.isNothingVisible()) {
            String maskFolder = this.chrData.maskFolder;
            Consumer<CharacterMask.Part> consumer = Lambda.consumer(maskFolder, this.texturesNotReady, (part, lMaskFolder, lTexturesNotReady) -> {
                Texture tex1 = getTextureWithFlags(lMaskFolder + "/" + part + ".png");
                if (tex1 != null && !tex1.isReady()) {
                    lTexturesNotReady.add(tex1);
                }
            });
        ...
        for (int i = 0; i < BloodBodyPartType.MAX.index(); i++) {
            ...
            String hole = "media/textures/HoleTextures/" + CharacterSmartTexture.MaskFiles[bpt.index()] + ".png";
            tex = getTextureWithFlags(hole);
            if (tex != null && !tex.isReady()) {
                this.texturesNotReady.add(tex);
            }

            String basicPatchMask = "media/textures/patches/" + CharacterSmartTexture.BasicPatchesMaskFiles[bpt.index()] + ".png";
            tex = getTextureWithFlags(basicPatchMask);
            if (tex != null && !tex.isReady()) {
                this.texturesNotReady.add(tex);
            }

            String denimPatchMask = "media/textures/patches/" + CharacterSmartTexture.DenimPatchesMaskFiles[bpt.index()] + ".png";
            tex = getTextureWithFlags(denimPatchMask);
            if (tex != null && !tex.isReady()) {
                this.texturesNotReady.add(tex);
            }

            String leatherPatchMask = "media/textures/patches/" + CharacterSmartTexture.LeatherPatchesMaskFiles[bpt.index()] + ".png";
        ...
        if (!this.rendered) {
            if (this.chrData.modelInstance != null) {
                ...
                try {
                    ...
                    for (int i = this.localItemData.size() - 1; i >= 0; i--) {
                        ModelInstanceTextureCreator.ItemData data = this.localItemData.get(i);
                        Texture itemTexture;
    ...
    private static final class ItemData {
        ...
        static final ObjectPool<ModelInstanceTextureCreator.ItemData> pool = new ObjectPool<>(ModelInstanceTextureCreator.ItemData::new, "ItemData.pool");
    }
}
