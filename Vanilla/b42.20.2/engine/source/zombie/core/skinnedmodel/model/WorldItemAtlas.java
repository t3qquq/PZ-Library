...
public final class WorldItemAtlas {
    ...
    private final ArrayList<WorldItemAtlas.Atlas> atlasListMain = new ArrayList<>();
    private final ConcurrentLinkedQueue<WorldItemAtlas.Atlas> newAtlasQ = new ConcurrentLinkedQueue<>();
    ...
    private final ObjectPool<WorldItemAtlas.ItemTextureDrawer> itemTextureDrawerPool = new ObjectPool<>(
        WorldItemAtlas.ItemTextureDrawer::new, "WorldItemAtlas.itemTextureDrawerPool"
    );
    private final ObjectPool<WorldItemAtlas.ItemTextureDepthDrawer> itemTextureDepthDrawerPool = new ObjectPool<>(
        WorldItemAtlas.ItemTextureDepthDrawer::new, "WorldItemAtlas.itemTextureDepthDrawerPool"
    );
    private final ObjectPool<WorldItemAtlas.WeaponPartParams> weaponPartParamPool = new ObjectPool<>(
        WorldItemAtlas.WeaponPartParams::new, "WorldItemAtlas.weaponPartParamPool"
    );
    ...
    private void updateAtlasList() {
        for (WorldItemAtlas.Atlas atlas = this.newAtlasQ.poll(); atlas != null; atlas = this.newAtlasQ.poll()) {
            this.atlasListMain.add(atlas);
        }
    }

    public void render() {
        this.updateAtlasList();

        for (int i = 0; i < this.atlasListMain.size(); i++) {
            WorldItemAtlas.Atlas atlas = this.atlasListMain.get(i);
            if (atlas.clear) {
        ...
        if (!this.renderJobs.isEmpty()) {
            for (int i = 0; i < this.renderJobs.size(); i++) {
                WorldItemAtlas.RenderJob job = this.renderJobs.get(i);
    ...
    public void Reset() {
        ...
        this.newAtlasQ.clear();
        this.atlasListMain.clear();
        ...
        boolean init(InventoryItem item, boolean bHighRes) {
            ...
            if (worldStaticItem != null) {
                ...
                if (clothing != null || item.getClothingItem() != null) {
                    ...
                    ImmutableColor clothingTint = item.isCustomColor()
                        ? new ImmutableColor(item.getColorRed(), item.getColorGreen(), item.getColorBlue())
                        : itemVisual.getTint(clothingItem);
                ...
                if (clothingItem != null
                    && itemVisual != null
                    && !StringUtils.isNullOrWhitespace(meshName)
                    ...
                    if (model != null && model.isReady() && model.mesh != null && model.mesh.isReady()) {
                        ...
                        ImmutableColor tint = item.isCustomColor()
                            ? new ImmutableColor(item.getColorRed(), item.getColorGreen(), item.getColorBlue())
                            : itemVisual.getTint(clothingItem);
    ...
    public static final class ItemTexture {
        ...
        public void render(float originX, float originY, float originZ, float screenX, float screenY, float r, float g, float b, float a) {
            if (PerformanceSettings.fboRenderChunk) {
                ...
                SpriteRenderer.instance
                    ...
                    .render(
                        this.entry.tex,
                        screenX - (this.entry.w / 2.0F - this.entry.offsetX) / maxZoom,
                        screenY - (this.entry.h / 2.0F - this.entry.offsetY) / maxZoom,
                        this.entry.w / maxZoom,
                        this.entry.h / maxZoom,
                        r,
                        g,
                        b,
                        a,
        ...
        public float getRenderX(float sx) {
            ...
            return sx - (this.entry.w / 2.0F - this.entry.offsetX) / maxZoom;
        }
        ...
        public float getRenderY(float sy) {
            ...
            return sy - (this.entry.h / 2.0F - this.entry.offsetY) / maxZoom;
        }
        ...
        public void render() {
            ...
            if (entry != null && entry.ready && entry.tex.isReady()) {
                ...
                if (DeadBodyAtlas.deadBodyAtlasShader.getShaderProgram().isCompiled()) {
                    ...
                    float x1 = this.x - (entry.w / 2.0F - entry.offsetX) / maxZoom;
                    float y1 = this.y - (entry.h / 2.0F - entry.offsetY) / maxZoom;
                    float w = entry.w / maxZoom;
                    float h = entry.h / maxZoom;
                    ...
                    float depthNear = IsoDepthHelper.getSquareDepthData(
                            PZMath.fastfloor(playerX), PZMath.fastfloor(playerY), this.ox + jx + 0.5F * f, this.oy + jy + 0.5F * f, this.oz + 1.0F
                        )
                        .depthStart;
                    float depthFar = IsoDepthHelper.getSquareDepthData(
                            PZMath.fastfloor(playerX), PZMath.fastfloor(playerY), this.ox + jx - 0.5F * f, this.oy + jy - 0.5F * f, this.oz
                        )
                        .depthStart;
    ...
    private static final class ItemTextureDrawer extends TextureDraw.GenericDrawer {
        ...
        public void render() {
            ...
            if (entry != null && entry.ready && entry.tex.isReady()) {
                ...
                int x = (int)(this.x - (entry.w / 2.0F - entry.offsetX) / maxZoom);
                int y = (int)(this.y - (entry.h / 2.0F - entry.offsetY) / maxZoom);
                int w = (int)(entry.w / maxZoom);
                int h = (int)(entry.h / maxZoom);
                entry.tex.bind();
                GL11.glBegin(7);
