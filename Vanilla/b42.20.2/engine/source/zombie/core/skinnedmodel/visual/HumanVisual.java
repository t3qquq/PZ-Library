...
public class HumanVisual extends BaseVisual {
    ...
    public float getBlood(BloodBodyPartType bodyPartType) {
        return (this.blood[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getDirt(BloodBodyPartType bodyPartType) {
        return (this.dirt[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getHole(BloodBodyPartType bodyPartType) {
        return (this.holes[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public void copyFrom(BaseVisual baseVisual) {
        if (baseVisual == null) {
            this.clear();
        } else if (baseVisual instanceof HumanVisual other) {
            other.getHairColor();
    ...
    private ItemVisual addClothingItem(ItemVisuals itemVisuals, List<ItemBodyLocation> itemVisualLocations, String clothingName, ClothingItemReference itemRef) {
        ...
        if (location == null) {
            DebugType.General.error("The game can't found location '" + locationId + "' for the item '" + scriptItem.name + "'");
            return null;
        }
        ...
        for (int i = 0; i < itemVisuals.size(); i++) {
            if (bodyLocationGroup.indexOf(itemVisualLocations.get(i)) > locationIndex) {
                insertAt = i;
                break;
            }
        }
