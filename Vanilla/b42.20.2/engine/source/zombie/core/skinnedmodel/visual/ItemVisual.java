...
public final class ItemVisual {
    ...
    public ClothingItem getClothingItem() {
        ...
        if (!StringUtils.isNullOrWhitespace(this.alternateModelName)) {
            ...
            if ("WithoutDrainable".equalsIgnoreCase(this.alternateModelName)) {
                Item withoutDrainableItem = ScriptManager.instance.getItem(scriptItem.getWithoutDrainable());
    ...
    public void getClothingItemCombinedMask(CharacterMask mask) {
        ClothingItem.tryGetCombinedMask(this.getClothingItem(), mask);
    }
    ...
    public ImmutableColor getTint(ClothingItem clothingItem) {
        ...
        if (clothingItem.allowRandomTint) {
            if (this.tint == null) {
    ...
    public void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        for (int i = 0; i < var8; i++) {
            byte amount = input.get();
            if (i < this.basicPatches.length) {
                this.basicPatches[i] = amount;
            }
        }
        ...
        for (int i = 0; i < var8; i++) {
            byte amount = input.get();
            if (i < this.denimPatches.length) {
                this.denimPatches[i] = amount;
            }
        }
        ...
        for (int i = 0; i < var8; i++) {
            byte amount = input.get();
            if (i < this.leatherPatches.length) {
                this.leatherPatches[i] = amount;
            }
        }
    }
    ...
    public float getDenimPatch(BloodBodyPartType bodyPartType) {
        return this.denimPatches == null ? 0.0F : (this.denimPatches[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getLeatherPatch(BloodBodyPartType bodyPartType) {
        return this.leatherPatches == null ? 0.0F : (this.leatherPatches[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getBasicPatch(BloodBodyPartType bodyPartType) {
        return this.basicPatches == null ? 0.0F : (this.basicPatches[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getHole(BloodBodyPartType bodyPartType) {
        return this.holes == null ? 0.0F : (this.holes[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getBlood(BloodBodyPartType bodyPartType) {
        return this.blood == null ? 0.0F : (this.blood[bodyPartType.index()] & 0xFF) / 255.0F;
    }

    public float getDirt(BloodBodyPartType bodyPartType) {
        return this.dirt == null ? 0.0F : (this.dirt[bodyPartType.index()] & 0xFF) / 255.0F;
    }
    ...
    public float getTotalBlood() {
        ...
        if (this.blood != null) {
            for (int i = 0; i < this.blood.length; i++) {
                total += (this.blood[i] & 0xFF) / 255.0F;
            }
        }
