...
public class Texture extends Asset implements IDestroyable, ITexture, Serializable {
    ...
    public static Texture getErrorTexture() {
        if (errorTexture == null) {
            ...
            RenderThread.invokeOnRenderContext(() -> {
                ...
                int numSquares = 8;
                int pixelsPerSquare = errorTexture.width / 8;
    ...
    public static Texture trygetTexture(String name) {
        ...
        if (texture == null) {
            if (StringUtils.containsDoubleDot(name)) {
                DebugType.FileIO.error("Failed to load texture: " + name);
                return null;
            }
    ...
    public void renderwallnw(float x, float y, float width, float height, int u, int d, int u2, int d2, int r, int r2) {
        ...
        float y1 = y - 96 * scale + height / 2.0F - 1.0F - 0.0F;
        float x2 = x + 0.0F;
        float y2 = y - 96 * scale - 2.0F - 0.0F;
        float x3 = x + 0.0F;
        ...
        y2 = y - 96 * scale + height / 2.0F - 0.0F;
    ...
    public void renderwallw(float x, float y, float width, float height, int u, int d, int u2, int d2) {
        ...
        float x1 = x - width / 2.0F - 0.0F;
        float y1 = y - 96 * scale + height / 2.0F - 1.0F - 0.0F;
        float x2 = x + scale + 0.0F;
        float y2 = y - 96 * scale - 3.0F - 0.0F;
        float x3 = x + scale + 0.0F;
    ...
    public void renderwalln(float x, float y, float width, float height, int u, int d, int u2, int d2) {
        ...
        float y1 = y - 96 * scale - 3.0F - 0.0F;
        float x2 = x + width / 2.0F + 0.0F;
        float y2 = y - 96 * scale + height / 2.0F - 0.0F;
    ...
    public void saveToZomboidDirectory(String filename) {
        if (StringUtils.containsDoubleDot(filename)) {
            throw new IllegalArgumentException("Unable to save to %s".formatted(this.name));
        }
    ...
    private void syncReadSize() {
        ...
        if (this.dataid != null && this.dataid.getPath() != null && this.dataid.getPath().isValid()) {
            fileName = this.dataid.getPath().getPath();
        }

        ZomboidFileSystem.instance.validatePrefix(fileName);
        pngSize.readSize(fileName);
