...
public class SmartTexture extends Texture {
    ...
    private static void addMaskFlags(SmartTexture tex, CharacterMask mask, String masksFolder, Texture base, int category) {
        Consumer<CharacterMask.Part> consumer = Lambda.consumer(
            ...
            base,
            category,
            (part, lTex, lMasksFolder, lBase, lCategory) -> lTex.mask(lBase, getTextureWithFlags(lMasksFolder + "/" + part + ".png"), lCategory)
        );
    ...
    private static void addMaskFlagsHue(SmartTexture tex, CharacterMask mask, String masksFolder, Texture base, int category, float hue) {
        Consumer<CharacterMask.Part> consumer = Lambda.consumer(
            ...
            category,
            hue,
            (part, lTex, lMasksFolder, lBase, lCategory, lHue) -> lTex.maskHue(lBase, getTextureWithFlags(lMasksFolder + "/" + part + ".png"), lCategory, lHue)
        );
        mask.forEachVisible(consumer);
    }
    ...
    private static void addMaskFlagsTint(SmartTexture tex, CharacterMask mask, String masksFolder, Texture base, int category, ImmutableColor tint) {
        Consumer<CharacterMask.Part> consumer = Lambda.consumer(
            ...
            category,
            tint,
            (part, lTex, lMasksfolder, lBase, lCategory, lTint) -> lTex.maskTint(
                lBase, getTextureWithFlags(lMasksfolder + "/" + part + ".png"), lCategory, lTint.r, lTint.g, lTint.b
            )
        );
        mask.forEachVisible(consumer);
    }
    ...
    public void calculate() {
        synchronized (this) {
            ...
            try {
                ...
                DebugType.General.error(e.getClass().getSimpleName() + " encountered while combining texture.");
                DebugType.General.error("Intended width : " + TextureCombiner.getResultingWidth(this.commands));
                DebugType.General.error("Intended height: " + TextureCombiner.getResultingHeight(this.commands));
                DebugType.General.error("");
                DebugType.General.error("Commands list: " + PZArrayUtil.arrayToString(this.commands));
                DebugType.General.error("");
                DebugType.General.error("Stack trace: ");
                ExceptionLogger.logException(e);
                DebugType.General.error("This SmartTexture will no longer be valid.");
                this.width = -1;
                this.height = -1;
                this.dirty = false;
                return;
