...
public class HLAStar extends AStar {
            ...
            if (initialNode.levelTransition == null || initialNode.levelTransition != goalNode1.levelTransition) {
                ...
                for (int pi = 0; pi < path.size(); pi++) {
                    ...
                    if (searchNode.chunkRegion != null) {
                        if (bRender && DebugOptions.instance.pathfindRenderClusters.getValue()) {
                            searchNode.chunkRegion.renderDebug();
                        }

                        this.addChunkLevelAndAdjacentToList(searchNode.chunkRegion, chunkList);
                    }
    ...
    void getSuccessors(HLSearchNode searchNode, ArrayList<ISearchNode> successors) {
        try (AbstractPerformanceProfileProbe var3 = PerfGetSuccessors.profile()) {
    ...
    void getSuccessorsInternal(HLSearchNode searchNode, ArrayList<ISearchNode> successors) {
        ...
        if (!searchNode.inUnloadedArea) {
            ...
            if (searchNode.levelTransition != null) {
                ...
                try (AbstractPerformanceProfileProbe var3 = PerfGetSuccessors_OnSameChunk.profile()) {
                    this.getSuccessorsOnSameChunk(searchNode, successors);
                }

                try (AbstractPerformanceProfileProbe var12 = PerfGetSuccessors_OnAdjacentChunks.profile()) {
                ...
                try (AbstractPerformanceProfileProbe var13 = PerfGetSuccessors_VisibilityGraphs.profile()) {
    ...
    void getSuccessorsGoingUp(HLSearchNode searchNode, ArrayList<ISearchNode> successors) {
        ...
        for (int i = 0; i < levelData1.slopedSurfaces.size(); i++) {
    ...
    void getSuccessorsFromVisibilityGraph(HLSearchNode searchNode, ArrayList<ISearchNode> successors) {
        ...
        for (int i = 0; i < vgNode.visible.size(); i++) {
    ...
    void addChunkLevelAndAdjacentToList(HLChunkRegion chunkRegion, ArrayList<HLChunkLevel> chunkList) {
        HLChunkLevel chunkLevel = chunkRegion.levelData;
        if (!chunkList.contains(chunkLevel)) {
            chunkList.add(chunkLevel);
        }

        HLSearchNode searchNode = this.nodeMapChunkRegion.get(chunkRegion);
        int wx = chunkLevel.getChunk().wx;
        int wy = chunkLevel.getChunk().wy;
        int level = chunkLevel.getLevel();
        ...
        for (int i = 0; i < DIRECTIONS.length; i++) {
            ...
            if (chunk2 != null && chunk2.isValidLevel(level)) {
                HLChunkLevel levelData2 = chunk2.getLevelData(level).getHighLevelData();
                if (searchNode != null) {
                    if (searchNode.successors.isEmpty() && this.goalNode.searchNode == searchNode) {
                        ArrayList<ISearchNode> successors = new ArrayList<>();
                        searchNode.getSuccessors(successors);
                        if (successors.stream().filter(iSearchNode -> {
                            HLSearchNode hlSearchNode = (HLSearchNode)iSearchNode;
                            return hlSearchNode.chunkRegion != null && hlSearchNode.chunkRegion.levelData == levelData2;
                        }).findFirst().isEmpty()) {
                            continue;
                        }
                    } else if (searchNode.successors
                        .stream()
                        .filter(hlSuccessor -> hlSuccessor.searchNode.chunkRegion != null && hlSuccessor.searchNode.chunkRegion.levelData == levelData2)
                        .findFirst()
                        .isEmpty()) {
                        continue;
                    }
