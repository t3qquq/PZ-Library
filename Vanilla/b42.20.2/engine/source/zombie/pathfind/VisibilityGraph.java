...
public final class VisibilityGraph {
    ...
    boolean isVisible(Node node1, Node node2) {
        ...
        for (int i = 0; i < this.perimeterEdges.size(); i++) {
            Edge edge = this.perimeterEdges.get(i);
            if (this.intersects(node1, node2, edge)) {
                return false;
            }
        }
                    ...
                    for (int y = minY; y < maxY; y++) {
                        for (int x = minX; x < maxX; x++) {
                            ...
                            if (squareCanPathInside != null && obstacle.isPointInside(x + 0.5F, y + 0.5F)) {
                                ...
                                for (int j = nodeInside.visible.size() - 1; j >= 0; j--) {
                                    Connection cxn = nodeInside.visible.get(j);
                                    if (cxn.has(1)) {
        ...
        for (int y = 0; y < bounds.h; y++) {
            for (int x = 0; x < bounds.w; x++) {
                ...
                if (f != null && (f.w || f.n || f.e || f.s || f.innerCorner)) {
                    Square square = PolygonalMap2.instance.getSquare(bounds.x + x, bounds.y + y, z);
                ...
                for (int j = i + 1; j < nodes1.size(); j++) {
                    ...
                    if (!node2.ignore && (node2.square == null || !node2.square.has(504)) && (!node1.hasFlag(8) || !node2.hasFlag(8))) {
                        if (node1.isConnectedTo(node2)) {
                            if ((node1.square == null || !node1.square.isCanPathW() && !node1.square.isCanPathN())
    ...
    void release() {
        ...
        for (int i = 0; i < this.perimeterEdges.size(); i++) {
            this.perimeterEdges.get(i).node1.release();
    ...
    void render() {
        ...
        for (Obstacle obstacle : this.obstacles) {
            ...
            if (DebugOptions.instance.pathfindRenderCrawling.getValue()) {
        ...
        for (Node node1 : this.perimeterNodes) {
            ...
            if (DebugOptions.instance.pathfindRenderNodes.getValue()) {
                float r1 = 1.0F;
                float g1 = 0.5F;
                float b1 = 0.0F;
                if (node1.ignore) {
        ...
        for (Node node1 : this.nodes) {
            ...
            if (DebugOptions.instance.pathfindRenderNodes.getValue() || node1.ignore) {
