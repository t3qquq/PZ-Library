...
public final class ItemContainer {
    ...
    private static final String TYPE_FRIDGE = "fridge";
    private static final String TYPE_FREEZER = "freezer";
    ...
    public void setCapacity(int capacity) {
        if (this.getVehiclePart() != null && this.capacity > 1000) {
            DebugType.General.warn("Attempting to set capacity of " + this + "over maximum capacity of 1000");
        } else if (this.containingItem != null && this.containingItem instanceof InventoryItem && this.capacity > 50) {
            DebugType.General.warn("Attempting to set capacity of " + this.containingItem + "over maximum capacity of 50");
        } else if (capacity > 100) {
            DebugType.General.warn("Attempting to set capacity of " + this + "over maximum capacity of 100");
        }
    ...
    public int getEffectiveCapacity(IsoGameCharacter chr) {
        ...
        if (chr != null && !(this.parent instanceof IsoGameCharacter) && !(this.parent instanceof IsoDeadBody) && !"floor".equals(this.getType())) {
            if (chr.hasTrait(CharacterTrait.ORGANIZED)) {
                return (int)Math.max(capacity * 1.3F, capacity + 1);
            }
    ...
    public boolean hasRoomFor(IsoGameCharacter chr, InventoryItem item) {
        if (chr != null && chr.getVehicle() != null && item.hasTag(ItemTag.HEAVY_ITEM) && this.parent instanceof IsoGameCharacter) {
            ...
            return chr != null
                ...
                : false;
        } else if (this.hasWorldItem() && item.isOnGroundOnSquare(this.getWorldItem().getSquare())) {
            return this.hasRoomFor(chr, item.getUnequippedWeight(), 0.0F);
        } else {
            return this.hasWorldItem() && item.isInsideBagOnSquare(this.getWorldItem().getSquare())
                ? this.hasRoomFor(chr, item.getUnequippedWeight(), 0.0F)
                : this.hasRoomFor(chr, item.getUnequippedWeight());
        }
    }
    ...
    public boolean hasRoomFor(IsoGameCharacter chr, float weightVal, float weightAddedToFloor) {
        ...
        if (chr != null
            ...
            && this.getWorldItem() instanceof IsoWorldInventoryObject worldItem
            && worldItem.getSquare() != null) {
            IsoGridSquare sq = worldItem.getSquare();
            float ground = sq.getTotalWeightOfItemsOnFloor();
            if (ground + weightAddedToFloor > 50.0F) {
                return false;
            }
        }
    ...
    public float getFreeCapacity(IsoGameCharacter chr) {
        return Math.max(this.getEffectiveCapacity(chr) - floatingPointCorrection(this.getCapacityWeight()), 0.0F);
    }
    ...
    private boolean isInvalidClothingRackItem(InventoryItem item) {
        return this.getType().equals("clothingrack") && !(item instanceof Clothing) && !item.hasTag(ItemTag.FITS_CLOTHING_RACK);
    }
    ...
    public boolean isItemAllowed(InventoryItem item) {
        ...
        if (this.isInvalidClothingRackItem(item)) {
            return false;
        }

        if (this.getParent() != null
            && this.getParent().getProperties() != null
            && this.getParent().getProperties().get(IsoPropertyType.CUSTOM_NAME) != null
            && this.getParent().getProperties().get(IsoPropertyType.CUSTOM_NAME).equals("Toaster")
        ...
        if (this.getParent() != null && this.getParent().getProperties() != null && this.getParent().getProperties().get(IsoPropertyType.GROUP_NAME) != null) {
            boolean coffeeMaker = this.getParent().getProperties().get(IsoPropertyType.GROUP_NAME).equals("Coffee")
                || this.getParent().getProperties().get(IsoPropertyType.GROUP_NAME).equals("Espresso");
    ...
    public InventoryItem AddItemBlind(InventoryItem item) {
        ...
        if (item.getWeight() + this.getCapacityWeight() > this.getCapacity()) {
            return null;
        }
    ...
    public InventoryItem findItem(String type, boolean doInv, boolean ignoreBroken) {
        if (type.contains("/")) {
            ...
            return ignoreBroken
                ...
                : this.findItem(type.trim(), (lType, item) -> compareType(lType, item) && !item.isBroken(), doInv);
        }
    }
    ...
    public boolean canHumanCorpseFit(IsoGameCharacter chr) {
        ...
        if (this.isVehicleSeat()) {
            ...
            if (this.isVehiclePart()) {
                float availableSpace = hasChr ? this.getEffectiveCapacity(chr) - this.getContentsWeight() : this.getAvailableWeightCapacity();
    ...
    public boolean canItemFit(InventoryItem item, IsoGameCharacter chr) {
        if (item.isHumanCorpse()) {
            return this.canHumanCorpseFit(chr);
        }

        float availableSpace = this.getAvailableWeightCapacity();
        float requiredSpace = item.getUnequippedWeight();
        return availableSpace >= requiredSpace;
    }
    ...
    public InventoryItem getBestType(String type, Comparator<InventoryItem> comparator) {
        ...
        try {
            return this.getBest(predicate, comparator);
        } finally {
    ...
    public InventoryItem getBestTypeRecurse(String type, Comparator<InventoryItem> comparator) {
        ...
        try {
            return this.getBestRecurse(predicate, comparator);
        } finally {
            TL_predicates.get().type.release(predicate);
        }
    }
    ...
    public ArrayList<InventoryItem> getAllRecurse(Predicate<InventoryItem> predicate) {
        ArrayList<InventoryItem> result = new ArrayList<>();
        return this.getAllRecurse(predicate, result);
    }
    ...
    private static boolean isSquarePowered(IsoGridSquare square, boolean includeGenerators) {
        if (square == null) {
            return false;
        }
        ...
        if (includeGenerators && square.haveElectricity()) {
            return true;
        }
    ...
    public static boolean isObjectPowered(IsoObject parent, boolean includeGenerators) {
        if (parent != null && parent.getObjectIndex() != -1) {
            ...
            if (tempObjects.isEmpty()) {
                ...
                return parent.getPipedFuelAmount() <= 0
                    ? isSquarePowered(sq, includeGenerators)
                    : includeGenerators && sq.haveElectricity() || sq.hasGridPower();
            } else {
                IsoLightSwitch lightSwitch = null;

                for (int i = 0; i < tempObjects.size(); i++) {
                    IsoObject object = tempObjects.get(i);
                ...
                for (int i = 0; i < tempObjects.size(); i++) {
                    ...
                    if (object.getPipedFuelAmount() > 0) {
                        return includeGenerators && testSq.haveElectricity() || testSq.hasGridPower();
                    }

                    if (isSquarePowered(testSq, includeGenerators)) {
                        return true;
                    }
                }
    ...
    public int getNumItems(String itemLike) {
        ...
        if (itemLike.contains("Type:")) {
            for (int i = 0; i < this.items.size(); i++) {
                ...
                if (item instanceof HandWeapon && itemLike.contains("Weapon")) {
                    ncount += item.getCurrentUses();
                }
            }
        } else {
    ...
    public int getItemCount(ItemKey type) {
        return this.getCountType(type.toString());
    }
    ...
    public String getOpenSound() {
        if (this.openSound == null) {
            if (this.getVehiclePart() instanceof VehiclePart part) {
                return part.getContainerOpenSound();
            } else {
                IsoObject parent1 = this.getParent();
                return parent1 != null && parent1.getSprite() != null ? parent1.getProperties().get(IsoPropertyType.CONTAINER_OPEN_SOUND) : null;
            }
        } else {
    ...
    public String getCloseSound() {
        if (this.closeSound == null) {
            if (this.getVehiclePart() instanceof VehiclePart part) {
                return part.getContainerCloseSound();
            } else {
                IsoObject parent1 = this.getParent();
                return parent1 != null && parent1.getSprite() != null ? parent1.getProperties().get(IsoPropertyType.CONTAINER_CLOSE_SOUND) : null;
            }
        } else {
    ...
    public String getPutSound() {
        if (this.putSound == null) {
            if (this.getVehiclePart() instanceof VehiclePart part) {
                return part.getContainerPutSound();
            } else {
                IsoObject parent1 = this.getParent();
                return parent1 != null && parent1.getSprite() != null ? parent1.getProperties().get(IsoPropertyType.CONTAINER_PUT_SOUND) : null;
            }
        } else {
    ...
    public String getTakeSound() {
        if (this.takeSound == null) {
            if (this.getVehiclePart() instanceof VehiclePart part) {
                return part.getContainerTakeSound();
            } else {
                IsoObject parent1 = this.getParent();
                return parent1 != null && parent1.getSprite() != null ? parent1.getProperties().get(IsoPropertyType.CONTAINER_TAKE_SOUND) : null;
            }
        } else {
    ...
    public VehiclePartOwner getVehiclePartOwner() {
        return this.isVehiclePart() ? this.getVehiclePart().getOwner() : null;
    }
    ...
    public VehiclePart getVehicleDoorPart() {
        ...
        if (part.isVehicleTrunk()) {
            VehiclePartOwner vehicle = this.getVehiclePartOwner();
            if (vehicle == null) {
                return null;
            }
    ...
    public IsoGridSquare getSquare() {
        ...
        return this.getWorldItem() instanceof IsoWorldInventoryObject worldItem ? worldItem.getSquare() : null;
    }

    public IsoWorldInventoryObject getWorldItem() {
        if (this.getContainingItem() != null) {
            IsoWorldInventoryObject var2 = this.getContainingItem().getWorldItem();
    ...
    public Vector2 getWorldPosition(Vector2 result) {
        ...
        if (vehicle != null && vehiclePart != null) {
            ...
            Vector2 partPos = vehicle.getAreaFacingPosition(vehiclePartArea, result);
            if (partPos != null) {
                return partPos;
            }
        }
    ...
    public List<InventoryItem> getSoapList(List<InventoryItem> result, boolean includeLiquidSoap) {
        ...
        for (InventoryItem item : this.items) {
            if (item.is(ItemKey.Drainable.SOAP_2)) {
                result.add(item);
            } else if (includeLiquidSoap) {
                FluidContainer fc = item.getFluidContainer();
                if (fc != null) {
                    float cleanRatio = fc.getRatioForFluid(Fluid.CleaningLiquid);
                    if (cleanRatio >= 0.499F && fc.getAmount() >= ZomboidGlobals.cleanStainCleaningFluidAmount) {
                        result.add(item);
                    }
                }
            }

            if (item instanceof InventoryContainer container && item.isEquipped()) {
                ItemContainer bag = container.getInventory();
                if (bag != null) {
    ...
    public boolean isFreezer() {
        return this.type != null && !"none".equals(this.type) ? this.type.equals(ContainerType.FREEZER.toString()) || this.type.equals("freezer") : false;
    }

    public boolean isFridge() {
        if (this.type == null || "none".equals(this.type)) {
            return false;
        } else if (this.isFreezer()) {
            return false;
        } else if (this.type.equals(ContainerType.FRIDGE.toString()) || this.type.equals("fridge")) {
            return true;
        } else {
    ...
    private static final class Comparators {
        ObjectPool<ItemContainer.ConditionComparator> condition = new ObjectPool<>(ItemContainer.ConditionComparator::new, "Comparators.condition");
        ObjectPool<ItemContainer.EvalComparator> eval = new ObjectPool<>(ItemContainer.EvalComparator::new, "Comparators.eval");
        ObjectPool<ItemContainer.EvalArgComparator> evalArg = new ObjectPool<>(ItemContainer.EvalArgComparator::new, "Comparators.evalArg");
    }
    ...
    private static final class InventoryItemListPool extends ObjectPool<ItemContainer.InventoryItemList> {
        public InventoryItemListPool() {
            super(ItemContainer.InventoryItemList::new, ItemContainer.InventoryItemList.class.getSimpleName());
        }
    ...
    private static final class Predicates {
        private final ObjectPool<ItemContainer.CategoryPredicate> category = new ObjectPool<>(ItemContainer.CategoryPredicate::new, "Predicates.category");
        private final ObjectPool<ItemContainer.EvalPredicate> eval = new ObjectPool<>(ItemContainer.EvalPredicate::new, "Predicates.eval");
        private final ObjectPool<ItemContainer.EvalArgPredicate> evalArg = new ObjectPool<>(ItemContainer.EvalArgPredicate::new, "Predicates.evalArg");
        private final ObjectPool<ItemContainer.TagPredicate> tag = new ObjectPool<>(ItemContainer.TagPredicate::new, "Predicates.tag");
        private final ObjectPool<ItemContainer.TagEvalPredicate> tagEval = new ObjectPool<>(ItemContainer.TagEvalPredicate::new, "Predicates.tagEval");
        private final ObjectPool<ItemContainer.TagEvalArgPredicate> tagEvalArg = new ObjectPool<>(
            ItemContainer.TagEvalArgPredicate::new, "Predicates.tagEvalArg"
        );
        private final ObjectPool<ItemContainer.TypePredicate> type = new ObjectPool<>(ItemContainer.TypePredicate::new, "Predicates.type");
        private final ObjectPool<ItemContainer.TypeEvalPredicate> typeEval = new ObjectPool<>(ItemContainer.TypeEvalPredicate::new, "Predicates.typeEval");
        private final ObjectPool<ItemContainer.TypeEvalArgPredicate> typeEvalArg = new ObjectPool<>(
            ItemContainer.TypeEvalArgPredicate::new, "Predicates.typeEvalArg"
        );
    }
