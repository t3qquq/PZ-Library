...
public class ItemRecipe {
    ...
    public static ItemRecipe Alloc(
        ...
        if (selectedItem != null && (selectedItem.getContainer() == null || !selectedItem.getContainer().contains(selectedItem))) {
            DebugType.Recipe.warn("recipe: item appears to have been used already, ignoring " + selectedItem.getFullType());
            selectedItem = null;
        }
    ...
    public ArrayList<InventoryItem> getSourceItems() {
        this.collectSourceItems();
        return new ArrayList<>(this.allSourceItems);
    }

    public ArrayList<InventoryItem> getSourceItems(int sourceIndex) {
        this.collectSourceItems();
        if (sourceIndex >= 0 && sourceIndex < this.sourceRecords.size()) {
            return new ArrayList<>(this.sourceRecords.get(sourceIndex).getCollectedItems());
        }

        DebugType.General.error("Index '" + sourceIndex + "' is not valid for recipe '" + this.recipe.getName() + "'.");
        return null;
    }
