...
public final class SurroundVehicle {
    private static final ObjectPool<SurroundVehicle.Position> s_positionPool = new ObjectPool<>(SurroundVehicle.Position::new, "SurroundVehicle.s_positionPool");
    ...
    public Vector2f getPositionForZombie(IsoZombie zombie, Vector2f out) {
        if ((!zombie.isOnFloor() || zombie.isCanWalk()) && PZMath.fastfloor(zombie.getZ()) == PZMath.fastfloor(this.vehicle.getZ())) {
            ...
            SurroundVehicle.Position position = this.getClosestPositionFor(zombie);
            if (position == null) {
                return null;
            }
