...
public final class VehiclePart extends GameEntity implements ChatElementOwner, WaveSignalDevice {
    protected VehiclePartOwner vehicle;
    ...
    protected VehicleEngine engine;
    ...
    public BaseVehicle getVehicle() {
        return Type.tryCastTo(this.vehicle, BaseVehicle.class);
    }
    ...
    public int getIndex() {
        return this.vehicle.getParts().indexOf(this);
    }
    ...
    public void setInventoryItem(InventoryItem item, int mechanicSkill) {
        ...
        this.getOwner().updateTotalMass();
        this.getOwner().updateDamageOverlayLater();
        if (this.isSetAllModelsVisible()) {
            this.setAllModelsVisible(item != null);
        }

        this.getOwner().updatePartStats();
        if (!GameServer.server) {
            this.getOwner().updateBulletStats();
        }
    }
    ...
    public void setRandomCondition(InventoryItem item) {
        ...
        if (this.getVehicle().isGoodCar()) {
            ...
            if (type != null) {
                max = PZMath.fastfloor(max * type.getRandomBaseVehicleQuality());
            }
    ...
    public String getContainerCloseSound() {
        return !this.isContainer() ? null : this.scriptPart.container.getSoundByID(SoundMapKey.CONTAINER_CLOSE);
    }

    public String getContainerOpenSound() {
        return !this.isContainer() ? null : this.scriptPart.container.getSoundByID(SoundMapKey.CONTAINER_OPEN);
    }

    public String getContainerPutSound() {
        return !this.isContainer() ? null : this.scriptPart.container.getSoundByID(SoundMapKey.CONTAINER_PUT);
    }

    public String getContainerTakeSound() {
        return !this.isContainer() ? null : this.scriptPart.container.getSoundByID(SoundMapKey.CONTAINER_TAKE);
    }
    ...
    public void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        if (zombie.scripting.objects.VehiclePart.ENGINE.toString().equals(this.partId)) {
            this.engine = new VehicleEngine(this);
            this.engine.addListener(this.getOwner());
        }
    ...
    public int getWheelIndex() {
        if (this.scriptPart != null && this.scriptPart.wheel != null) {
            for (int i = 0; i < this.vehicle.getScript().getWheelCount(); i++) {
                VehicleScript.Wheel scriptWheel = this.vehicle.getScript().getWheel(i);
    ...
    public void setLightActive(boolean active) {
        if (this.light != null && this.light.active != active) {
            ...
            this.vehicle.transmitPartLight(this);
        }
    }
    ...
    public void setCondition(int condition) {
        ...
        if (this.getOwner().getDriverRegardlessOfTow() != null) {
            if (this.condition > 60 && condition < 60 && condition > 40) {
                LuaEventManager.triggerEvent("OnVehicleDamageTexture", this.getOwner().getDriverRegardlessOfTow());
            }
        ...
        if (condition <= 0 && "lightbar".equals(this.getId())) {
            this.getOwner().setLightbarSirenMode(0);
        }
    ...
    public void damage(int amount) {
        if (this.getWindow() != null) {
            ...
            this.getOwner().transmitPartCondition(this);
        }
    }
