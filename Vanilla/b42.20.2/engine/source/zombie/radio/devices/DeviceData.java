...
public final class DeviceData implements Cloneable, IFMODParameterUpdater {
    ...
    public void generatePresets() {
        ...
        if (this.isTelevision) {
            ...
            if (category != null) {
                for (Entry<Integer, String> entry : category.entrySet()) {
                    if (entry.getKey() >= this.minChannelRange && entry.getKey() <= this.maxChannelRange && Rand.Next(1000) < radiochance) {
                        this.presets.addPreset(entry.getValue(), entry.getKey());
                    }
                }
            }
    ...
    protected Object clone() throws CloneNotSupportedException {
        ...
        c.setDevicePresets(new DevicePresets(this.getDevicePresets()));
        c.setParent(null);
        c.emitter = null;
    ...
    public void cloneDevicePresets(DevicePresets p) throws CloneNotSupportedException {
        this.presets.clearPresets();
        if (p != null) {
            for (int i = 0; i < p.getPresets().size(); i++) {
                PresetEntry entry = p.getPresets().get(i);
                this.presets.addPreset(entry.getName(), entry.getFrequency());
            }
        }
    }
    ...
    public void setIsTurnedOn(boolean b) {
        if (this.canBePoweredHere()) {
            if (this.isBatteryPowered && !(this.powerDelta > 0.0F)) {
                this.setIsTurnedOnInternal(false);
            } else {
                this.setIsTurnedOnInternal(b);
            }
    ...
    protected void updateEmitter() {
        if (!GameServer.server) {
            ...
            float distSq = isoObject == null
                ? Float.MAX_VALUE
                : FMODParameterUtils.getClosestListenerDistanceSquared(isoObject.getX(), isoObject.getY(), isoObject.getZ());
    ...
    private void sendDeviceDataStatePacket(UdpConnection connection, short type) {
        ...
        if (this.isIsoDevice()) {
            ...
            if (radio.getEquipParent() != null && radio.getEquipParent() instanceof IsoPlayer isoPlayer) {
                player = isoPlayer;
            } else if (radio.getContainer() != null && radio.getContainer().parent instanceof IsoPlayer isoPlayer) {
                player = isoPlayer;
            }
        ...
        if (validInfoHeader) {
            ...
            switch (type) {
                ...
                case 7:
                    ...
                    if (bb.putBoolean(this.mediaItem != null)) {
            ...
            switch (type) {
                case 0:
                    if (isServer && isIso) {
                        ...
                        this.setIsTurnedOnInternal(bb.getBoolean());
                    }
