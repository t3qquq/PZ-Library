...
public final class ZomboidRadio {
    ...
    public int getRandomFrequency(int rangemin, int rangemax) {
        int freq;
        do {
            freq = Rand.Next(rangemin, rangemax) / 200 * 200;
    ...
    public void Init(int savedWorldVersion) {
        ...
        if (bDebugEnabled) {
            DebugType.Radio.println();
            DebugType.Radio.println("################## Radio Init ##################");
        }
        ...
        if (mode == GameMode.Client) {
            ...
            if (bDebugEnabled) {
                DebugType.Radio.println("Radio (Client) loaded.");
                DebugType.Radio.println("################################################");
            }
            ...
            try {
                ...
                for (RadioData radioData : RadioData.fetchAllRadioData()) {
                    for (RadioChannel channel : radioData.getRadioChannels()) {
                        ...
                        if (found != null && (!found.getRadioData().isVanilla() || channel.getRadioData().isVanilla())) {
                            if (bDebugEnabled) {
                                DebugType.Radio.println("Unable to add channel: " + channel.GetName() + ", frequency '" + channel.GetFrequency() + "' taken.");
                            }
                        } else {
                ...
                if (savedWorldVersion == -1) {
                    if (bDebugEnabled) {
                        DebugType.Radio.println("Radio setting new game start times");
                    }
                    ...
                    if (months > 0) {
                        ...
                        if (bDebugEnabled) {
                            DebugType.Radio.println("Time since the apocalypse in days: " + this.daysSinceStart);
                        }
            ...
            if (bDebugEnabled) {
                if (success) {
                    DebugType.Radio.println("Radio loaded.");
                }
    ...
    private void checkGameModeSpecificStart() {
        if (Core.gameMode.equals("Initial Infection")) {
            ...
            for (Entry<Integer, RadioChannel> entry : this.scriptManager.getChannels().entrySet()) {
                ...
                } else if (entry.getValue().GetName().equals("NNR Radio")) {
            ...
            if ((mode == GameMode.Server || mode == GameMode.SinglePlayer) && this.scriptManager != null) {
                ...
                if (path.exists() && path.isDirectory()) {
                    ...
                    if (DebugLog.isEnabled(DebugType.Radio)) {
                        DebugType.Radio.println("Saving radio: " + fileName);
                    }
