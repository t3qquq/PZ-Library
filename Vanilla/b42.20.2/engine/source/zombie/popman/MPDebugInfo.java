...
public final class MPDebugInfo {
    ...
    public final ObjectPool<MPDebugInfo.MPCell> cellPool = new ObjectPool<>(MPDebugInfo.MPCell::new, "MPDebugInfo.cellPool");
    ...
    public final ObjectPool<MPDebugInfo.MPRepopEvent> repopEventPool = new ObjectPool<>(MPDebugInfo.MPRepopEvent::new, "MPDebugInfo.repopEventPool");
    ...
    public void render(ZombiePopulationRenderer renderer, float zoom) {
        ...
        for (int i = 0; i < this.loadedAreas.count; i++) {
            int n = i * 4;
        ...
        for (int i = 0; i < this.repopEvents.size(); i++) {
            MPDebugInfo.MPRepopEvent evt = this.repopEvents.get(i);
        ...
        if (zoom > 0.25F) {
            for (int i = 0; i < this.loadedCells.size(); i++) {
                MPDebugInfo.MPCell cell = this.loadedCells.get(i);
                renderer.renderCellInfo(cell.cx, cell.cy, cell.currentPopulation, cell.desiredPopulation, cell.lastRepopTime + 1.0F - worldAge);
            }
        }

        if (renderer.getBoolean("WorldSounds")) {
            try {
                this.renderWorldSounds(renderer);
            } catch (Exception var11) {
            }
        }
    }

    private void renderWorldSounds(ZombiePopulationRenderer renderer) {
