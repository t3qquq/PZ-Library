...
public final class ZombiePopulationRenderer {
    ...
    private final ZombiePopulationRenderer.BooleanDebugOption worldSounds = new ZombiePopulationRenderer.BooleanDebugOption("WorldSounds", false);
        ...
        if (this.cellInfo.getValue()) {
            ...
            this.currentDrawer
                .renderRectFilledUI(
                    ...
                    ty - 2.0F,
                    TextManager.instance.MeasureStringX(UIFont.Small, str) + 4,
                    TextManager.instance.MeasureStringY(UIFont.Small, str) + 4,
                    0.0F,
                    0.0F,
                    0.0F,
                    0.75F
    ...
    private void _render(UIElement ui, float zoom, float xPos, float yPos) {
                    ...
                    if (metaGrid.hasCell(xx - metaGrid.minX, y - metaGrid.minY)) {
                        ...
                        if (metaCell != null) {
                            for (int n = 0; n < metaCell.buildings.size(); n++) {
                                ...
                                for (int r = 0; r < def.getEmptyOutside().size(); r++) {
                                    if (def.getEmptyOutside().get(r).level <= 0) {
                                        ArrayList<RoomDef.RoomRect> rects = def.getEmptyOutside().get(r).getRects();
        ...
        if (this.cellGrid.getValue()) {
            ...
            for (int y = minCellY; y <= maxCellY; y += dxy) {
        ...
        if (bShowZombieIntensity) {
                            ...
                            for (int chunkY = 0; chunkY < 32; chunkY++) {
                                for (int chunkX = 0; chunkX < 32; chunkX++) {
                                    ...
                                    if (intensity > 0) {
                                        ...
                                        float b = 0.0F;
                                        float a = 0.9F;
                                        this.renderRect(cellX * 256 + chunkX * 8, cellY * 256 + chunkY * 8, 8.0F, 8.0F, r, 0.0F, 0.0F, 0.9F);
                                    }
                                }
                            }
                        }
            ...
            if (metaCell != null && metaCell.info != null) {
                int chunkX = (int)(worldX - cellX * 256) / 8;
                ...
                this.textDrawer
                    .renderRectFilledUI(
                        textX - 4.0F,
                        textY,
                        TextManager.instance.MeasureStringX(UIFont.Small, intensityStr) + 8,
                        TextManager.instance.MeasureStringY(UIFont.Small, intensityStr),
                        0.0F,
                        0.0F,
                        0.0F,
                        0.75F
        ...
        if (!GameClient.client && this.worldSounds.getValue()) {
            this.renderWorldSounds();
        }
    ...
    private void renderWorldSounds() {
        for (WorldSoundManager.WorldSound worldSound : WorldSoundManager.instance.soundList) {
            Color c = Colors.LightBlue;
            if (worldSound.sourceIsZombie) {
                c = Colors.GreenYellow;
            } else if (worldSound.stressAnimals && !worldSound.stresshumans) {
                c = Colors.Tan;
            } else if (worldSound.repeating) {
                c = Colors.Coral;
            }

            float a = 1.0F;
            this.renderCircle(worldSound.x, worldSound.y, worldSound.radius, c.r, c.g, c.b, 1.0F);
        }
    }
