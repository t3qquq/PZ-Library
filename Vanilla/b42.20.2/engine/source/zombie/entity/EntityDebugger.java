...
public class EntityDebugger {
    ...
    void reloadDebug() throws Exception {
        if (Core.debug && !GameClient.client && !GameServer.server) {
            if (this.engine.isProcessing()) {
                DebugType.General.println("Cannot reload entities when engine is processing.");
                return;
            }

            ImmutableArray<GameEntity> entities = this.engine.getIsoObjectBucket().getEntities();
            DebugType.General.println("-- Reloading '" + entities.size() + "' Instantiated IsoObject Entities --");
            if (entities.size() == 0) {
                return;
            }
            ...
            for (int i = 0; i < entities.size(); i++) {
                ...
                try {
                    DebugType.General.println("Reloading: " + gameEntity.getEntityFullTypeDebug());
    ...
    void reloadDebugEntity(GameEntity gameEntity) throws Exception {
        if (Core.debug && !GameClient.client && !GameServer.server) {
            ...
            DebugType.General.println("-- Reloading Entity --");
            if (gameEntity.getGameEntityType() != GameEntityType.IsoObject || !(gameEntity instanceof IsoObject)) {
                DebugType.General.println("Failed to reload entity, not IsoObject");
                return;
            }
    ...
    void reloadEntityFromScriptDebug(GameEntity gameEntity) throws Exception {
        if (Core.debug && !GameClient.client && !GameServer.server) {
            ...
            DebugType.General.println("-- Reloading Entity From Script --");
            if (gameEntity.getGameEntityType() != GameEntityType.IsoObject || !(gameEntity instanceof IsoObject)) {
                DebugType.General.println("Failed to reload entity from script, not IsoObject");
                return;
            }
            ...
            if (entityScript != null && entityScript.getScript() != null) {
                ...
                DebugType.General.warn("Failed to reload from script, no script component or component has no valid script.");
            }
        }
    }
}
