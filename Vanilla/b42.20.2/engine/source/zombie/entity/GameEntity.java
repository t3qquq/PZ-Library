...
public abstract class GameEntity {
    ...
    BitSet getBucketBits() {
        ...
        DebugType.General.error("Getting bucketBits while no Component data");
        if (Core.debug) {
    ...
    final boolean addComponent(Component component) {
        if (component == null) {
            DebugType.General.error("Trying to add 'null' component.");
            return false;
        }

        if (this.components != null && this.components.size() >= 127) {
            DebugType.General.error("Maximum component size '127' for entity reached.");
            return false;
        }
        ...
        if (!this.hasComponent(component.getComponentType())) {
            ...
            if (component.getOwner() != null) {
                DebugType.General.warn("Component added has not been removed from previous Entity.");
            ...
            DebugType.General.error("Trying to add component but component type already exists: " + component.getComponentType());
            return false;
        }
    }
    ...
    final Component removeComponent(ComponentType componentType) {
        if (!this.hasComponents()) {
            DebugType.General.error("Trying to remove component but has no components: " + componentType);
            return null;
        } else {
    ...
    final Component removeComponent(Component component) {
        if (component == null) {
            DebugType.General.error("Trying to remove 'null' component.");
            return null;
        } else if (!this.hasComponents()) {
            DebugType.General.error("Trying to remove component but has no components: " + component.getComponentType());
            return null;
    ...
    public final void sendRequestSyncGameEntity() {
        if (!GameClient.client) {
            DebugType.General.warn("Can only call on client.");
        } else {
            ...
            try {
                ...
                if (size > 0) {
                    for (int i = 0; i < size; i++) {
                        ...
                        component.loadSyncData(input.bb);
                        component.onAddedToOwner();
                    }
                }
