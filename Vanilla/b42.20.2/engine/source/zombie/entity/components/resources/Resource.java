...
public abstract class Resource {
    ...
    public void setDirty() {
        if (!GameClient.client) {
            ...
            } else if (Core.debug && !this.flags.contains(ResourceFlag.Temporary)) {
    ...
    public Resources getResourcesComponent() {
        if (this.resourcesComponent == null) {
            DebugType.CraftLogic.warn("ResourcesComponent (currently) not set!");
        }
    ...
    public boolean isTemporary() {
        return this.flags.contains(ResourceFlag.Temporary);
    }
    ...
    public void setProgress(double progress) {
        if (GameClient.client) {
            DebugType.General.warn("Not allowed on client");
        } else {
    ...
    public void sync() {
        if (GameServer.server) {
            INetworkPacket.sendToRelative(
                PacketTypes.PacketType.SyncEntityResource, this.getResourcesComponent().getOwner().getX(), this.getResourcesComponent().getOwner().getY(), this
            );
        }

        if (GameClient.client) {
            DebugType.General.warn("Resource.sync() function was called on client side");
        }
    }
}
