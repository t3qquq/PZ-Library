...
public class FluidUtil {
    ...
    public static String getFractionFormatted(float numerator, float denominator) {
        ...
        if (amount >= 1000.0F) {
            return String.format("%s / %s", df_liter1000.format(numerator), df_liter1000.format(denominator) + Translator.getText("IGUI_Liter"));
        } else if (amount >= 10.0F) {
            return String.format("%s / %s", df_liter10.format(numerator), df_liter10.format(denominator) + Translator.getText("IGUI_Liter"));
        } else {
            return amount >= 1.0F
                ? String.format("%s / %s", df_liter.format(numerator), df_liter.format(denominator) + Translator.getText("IGUI_Liter"))
                : String.format("%s / %s", Math.round(numerator * 1000.0F), Math.round(denominator * 1000.0F) + Translator.getText("IGUI_Milliliter"));
        }
    }

    public static String getAmountLiter1000(float amount) {
        return df_liter1000.format(amount) + Translator.getText("IGUI_Liter");
    }

    public static String getAmountLiter10(float amount) {
        return df_liter10.format(amount) + Translator.getText("IGUI_Liter");
    }

    public static String getAmountLiter(float amount) {
        return df_liter.format(amount) + Translator.getText("IGUI_Liter");
    }

    public static String getAmountMilli(float amount) {
        int ml = Math.round(amount * 1000.0F);
        return ml + Translator.getText("IGUI_Milliliter");
    }
