    ...
    public CraftRecipeMonitor seal() {
        if (!this.sealed) {
            ...
            if (this.openedBlocks > 0) {
                DebugType.General.warn("seal called but '" + this.openedBlocks + "' open blocks remain, auto resolving...");
    ...
    public void close() {
        if (this.canLog()) {
            if (this.openedBlocks > 0) {
                ...
                DebugType.General.warn("close called but no more opened blocks");
            }
        }
    }
    ...
    public void warn(String s) {
        if (this.canLog()) {
            ...
            if (this.printToConsole) {
                DebugType.General.debugln(this.lines.get(this.lines.size() - 1));
            }
        }
    }
