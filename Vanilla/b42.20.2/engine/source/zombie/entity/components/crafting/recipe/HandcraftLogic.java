...
public class HandcraftLogic extends BaseCraftingLogic {
    ...
    public void startCraftAction(KahluaTableImpl actionTable) {
        ...
        if (this.getRecipe() == actionTable.rawget("craftRecipe")) {
            ...
            if (items != null) {
                this.populateInputs(this.player, items, null, true);
            }
        }
    ...
    public void stopCraftAction(boolean stopAll) {
        ...
        if (stopAll) {
            this.numCraftActionInProgress = 0;
        }
    ...
    public void setRecipeFromContextClick(CraftRecipe recipe, InventoryItem inventoryItem) {
        ...
        this.populateInputs(this.player, this.allItems, this.sourceResources, false);
    ...
    public IsoObject findCraftSurface(IsoGameCharacter player, int radius) {
        IsoGridSquare currentSquare = player.getCurrentSquare();
        if (currentSquare == null) {
            return null;
        }

        int chrX = currentSquare.getX();
        int chrY = currentSquare.getY();
        int chrZ = currentSquare.getZ();

        for (int x = chrX - radius; x < chrX + radius; x++) {
            for (int y = chrY - radius; y < chrY + radius; y++) {
                IsoGridSquare square = IsoCell.getInstance().getGridSquare(x, y, chrZ);
                if (square != null && player.getSquare().canReachTo(square)) {
                    for (int i = 0; i < square.getObjects().size(); i++) {
                        IsoObject object = square.getObjects().get(i);
                        if (player.canUseAsGenericCraftingSurface(object)) {
                            return object;
                        }
                    }
                }
