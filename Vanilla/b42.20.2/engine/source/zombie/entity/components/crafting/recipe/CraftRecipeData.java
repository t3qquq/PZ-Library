...
public class CraftRecipeData {
    ...
    public boolean canOfferInputItem(InputScript inputScript, InventoryItem item, boolean verbose) {
        ...
        if (!this.recipe.containsIO(inputScript)) {
            if (verbose) {
                DebugType.CraftLogic.warn("Input script not part of current recipe.");
            }
            ...
            if (data == null) {
                if (verbose) {
                    DebugType.CraftLogic.warn("Data is null for input script");
                }

                return false;
            } else if (data.getInputScript().getResourceType() != ResourceType.Item) {
                if (verbose) {
                    DebugType.CraftLogic.warn("Cannot offer items to input scripts that are not ResourceType.Item");
                }
        ...
        for (CraftRecipe.LuaCall luaCall : CraftRecipe.LuaCall.values()) {
            if (this.recipe.hasLuaCall(luaCall)) {
                ...
                if (functionObject != null) {
                    ...
                    DebugType.CraftLogic.warn("Could not find lua function: " + this.recipe.getLuaCallString(luaCall));
                }
            }
        }
    ...
    public void processDestroyAndUsedItems(IsoGameCharacter character) {
        ...
        for (InventoryItem inventoryItem : this.consumedUsedItems) {
            ...
            DebugType.CraftLogic.println("post process -> Sync using item: " + inventoryItem.getFullType());
    ...
    private boolean consumeRecipeInputs(
        ...
        for (int i = 0; i < this.inputs.size(); i++) {
            ...
            if (data.inputScript.hasFlag(InputFlag.HandcraftOnly) && this.craftMode != CraftMode.Handcraft) {
                ...
                boolean consumed = false;
                List<InventoryItem> items = overrideInputItems != null ? overrideInputItems : data.inputItems;
                ...
                for (int i = 0; i < items.size(); i++) {
                    ...
                    if (inventoryItem.getScriptItem().getFullName().equals(inputItems.get(j).getFullName())
                        ...
                        if (CraftRecipeManager.consumeInputItem(input, inventoryItem, testOnly, cacheData, character)) {
                            ...
                            float useQueued = testOnly ? CraftRecipeManager.itemGetQueuedAmount(character, inventoryItem, cacheData) : 0.0F;
                            if (useQueued != Float.MAX_VALUE) {
                                uses = Math.max(uses - useQueued, 0.0F);
                                    ...
                                    switch (script.getResourceType()) {
                                        case Fluid:
                                            if (!testOnly) {
                                                ...
                                                uses = Math.max(inventoryItem.getFluidContainer().getAmount() - useQueued, 0.0F);
                                                requses = uses;
                                            }
                                    }
                                }
    ...
    private boolean consumeInputFromResources(
        ...
        Set<Resource> tempUsedResources = new HashSet<>(this.usedResources);
    ...
    private void processKeepInputItem(CraftRecipeData.InputScriptData inputData, IsoGameCharacter character, boolean testOnly, InventoryItem inventoryItem) {
        ...
        if (inputData.getInputScript().isKeep() && !testOnly) {
            DebugType.CraftLogic.debugln("Recipe is " + inputData.getRecipeData().getRecipe().getName());
            DebugType.CraftLogic.debugln("Item is " + inventoryItem.getType());
            InputScript input = inputData.getInputScript();
            ...
            if (GameServer.server && character instanceof IsoPlayer player && inventoryItem.isEquipped()) {
                INetworkPacket.sendToRelative(PacketTypes.PacketType.SyncItemActivated, character.getX(), character.getY(), player, inventoryItem);
            }
        }
    }
            ...
            if (testOnly) {
                ...
                for (int i = 0; i < createQty; i++) {
                    if (!CraftRecipeManager.createOutputItem(output, outputItem, testOnly, cacheData, character)) {
                        DebugType.CraftLogic.warn("Failed to create output item for: " + output.getOriginalLine());
                        if (this.craftRecipeMonitor != null) {
                    ...
                    if (cacheData.getMostRecentItem() != null) {
                        ...
                        if (inheritColorItem != null) {
                            item.setColorRed(inheritColorItem.getColor().r);
                            item.setColorGreen(inheritColorItem.getColor().g);
                            item.setColorBlue(inheritColorItem.getColor().b);
                            item.setColor(new Color(inheritColorItem.getColor().r, inheritColorItem.getColor().g, inheritColorItem.getColor().b));
                        ...
                        if (inheritConditionItem != null && !item.hasTag(ItemTag.DONT_INHERIT_CONDITION) && !output.hasFlag(OutputFlag.DontInheritCondition)) {
        ...
        if (!recipeInvalidated) {
            try {
                ...
                if (size != this.inputs.size()) {
                    DebugType.CraftLogic.warn("Recipe inputs changed or mismatch with saved data.");
                    valid = false;
                } else {
                ...
                this.hasConsumedInputs = input.get() != 0;
                if (worldVersion >= 235) {
    ...
    private void destroyAllSurvivingDestroyInputs() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            if (item != null) {
                DebugType.CraftLogic.debugln("Destroying surviving destroy input item " + item.getFullType());
        ...
        if (this.getRecipe() != null && this.getRecipe().containsIO(inputScript)) {
            ...
            if (inputScript.getResourceType() == ResourceType.Item) {
                ...
                for (InventoryItem inventoryItem : list) {
                    DebugType.CraftLogic.println("get m-input: " + inventoryItem.getFullType());
                }
            }
        ...
        if (this.getRecipe() != null && this.getRecipe().containsIO(inputScript)) {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                if ((inputItem.getContainer() != null || inputItem.getWorldItem() != null && inputItem.getWorldItem().getWorldObjectIndex() != -1)
                    ...
                    DebugType.CraftLogic.println("add m-input: " + list.get(i).getFullType());
                }
            }
    ...
    public void populateInputs(IsoGameCharacter player, List<InventoryItem> inputItems, List<Resource> resources, boolean clearExisting) {
    ...
    public static class InputScriptData extends CraftRecipeData.CacheData {
        ...
        public boolean isInputItemsSatisfied() {
            return this.inputScript.getResourceType() == ResourceType.Item && !this.inputItems.isEmpty()
                ? this.recipeData.consumeInputFromItems(this.inputScript, this.inputItems, true, null, true, this.recipeData.getCharacter())
                : false;
        }
        ...
        public boolean isInputItemsSatisifiedToMaximum() {
            if (this.inputScript.getResourceType() == ResourceType.Item
                ...
                int targetFillAmount = (int)Math.min(
                    this.inputScript.getAmount() * this.recipeData.targetVariableInputRatio, this.inputScript.getIntMaxAmount()
                );
        ...
        public boolean acceptsInputItem(InventoryItem inventoryItem) {
            if (this.inputScript.getResourceType() == ResourceType.Item) {
                return !this.inputItems.isEmpty() && this.inputItems.contains(inventoryItem)
                    ? false
                    : CraftRecipeManager.consumeInputItem(this.inputScript, inventoryItem, true, null, this.recipeData.getCharacter());
            } else {
                return false;
            }
        }
        ...
        public boolean addInputItem(InventoryItem inventoryItem) {
            if (this.inputScript.getResourceType() == ResourceType.Item) {
                ...
                DebugType.CraftLogic.warn("input script does not accept items, line=" + this.inputScript.getOriginalLine());
            }
