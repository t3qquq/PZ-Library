...
public class CraftLogicUILogic {
    ...
    private final List<InventoryItem> selectedInventoryItems = new ArrayList<>();
    ...
    public int getPossibleCraftCount(boolean forceRecache) {
        if (forceRecache || this.cachedPossibleCraftCount == -1) {
            ...
            List<InventoryItem> selectedItems = this.getRecipeData().getAllInputItems();
            this.cachedPossibleCraftCount = recipeData.getPossibleCraftCount(
                this.getCraftLogic().getInputResources(), selectedItems, new ArrayList<>(), new ArrayList<>(), false
            );
    ...
    public void offerInputItem(InventoryItem inventoryItem) {
        List<InventoryItem> itemsToTest = new ArrayList<>(this.selectedInventoryItems);
        itemsToTest.add(inventoryItem);
        if (this.getCraftLogic().willInputsAccommodate(itemsToTest)
            && this.getCraftLogic().canStartWithInventoryItems(this.player, itemsToTest)
            && this.getRecipeData().offerInputItem(this.manualSelectInputScriptFilter, inventoryItem)) {
    ...
    public void clearManualInputsFor(CraftRecipeData.InputScriptData input) {
        List<InventoryItem> existingLineItems = this.getRecipeData().getManualInputsFor(input.getInputScript(), new ArrayList<>());
    ...
    public KahluaTable getInventoryItemsToTransfer() {
        ...
        for (InventoryItem inventoryItem : this.selectedInventoryItems) {
            table.rawset(table.size() + 1, inventoryItem);
        }
