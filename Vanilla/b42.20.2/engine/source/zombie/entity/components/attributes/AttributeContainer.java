...
public class AttributeContainer extends Component {
    ...
    protected void readFromScript(ComponentScript componentScript) {
        ...
        if (attributesScript.getTemplateContainer() != null) {
            ...
            DebugType.General.error("Unable to create AttributeContainer from script: " + componentScript.getName());
        }
    }
        ...
        if (savedMode != 0) {
            ...
            if (savedMode == 1) {
                ...
                for (int i = 0; i < attrCount; i++) {
                    AttributeType type;
                    if (savedMode == 8) {
                        type = Attribute.TypeFromId(input.get());
                    } else {
                        type = Attribute.TypeFromId(input.getShort());
                    }
