...
public final class FBORenderTracerEffects {
    ...
    private final ObjectPool<FBORenderTracerEffects.Effect> effectPool = new ObjectPool<>(
        FBORenderTracerEffects.Effect::new, "FBORenderTracerEffects.effectPool"
    );
    private final ObjectPool<FBORenderTracerEffects.Drawer> drawerPool = new ObjectPool<>(
        FBORenderTracerEffects.Drawer::new, "FBORenderTracerEffects.drawerPool"
    );
    ...
    public void storeWeaponTransform(IsoGameCharacter chr, Matrix4f xfrm) {
        if (DebugOptions.instance.fboRenderChunk.bulletTracers.getValue()) {
            ...
            if (chr != null && chr.primaryHandModel != null && chr.primaryHandModel.modelScript != null) {
                ModelAttachment attachment = chr.primaryHandModel.modelScript.getAttachmentById(ModelAttachmentId.MUZZLE);
                if (attachment != null) {
    ...
    private static final class Drawer extends TextureDraw.GenericDrawer {
        ...
        static Matrix4f calculateProjectViewXfrm(Matrix4f projection, Matrix4f view, boolean renderThread) {
            ...
            float offscreenWidth = renderThread ? cam.offscreenWidth : IsoCamera.getOffscreenWidth(playerIndex);
            float offscreenHeight = renderThread ? cam.offscreenHeight : IsoCamera.getOffscreenHeight(playerIndex);
        ...
        static void calculateModelXfrm(float ox, float oy, float oz, float useangle, boolean vehicle, Matrix4f model, boolean renderThread) {
            int playerIndex = renderThread ? SpriteRenderer.instance.getRenderingPlayerIndex() : IsoCamera.frameState.playerIndex;
            float cx = renderThread ? Core.getInstance().floatParamMap.get(0) : IsoCamera.frameState.camCharacterX;
            float cy = renderThread ? Core.getInstance().floatParamMap.get(1) : IsoCamera.frameState.camCharacterY;
            float cz = renderThread ? Core.getInstance().floatParamMap.get(2) : IsoCamera.frameState.camCharacterZ;
            double x = cx;
            double y = cy;
            double z = cz;
            PlayerCamera cam = renderThread ? SpriteRenderer.instance.getRenderingPlayerCamera(playerIndex) : IsoCamera.cameras[playerIndex];
