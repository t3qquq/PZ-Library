...
public class IsoWorldInventoryObject extends IsoObject implements IItemProvider {
    ...
    public void loadChange(IsoObjectChange change, ByteBufferReader bb) {
        if (change == IsoObjectChange.SWAP_ITEM) {
            try {
                InventoryItem newItem = InventoryItem.loadItem(bb.bb, 249);
                if (newItem != null) {
        ...
        if (this.getItem().getScriptItem().isWorldRender()) {
            ...
            if (status == ItemModelRenderer.RenderStatus.Loading || status == ItemModelRenderer.RenderStatus.Ready) {
                ...
                if (tex != null) {
                    ...
                    this.sprite
                        .render(
                            this, x + this.xoff, y + this.yoff, z + this.zoff, this.getForwardIsoDirection(), this.offsetX + dx, this.offsetY + dy, col, true
                        );
                    col.set(r, g, b, a);
    ...
    public float getScreenPosY(int playerIndex) {
        Texture tex = this.sprite == null ? null : this.sprite.getTextureForCurrentFrame(this.getForwardIsoDirection(), this);
