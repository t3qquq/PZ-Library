...
public final class IsoFeedingTrough extends IsoObject {
    ...
    public void handleBurning() {
        ...
        if (this.hasSpriteGrid()) {
            for (IsoObject object : this.getSpriteGridObjectsExcludingSelf(new ArrayList<>())) {
                if (object instanceof IsoFeedingTrough trough) {
                    trough.removeFromWorld();
                    trough.getSquare().transmitRemoveItemFromSquare(trough);
                }

                if (object.getSquare() != null) {
                    burnSquares.add(object.getSquare());
                }
            }
        }

        this.removeFromWorld();
        IsoObjectUtils.safelyRemoveTileObjectFromSquare(this);
