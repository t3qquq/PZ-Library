...
public final class ClothingWasherLogic implements IClothingWasherDryerLogic {
                    ...
                    if (elapsedMinutes >= 1) {
                        ...
                        for (int i = 0; i < this.getContainer().getItems().size(); i++) {
                            ...
                            if (item instanceof Clothing clothing) {
                                ...
                                float dirtyness = clothing.getDirtiness();
                                if (dirtyness > 0.0F) {
                                ...
                                if (GameServer.server) {
                                    GameServer.sendSyncItemFields(clothing);
                                }
                            } else {
                                boolean modDataChanged = false;
                                if (item.getModData().rawget("ItemAfterCleaning") != null) {
                                    ...
                                    if (cleaningProgress == null) {
                                        ...
                                        if (cleaningProgress >= 100.0) {
                                            InventoryItem replacement = InventoryItemFactory.CreateItem(resultItem);
                                            this.getContainer().getItems().remove(item);
                                            this.getContainer().addItem(replacement);
                                            if (GameServer.server) {
                                                GameServer.sendRemoveItemFromContainer(this.getContainer(), item);
                                                GameServer.sendAddItemToContainer(this.getContainer(), replacement);
                                            }
                                ...
                                boolean containerBloodChanged = false;
                                if (item instanceof InventoryContainer invContainer && invContainer.getBloodLevel() > 0.0F) {
                                    ...
                                    containerBloodChanged = true;
                                }

                                if ((modDataChanged || containerBloodChanged) && GameServer.server) {
    ...
    private boolean cycleFinished() {
        if (this.isActivated()) {
            ...
            this.setActivated(false);
            if (GameServer.server) {
                this.getObject().sendObjectChange(IsoObjectChange.WASHER_STATE);
            }
        }
