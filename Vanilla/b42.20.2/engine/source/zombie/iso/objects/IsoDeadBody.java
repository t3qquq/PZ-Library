...
public final class IsoDeadBody
    ...
    private final List<AnimalGene> animalGenome = new ArrayList<>();
    private final List<String> animalGeneticDisorder = new ArrayList<>();
    ...
    private InventoryItem createdCorpseItem;
    ...
    public IsoDeadBody(IsoGameCharacter died, boolean wasCorpseAlready, boolean bAddToSquareAndWorld) {
        ...
        IsoGridSquare realSquare = IsoWorld.instance.currentCell.getGridSquare(died.getX(), died.getY(), died.getZ());
        if (GameServer.server && realSquare != null && sq != realSquare) {
            sq = realSquare;
        }
        ...
        if (died.getZ() < -32.0F) {
            DebugType.Death.error("invalid z-coordinate %.2f,%.2f,%.2f", died.getX(), died.getY(), died.getZ());
            died.setZ(0.0F);
        }
        ...
        this.setForwardIsoDirection(died.getForwardIsoDirection());
        ...
        if (bAddToSquareAndWorld && !this.wasZombie && !(died instanceof IsoAnimal)) {
            ...
            for (int i = 0; i < movingObj.size(); i++) {
                ...
                ((IsoZombie)movingObj.get(i)).bodyToEat = this;
            }
        }
    ...
    public List<AnimalGene> getAnimalGenome() {
        return this.animalGenome;
    }

    public List<String> getAnimalGeneticDisorder() {
        return this.animalGeneticDisorder;
    }
    ...
    public InventoryItem getItem() {
        if (this.createdCorpseItem != null) {
            return this.createdCorpseItem;
        }

        InventoryItem item = InventoryItemFactory.CreateItem(this.getCorpseItemType());
        if (this.isAnimal()) {
            item.copyModData(this.getModData());
            String icon = this.isAnimalSkeleton() ? this.skelInvIcon : this.invIcon;
            item.setIcon(Texture.getSharedTexture(icon));
            Texture rottenIcon = Texture.getSharedTexture(this.invIcon + "Rotten");
            if (rottenIcon != null) {
                item.setTexturerotten(rottenIcon);
            } else {
                item.setTexturerotten(Texture.getSharedTexture(icon));
            }
        ...
        this.createdCorpseItem = item;
        return item;
    }

    private String getCorpseItemType() {
        if (this.isAnimal()) {
            return "Base.CorpseAnimal";
        } else {
            return this.isFemale() ? "Base.CorpseFemale" : "Base.CorpseMale";
        }
    }
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        if (input.get() != 0) {
            ...
            if (worldVersion >= 246) {
                this.loadAnimalGenetics(input, worldVersion, isDebugSave);
            }
        ...
        this.forwardDirection.setDirection(this.angle);
    ...
    public void save(ByteBuffer output, boolean isDebugSave) throws IOException {
        ...
        if (this.isAnimal()) {
            ...
            this.saveAnimalGenetics(output, isDebugSave);
    ...
    private void saveAnimalGenetics(ByteBuffer output, boolean isDebugSave) throws IOException {
        output.put((byte)this.animalGenome.size());

        for (AnimalGene gene : this.animalGenome) {
            gene.save(output, isDebugSave);
        }

        output.put((byte)this.animalGeneticDisorder.size());

        for (int i = 0; i < this.animalGeneticDisorder.size(); i++) {
            GameWindow.WriteString(output, this.animalGeneticDisorder.get(i));
        }
    }

    private void loadAnimalGenetics(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        this.animalGenome.clear();
        int geneCount = input.get() & 255;

        for (int i = 0; i < geneCount; i++) {
            AnimalGene gene = new AnimalGene();
            gene.load(input, worldVersion, isDebugSave);
            this.animalGenome.add(gene);
        }

        this.animalGeneticDisorder.clear();
        int disorderCount = input.get() & 255;

        for (int i = 0; i < disorderCount; i++) {
            this.animalGeneticDisorder.add(GameWindow.ReadString(input));
        }
    }
    ...
    public void saveChange(IsoObjectChange change, KahluaTable tbl, ByteBufferWriter bb) {
        ...
        } else if (change == IsoObjectChange.ANIMAL_ROT_STAGE) {
            bb.putInt(this.getAnimalVisual().animalRotStage);
    ...
    public void loadChange(IsoObjectChange change, ByteBufferReader bb) {
        if (change == IsoObjectChange.BECOME_SKELETON) {
            ...
            this.getAnimalVisual().animalRotStage = bb.getInt();
            this.getModData().rawset("animalRotStage", (double)this.getAnimalVisual().animalRotStage);
            this.atlasTex = null;
        } else if (change == IsoObjectChange.ZOMBIE_ROT_STAGE) {
    ...
    public void checkClothing(InventoryItem removedItem) {
        ...
        for (int i = 0; i < this.attachedItems.size(); i++) {
            InventoryItem item = this.attachedItems.getItemByIndex(i);
            if (this.container == null || this.container.getItems().indexOf(item) == -1) {
    ...
    public void addToWorld() {
        ...
        if (!GameClient.client) {
            if (this.reanimateTime > 0.0F) {
                ...
                DebugType.Death.debugln("reanimate: addToWorld reanimateTime=" + this.reanimateTime + this);
            }
                ...
                for (int i = 0; i < tempBodies.size(); i++) {
                    ...
                    if (body.getHumanVisual() != null) {
                        ...
                        if (!body.updateFakeDead() && (ServerOptions.instance.removePlayerCorpsesOnCorpseRemoval.getValue() || body.wasZombie)) {
                            ...
                            if (!(age < hoursForCorpseRemoval + (body.isSkeleton() ? hoursPerRotStage : 0.0F))) {
                                int stages = (int)(age / hoursPerRotStage);
                                DebugType.Death.noise("%s REMOVE %d -> %d age=%.2f stages=%d", body, stageOld, stageNew, age, stages);
                                if (GameServer.server) {
    ...
    public void changeRotStage(int newStage) {
        if (newStage <= 4) {
            int rotStageDueToAge = this.getAnimalVisual().animalRotStage - this.animalRotStageAtDeath;
            this.getAnimalVisual().animalRotStage = rotStageDueToAge + newStage;
            this.animalRotStageAtDeath = newStage;
            if (GameServer.server) {
                this.sendObjectChange(IsoObjectChange.ANIMAL_ROT_STAGE);
            } else {
                this.atlasTex = null;
                this.invalidateCorpse();
                this.invalidateRenderChunkLevel(2L);
    ...
    private void updateRotting(float worldAge, float hoursPerRotStage) {
        if (!this.isSkeleton()) {
            ...
            if (newStage <= 3 && newStage != this.getHumanVisual().zombieRotStage) {
                ...
                DebugType.Death.noise("%s zombieRotStage %d -> %d age=%.2f stages=%d", this, this.getHumanVisual().zombieRotStage, newStage, age, stages);
                            ...
                            if (this.wasZombie) {
                                ...
                                if (Rand.Next(maggotChance * 2) == 0 && SandboxOptions.instance.maggotSpawn.getValue() != 2) {
                                    ...
                                    if (maggot != null && this.getSquare() != null) {
                                        this.getSquare().AddWorldInventoryItem(maggot, Rand.Next(10) / 10, Rand.Next(10) / 10, 0.0F);
                                        if (maggot instanceof Food food) {
                                            food.setPoisonPower(5);
                                        }
                                    }
                ...
                if (stages == 3 && Rand.NextBool(7)) {
                    DebugType.Death.noise("%s zombieRotStage %d -> x age=%.2f stages=%d", this, this.getHumanVisual().zombieRotStage, age, stages);
                    this.getHumanVisual().setBeardModel("");
                    this.getHumanVisual().setHairModel("");
    ...
    public void setReanimateTime(float hours) {
        ...
        if (!GameClient.client) {
            ...
            if (this.reanimateTime > 0.0F) {
                cell.addToStaticUpdaterObjectList(this);
            } else {
    ...
    public IsoGameCharacter reanimate() {
        ...
        zombie.setDir(this.getForwardIsoDirection());
        ...
        if (GameServer.server) {
            ...
            zombie.getNetworkCharacterAI().reanimatedBodyId.set(this.id);
        }
        ...
        try {
            zombie.getAdvancedAnimator().update(GameTime.getInstance().getTimeDelta());
        } finally {
    ...
    private IsoAnimal reanimateAnimal() {
        ...
        animal.copyGenome(this.getAnimalGenome());
        animal.copyGeneticDisorder(this.getAnimalGeneticDisorder());
        ...
        animal.setDir(this.getForwardIsoDirection());
    ...
    public void setAnimalData(IsoAnimal died) {
        ...
        for (AnimalGene gene : died.getFullGenomeList()) {
            this.animalGenome.add(new AnimalGene(gene));
        }

        this.animalGeneticDisorder.clear();
        this.animalGeneticDisorder.addAll(died.getGeneticDisorder());
    ...
    public String getInvIcon() {
        if (this.isAnimal()) {
            return switch (this.getAnimalVisual().animalRotStage) {
                case -1, 0 -> this.invIcon;
                case 1 -> this.invIcon + "Rotten";
                default -> this.skelInvIcon;
            };
        } else {
            return this.invIcon;
        }
    ...
    public InventoryItem becomeCorpseItem(boolean isRemote) {
        ...
        this.square.removeCorpse(this, isRemote);
    ...
    public boolean canBeGrabbedFrom(float x, float y) {
        return this.canBeGrabbed() && canPickUpBodyFromSquare(this.getCell().getGridSquare(x, y, this.getZ()), this.getSquare());
    }

    public boolean canBeGrabbed() {
        return this.getCell() != null && this.getSquare() != null && !this.isAnimal() && this.getStaticMovingObjectIndex() > -1;
    }

    public static boolean canPickUpBodyFromSquare(IsoGridSquare fromSquare, IsoGridSquare toSquare) {
        return fromSquare != null && fromSquare.canReachTo(toSquare) && !toSquare.haveFire();
    }
}
