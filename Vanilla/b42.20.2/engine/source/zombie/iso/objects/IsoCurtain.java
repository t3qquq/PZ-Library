    ...
    public void ToggleDoor(IsoGameCharacter chr) {
        if (!this.barricaded) {
            ...
            if (!this.locked || chr == null || chr.getCurrentSquare().getRoom() != null || this.open) {
                ...
                if (chr != null) {
                    String sound = this.getSoundPrefix() + (this.open ? "Open" : "Close");
                    if (GameServer.server) {
                        GameServer.sendCharacterSound(chr, sound, (byte)0);
                    } else {
                        chr.playSound(sound);
                    }
                }
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoAttached, boolean bWallLightingPass, Shader shader) {
        ...
        for (int i = 0; i < this.getSquare().getObjects().size(); i++) {
            ...
            if (!(object instanceof IsoWorldInventoryObject)) {
                ...
                if (sprite != null
                    && sprite.getTileType() != IsoObjectType.curtainN
                    && sprite.getTileType() != IsoObjectType.curtainS
                    && sprite.getTileType() != IsoObjectType.curtainW
                    && sprite.getTileType() != IsoObjectType.curtainE) {
