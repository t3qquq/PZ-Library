...
public class IsoCarBatteryCharger extends IsoObject implements IItemProvider {
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoChild, boolean bWallLightingPass, Shader shader) {
        if (!PerformanceSettings.fboRenderChunk || !Core.getInstance().isOption3DGroundItem()) {
            ...
            if (!this.chargerSprite.hasNoTextures()) {
                Texture tex = this.chargerSprite.getTextureForCurrentFrame(this.getForwardIsoDirection(), this);
                if (tex != null) {
    ...
    public void syncIsoObjectReceive(ByteBufferReader bb) {
        if (bb.getBoolean()) {
            try {
                this.battery = InventoryItem.loadItem(bb.bb, 249);
            } catch (Exception ex) {
                DebugType.General.printException(ex, LogSeverity.Error);
            }
        } else {
    ...
    public float getGeneratorPowerConsumption() {
        return this.isActivated() ? 0.05F : 0.0F;
    }
