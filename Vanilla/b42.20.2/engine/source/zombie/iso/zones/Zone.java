...
public class Zone {
    ...
    public boolean intersects(int x, int y, int z, int w, int h) {
        if (z != Integer.MAX_VALUE && this.z != z) {
            return false;
        }
    ...
    public boolean difference(int x, int y, int z, int w, int h, ArrayList<Zone> result) {
        ...
        if (this.isRectangle()) {
            if (this.x < x) {
                ...
                result.add(new Zone(this.name, this.type, this.x, top, this.z, x - this.x, bottom - top));
            }

            if (x + w < this.x + this.w) {
                int top = Math.max(y, this.y);
                int bottom = Math.min(y + h, this.y + this.h);
                result.add(new Zone(this.name, this.type, x + w, top, this.z, this.x + this.w - (x + w), bottom - top));
            }

            if (this.y < y) {
                result.add(new Zone(this.name, this.type, this.x, this.y, this.z, this.w, y - this.y));
            }

            if (y + h < this.y + this.h) {
                result.add(new Zone(this.name, this.type, this.x, y + h, this.z, this.w, this.y + this.h - (y + h)));
            }
