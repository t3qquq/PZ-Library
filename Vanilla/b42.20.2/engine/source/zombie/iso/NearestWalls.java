...
public class NearestWalls {
    ...
    private static int getNearestWallWest(IsoChunk chunk, int x, int y, int z, boolean exterior) {
        ...
        for (int d = 1; d <= 3; d++) {
            ...
            int y2 = y;
            dist = getNearestWallOnSameChunk(chunk2, x2, y2, z, 0, exterior);
            if (dist != -1) {
                return x2 - dist;
            }
        }
    ...
    private static int getNearestWallEast(IsoChunk chunk, int x, int y, int z, boolean exterior) {
        ...
        for (int d = 1; d <= 3; d++) {
            ...
            int x2 = chunk2.wx * 8;
            int y2 = y;
            dist = hasWall(chunk2, x2, y2, z, 0, exterior) ? 0 : getNearestWallOnSameChunk(chunk2, x2, y2, z, 1, exterior);
            if (dist != -1) {
                return x2 + dist;
            }
        }
    ...
    private static int getNearestWallNorth(IsoChunk chunk, int x, int y, int z, boolean exterior) {
        ...
        for (int d = 1; d <= 3; d++) {
            ...
            dist = getNearestWallOnSameChunk(chunk2, x2, y2, z, 2, exterior);
            if (dist != -1) {
                return y2 - dist;
            }
        }
    ...
    private static int getNearestWallSouth(IsoChunk chunk, int x, int y, int z, boolean exterior) {
        ...
        for (int d = 1; d <= 3; d++) {
            ...
            int x2 = x;
            int y2 = chunk2.wy * 8;
            dist = hasWall(chunk2, x2, y2, z, 2, exterior) ? 0 : getNearestWallOnSameChunk(chunk2, x2, y2, z, 3, exterior);
            if (dist != -1) {
                return y2 + dist;
            }
        }
