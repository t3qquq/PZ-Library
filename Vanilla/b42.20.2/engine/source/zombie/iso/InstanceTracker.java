...
public abstract class InstanceTracker {
    ...
    public static void exportFile(List<String> groups, String filename, InstanceTracker.Format format, InstanceTracker.Sort sort) {
        if (StringUtils.containsDoubleDot(filename)) {
            throw new IllegalArgumentException("Unable to export to: %s".formatted(filename));
        }

        if (!filename.isBlank() && !new File(filename).isAbsolute()) {
        ...
        if (!GameClient.client && !Core.getInstance().isNoSave()) {
            try {
                ...
                try {
                    ...
                    DebugType.General.debugln("InstanceTracker Overflow");
                    ExceptionLogger.logException(ex);
                    return;
                }
