...
public class BentFences {
    ...
    public void collapse(IsoObject obj, IsoDirections dir, BentFences.Entry entry, int index) {
        ...
        for (int i = 0; i < tripZombies.size(); i++) {
            if (tripZombies.get(i) instanceof IsoZombie zombie && !zombie.isObjectBehind(obj)) {
        ...
        for (int i = 0; i < crushZombies.size(); i++) {
            if (crushZombies.get(i) instanceof IsoZombie zombie) {
                zombie.setOnFloor(true);
                zombie.setFallOnFront(zombie.isObjectBehind(obj));
                zombie.setHitReaction("Floor");
            } else {
                IsoPlayer player = Type.tryCastTo(crushZombies.get(i), IsoPlayer.class);
                if (player != null) {
        ...
        if (bendStage != -1 && (bending || bendStage != 0)) {
            ...
            if (bendStage != entry.stages.size() || !bending) {
                ...
                if (tiles != null && swapTiles != null && baseTiles != null && tiles.size() == swapTiles.size()) {
                    ...
                    for (int i = 0; i < squaresToUpdate.size(); i++) {
                        doSquareUpdateTasks(squaresToUpdate.get(i));
                    }
    ...
    public void bendFence(IsoObject obj, IsoDirections dir) {
        ...
        if (entry != null) {
            ...
            SoundKey thumpSound = obj.getProperties().get("ThumpSound") != null
                ? SoundKey.get(ResourceLocation.of(obj.getProperties().get("ThumpSound")))
                : SoundKey.ZOMBIE_THUMP_GENERIC;
            this.swapTiles(obj, dir, true);
            int bendStageNew = this.getBendStage(obj, this.getEntryForObject(obj));
            if (bendStageNew > bendStageOld) {
                SoundKey soundName = IsoThumpable.DEFAULT_BREAK_SOUND;
                if (SoundKey.ZOMBIE_THUMP_CHAINLINK_FENCE == thumpSound) {
                    if (bendStageNew == 1) {
                        soundName = SoundKey.ZOMBIE_THUMP_CHAINLINK_FENCE_DAMAGE_LOW;
                    } else if (bendStageNew < entry.stages.size() - 1) {
                        soundName = SoundKey.ZOMBIE_THUMP_CHAINLINK_FENCE_DAMAGE_HIGH;
                    } else {
                        soundName = SoundKey.ZOMBIE_THUMP_CHAINLINK_FENCE_DAMAGE_COLLAPSE;
                    }
                } else if (SoundKey.ZOMBIE_THUMP_METAL_POLE_FENCE == thumpSound) {
                    if (bendStageNew == 1) {
                        soundName = SoundKey.ZOMBIE_THUMP_METAL_POLE_FENCE_DAMAGE_LOW;
                    } else if (bendStageNew < entry.stages.size() - 1) {
                        soundName = SoundKey.ZOMBIE_THUMP_METAL_POLE_FENCE_DAMAGE_HIGH;
                    } else {
                        soundName = SoundKey.ZOMBIE_THUMP_METAL_POLE_FENCE_DAMAGE_COLLAPSE;
                    }
