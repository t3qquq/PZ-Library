...
public final class WorldConverter {
    ...
    public void convert(String worldName, IsoSpriteManager manager) throws IOException {
        ...
        if (inFile.exists()) {
            ...
            input.close();
            if (convertingVersion < 249) {
                if (convertingVersion < 1) {
                    GameLoadingState.worldVersionError = true;
                    return;
                }

                DebugType.General.println("WorldConverter.convert() start");

                try {
                    this.convert(worldName, convertingVersion, 249);
                } catch (Exception ex) {
                ...
                DebugType.General.println("WorldConverter.convert() end");
            }
