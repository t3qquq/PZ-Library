...
public class WeatherPeriod {
    ...
    public void writeNetWeatherData(ByteBufferWriter output) throws IOException {
        if (output.putBoolean(this.isRunning)) {
            output.putBoolean(this.isThunderStorm);
            output.putBoolean(this.isTropicalStorm);
            output.putBoolean(this.isBlizzard);
    ...
    public void readNetWeatherData(ByteBufferReader input) throws IOException {
        this.isRunning = input.getBoolean();
        if (this.isRunning) {
            this.isThunderStorm = input.getBoolean();
            this.isTropicalStorm = input.getBoolean();
            this.isBlizzard = input.getBoolean();
    ...
    private void linkWeatherStages() {
        ...
        for (int i = 0; i < this.weatherStages.size(); i++) {
            WeatherPeriod.WeatherStage curr = this.weatherStages.get(i);
    ...
    private void createWeatherPattern() {
        ...
        if (this.frontCache.getType() == 1) {
            ...
            while (dur < maxDuration) {
                WeatherPeriod.WeatherStage ws = switch (this.RandNext(0, 10)) {
            ...
            while (dur < maxDuration) {
                WeatherPeriod.WeatherStage var21 = switch (this.RandNext(0, 10)) {
                ...
                dur = (float)(dur + var21.getStageDuration());
                stages.add(var21);
            }
        }
        ...
        for (int i = 0; i < this.weatherStages.size(); i++) {
            this.weatherStages.get(i).stageDuration *= rainTimeMultiplier;
        }
        ...
        for (int i = 0; i < this.weatherStages.size(); i++) {
            timeWorldAgeHours = this.weatherStages.get(i).setStageStart(timeWorldAgeHours);
        }
    ...
    private WeatherPeriod.WeatherStage createStage(int typeID, double duration, String moddedID) {
        WeatherPeriod.WeatherStage stage;
