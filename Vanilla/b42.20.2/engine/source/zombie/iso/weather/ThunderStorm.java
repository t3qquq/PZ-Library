...
public class ThunderStorm {
    ...
    public void update(double currentTime) {
                        ...
                        if (cloud.suspendTimer.finished()) {
                            ...
                            if (Rand.Next(0.0F, 1.0F) < cloud.thunderRatio) {
                                ...
                                this.triggerThunderEvent(
                                    ...
                                    true,
                                    !Core.getInstance().getOptionDisableLightningDuringStorms(),
        ...
        if (GameClient.client || !GameServer.server) {
            for (int i = 0; i < 4; i++) {
                ThunderStorm.PlayerLightningInfo linfo = this.lightningInfos[i];
            ...
            for (int i = 0; i < this.events.length; i++) {
                ThunderStorm.ThunderEvent event = this.events[i];
                if (event.isRunning) {
            ...
            if (dist < 10000) {
                ...
                if (event != null) {
                    ...
                    if (doLightning) {
                        for (int i = 0; i < IsoPlayer.numPlayers; i++) {
                            IsoPlayer player = IsoPlayer.players[i];
                            if (player != null && this.lightningInfos[i].distance < 7500.0F) {
                                float ls = 1.0F - this.lightningInfos[i].distance / 7500.0F;
                                this.lightningInfos[i].lightningState = ThunderStorm.LightningState.ApplyLightning;
                                if (ls > this.lightningInfos[i].lightningStrength) {
