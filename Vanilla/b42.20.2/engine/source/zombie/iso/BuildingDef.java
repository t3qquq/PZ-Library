...
public final class BuildingDef {
    ...
    public void CalculateBounds(ArrayList<RoomDef> tempRooms) {
        ...
        for (int n = 0; n < tempRooms.size(); n++) {
            ...
            for (int nn = 0; nn < r.rects.size(); nn++) {
                ...
                minX = (rect.x - 1) / 8;
                minY = (rect.y - 1) / 8;
                maxX = (rect.x + rect.w + 0) / 8;
                maxY = (rect.y + rect.h + 0) / 8;
    ...
    public void calculateCollapseRect() {
        if (this.collapseRectX2 == -1) {
            ...
            minX = Math.min(minX, roomXFloor);
            maxX = Math.max(maxX, roomX2Floor);
            minY = Math.min(minY, roomYFloor);
            maxY = Math.max(maxY, roomY2Floor);
            if (this.collapseRectX == -1) {
                ...
                this.collapseRectX = (int)Math.min(minX, this.collapseRectX);
                this.collapseRectY = (int)Math.min(minY, this.collapseRectY);
                this.collapseRectX2 = (int)Math.max(maxX, this.collapseRectX) - 1;
                this.collapseRectY2 = (int)Math.max(maxY, this.collapseRectY) - 1;
            }
        }
    }
    ...
    public List<IsoObject> getObjects() {
        List<IsoObject> objList = new ArrayList<>();
        List<IsoGridSquare> sqList = this.getSquares();

        for (int i = 0; i < sqList.size(); i++) {
            IsoGridSquare sq = sqList.get(i);
    ...
    public List<IsoGridSquare> getSquares() {
        List<IsoGridSquare> squareList = new ArrayList<>();
        IsoCell cell = IsoWorld.instance.currentCell;

        for (int x1 = this.x - 1; x1 < this.x2 + 1; x1++) {
            for (int y1 = this.y - 1; y1 < this.y2 + 1; y1++) {
                for (int z = -32; z < 31; z++) {
                    IsoGridSquare sq = cell.getGridSquare(x1, y1, z);
                    if (sq != null) {
                        squareList.add(sq);
                    }
                }
