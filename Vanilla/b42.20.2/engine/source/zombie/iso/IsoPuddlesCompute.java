...
public final class IsoPuddlesCompute {
    ...
    private static final ObjectPool<Vector3f> pool_vector3f = new ObjectPool<>(Vector3f::new, "IsoPuddlesCompute.pool_vector3f");
    ...
    public static float computePuddle(IsoGridSquare square) {
        ...
        if (GameServer.server || Core.getInstance().getPerfPuddlesOnLoad() != 3 && Core.getInstance().getPerfPuddles() != 3) {
            if (Core.getInstance().getPerfPuddles() > 0 && square.z > 0) {
                ...
                if (puddlesSize <= 0.0F) {
                    ...
                    puddlesDirAll = 1.0F;
                    puddlesDirNone = 0.0F;
                    float rawX = IsoUtils.XToScreen(square.x + 0.5F - square.z * 3.0F, square.y + 0.5F - square.z * 3.0F, 0.0F, 0);
                    float rawY = IsoUtils.YToScreen(square.x + 0.5F - square.z * 3.0F, square.y + 0.5F - square.z * 3.0F, 0.0F, 0);
                    return GetPuddlesFromPerlinNoise(
                        temp_vector2f.set((rawX - 90000.0F) * 8.0E-4F + square.z * 7.0F, (rawY - 640000.0F) * 8.0E-4F + square.z * 7.0F)
                    );
                } else {
    ...
    private static Vector2f CalculatePuddlesUvMain(IsoGridSquare square, IsoPuddles isoPuddles) {
        ...
        float fragCoordX = x;
        float fragCoordY = y;
        return temp_vector2f.set(
            (fragCoordX * wOffset.z + wOffset.x) * 8.0E-4F + square.z * 7.0F, (fragCoordY * wOffset.w + wOffset.y) * 8.0E-4F + square.z * 7.0F
        );
    }
