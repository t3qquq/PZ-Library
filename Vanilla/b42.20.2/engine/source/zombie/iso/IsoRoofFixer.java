...
public final class IsoRoofFixer {
    ...
    private static void FixRoofsPerRoomAt(IsoGridSquare current) {
        ...
        for (int i = 0; i < floorInfoIndex; i++) {
            placeFloorInfos[i].square.addFloor(roofGroups.get(placeFloorInfos[i].floorType));
        }
    }
        ...
        while (true) {
            label163: {
                ...
                for (int index = 0; index < count; index++) {
                    IsoGridSquare test = sqCache[index];
                    if (test.getRoom() == null && isInteriorAirSpace(test.getX(), test.getY(), test.getZ())) {
                        return null;
                    }

                    if (isRoofAt(test, true)) {
                        return test;
                    }
                    ...
                    for (int x = test.x - 3; x <= test.x + 3; x++) {
                        for (int y = test.y - 3; y <= test.y + 3; y++) {
                            if (x != test.x || y != test.y) {
                                IsoGridSquare gs = cell.getGridSquare(x, y, test.z);
                                if (gs != null) {
                                    ...
                                    if (above != null && !above.getObjects().isEmpty()) {
                                        for (int i = 0; i < above.getObjects().size(); i++) {
                                            IsoObject obj = above.getObjects().get(i);
