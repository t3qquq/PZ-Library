...
public class WorldGenReader {
    public Map<String, Biome> loadBiomes(KahluaTable worldgenTable, String tableKey) {
        ...
        while (iterMainTable.advance()) {
            ...
            DebugType.WorldGen.debugln(biomeName.toString());
            ...
            (k, v) -> {
                ...
                while (true) {
                    ...
                    if (biome.parent() == null || biome.parent().isEmpty()) {
                        ...
                        Map<FeatureType, Map<FeatureType, List<IBiome>>> subBiomes = biomeList.get(0).subBiomes();
                        List<String> protectedBiomes = biomeList.get(0).protectedList();
                        ...
                        for (Biome biomex : biomeList) {
                            ...
                            if (biomex.subBiomes() != null && !biomex.subBiomes().isEmpty()) {
                                subBiomes = biomex.subBiomes();
                            }

                            if (biomex.protectedList() != null && !biomex.protectedList().isEmpty()) {
                                protectedBiomes = biomex.protectedList();
                            }
    ...
    public Biome loadBiome(String biomeName, KahluaTable biomeTable) {
        ...
        Map<FeatureType, List<Feature>> features = this.loadBiomeFeatures(biomeName, featuresTable);
        ...
        Map<FeatureType, Map<FeatureType, List<IBiome>>> subBiomes = this.loadSubBiomes((KahluaTable)paramsTable.rawget("subbiomes"));
    ...
    private Map<FeatureType, List<Feature>> loadBiomeFeatures(String name, KahluaTable mainTable) {
        if (mainTable == null) {
            return null;
        }
        ...
        while (iterTypesTable.advance()) {
            ...
            while (iterTypeTable.advance()) {
                ...
                if (featuresObj == null || probaObj == null) {
                    try {
                        Field curKeyField = iterTypeTable.getClass().getDeclaredField("curKey");
                        Field keyIndexField = iterTypeTable.getClass().getDeclaredField("keyIndex");
                        curKeyField.setAccessible(true);
                        keyIndexField.setAccessible(true);
                        Double curKey = (Double)curKeyField.get(iterTypeTable);
                        int keyIndex = (Integer)keyIndexField.get(iterTypesTable);
                        throw new RuntimeException(
                            String.format(
                                "%s{%s:%s/%s} => Features not found or probability absents | %s | %s", name, typeName, curKey, keyIndex, featuresObj, probaObj
                            )
                        );
    ...
    public Map<FeatureType, Map<FeatureType, List<IBiome>>> loadSubBiomes(KahluaTable subBiomesTable) {
        if (subBiomesTable == null) {
            return new HashMap<>();
        }

        KahluaTableIterator iter = subBiomesTable.iterator();
        Map<FeatureType, Map<FeatureType, List<IBiome>>> subBiomes = new HashMap<>();

        while (iter.advance()) {
            String typeName = (String)iter.getKey();
            KahluaTable subTypeTable = (KahluaTable)iter.getValue();
            KahluaTableIterator subTypeIter = subTypeTable.iterator();
            Map<FeatureType, List<IBiome>> subTypeMap = new HashMap<>();

            while (subTypeIter.advance()) {
                String subTypeName = (String)subTypeIter.getKey();
                KahluaTable innerBiomesTable = (KahluaTable)subTypeIter.getValue();
                KahluaTableIterator innerBiomesIter = innerBiomesTable.iterator();
                List<IBiome> innerBiomes = new ArrayList<>();

                while (innerBiomesIter.advance()) {
                    Double key = (Double)innerBiomesIter.getKey();
                    Biome biome = this.loadBiome(key.toString(), (KahluaTable)innerBiomesIter.getValue());
                    innerBiomes.add(biome);
                }

                subTypeMap.put(FeatureType.valueOf(subTypeName), innerBiomes);
            }

            subBiomes.put(FeatureType.valueOf(typeName), subTypeMap);
        }
    ...
    public List<AnimalsPathConfig> loadAnimalsPath(KahluaTable animalsPathTable) {
        ...
        while (iterAnimalsPath.advance()) {
            ...
            List<Double> radiusDouble = radiusObj instanceof KahluaTable table ? this.loadList(table) : List.of(this.loadDouble(radiusObj, -1.0));
            List<Double> extensionDouble = extensionObj instanceof KahluaTable table ? this.loadList(table) : List.of(this.loadDouble(extensionObj, -1.0));
