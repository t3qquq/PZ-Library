...
public class AttachmentsHandler {
            ...
            if (!materialsFoundNotNull.isEmpty() && (materialsFoundNotNull.size() != 1 || !materialsFoundNotNull.get(0).equals(floorMaterial))) {
                ...
                if (materialsFound.size() > 1) {
                    for (Direction dir : Direction.diagonals()) {
                        List<String> mat = Stream.of(materials.get(dir), materials.get(dir.prev()), materials.get(dir.next())).distinct().toList();
                        if ((mat.size() != 1 || mat.get(0) != null) && mat.size() == 1 && !mat.get(0).equals(floorMaterial)) {
                            validDirs.remove(dir);
                            validDirs.remove(dir.prev());
                            validDirs.remove(dir.next());
                            this.applyAttachment(dir, wgChunk, doPriority, true, rnd, floor, floorMaterial, materials.get(dir));
                        }
                    }
                }

                for (Direction dir : Direction.cardinals()) {
                    if (materials.containsKey(dir) && materials.get(dir) != null && validDirs.contains(dir)) {
                        IsoObject floorRemote = WorldGenUtils.INSTANCE.doesFloorExit(cell, x + dir.x, y + dir.y, 0);
                        if (floorRemote != null) {
                            validDirs.remove(dir);
                            this.applyAttachment(dir.opposite(), wgChunk, doPriority, false, rnd, floorRemote, materials.get(dir), floorMaterial);
                        }
                    }
                }
            }
    ...
    private void applyAttachment(
        Direction dir, WorldGenChunk wgChunk, boolean doPriority, boolean priorityOrder, Random rnd, IsoObject floor, String material, String materialRemote
    ) {
        boolean priorityValid = priorityOrder ? wgChunk.priority(material, materialRemote) : wgChunk.priority(materialRemote, material);
        if (!Objects.equals(material, materialRemote) && (!doPriority || priorityValid)) {
