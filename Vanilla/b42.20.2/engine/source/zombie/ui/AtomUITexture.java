...
public class AtomUITexture extends AtomUI {
    ...
    private void updateSlices9() {
        ...
        if (this.sliceLeft != 0.0 && this.sliceTop != 0.0) {
            this.slices.add(new AtomUITexture.Slice(x0, x1, y0, y1, u0, u1, v0, v1));
        }

        if (this.sliceTop != 0.0) {
            this.slices.add(new AtomUITexture.Slice(x1, x2, y0, y1, u1, u2, v0, v1));
        }

        if (this.sliceRight != 0.0 && this.sliceTop != 0.0) {
            this.slices.add(new AtomUITexture.Slice(x2, x3, y0, y1, u2, u3, v0, v1));
        }

        if (this.sliceLeft != 0.0) {
            this.slices.add(new AtomUITexture.Slice(x0, x1, y1, y2, u0, u1, v1, v2));
        }
        ...
        if (this.sliceLeft != 0.0 && this.sliceDown != 0.0) {
            this.slices.add(new AtomUITexture.Slice(x0, x1, y2, y3, u0, u1, v2, v3));
        }
    ...
    Texture tryGetTexture(String key) {
        ...
        switch (var3) {
            case Texture var9:
                return (Texture)var3;
            case KahluaTable valueTable:
                KahluaThread thread = UIManager.getDefaultThread();
                Object getTextureFunc = thread.tableget(valueTable, "getTexture");
                return KahluaUtil.isFunction(getTextureFunc) ? Type.tryCastTo(thread.call(getTextureFunc, valueTable, null, null), Texture.class) : null;
            case null:
            default:
                return null;
        }
