...
public final class UIDebugConsole extends NewWindow {
    ...
    public void render() {
        if (this.isVisible()) {
            super.render();
            this.DrawTextCentre(UIFont.DebugConsole, Translator.getText("IGUI_CommandConsole"), this.getWidth() / 2.0, 2.0, 1.0, 1.0, 1.0, 1.0);
            this.DrawText(UIFont.DebugConsole, Translator.getText("IGUI_OutputLog"), 7.0, this.titleMiddle.getHeight(), 0.7F, 0.7F, 1.0, 1.0);
            this.DrawText(
                UIFont.DebugConsole,
                Translator.getText("IGUI_LuaCommandLine"),
                7.0,
    ...
    public int levenshteinDistance(CharSequence lhs, CharSequence rhs) {
        ...
        for (int j = 1; j < len1; j++) {
            ...
            for (int ix = 1; ix < len0; ix++) {
                ...
                int costReplace = cost[ix - 1] + match;
                int costInsert = cost[ix] + 1;
                int costDelete = newcost[ix - 1] + 1;
                newcost[ix] = Math.min(Math.min(costInsert, costDelete), costReplace);
            }
