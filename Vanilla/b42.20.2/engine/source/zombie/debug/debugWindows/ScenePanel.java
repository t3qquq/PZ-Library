...
public class ScenePanel extends BaseDebugWindow {
    ...
    protected void doWindowContents() {
        if (this.doTreeNode("optionsTree", "Settings", false)) {
            ...
            if (this.doTreeNode("logOptionsTree", "Logging", false)) {
                ...
                for (int i = 0; i < debugs.size(); i++) {
                    if (!this.isFiltered("logging", debugs.get(i).name())) {
                        boolean showLogSeverityCombo = Wrappers.checkbox(debugs.get(i).name(), DebugLog::isEnabled, DebugLog::setLogEnabled, debugs.get(i));
                        if (showLogSeverityCombo) {
                            DebugType debugType = debugs.get(i);
                            LogSeverity current = debugType.getLogSeverity();
                            ...
                            if (ImGui.combo(comboLabel, currentIndex, items, items.length)) {
                                debugType.setLogSeverity(values[currentIndex.get()]);
                            }
        ...
        if (IsoWorld.instance.currentCell != null && this.doTreeNode("worldTree", "IsoWorld", false, true)) {
            ...
            if (this.doTreeNode("vehicleTree", "BaseVehicle", false)) {
                this.doAllVehicles(new ArrayList<>(IsoWorld.instance.currentCell.vehicles));
                ImGui.treePop();
            }
    ...
    private void highlight(IsoMovingObject obj) {
        obj.setAlphaAndTarget(IsoPlayer.getInstance().getIndex(), 1.0F);
        obj.setOutlineHighlightCol(Core.getInstance().getBadHighlitedColor());
        obj.setOutlineHighlight(IsoPlayer.getInstance().getIndex(), true);
    }
    ...
    private void doAnimTable(String id, IsoGameCharacter character) {
        if (character.getAdvancedAnimator() != null) {
            if (this.doTreeNode(id, "Animation Data", false)) {
                ImGui.textWrapped(character.getAdvancedAnimator().getRootLayer().GetDebugString());
    ...
    enum ObjectTreeSort {
            ...
            (o1, o2) -> {
                if (o1 instanceof IsoZombie isoZombie) {
                    ...
                    return o1 instanceof IsoPlayer isoPlayer
                        ? Integer.compare(isoPlayer.getIndex(), ((IsoPlayer)o2).getIndex())
        ...
        ObjectTreeSort(final Comparator<IsoMovingObject> comparator) {
