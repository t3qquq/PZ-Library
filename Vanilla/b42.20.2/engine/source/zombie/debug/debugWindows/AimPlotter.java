                    ...
                    if (dist <= weapon.getMaxRange()) {
                        value += PZMath.max(
                            this.combatManager.getDistanceModifierSightless(dist, false)
                                - (this.showCurrent ? this.combatManager.getAimDelayPenaltySightless(PZMath.max(0.0F, player.getAimingDelay()), dist) : 0.0F),
                            this.combatManager.getDistanceModifier(dist, min, max, false)
                                - (this.showCurrent ? this.combatManager.getAimDelayPenalty(PZMath.max(0.0F, player.getAimingDelay()), dist, min, max) : 0.0F)
                        );
                        critv += PZMath.max(
                            this.combatManager.getDistanceModifierSightless(dist, false)
                                - (this.showCurrent ? this.combatManager.getAimDelayPenaltySightless(PZMath.max(0.0F, player.getAimingDelay()), dist) : 0.0F),
