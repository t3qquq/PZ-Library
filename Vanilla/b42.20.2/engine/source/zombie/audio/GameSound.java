...
public final class GameSound {
    ...
    public float getMaxDistanceOfClips() {
        ...
        for (int i = 0; i < this.clips.size(); i++) {
            ...
            if (clip.hasMaxDistance()) {
                max = PZMath.max(max, clip.getMaxDistance());
            }
        }
    ...
    public int numClipsUsingParameter(boolean remote, String parameterName) {
        ...
        for (int i = 0; i < this.clips.size(); i++) {
            GameSoundClip clip = this.clips.get(i);
            if (clip.hasParameter(remote, parameterDescription)) {
                result++;
            }
        }
