...
public class TaggedObjectManager<T extends TaggedObjectManager.TaggedObject> {
    ...
    public void registerObjectsFromBackingList(boolean clear) {
        if (this.verbose) {
            DebugType.General.println("Registering objects from backing list...");
        }
    ...
    public void registerObject(T taggedObject, boolean bSetDirty) {
        if (this.verbose) {
            DebugType.General.println("register tagged object: " + taggedObject);
        }
    ...
    private int registerTag(String tag, boolean bSetDirty) {
        ...
        if (!this.registeredTags.contains(tag)) {
            if (this.verbose) {
                DebugType.General.println("register new tag: " + tag);
            }
    ...
    public List<T> queryTaggedObjects(String tagQueryString) {
        if (StringUtils.isNullOrWhitespace(tagQueryString)) {
            DebugType.General.warn("manager-> returning empty list for: " + tagQueryString);
        ...
        if (tagGroup != null) {
            if (this.verbose) {
                DebugType.General.println("manager-> returning cached list for: " + tagQueryString);
            }
            ...
            if (alias != null) {
                ...
                if (tagGroup != null) {
                    if (this.verbose) {
                        DebugType.General.println("manager-> returning cached list for alias '" + alias + "', cache: " + tagQueryString);
                    }
            ...
            if (tagGroup != null) {
                if (this.verbose) {
                    DebugType.General.println("manager-> created new alias '" + tagQueryString + "' for: " + formattedQueryStr);
                }
                ...
                if (!hasWhiteList && !hasBlackList) {
                    DebugType.General.warn("manager-> could not gather objects for key: " + tagQueryString);
                    return this.emptyTagObjects;
                }
                ...
                if (this.verbose) {
                    DebugType.General.println("manager-> created new set for: " + formattedQueryStr);
                }
            ...
            for (int i = 0; i < tags.length; i++) {
                ...
                if (bitIndex != null) {
                    ...
                    if (this.warnNonPreprocessedNewTag) {
                        DebugType.General.warn("manager-> new tag discovered that was not preprocessed, tag: " + tag);
                    }
    ...
    public List<T> filterList(String tagQueryString, List<T> listToPopulate, List<T> sourceList, boolean clearList) {
        ...
        if (StringUtils.isNullOrWhitespace(tagQueryString)) {
            if (this.verbose) {
                DebugType.General.warn("manager-> query string empty, returning input list for: " + tagQueryString);
            }
