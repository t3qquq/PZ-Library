...
public class UIWorldMap extends UIElement {
    ...
    public void render() {
        if (this.isVisible()) {
            if (this.parent == null || this.parent.getMaxDrawHeight() == -1.0 || !(this.parent.getMaxDrawHeight() <= this.getY())) {
                ...
                if (this.renderer.getBoolean("VirtualVehicles")) {
                    if (GameClient.client) {
                        Manager.getInstance().renderWorldMap(this);
                    } else {
                        VirtualVehicleManager.getInstance().renderWorldMap(this);
                    }
                }
    ...
    private boolean shouldShowRemotePlayer(WorldMapRemotePlayer remotePlayer) {
        ...
        for (int pn = 0; pn < IsoPlayer.numPlayers; pn++) {
            IsoPlayer player = IsoPlayer.players[pn];
            if (player != null) {
                if (Faction.isInSameFaction(player, remotePlayer.getUsername())
                    || SafeHouse.isInSameSafehouse(player.getUsername(), remotePlayer.getUsername())) {
                    sameFaction = true;
                }

                if (!player.isInvisible() || remotePlayer.canSeeInvisiblePlayer()) {
                    cansee = true;
                }
                ...
                if (remoteIsoPlayer != null && player.checkCanSeeClient(remoteIsoPlayer)) {
                    visible = true;
                }
            }
        }
        ...
        return switch (mapRemotePlayerVisibility) {
            case FACTION_ONLY -> sameFaction;
            case FACTION_AND_VISIBLE_ONLY -> sameFaction || visible;
            case ALL -> cansee;
            default -> false;
        };
    }
