...
public final class WorldMapStreets {
    ...
    void getStreetsOverlapping(UIWorldMap ui, float uiMinX, float uiMinY, float uiMaxX, float uiMaxY, THashSet<WorldMapStreet> result) {
        ...
        if (!(ui.getAPI().getZoomF() < 13.0F)) {
            ...
            minX = PZMath.max(minX, ui.getWorldMap().getMinXInSquares());
            minY = PZMath.max(minY, ui.getWorldMap().getMinYInSquares());
            maxX = PZMath.min(maxX, ui.getWorldMap().getMaxXInSquares());
            maxY = PZMath.min(maxY, ui.getWorldMap().getMaxYInSquares());
