...
public final class ScriptManager implements IScriptObjectStore {
    ...
    public static void println(ScriptType type, String msg) {
        if (debugTypes.contains(type)) {
            DebugType.Script.println("[" + type.toString() + "] " + msg);
        }
    }
    ...
    public ArrayList<?> getScriptsForType(ScriptType type) {
        ...
        DebugType.General.warn("Type has no bucket collection: " + type);
        ...
        if (Core.debug) {
            ...
            for (CraftRecipe craftRecipe : failedVerificationRecipes) {
                ...
                for (CharacterProfessionDefinition characterProfessionDefinition : characterProfessionDefinitions) {
                    if (characterProfessionDefinition.isGrantedRecipe(craftRecipe.getName())) {
            ...
            for (String learnableRecipe : allMagazineRecipes) {
                ...
                for (CraftRecipe craftRecipe : allRecipes) {
                    ...
                    if (craftRecipe.getMetaRecipe() != null && craftRecipe.getMetaRecipe().equalsIgnoreCase(learnableRecipe)) {
                        found = true;
                        break;
                    }
                }
    ...
    public void LoadFile(ScriptLoadMode loadMode, String filename, boolean bLoadJar) throws FileNotFoundException {
        if (DebugLog.isEnabled(DebugType.Script)) {
            DebugType.Script.debugln(filename + (bLoadJar ? " bLoadJar" : ""));
        }
        ...
        if (filename.contains(".tmx")) {
            ...
            DebugType.Script.debugln("  file is a .tmx (map) file. Set mapPath to " + IsoWorld.mapPath + (IsoWorld.mapUseJar ? " mapUseJar" : ""));
        } else if (!filename.endsWith(".txt")) {
            DebugType.Script.warn(" file is not a .txt (script) file: " + filename);
        } else {
            ...
            label108: {
                try {
                    ...
                    DebugType.Script.error("Exception thrown reading file " + filename + "\n  " + ex);
                } finally {
                    try {
                        br.close();
                        isr.close();
    ...
    private void CreateFromToken(ScriptLoadMode loadMode, String token) {
        ...
        if (token.indexOf("module") == 0) {
            ...
            if (way == null) {
                if (DebugLog.isEnabled(DebugType.Script)) {
                    DebugType.Script.debugln("Adding new module: " + name);
                }
    ...
    public void Load() throws IOException {
        try {
            ...
            if (GameClient.client || GameServer.server) {
                ...
                if (GameServer.server) {
                    DebugType.General.println("scriptChecksum: " + this.checksum);
                }
            }
        } catch (Exception ex) {
    ...
    public void ReloadScripts(EnumSet<ScriptType> types) {
        DebugType.General.debugln("Reloading scripts = " + types);
    ...
    private void loadScripts(ScriptLoadMode loadMode, EnumSet<ScriptType> toLoadTypes) {
        try {
            ...
            if (loadMode == ScriptLoadMode.Reload) {
                ...
                for (ScriptBucketCollection<?> collection : this.bucketCollectionList) {
                    collection.setReloadBuckets(false);
                }
            }

            for (ScriptBucketCollection<?> collection : this.bucketCollectionList) {
                if (toLoadTypes.contains(collection.getScriptType())) {
                    collection.LoadScripts(loadMode);
                }
            }

            for (ScriptBucketCollection<?> collection : this.bucketCollectionList) {
                if (toLoadTypes.contains(collection.getScriptType())) {
                    collection.PostLoadScripts(loadMode);
                }
            }

            for (ScriptBucketCollection<?> collection : this.bucketCollectionList) {
                if (toLoadTypes.contains(collection.getScriptType())) {
                    collection.OnScriptsLoaded(loadMode);
                }
            }
    ...
    private void debugItems() {
        for (Item item : instance.getAllItems()) {
            if (item.isItemType(ItemType.DRAINABLE) && item.getReplaceOnUse() != null) {
                DebugType.Script.warn("%s ReplaceOnUse instead of ReplaceOnDeplete", item.getFullName());
            }
    ...
    private void createClothingItemMap() {
        for (Item item : this.getAllItems()) {
            if (!StringUtils.isNullOrWhitespace(item.getClothingItem())) {
                if (DebugLog.isEnabled(DebugType.Script)) {
                    DebugType.Script.noise("ClothingItem \"%s\" <---> Item \"%s\"", item.getClothingItem(), item.getFullName());
                }
