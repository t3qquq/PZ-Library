...
public class SpriteOverlayConfigScript extends ComponentScript {
        ...
        for (ScriptParser.Block child : block.children) {
            if (child.type.equals("progress")) {
                ...
                if (this.allStyles.containsKey(styleMapKey)) {
                    DebugType.General.error("Duplicate style name and progress (not allowed): " + styleMapKey);
                } else {
    ...
    private void loadFaces(String styleName, ScriptParser.Block block) {
        ...
        for (ScriptParser.Block child : block.children) {
            if (child.type.equalsIgnoreCase("face") && !style.isSingleFace) {
                ...
                if (faceID == -1) {
                    DebugType.General.error("Cannot find face for: " + child.id);
                } else {
                ...
                DebugType.General.error("Unknown block '" + child.type + "' in entity iso script: " + this.getName());
            }
        }
    ...
    private void warnOrError(String msg) {
        ...
        DebugType.General.warn("[" + this.getName() + "] " + msg);
    }
            ...
            for (SpriteOverlayConfigScript.FaceScript faceScript : style.faces) {
                if (faceScript != null) {
                    ...
                    if (complementaryFace != -1
                        ...
                        && (
                            faceScript.totalWidth != style.getFace(complementaryFace).totalWidth
                                || faceScript.totalHeight != style.getFace(complementaryFace).totalHeight
                        )) {
                        DebugType.General.error("Complementary faces are different sizes. This is not supported. In entity iso script: " + this.getName());
                    }
                }
            }
