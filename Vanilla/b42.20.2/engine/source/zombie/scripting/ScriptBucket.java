        ...
        try {
            if (this.scriptType.getScriptTag().equals(type)) {
                ...
                if (this.loadData.containsKey(name)) {
                    ...
                    DebugType.General.warn("Found new script but was unable to load, possibly due to not being allowed during reload...");
        ...
        for (ScriptBucket.LoadData<E> data : this.loadData.values()) {
            try {
                ...
                if (loadMode != ScriptLoadMode.Reload || data.reloaded) {
                    ...
                    if (this.isVerbose()) {
                        DebugType.General.debugln("[" + this.scriptType.getScriptTag() + "] load script = " + script.getScriptObjectName());
                    }
                    ...
                    if (script != null) {
                        if (script.getObsolete()) {
                            DebugType.Script.debugln("[" + this.scriptType.getScriptTag() + "] ignoring script, obsolete = " + script.getScriptObjectName());
                            continue;
                        }

                        if (!script.isEnabled()) {
                            DebugType.Script.debugln("[" + this.scriptType.getScriptTag() + "] ignoring script, disabled = " + script.getScriptObjectName());
                            continue;
                        }
