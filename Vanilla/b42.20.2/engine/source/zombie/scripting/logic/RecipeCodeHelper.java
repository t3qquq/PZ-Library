...
public class RecipeCodeHelper {
    ...
    protected static void setColor(InventoryItem result, Color color) {
        ...
        result.getVisual().setTint(new ImmutableColor(color));
        result.setCustomColor(true);
        if (result instanceof Clothing clothing) {
            ItemVisual visual = clothing.getVisual();
            if (visual == null) {
                return;
            }

            visual.setTint(new ImmutableColor(color.getR(), color.getG(), color.getB()));
        }
    }
