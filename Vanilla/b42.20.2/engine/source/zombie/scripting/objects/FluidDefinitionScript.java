    ...
    public void Load(String name, String body) throws Exception {
        ...
        for (ScriptParser.Value value : block.values) {
            ...
            if (!key.isEmpty() && !val.isEmpty()) {
                if (key.equalsIgnoreCase("displayName")) {
                    ...
                    DebugType.General.error("Unknown key '" + key + "' val(" + val + ") in fluid definition: " + this.getScriptObjectFullType());
                    if (Core.debug) {
        ...
        for (ScriptParser.Block child : block.children) {
            if ("blendWhiteList".equalsIgnoreCase(child.type)) {
                ...
                DebugType.General.error("Unknown block '" + block.type + "' val(" + block.id + ") in fluid definition: " + this.getScriptObjectFullType());
                if (Core.debug) {
                    throw new Exception("FluidDefinition error.");
                }
            }
        ...
        for (ScriptParser.Value value : block.values) {
            ...
            if (!key.isEmpty() && !val.isEmpty()) {
                ...
                if ("maxEffect".equalsIgnoreCase(key)) {
                    ...
                    DebugType.General.error("Unknown key '" + key + "' val(" + val + ") in fluid poison definition: " + this.getScriptObjectFullType());
                    if (Core.debug) {
                        throw new Exception("FluidDefinition error.");
                    }
                }
        ...
        for (ScriptParser.Value value : block.values) {
            ...
            if (!key.isEmpty() && !val.isEmpty()) {
                if ("fatigueChange".equalsIgnoreCase(key)) {
                    ...
                    DebugType.General.error("Unknown key '" + key + "' val(" + val + ") in fluid properties definition: " + this.getScriptObjectFullType());
                    if (Core.debug) {
                        throw new Exception("FluidDefinition error.");
                    }
                }
