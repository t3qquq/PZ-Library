...
public final class VehicleScript extends BaseScriptObject implements IModelAttachmentOwner {
    ...
    private final List<VehicleScript.Wheel> crawlThroughWheels = new ArrayList<>();
        ...
        for (ScriptParser.BlockElement element : block.elements) {
            if (element.asValue() != null) {
                ...
                if ("area".equals(child.type)) {
                    ...
                    this.LoadWheel(child, false);
                } else if ("crawlThroughWheel".equals(child.type)) {
                    this.LoadWheel(child, true);
    ...
    public void Loaded() {
        ...
        if (this.hasPhysicsChassisShape() && !this.extents.equals(this.physicsChassisShape)) {
            DebugType.Script.warn("vehicle \"" + this.name + "\" extents != physicsChassisShape");
        }
        ...
        for (int i = 0; i < this.parts.size(); i++) {
            ...
            if (part.specificItem && part.itemType != null) {
                for (int j = 0; j < part.itemType.size(); j++) {
    ...
    private VehicleScript.Wheel LoadWheel(ScriptParser.Block block, boolean crawlThroughWheel) {
        VehicleScript.Wheel wheel = this.getWheelById(block.id);
        if (wheel == null) {
            wheel = new VehicleScript.Wheel();
            wheel.id = block.id;
            if (crawlThroughWheel) {
                this.crawlThroughWheels.add(wheel);
            } else {
    ...
    private VehicleScript.Container LoadContainer(ScriptParser.Block block, VehicleScript.Container existing) {
        ...
        for (ScriptParser.Value value : block.values) {
            ...
            if ("capacity".equals(k)) {
                ...
                container.seatId = v;
            } else if ("soundMap".equals(k)) {
                String[] ss = v.split("\\s+");
                if (ss.length == 2 && !ss[0].trim().isEmpty()) {
                    container.soundMap.put(ss[0].trim(), ss[1].trim());
                }
    ...
    private void initCrawlOffsets() {
        ...
        for (int i = 0; i < this.getCrawlThroughWheel(); i++) {
            VehicleScript.Wheel wheel = this.getCrawlThroughWheel(i);
            if (wheel.id.contains("Left")) {
                this.initCrawlOffsets(wheel);
            }
        }
        ...
        for (int i = 0; i < this.crawlOffsets.size(); i++) {
    ...
    public int getCrawlThroughWheel() {
        return this.crawlThroughWheels.size();
    }
    ...
    public VehicleScript.Wheel getCrawlThroughWheel(int index) {
        return this.crawlThroughWheels.get(index);
    }
    ...
    public static final class Container {
        ...
        VehicleScript.Container makeCopy() {
            ...
            copy.soundMap.putAll(this.soundMap);
            return copy;
        }
    }
