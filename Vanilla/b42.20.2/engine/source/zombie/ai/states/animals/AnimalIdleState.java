...
public final class AnimalIdleState extends State {
    private static final AnimalIdleState INSTANCE = new AnimalIdleState();
    public static final State.Param<Long> PET_SOUND_TIME = State.Param.ofLong("pet_sound_time", 0L);

    public static AnimalIdleState instance() {
        return INSTANCE;
    }

    private AnimalIdleState() {
    ...
    public void execute(IsoGameCharacter owner) {
        ...
        if (animal.isLocalPlayer()) {
            ...
            if (animal.isAnimalEating()) {
                animal.getStateMachine().changeState(AnimalEatState.instance(), null, false);
            }

            if (animal.getVariableBoolean("bMoving")) {
                animal.getStateMachine().changeState(AnimalWalkState.instance(), null, false);
            }
        }
    }
    ...
    public void animEvent(IsoGameCharacter owner, AnimLayer layer, AnimationTrack track, AnimEvent event) {
        ...
        if ("PlayBreedSound".equalsIgnoreCase(event.eventName)) {
            if ("petting".equalsIgnoreCase(event.parameterValue)) {
                long petTime = owner.get(PET_SOUND_TIME);
                if (System.currentTimeMillis() - petTime < 5000L) {
                    return;
                }

                owner.set(PET_SOUND_TIME, System.currentTimeMillis());
            }
