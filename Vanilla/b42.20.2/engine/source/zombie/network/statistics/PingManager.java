...
public class PingManager {
    ...
    public static boolean doKickWhileLoading(UdpConnection connection, int ping) {
        ...
        return pingLimit > 0 && ping > pingLimit && !connection.getRole().hasCapability(Capability.CantBeKickedIfTooLaggy);
    }
        ...
        if (c.pingHistory.size() >= pingIntervalCount) {
            ...
            if (count >= pingLimitCount) {
                return (int)Math.ceil(
                    (float)c.pingHistory.stream().limit(pingIntervalCount).mapToLong(v -> Math.min(maxPingToSum, v.intValue())).sum()
    ...
    private static void limitPing(UdpConnection connection, int pingLimit, int lastPing) {
        connection.pingHistory.addFirst(lastPing);
        ...
        if (doKick(connection, ping)) {
            ...
            AntiCheat.log(connection, "ping: user \"" + connection.getUserName() + "\" is disconnected because ping is too high");
        }
