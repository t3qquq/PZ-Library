...
public class StatisticManager implements Iterable<Statistic> {
    ...
    static final double coordConvertA = 5.3648E-6;
    static final double coordConvertB = -179.913143;
    static final double coordConvertC = -2.3152E-7;
    static final double coordConvertD = 85.052166;
    ...
    public void init() {
        ...
        if (GameServer.server) {
            ...
            statistics.put(PoolStatistic.getInstance().getName(), PoolStatistic.getInstance());
        }
        ...
        if (prometheusPort != null) {
            ...
            this.playerX = (Gauge)Gauge.builder().name("player_x").help("X coordinate").labelNames("id", "name").register();
            this.playerY = (Gauge)Gauge.builder().name("player_y").help("Y coordinate").labelNames("id", "name").register();
            this.playerLon = (Gauge)Gauge.builder().name("player_lon").help("longitude").labelNames("id", "name").register();
            this.playerLat = (Gauge)Gauge.builder().name("player_lat").help("latitude").labelNames("id", "name").register();
        ...
        if (period != 0 && this.updateLimit.Check()) {
            ...
            if (GameServer.server) {
                        ...
                        if (c.isFullyConnected()) {
                            for (int j = 0; j < c.players.length; j++) {
                                ...
                                if (player != null) {
                                    ...
                                    ((GaugeDataPoint)this.playerLon.labelValues(new String[]{onlineId, username}))
                                        .set(5.3648E-6 * (player.getX() - player.getY()) + -179.913143);
                                    ((GaugeDataPoint)this.playerLat.labelValues(new String[]{onlineId, username}))
                                        .set(-2.3152E-7 * (player.getX() + player.getY()) + 85.052166);
                                }
                            }
                        }
                    }
                ...
                for (UdpConnection connection : GameServer.udpEngine.connections) {
                    if (connection.getRole() != null && connection.getRole().hasCapability(Capability.GetStatistic) && connection.statisticTransmissionEnabled) {
                        INetworkPacket.send(connection, PacketTypes.PacketType.Statistics);
                    }
                }
            }
        }
