...
public final class WarManager {
    ...
    public static void updateWar(int onlineId, String attacker, WarManager.State state, long timestamp) {
        ...
        WarManager.War war = new WarManager.War(onlineId, attacker, state, timestamp);
        wars.add(war);
    }
    ...
    public static void update() {
        if (ServerOptions.getInstance().war.getValue()) {
            ...
            while (iterator.hasNext()) {
                WarManager.War war = iterator.next();
                war.setState(WarManager.State.Ended);
                iterator.remove();
            }
        }
    }
    ...
    public enum State {
        ...
        State(final WarManager.State next) {
            this.next = next;
        }
    ...
    public static class War {
        ...
        public void setState(WarManager.State state) {
            DebugType.Multiplayer.debugln("War id=%d state=%s->%s", this.onlineId, this.state, state);
            this.state = state;
        ...
        public void setTimestamp(long timestamp) {
            DebugType.Multiplayer.debugln("War id=%d time=%d", this.onlineId, timestamp - GameTime.getServerTimeMills());
