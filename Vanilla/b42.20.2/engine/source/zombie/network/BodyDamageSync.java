...
public class BodyDamageSync {
    ...
    public void startSendingUpdates(short localId, short remoteID) {
        if (GameServer.server) {
            noise("start sending updates to " + remoteID);

            for (BodyDamageSync.Updater updater : this.updaters) {
                if (updater.localId == localId && updater.remoteId == remoteID) {
                    updater.bdSent.RestoreToFullHealth();
                    updater.bdSent.setBodyPartsLastState();
                    return;
                }
            }

            IsoPlayer player = GameServer.IDToPlayerMap.get(localId);
            BodyDamageSync.Updater updater = new BodyDamageSync.Updater();
            updater.localId = localId;
    ...
    public void stopSendingUpdates(short localIndex, short remoteID) {
        if (GameServer.server) {
            ...
            for (int i = 0; i < this.updaters.size(); i++) {
                ...
                if (updater.localId == localIndex && updater.remoteId == remoteID) {
    ...
    public static final class Updater {
        ...
        private void update() {
            ...
            if (currentTime - this.sendTime >= 500L) {
                ...
                if (bb.position() > 0) {
                    bb.put((byte)65);
                    IsoPlayer playerLocal = GameServer.IDToPlayerMap.get(this.localId);
                    IsoPlayer playerRemote = GameServer.IDToPlayerMap.get(this.remoteId);
                    UdpConnection connection = GameServer.getConnectionByPlayerOnlineID(this.remoteId);
                    if (playerLocal != null && playerRemote != null && connection != null && !GameServer.isDelayedDisconnect(connection)) {
                        BodyDamageUpdatePacket packet = new BodyDamageUpdatePacket();
                        packet.setUpdate(playerLocal, playerRemote, bb);
                        ByteBufferWriter b = connection.startPacket();
                        PacketTypes.PacketType.BodyDamageUpdate.doPacket(b);
                        packet.write(b);
                        PacketTypes.PacketType.BodyDamageUpdate.send(connection);
                    }
                }
            }
        }
