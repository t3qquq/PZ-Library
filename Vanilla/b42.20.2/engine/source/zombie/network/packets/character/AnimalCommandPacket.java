...
public class AnimalCommandPacket implements INetworkPacket {
    ...
    public void setData(Object... values) {
        if (values.length == 2) {
        ...
        } else if (values.length >= 4 && values[3] instanceof BaseVehicle) {
            this.setForTrailer(values);
            ...
            DebugType.Animal.error("No such packet set: %s", values[0]);
        }
    }

    private void setForTrailer(Object... values) {
        if (values[1] instanceof IsoAnimal animal) {
            this.set((AnimalCommandPacket.Type)values[0], animal, (IsoPlayer)values[2], (BaseVehicle)values[3], (InventoryItem)values[4]);
        } else if (values[1] instanceof IsoDeadBody deadBody) {
            this.set((AnimalCommandPacket.Type)values[0], deadBody, (IsoPlayer)values[2], (BaseVehicle)values[3], (InventoryItem)values[4]);
        }
    }
    ...
    public void set(AnimalCommandPacket.Type operation, IsoDeadBody deadBody, IsoPlayer player, BaseVehicle vehicle, InventoryItem item) {
        this.type = operation;
        this.bodyId.set(deadBody.getObjectID());
        this.playerId.set(player);
        this.vehicleId.set(vehicle);
        this.item = item;
    }
    ...
    public void write(ByteBufferWriter b) {
        ...
        if (AnimalCommandPacket.Type.UpdateGenome == this.type) {
            ...
            try {
                ...
                DebugType.Multiplayer.printException(e, "Animal genome write failed", LogSeverity.Error);
                b.position(sizePos);
                b.putInt(0);
            }
        } else {
            ...
            if (this.item != null) {
                ...
                try {
                    ...
                    DebugType.Multiplayer.printException(e, "AnimalInventoryItem save failed", LogSeverity.Error);
                }
            } else {
                b.putInt(-1);
            }
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        this.type = b.getEnum(AnimalCommandPacket.Type.class);
        ...
        if (AnimalCommandPacket.Type.UpdateGenome == this.type) {
            ...
            try {
                ...
                DebugType.Multiplayer.printException(e, "Animal genome read failed", LogSeverity.Error);
                return;
            }
        } else {
            ...
            if (itemID != -1) {
                try {
                    ...
                    DebugType.Multiplayer.printException(e, "AnimalInventoryItem load failed", LogSeverity.Error);
                }
            }
        }
    }
        ...
        if (this.isConsistent(connection) && AnimalCommandPacket.Type.UpdateGenome == this.type) {
            for (UdpConnection c : GameServer.udpEngine.connections) {
                if (connection.getConnectedGUID() != c.getConnectedGUID()
                    && c.isFullyConnected()
                    && c.isRelevantTo(this.animalId.getAnimal().getX(), this.animalId.getAnimal().getY())) {
    ...
    public void processClient(UdpConnection connection) {
        if (this.isConsistent(connection)) {
            switch (this.type) {
                case None:
                default:
                    DebugType.Animal.warn("AnimalPacket \"%s\" is not supported", this.type);
                    break;
                case AddAnimalFromHandsInTrailer:
                    if (this.animalId.isConsistent(connection) && this.playerId.isConsistent(connection) && this.vehicleId.isConsistent(connection)) {
                        ...
                        if (item != null) {
                            ...
                            DebugType.Animal.error("Animal not found");
                        }
                    }
                    break;
                case AddAnimalInTrailer:
                    if (this.animalId.isConsistent(connection)
                        ...
                        this.vehicleId.getVehicle().addAnimalInTrailer(this.animalId.getAnimal());
                    }
                    break;
                case RemoveAnimalFromTrailer:
                    if (this.animalId.isConsistent(connection) && this.playerId.isConsistent(connection) && this.vehicleId.isConsistent(connection)) {
                        boolean animalRemoved = this.vehicleId.getVehicle().animals.remove(this.animalId.getAnimal());
                        if (animalRemoved && this.animalId.getAnimal().isAlive()) {
                ...
                case RemoveAndGrabAnimalFromTrailer:
                    if (this.animalId.isConsistent(connection)
                        ...
                        if (this.item instanceof InventoryItem invItem) {
                            this.playerId.getPlayer().getInventory().AddItem(invItem);
                            this.playerId.getPlayer().setPrimaryHandItem(invItem);
                            this.playerId.getPlayer().setSecondaryHandItem(invItem);
                            this.animalId.getAnimal().itemId = this.item.id;
                            if (invItem instanceof AnimalInventoryItem animalItem) {
                                AnimalInstanceManager.getInstance().add(animalItem.getAnimal(), this.animalId.getID());
                            }
                        } else {
                            DebugType.Animal.warn("AnimalInventoryItem is null");
                        }
                    }
                    break;
                ...
                case ButcherAnimal:
                    ...
                    if (deadBody != null && this.playerId.isConsistent(connection)) {
                        ...
                        DebugType.Animal.warn("IsoDeadBody is null");
                    }
                    break;
                case FeedAnimalFromHand:
                    if (this.animalId.isConsistent(connection) && this.playerId.isConsistent(connection)) {
                        if (this.item instanceof InventoryItem inventoryItem) {
                            this.animalId.getAnimal().feedFromHand(this.playerId.getPlayer(), inventoryItem);
                        } else {
                            DebugType.Animal.warn("InventoryItem is null");
                        }
                    }
                    break;
                    ...
                    if (this.animalId.isConsistent(connection) && this.playerId.isConsistent(connection) && this.objectId.isConsistent(connection)) {
                        ...
                        if (inventoryItem instanceof InventoryItem) {
                            if (this.objectId.getObject() instanceof IsoHutch hutch) {
                                ...
                                DebugType.Animal.warn("IsoHutch is null");
                            }
                        } else {
                            DebugType.Animal.warn("InventoryItem is null");
                        }
                ...
                case DropAnimal:
                    if (this.animalId.isConsistent(connection)) {
                        ...
                        newAnimal.itemId = 0;
                        newAnimal.attachBackToMotherTimer = 10000.0F;
                        IsoAnimal prevHolder = this.animalId.getAnimal();
                        if (prevHolder != null && prevHolder != newAnimal) {
                ...
                case UpdateGenome:
                    break;
                case PickupAnimalFromTrap:
                    if (this.playerId.isConsistent(connection) && this.animalId.getID() != -1 && this.item instanceof AnimalInventoryItem animal) {
                        AnimalInstanceManager.getInstance().add(animal.getAnimal(), this.animalId.getID());
                        this.playerId.getPlayer().getInventory().AddItem(animal);
                    }
            }
