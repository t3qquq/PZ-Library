...
public class VariableSyncPacket implements INetworkPacket {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        this.key = b.getUTF();
        this.varType = b.getEnum(VariableSyncPacket.VariableType.class);
        switch (this.varType) {
            case String:
                this.value = b.getUTF();
                break;
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        ...
        for (int n = 0; n < GameServer.udpEngine.connections.size(); n++) {
            ...
            if (c.getConnectedGUID() != connection.getConnectedGUID() && c.isRelevantTo(this.player.getPlayer().getX(), this.player.getPlayer().getY())) {
