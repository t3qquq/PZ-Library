...
public class AddCorpseToMapPacket implements INetworkPacket {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        if (this.body != null) {
            ...
            long lastObjectId = this.objectId.getObjectID();
            ...
            if (GameServer.server) {
                ...
                if (sq != null) {
                    ...
                    if (lastObjectId != this.body.getObjectID().getObjectID()) {
                        this.objectId.set(this.body.getObjectID());
                        b.position(objectIdPos);
                        this.objectId.save(b.bb);
                        if (connection.isRelevantTo(this.x, this.y)) {
                            this.body.sendObjectChange(IsoObjectChange.OBJECT_ID);
                        }
                    }

                    for (int n = 0; n < GameServer.udpEngine.connections.size(); n++) {
                        UdpConnection c = GameServer.udpEngine.connections.get(n);
                        if (c.getConnectedGUID() != connection.getConnectedGUID() && c.isRelevantTo(this.x, this.y)) {
                ...
                LoggerManager.getLogger("item")
                    .write(connection.getIDStr() + " \"" + connection.getUserName() + "\" corpse +1 " + this.x + "," + this.y + "," + this.z);
            } else {
