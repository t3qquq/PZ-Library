...
UIActions.createNewAction = function(name, title, textureFilename, save, load, performAction, actionIsFinished, renderFunc, optionsUI, doAddToUI)
	...
	local element = LuaUI.createElement(action, doAddToUI);
...
UIActions.save = function()
	local output = getFileOutput("hotkeys/onscreenaction.data");
...
UIActions.load = function()
	local input = getFileInput("hotkeys/onscreenaction.data");
	if input ~= nil then
		...
		for i = 0, count do
			local str = input:readUTF();
			
			local action = ActionFactory.createFromName(str);
			action:load(input);
			action.javaObject:setX(input:readInt());
			action.javaObject:setY(input:readInt());
			action:addToUI();
			UIActions.onScreenActions[action.ID] = action;
		end
