...
HotkeyBar.addHotkeySlot = function(name, keycode, displaykey)	
	local hotkey = HotkeySlots.create(name, keycode, displaykey, HotkeyBar.window);
...
HotkeyBar.addSeperator = function()	
	local hotkey = {seperator = {};}
	table.insert(HotkeyBar.window.slots, hotkey);	
	HotkeyBar.layout();
end

HotkeyBar.addNewLine = function()	
	local hotkey = {newline = {};}
	table.insert(HotkeyBar.window.slots, hotkey);	
	HotkeyBar.layout();
end
...
HotkeyBar.save = function()
	local output = getFileOutput("hotkeys/hotkeys.data");
...
HotkeyBar.load = function()
	local input = getFileInput("hotkeys/hotkeys.data");
	if input ~= nil then
		
		local count = input:readInt();
		for i = 0, count do
			local str = input:readUTF();
			local hotkey = HotkeyBar.hotkeys[str];
			hotkey:load(input);
		end
