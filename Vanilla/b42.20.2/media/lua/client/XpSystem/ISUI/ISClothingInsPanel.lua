...
function ISClothingInsPanel:createChildren()
    ...
    self.coreRectangleX = self.bodyOutline:getWidth() + UI_BORDER_SPACING*2 + 1;
    self.coreRectangleW = self.width-self.coreRectangleX-UI_BORDER_SPACING-1;
    ...
    self.bpAnchorX = self.bodyOutline:getWidth();
    self.bpAnchorY = 50;
    ...
    y = self.bodyPartPanel:getHeight() + UI_BORDER_SPACING;

    if isDebugEnabled() then
        local btnx = x;
        local resetBtn;
        y, resetBtn = ISDebugUtils.addButton(self, {}, btnx, y, self.bpAnchorX, BUTTON_HGT, "-reset-", ISClothingInsPanel.onResetButton);
    end
    ...
    y = x*2; --all contents inside coreRectangle

    local c = self.btnTextColor;
    local name = getText("IGUI_Temp_CoreTemp");
    self.labelCoreTemp = ISLabel:new(self.coreRectangleX+self.coreRectangleW/2, y, BUTTON_HGT, name, c.r, c.g, c.b, c.a, UIFont.Small, true);
    ...
    self.labelCoreTempMid = ISLabel:new(self.coreRectangleX+self.coreRectangleW/2, y, BUTTON_HGT, getText("IGUI_Temp_Normal"), self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
    self.labelCoreTempMid.center = true;
    self:addMainGroup(self.labelCoreTempMid);

    self.labelCoreTempMin = ISLabel:new(self.coreRectangleX+x, y, BUTTON_HGT, getText("IGUI_Temp_Cold"), self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
    self:addMainGroup(self.labelCoreTempMin);

    self.labelCoreTempMax = ISLabel:new(self.coreRectangleX+self.coreRectangleW-x, y, BUTTON_HGT, getText("IGUI_Temp_Hot"), self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, false);
    self:addMainGroup(self.labelCoreTempMax);

    y = y + BUTTON_HGT;

    self.coreTemperatureBar = ISGradientBar:new(self.coreRectangleX+x, y, self.coreRectangleW-x*2, UI_BORDER_SPACING);
    ...
    self.coreTemperatureBar:setHighlightRadius(20);
    ...
    self.labelCoreHeat = ISLabel:new(self.coreRectangleX+self.coreRectangleW/2, y, BUTTON_HGT, name, c.r, c.g, c.b, c.a, UIFont.Small, true);
    self.labelCoreHeat.prefixName = name;
    self.labelCoreHeat.center = true;
    self:addMainGroup(self.labelCoreHeat);

    y = y + BUTTON_HGT;
    self.labelCoreHeatMid = ISLabel:new(self.coreRectangleX+self.coreRectangleW/2, y, BUTTON_HGT, getText("IGUI_Temp_Normal"), self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
    self.labelCoreHeatMid.center = true;
    self:addMainGroup(self.labelCoreHeatMid);

    self.labelCoreHeatMin = ISLabel:new(self.coreRectangleX+x, y, BUTTON_HGT, getText("IGUI_Temp_Low"), self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
    self:addMainGroup(self.labelCoreHeatMin);

    self.labelCoreHeatMax = ISLabel:new(self.coreRectangleX+self.coreRectangleW-x, y, BUTTON_HGT, getText("IGUI_Temp_High"), self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, false);
    self:addMainGroup(self.labelCoreHeatMax);

    y = y + BUTTON_HGT;

    self.coreHeatBar = ISGradientBar:new(self.coreRectangleX+x, y, self.coreRectangleW-x*2, UI_BORDER_SPACING);
    self.coreHeatBar:setGradientTexture(getTexture("media/ui/BodyInsulation/heatbar_horz"));
    self.coreHeatBar:setValue(0.1);
    self.coreHeatBar:setHighlightRadius(20 );
    ...
    self.coreRectangleH = self.maxViewsY;
    ...
    y = math.max(320, self.maxViewsY + UI_BORDER_SPACING*2+1);
    local c = self.btnTextColor;
    self.labelCurrentView = ISLabel:new(self.width/2, y, FONT_HGT_MEDIUM+6, getText("IGUI_Temp_SkinTemperature"), c.r, c.g, c.b, c.a, UIFont.Medium, true);
    self.labelCurrentView.center = true;
    self:addChild(self.labelCurrentView);

    y = y + FONT_HGT_MEDIUM+6;

    self.labelCurrentViewMid = ISLabel:new(self.width/2, y, BUTTON_HGT, "Mid", self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
    self.labelCurrentViewMid.center = true;
    self:addChild(self.labelCurrentViewMid);

    self.labelCurrentViewMin = ISLabel:new(x, y, BUTTON_HGT, "Min", self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
    self:addChild(self.labelCurrentViewMin);

    self.labelCurrentViewMax = ISLabel:new(self.width-x, y, BUTTON_HGT, "Max", self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, false);
    self:addChild(self.labelCurrentViewMax);

    y = y + BUTTON_HGT

    self.legendBar = ISGradientBar:new(x, y, self.width - x*2, UI_BORDER_SPACING*2);
    self.legendBar:setGradientTexture(getTexture("media/ui/BodyInsulation/heatbar_horz"));
    self.legendBar:setValue(0.5);
    self.legendBar:setHighlightRadius(20);
    self.legendBar:setDoKnob(false);
    self.legendBar:setDarkAlpha(0);
    self:addChild(self.legendBar);

    y = y + UI_BORDER_SPACING*3

    y, self.toggleAdvBtn = ISDebugUtils.addButton(self, {}, x, y, self.width - x*2, BUTTON_HGT, getText("IGUI_Temp_AdvView"), ISClothingInsPanel.onToggleViewStyle);
...
function ISClothingInsPanel:addView(_viewID, _viewTable, _y)
    ...
    for i=1,#_viewTable do
        y, btn = ISDebugUtils.addButton(self, {index = i}, self.coreRectangleX+UI_BORDER_SPACING+1, y, self.coreRectangleW-UI_BORDER_SPACING*2-2, BUTTON_HGT, _viewTable[i].title, ISClothingInsPanel.onClickViewButton);
        if i==1 then
            btn.textColor = self.titleColor;
        end
    ...
    self.maxViewsY = math.max(self.maxViewsY, y - UI_BORDER_SPACING)
    ...
    for i=1,#_viewTable do
        ...
        t.labelTitle = ISLabel:new(self.coreRectangleX+self.coreRectangleW/2, y, BUTTON_HGT, name, c.r, c.g, c.b, c.a, UIFont.Small, true);
        t.labelTitle.prefixName = name;
        t.labelTitle.center = true;
        ...
        y = y + BUTTON_HGT;
        if view.mid then
            t.labelMid = ISLabel:new(self.coreRectangleX+self.coreRectangleW/2, y, BUTTON_HGT, view.mid, self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
            t.labelMid.center = true;
            self:addNodeGroup(t.labelMid, _viewID);
        end

        if view.min then
            t.labelMin = ISLabel:new(self.coreRectangleX+x, y, BUTTON_HGT, view.min, self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, true);
            self:addNodeGroup(t.labelMin, _viewID);
        end

        if view.max then
            t.labelMax = ISLabel:new(self.coreRectangleX+self.coreRectangleW-x, y, BUTTON_HGT, view.max, self.defTextColor.r, self.defTextColor.g, self.defTextColor.b, self.defTextColor.a, UIFont.Small, false);
            self:addNodeGroup(t.labelMax, _viewID);
        end

        y = y + BUTTON_HGT;

        t.bar = ISGradientBar:new(self.coreRectangleX+x, y, self.coreRectangleW-x*2, UI_BORDER_SPACING);
        t.bar:setGradientTexture(view.texture);
        t.bar:setValue(0.5);
        if i>=6 then
            t.bar:setValue(0);
        end
        t.bar:setHighlightRadius(20);
        t.bar:setDoKnob(false);
        self:addNodeGroup(t.bar, _viewID);

        table.insert(self.nodeDetails[_viewID], t);

        y = t.bar:getBottom() + UI_BORDER_SPACING;
...
function ISClothingInsPanel:render()
    ISPanelJoypad.render(self);--self.bodyOutline:getWidth()
    self:drawRectBorder(self.coreRectangleX, UI_BORDER_SPACING+1, self.width-self.coreRectangleX-UI_BORDER_SPACING-1, self.coreRectangleH, 1.0, 0.5, 0.5, 0.5);
    if self.selectedBodyPart then
        self:drawRect(self.bpAnchorX, self.bpPanelY+self.bpAnchorY-1, self.coreRectangleX-self.bpAnchorX, 3, 1.0, 0.0, 0.0, 0.0);
        self:drawRect(self.bpAnchorX, self.bpPanelY+self.bpAnchorY, self.coreRectangleX-self.bpAnchorX, 1, 1.0, 1.0, 1.0, 1.0);
    end

    self:setWidthAndParentWidth(math.max(self.width, 100));
    self:setHeightAndParentHeight(self.toggleAdvBtn:getY()+self.toggleAdvBtn:getHeight()+UI_BORDER_SPACING+1);
end
