...
public final class CustomSandboxOptions {
    ...
    private void parse(String contents) {
        ...
        if (version >= 1 && version <= 1) {
            for (ScriptParser.Block block1 : block.children) {
                ...
                if (option == null) {
                    DebugType.General.warn("failed to parse custom sandbox option \"%s\"", block1.id);
                } else {
    ...
    private CustomSandboxOption parseOption(ScriptParser.Block block) {
        if (StringUtils.isNullOrWhitespace(block.id)) {
            DebugType.General.warn("missing or empty option id");
            return null;
        }
        ...
        if (type != null && !StringUtils.isNullOrWhitespace(type.getValue())) {
            switch (type.getValue().trim()) {
                ...
                default:
                    DebugType.General.warn("unknown option type \"%s\"", type.getValue().trim());
                    return null;
            }
        } else {
            DebugType.General.warn("missing or empty value \"type\"");
            return null;
        }
    }
}
