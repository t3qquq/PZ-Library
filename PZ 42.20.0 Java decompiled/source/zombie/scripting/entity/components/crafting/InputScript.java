...
public class InputScript extends CraftRecipe.IOScript {
    ...
    protected static InputScript LoadBlock(CraftRecipe parentRecipe, ScriptParser.Block block) throws Exception {
        ...
        for (ScriptParser.Value value : block.values) {
            if (!StringUtils.isNullOrWhitespace(value.string) && !value.string.contains("=") && StringUtils.containsWhitespace(value.string)) {
                DebugType.General.warn("Cannot load: " + value.string + ", recipe:" + parentRecipe.getScriptObjectFullType());
        ...
        if (input == null) {
            DebugType.General.warn("Cannot load input block. " + parentRecipe.getScriptObjectFullType());
        }
    ...
    protected static InputScript Load(CraftRecipe parentRecipe, String line, boolean isInternal) throws Exception {
                ...
                if (s.startsWith("mode:")) {
                    ...
                    switch (type) {
                        case Item:
                            if (!s.equalsIgnoreCase("use")) {
                                if (s.equalsIgnoreCase("keep")) {
                                    ...
                                    DebugType.General.error("Deprecated parameter in inputscript! for recipe " + recipeName);
                        ...
                        case Energy:
                            ...
                            break;
                        default:
                            DebugType.General.warn("Cannot set mode for type = " + type + " for recipe " + recipeName);
                            if (Core.debug) {
        ...
        if (input.amounts.size() > 1 && !input.isVariableAmount() && input.loadedFluids.isEmpty() && input.amounts.stream().distinct().count() > 1L) {
            input.amount = 1.0F;
    ...
    protected void OnPostWorldDictionaryInit() throws Exception {
        ...
        if (this.acceptsAnyItem) {
            ...
            for (String s : this.loadedItems) {
                ...
                if (item != null) {
                    ...
                    DebugType.General.error(this.getParentRecipe().getName() + " item not found: " + s + ". line: " + this.originalLine);
                }
            }
            ...
            for (ItemTag itemTag : this.itemTags) {
                ...
                for (Item item : tagItems) {
                    if (!this.items.contains(item.getFullName())) {
                        this.items.add(item.getFullName());
                        this.itemScriptCache.add(item);
                    }
        ...
        if (!this.isValid()) {
            DebugType.General.error("Invalid input. line: " + this.originalLine);
        }
    }
    ...
    public boolean doesItemPassFoodAndCookingTests(InventoryItem item) {
        if (!(item instanceof Food)) {
            ...
            return this.hasFlag(InputFlag.IsUncookedFoodItem) && item instanceof Food food && item.isCookable() && !food.isUncooked()
                ? false
