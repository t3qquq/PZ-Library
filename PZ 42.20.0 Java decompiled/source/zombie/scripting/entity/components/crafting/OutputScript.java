...
public class OutputScript extends CraftRecipe.IOScript {
    ...
    public ArrayList<Item> getPossibleResultItems() {
        if (this.outputMapper == null) {
            DebugType.General.warn("This output does not have items! returning empty list.");
            return _emptyItems;
        } else {
    ...
    public Item getItem(CraftRecipeData recipeData) {
        if (this.outputMapper == null) {
            DebugType.General.warn("This output does not have items! returning null.");
            return null;
        } else {
    ...
    protected static OutputScript LoadBlock(CraftRecipe parentRecipe, ScriptParser.Block block) throws Exception {
        ...
        if (output == null) {
            DebugType.General.warn("Cannot load output block. " + parentRecipe.getScriptObjectFullType());
        }
            ...
            if (s.startsWith("chance:")) {
                ...
                switch (type) {
                    ...
                    case Fluid:
                        ...
                        break;
                    default:
                        DebugType.General.warn("Cannot set mode for type = " + type);
                        if (Core.debug) {
