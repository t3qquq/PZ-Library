...
public final class CollideWithObstacles {
        ...
        if (!this.obstacles.isEmpty()) {
            ...
            for (int y = 0; y < boundsBottom - boundsTop; y++) {
                for (int x = 0; x < boundsRight - boundsLeft; x++) {
                    CollideWithObstacles.CCObjectOutline f = CollideWithObstacles.CCObjectOutline.get(x, y, z, this.oo);
                    if (f != null && f.nw && f.nwW && f.nwN) {
                        ...
                        if (!f.nodes.isEmpty()) {
                            ...
                            for (int i = f.nodes.size() - 1; i > 0; i--) {
                                CollideWithObstacles.CCNode node1 = f.nodes.get(i);
                                CollideWithObstacles.CCNode node2 = f.nodes.get(i - 1);
                                node1.x += boundsLeft;
                                node1.y += boundsTop;
    ...
    void checkEdgeIntersection() {
        ...
        for (int i = 0; i < this.obstacles.size(); i++) {
            ...
            for (int j = obstacle1.edges.size() - 1; j >= 0; j--) {
                CollideWithObstacles.CCEdge edge1 = obstacle1.edges.get(j);
    ...
    public CollideWithObstacles.CCIntersection getIntersection(CollideWithObstacles.CCEdge edge1, CollideWithObstacles.CCEdge edge2) {
        ...
        if (ua >= 0.0 && ua <= 1.0 && ub >= 0.0 && ub <= 1.0) {
            ...
            if (node1 != null && node2 != null) {
                CollideWithObstacles.CCIntersection intersection = CollideWithObstacles.CCIntersection.alloc().init(edge1, edge2, (float)ua, (float)ub, node1);
                edge1.intersections.add(intersection);
                this.intersections.add(intersection);
    ...
    private Vector2f resolveCollision(float x1, float y1, float z1, float x2, float y2, Vector2f finalPos) {
        ...
        if (closestEdge == null) {
            ...
            for (int i = 0; i < this.obstacles.size(); i++) {
                ...
                for (int j = 0; j < obstacle1.edges.size(); j++) {
                    CollideWithObstacles.CCEdge edge = obstacle1.edges.get(j);
                    if (edge.node1.visible.contains(edge.node2)) {
        ...
        if (nodeAtXY != null) {
            ...
            for (int i = 0; i < nodeAtXY.edges.size(); i++) {
                CollideWithObstacles.CCEdge edge = nodeAtXY.edges.get(i);
                if (edge.node1.visible.contains(edge.node2)
                    && edge != closestEdge
    ...
    private static final class CCObjectOutline {
        ...
        static void setWest(int x, int y, int z, CollideWithObstacles.CCObjectOutline[][] oo) {
            ...
            CollideWithObstacles.CCObjectOutline fff = f1;
            f1 = get(x, y + 1, z, oo);
            if (f1 == null) {
                if (fff != null) {
                    fff.wCutoff = true;
