...
public final class PathFindBehavior2 implements IPathfinder {
    ...
    private static final ObjectPool<PathFindBehavior2.DebugPt> actualPool = new ObjectPool<>(PathFindBehavior2.DebugPt::new, "PathFindBehavior2.actualPool");
    ...
    public void pathToCharacter(IsoGameCharacter target) {
        ...
        if (target.isSittingOnFurniture() && (target.getSquare().isSolid() || target.getSquare().isSolidTrans())) {
            ...
            switch (dir) {
                case N:
                    IsoGridSquare sqN = IsoWorld.instance.getCell().getGridSquare(x + 0.5F, y - 0.3F, z);
                    if (this.isGoodChairAdjacentSquare(target.getSquare(), sqN)) {
                        this.setData(x + 0.5F, y - 0.3F, z);
                        return;
                    }
                    break;
                case S:
                    IsoGridSquare sqS = IsoWorld.instance.getCell().getGridSquare(x + 0.5F, y + 1 + 0.3F, z);
                    if (this.isGoodChairAdjacentSquare(target.getSquare(), sqS)) {
                        this.setData(x + 0.5F, y + 1 + 0.3F, z);
                        return;
                    }
                    break;
                case W:
                    IsoGridSquare sqW = IsoWorld.instance.getCell().getGridSquare(x - 0.3F, y + 0.5F, z);
                    if (this.isGoodChairAdjacentSquare(target.getSquare(), sqW)) {
                        this.setData(x - 0.3F, y + 0.5F, z);
                        return;
                    }
                    break;
                case E:
                    IsoGridSquare sqE = IsoWorld.instance.getCell().getGridSquare(x + 1 + 0.3F, y + 0.5F, z);
                    if (this.isGoodChairAdjacentSquare(target.getSquare(), sqE)) {
                        this.setData(x + 1 + 0.3F, y + 0.5F, z);
                        return;
                    }
                    break;
                default:
                    DebugType.General.warn("unhandled sitting direction");
            }

            ArrayList<IsoGridSquare> testSquares = new ArrayList<>();

            for (int i = 0; i < 8; i++) {
                if (IsoDirections.fromIndex(i) != dir) {
                    IsoGridSquare testSquare = target.getSquare().getAdjacentSquare(IsoDirections.fromIndex(i));
                    if (this.isGoodChairAdjacentSquare(target.getSquare(), testSquare)) {
            ...
            if (!testSquares.isEmpty()) {
                for (IsoGridSquare testSquare : testSquares) {
                    ...
                    if (closestDistance == 0.0F || testDistance < closestDistance && this.isGoodChairAdjacentSquare(target.getSquare(), testSquare)) {
            ...
            if (closestSquare != null) {
                this.setData(closestSquare.getX() + 0.5F, closestSquare.getY() + 0.5F, closestSquare.getZ());
                return;
            }
        }
    ...
    private void getGrabCorpseLocations(IsoDeadBody targetBody, List<Vector2f> possibleTargetPositions) {
        if (!targetBody.isAnimal()) {
            ...
            AnimalDefinitions animalDef = AnimalDefinitions.getDef(targetBody.getAnimalType());
            float collisionSize = (animalDef == null ? 1.0F : PZMath.max(animalDef.collisionSize, 0.2F)) * targetBody.getAnimalSize();
            double radius = collisionSize * 2.0;
            int segments = 12;

            for (int i = 0; i < 12; i++) {
                double angle = Math.toRadians(i * 360.0 / 12.0);
                double cx = targetBody.getX() + radius * Math.cos(angle);
                double cy = targetBody.getY() + radius * Math.sin(angle);
    ...
    public void pathToGrabCorpse(IsoDeadBody targetBody) {
        List<Vector2f> possibleTargetPositions = new ArrayList<>();
        ...
        if (possibleTargetPositions.isEmpty()) {
            ...
            Vector2f firstPos = possibleTargetPositions.getFirst();
            this.isCancel = false;
            this.startedMoving = false;
            this.goal = PathFindBehavior2.Goal.GrabCorpse;
            this.setData(firstPos.x, firstPos.y, targetPosZ);

            for (int i = 1; i < possibleTargetPositions.size(); i++) {
                Vector2f possiblePos = possibleTargetPositions.get(i);
            ...
            if (state != ClimbOverFenceState.instance() && state != ClimbThroughWindowState.instance() && state != PlayerSitOnFurnitureState.instance()) {
                ...
                if (this.pathIndex == this.path.nodes.size() - 2) {
                    ...
                    if (distToEnd <= 0.05F) {
                        ...
                        this.chr.moveUnmodded(tempVector2_2.x, tempVector2_2.y);
                        this.stopping = true;
                        return PathFindBehavior2.BehaviorResult.Working;
                    }
                ...
                if (this.chr.isPerformingNoAimShortStrafe()) {
                    ...
                    this.chr.set(PlayerStrafeState.STRAFE_SPEED, 1.0F);
    ...
    private boolean checkDoorHoppableWindow(float nx, float ny, float z) {
        ...
        if (square != null && square != current) {
            ...
            } else if (object instanceof IsoThumpable door && door.isDoor() && !door.open) {
                if (this.chr instanceof IsoPlayer player && !player.isAnimal() && player.timeSinceCloseDoor < 50.0F) {
                    this.chr.setCollidable(false);
                } else {
                    if (!door.couldBeOpen(this.chr)) {
            ...
            if (window != null) {
                ...
                if (windowThumpable == null) {
                    ...
                    if (dx <= 0 || !square.has(IsoFlagType.TallHoppableW) && !square.has(IsoFlagType.WallW) && !square.has(IsoFlagType.WallWTrans)) {
                        if (dx >= 0 || !current.has(IsoFlagType.TallHoppableW) && !current.has(IsoFlagType.WallW) && !current.has(IsoFlagType.WallWTrans)) {
                            if (dy >= 0 || !current.has(IsoFlagType.TallHoppableN) && !current.has(IsoFlagType.WallN) && !current.has(IsoFlagType.WallNTrans)) {
                                if (dy > 0 && (square.has(IsoFlagType.TallHoppableN) || square.has(IsoFlagType.WallN) || square.has(IsoFlagType.WallNTrans))) {
                                    climbDir = IsoDirections.S;
                                }
                            } else {
                                climbDir = IsoDirections.N;
                    ...
                    if (climbDir != null && this.chr instanceof IsoPlayer player) {
    ...
    public boolean isStrafing() {
        if (this.chr.isZombie()) {
            ...
            return this.chr instanceof IsoPlayer player && !player.isLocal()
                ? false
    ...
    public boolean isGoodChairAdjacentSquare(IsoGridSquare targetSquare, IsoGridSquare adjacentSquare) {
        return adjacentSquare != null
            && !adjacentSquare.isSolid()
            && !adjacentSquare.isSolidTrans()
            && (!adjacentSquare.getProperties().has(IsoFlagType.water) || adjacentSquare.hasFloorOverWater())
            && adjacentSquare.canReachTo(targetSquare);
    }
