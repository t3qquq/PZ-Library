...
public class WeatherFxMask {
    ...
    private static void scanForTiles(int nPlayer) {
        ...
        if (!playerFxMask.diamondIterDone) {
            ...
            try (GameProfiler.ProfileArea bRender = profiler.profile("Calc Bounds")) {
            ...
            try (GameProfiler.ProfileArea ox = profiler.profile("scanTriangle")) {
                            ...
                            if (playerMasks[nPlayer].maskEnabled) {
                                ...
                                if (playerFxMask.maskEnabled) {
                                    ...
                                    if (maskingEnabled && playerFxMask.hasMaskToDraw) {
                                        ...
                                        try (GameProfiler.ProfileArea var3x = profiler.profile("scanForTiles")) {
                                            scanForTiles(nPlayer);
                                        }

                                        SpriteRenderer.instance.glIgnoreStyles(true);
                                        if (maskingEnabled) {
                                            try (GameProfiler.ProfileArea var12 = profiler.profile("drawFxMask")) {
                                                drawFxMask(nPlayer);
                                            }
                                        }
                                        ...
                                        try (GameProfiler.ProfileArea var13 = profiler.profile("drawFxLayered")) {
