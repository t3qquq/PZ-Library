            ...
            try {
                ...
                if (path.exists() && path.isDirectory()) {
                    ...
                    try (FileWriter w = new FileWriter(f, false)) {
                        ...
                        this.write("Normal warm: " + total.countNormalWarm + ", average: " + this.round((double)total.countNormalWarm / div));
                        this.write("Normal cold: " + total.countNormalCold + ", average: " + this.round((double)total.countNormalCold / div));
                        this.write("Normal storm: " + total.countStorm + ", average: " + this.round((double)total.countStorm / div));
                        this.write("Normal tropical: " + total.countTropical + ", average: " + this.round((double)total.countTropical / div));
                        this.write("Normal blizzard: " + total.countBlizzard + ", average: " + this.round((double)total.countBlizzard / div));
                        this.write("");
    ...
    private void printCountTable(FileWriter w, int[] table, int indexMod) throws Exception {
        if (table != null && table.length > 0) {
            ...
            if (t > 0) {
                for (int i = 0; i < table.length; i++) {
                    String s = "";
                    s = s + String.format("%1$8s", i * indexMod + "-" + (i * indexMod + indexMod));
                    int v = table[i];
