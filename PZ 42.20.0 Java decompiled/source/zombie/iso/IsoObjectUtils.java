...
public class IsoObjectUtils {
    ...
    private static final ThreadLocal<ArrayList<IsoObject>> threadLocalObjects1 = ThreadLocal.withInitial(ArrayList::new);
    private static final ThreadLocal<ArrayList<IsoObject>> threadLocalObjects2 = ThreadLocal.withInitial(ArrayList::new);
    private static final ThreadLocal<ArrayList<IsoObject>> threadLocalObjects3 = ThreadLocal.withInitial(ArrayList::new);
    ...
    public static int safelyRemoveTileObjectFromSquare(IsoObject object) {
        if (isObjectMultiSquare(object)) {
            ArrayList<IsoObject> objects = threadLocalObjects1.get();
            objects.clear();
            if (!getAllMultiTileObjects(object, objects)) {
                if (Core.debug) {
                    DebugType.General.warn("Failed to find all parts of a multi-tile object!");
                }
    ...
    public static void safelyBurnTileObjectFromSquare(IsoObject object) {
        if (isObjectMultiSquare(object)) {
            ArrayList<IsoObject> objects = threadLocalObjects2.get();
            objects.clear();
            getAllMultiTileObjects(object, objects);

            for (IsoObject obj : objects) {
                if (object != obj) {
                    IsoGridSquare sq = obj.square;
                    if (sq != null) {
                        sq.BurnWalls(true, false);
                    } else if (Core.debug) {
                        DebugType.General.warn("Failed to find all parts of a multi-tile object!");
                    }
                }
    ...
    public static boolean getAllMultiTileObjects(IsoObject object, ArrayList<IsoObject> outList) {
        ...
        if (ddIndex != -1) {
            ...
            if (gdIndex == -1) {
                ...
                for (IsoObject object1 = IsoDoor.getGarageDoorFirst(object); object1 != null; object1 = IsoDoor.getGarageDoorNext(object1)) {
                    outList.add(object1);
                }
    ...
    public static void dumpContentsInSquare(IsoObject object) {
        if (object.getSquare() != null) {
            ...
            if (isObjectMultiSquare(object)) {
                ArrayList<IsoObject> objects = threadLocalObjects3.get();
                objects.clear();
                if (!getAllMultiTileObjects(object, objects)) {
                    if (Core.debug) {
                        DebugType.General.warn("Failed to find all parts of a multi-tile object!");
