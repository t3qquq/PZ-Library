...
public final class FBORenderLevels {
    ...
    void calculateRenderBounds(int minLevel, int maxLevel) {
        ...
        y -= dh;
        if (minLevel == 0) {
            float y1 = IsoUtils.YToScreen(this.chunk.wx * 8, this.chunk.wy * 8, minLevel, 0);
            y1 += FBORenderChunk.FLOOR_HEIGHT;
            y1 -= FBORenderChunk.JUMBO_XXL_HEIGHT;
            float dh1 = y - y1;
            if (dh1 > 0.0F) {
                height += dh1;
                y -= dh1;
    ...
    public boolean calculateInStencilRect(int level, IsoCell cell) {
        this.calculateRenderBounds(level, level);
        float zoom = Core.getInstance().getZoom(this.playerIndex);

        for (IsoCell.StencilArea stencilArea : cell.getStencilAreas()) {
            int texW = stencilArea.texWidth() * Core.tileScale;
            int texH = stencilArea.texHeight() * Core.tileScale;
            float px = stencilArea.x() * zoom - stencilArea.offX();
            float py = stencilArea.y() * zoom - stencilArea.offY();
            ...
            if (this.renderX + this.renderW > stencilX1 && this.renderX < stencilX2 && this.renderY + this.renderH > stencilY1 && this.renderY < stencilY2) {
                return true;
            }
        }
    ...
    public void clearCachedSquares(int level) {
        ...
        nLevels.cutawayWindowFrames.clear();
    ...
    public List<IsoGridSquare> getCachedSquares_CutawayWindowFrames(int level) {
        return this.getNLevels(level).cutawayWindowFrames;
    }
    ...
    public List<IsoGridSquare> getCachedSquares_Water(int level) {
        return this.getNLevels(level).waterSquares;
    }

    public List<IsoGridSquare> getCachedSquares_WaterShore(int level) {
        return this.getNLevels(level).waterShoreSquares;
    }

    public List<IsoGridSquare> getCachedSquares_WaterAttach(int level) {
        return this.getNLevels(level).waterAttachSquares;
    }
    ...
    public static int extraHeightForJumboTrees(int minLevel, int maxLevel) {
        if (minLevel != 0) {
            ...
            return maxLevel == 1
                ? FBORenderChunk.JUMBO_L_HEIGHT - FBORenderChunk.PIXELS_PER_LEVEL * 2 - FBORenderChunk.FLOOR_HEIGHT / 2
                : FBORenderChunk.JUMBO_L_HEIGHT - FBORenderChunk.PIXELS_PER_LEVEL - FBORenderChunk.FLOOR_HEIGHT / 2;
        }
    }
    ...
    private static final class NLevels {
        ...
        final List<IsoGridSquare> animatedAttachments = new ArrayList<>();
        final List<IsoGridSquare> corpseSquares = new ArrayList<>();
        final List<IsoGridSquare> cutawayWindowFrames = new ArrayList<>();
        final List<IsoGridSquare> fliesSquares = new ArrayList<>();
        final List<IsoGridSquare> itemSquares = new ArrayList<>();
        final List<IsoGridSquare> puddleSquares = new ArrayList<>();
        final List<IsoGridSquare> translucentFloor = new ArrayList<>();
        final List<IsoGridSquare> translucentNonFloor = new ArrayList<>();
        final List<IsoGridSquare> waterSquares = new ArrayList<>();
        final List<IsoGridSquare> waterShoreSquares = new ArrayList<>();
        final List<IsoGridSquare> waterAttachSquares = new ArrayList<>();
        ...
        void invalidate(long dirtyFlags) {
            ...
            if (FBORenderLevels.clearCachedSquares) {
                if (!FBORenderChunkManager.instance.isCaching()) {
                    ...
                    this.cutawayWindowFrames.clear();
