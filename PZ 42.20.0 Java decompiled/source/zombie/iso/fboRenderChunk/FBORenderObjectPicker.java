...
public final class FBORenderObjectPicker {
    ...
    private final ObjectPool<IsoObjectPicker.ClickObject> clickObjectPool = new ObjectPool<>(
        IsoObjectPicker.ClickObject::new, "FBORenderObjectPicker.clickObjectPool"
    );
        ...
        for (int n = this.clickObjects.size() - 1; n >= 0; n--) {
            ...
            if (!(object instanceof IsoPlayer) || object != IsoPlayer.players[0]) {
                ...
                if (sprite != null) {
                    ...
                    if (targetAlpha == 0.0F || this.shouldIgnoreWallLikeObject(object, sprite, targetAlpha)) {
                        continue;
                    }
                }
    ...
    private boolean shouldIgnoreWallLikeObject(IsoObject object, IsoSprite sprite, float targetAlpha) {
        if (!sprite.hasProperty(IsoFlagType.cutW) && !sprite.hasProperty(IsoFlagType.cutN)) {
            ...
            return false;
        } else if (object instanceof IsoThumpable isoThumpable && isoThumpable.isDoor()) {
            return false;
        } else {
            return object instanceof IsoDoor ? false : targetAlpha < 1.0F;
        }
    }

    private boolean shouldIgnoreCutawayWall(IsoObject object) {
        if (object instanceof IsoWindowFrame windowFrame && !windowFrame.hasWindow()) {
            return false;
        } else {
            return object instanceof IsoThumpable thumpable && thumpable.isWindow() && object.getSquare().getWindow(thumpable.getNorth()) == null
                ? false
                : !(object instanceof IsoDoor);
        }
    }
    ...
    public void getClickObjects(int screenX, int screenY, PZArrayList<IsoObjectPicker.ClickObject> clickObjects) {
        ...
        for (int i = 0; i < this.objects.size(); i++) {
            ...
            if (renderChunk != null && !this.handleWaterShader(object, clickObjects)) {
                ...
                if (!renderInfo.cutaway || !this.shouldIgnoreCutawayWall(object)) {
                    ...
                    if (renderInfo.layer == ObjectRenderLayer.Translucent
                        || renderInfo.layer == ObjectRenderLayer.TranslucentFloor
                    ...
                    clickObject.z = object.getSquare().getZ();
    ...
    static int renderLayerIndex(IsoObjectPicker.ClickObject o) {
        return switch (o.tile.getRenderInfo(0).layer) {
            ...
            case MinusFloorSE, TranslucentSE -> 5;
        };
    }
    ...
    void getObjectsAt(int screenX, int screenY, ArrayList<IsoObject> objects) {
        ...
        if (player.getZi() < 0) {
            for (int z = -32; z < 0; z++) {
                float worldX = IsoUtils.XToIso(0, screenX, screenY, z);
                float worldY = IsoUtils.YToIso(0, screenX, screenY, z);
    ...
    public IsoObject PickDoor(int screenX, int screenY, boolean bTransparent) {
        return this.getFirst(screenX, screenY, (obj, x, y) -> {
            ...
            if (obj.tile.getRenderInfo(0).targetAlpha == 0.0F) {
                return 0;
            }

            if (bTransparent != obj.tile.getRenderInfo(0).targetAlpha < 1.0F) {
                return 0;
            }

            if (obj.contains(x, y, IsoPlayer.players[0].getZi())) {
        ...
        return this.getFirst(screenX, screenY, (obj, x, y) -> {
            ...
            if (obj.contains(x, y, IsoPlayer.players[0].getZi())) {
                ...
                if (obj.tile instanceof IsoWindow) {
                    ...
                    for (int ty = ly; ty < obj.height; ty++) {
                        if (obj.tile.isMaskClicked(lx, ty)) {
                            below = true;
                            break;
                        }
