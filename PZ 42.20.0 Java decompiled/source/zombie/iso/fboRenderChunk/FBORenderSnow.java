...
public final class FBORenderSnow {
    ...
    public boolean isSnowAnywhere() {
        return IsoWorld.instance.currentCell.getSnowTarget() > 0;
    }
    ...
    private static final class SnowGrid {
        ...
        public FBORenderSnow.SnowGrid init(int worldX, int worldY, int level, int frac) {
            ...
            if (!isWinter) {
                ...
                try (GameProfiler.ProfileArea s = profiler.profile("Noise")) {
                    for (int y = 0; y < this.h; y++) {
                        for (int x = 0; x < this.w; x++) {
                            for (int sx = 0; sx < 2; sx++) {
                                this.grid[x][y][sx] = null;
                            ...
                            if (level == 0) {
                                ...
                                for (int d = 0; d < FBORenderSnow.DIRECTIONS.length; d++) {
                                    IsoDirections dir = FBORenderSnow.DIRECTIONS[d];
                ...
                try (GameProfiler.ProfileArea var24 = profiler.profile("Check Set")) {
                    for (int y = 0; y < this.h; y++) {
                        for (int x = 0; x < this.w; x++) {
