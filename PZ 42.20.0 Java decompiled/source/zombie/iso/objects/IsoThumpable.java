...
import zombie.audio.parameters.ParameterSinkType;
...
public class IsoThumpable extends IsoObject implements BarricadeAble, Thumpable, IHasHealth, ILockableDoor {
    ...
    private static final String BUILD_MATERIAL_PREFIX = "need:";
    private KahluaTable table;
    private KahluaTable modData;
    private KahluaTable buildMaterials;
    private boolean isDoor;
    private boolean isDoorFrame;
    public static final SoundKey DEFAULT_BREAK_SOUND = SoundKey.BREAK_OBJECT;
    private String breakSound = DEFAULT_BREAK_SOUND.getSoundName();
    ...
    public void setModData(KahluaTable modData) {
        if (modData != null) {
            ...
            while (it.advance()) {
                if (it.getKey() instanceof String key && key.startsWith("need:")) {
                    this.getBuildMaterials().rawset(key.substring("need:".length()), it.getValue());
                    needKeys.add(key);
                }
            }

            for (String key : needKeys) {
                modData.rawset(key, null);
            }
        }
    ...
    public KahluaTable getBuildMaterials() {
        if (this.buildMaterials == null) {
            this.buildMaterials = LuaManager.platform.newTable();
        }
    ...
    public boolean hasBuildMaterials() {
        return this.buildMaterials != null && !this.buildMaterials.isEmpty();
    }
    ...
    public String getBreakSound() {
        return this.breakSound != null && !this.breakSound.equalsIgnoreCase(DEFAULT_BREAK_SOUND.getSoundName())
            ? this.breakSound
            : GetBreakFurnitureSound(this.getSprite());
    }
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        if (!bits.equals(0)) {
            ...
            if (bits.hasFlags(4398046511104L)) {
                if (this.buildMaterials == null) {
                    this.buildMaterials = LuaManager.platform.newTable();
                }

                this.buildMaterials.load(input, worldVersion);
            }
        }

        bits.release();
        if (worldVersion < 248 && this.modData != null) {
    ...
    public void save(ByteBuffer output, boolean isDebugSave) throws IOException {
        ...
        if (this.buildMaterials != null && !this.buildMaterials.isEmpty()) {
            bits.addFlags(4398046511104L);
            this.buildMaterials.save(output);
        }
    ...
    public void Thump(IsoMovingObject thumper, int thumpEventCount) {
        if (SandboxOptions.instance.lore.thumpOnConstruction.getValue()) {
            ...
            if (thumper instanceof IsoZombie isoZombie) {
                ...
                int maxThumpers = this.thumpDmg;
                int damagePerMaxThumpers = 1;
                if (totalThumpers >= maxThumpers) {
                    int amount = 1 * thumpEventCount;
                    this.setHealth(this.getHealth() - amount);
                } else {
                    float partialAmount = (float)(1 * totalThumpers) / maxThumpers;
                    this.partialThumpDmg += partialAmount * thumpEventCount;
                ...
                WorldSoundManager.instance
                    .addSound(thumper, this.square.getX(), this.square.getY(), this.square.getZ(), 20, 20, true, 4.0F, this.getStressModFromThumping());
                if (this.isDoor()) {
                    this.setRenderEffect(RenderEffectType.Hit_Door, true);
                }
            ...
            if (this.health <= 0) {
                if (!bBreakableFence) {
                    ((IsoGameCharacter)thumper).getEmitter().playSound(this.getBreakSound(), this);
                    if (GameServer.server) {
                        GameServer.PlayWorldSoundServer(
                            (IsoGameCharacter)thumper, this.getBreakSound(), false, thumper.getCurrentSquare(), 0.2F, 70.0F, 1.1F, true
                        );
                    }
                }
    ...
    public Thumpable getThumpableFor(IsoGameCharacter chr, HandWeapon weapon) {
        if (this.isWindow()) {
            IsoBarricade barricade = this.getBarricadeForCharacter(chr);
            if (barricade != null && !barricade.canAttackBypassIsoBarricade(chr, weapon)) {
                return barricade;
            }

            barricade = this.getBarricadeOppositeCharacter(chr);
            return barricade != null && !barricade.canAttackBypassIsoBarricade(chr, weapon) ? barricade : null;
        } else {
            ...
            if (thumpable != null) {
                if (thumpable instanceof IsoBarricade) {
                    ...
                    if (weapon.getDoorHitSound() != null) {
                        ParameterMeleeHitSurface.Material material = this.getMeleeHitSurface();
                        if (GameServer.server) {
                    ...
                    if (!this.IsStrengthenedByPushedItems() && this.health <= 0 || this.IsStrengthenedByPushedItems() && this.health <= -this.pushedMaxStrength
                     ...
                     {
                        owner.getEmitter().playSound(this.getBreakSound(), this);
                        WorldSoundManager.instance.addSound(owner, this.square.getX(), this.square.getY(), this.square.getZ(), 20, 20, false, 0.0F, 15.0F);
                        if (GameClient.client) {
                            GameClient.instance
                                .sendClientCommandV(
    ...
    public void ToggleDoorActual(IsoGameCharacter chr) {
        if (!(chr instanceof IsoAnimal)) {
            if (GameClient.client || GameServer.server) {
                short onlineID = chr != null ? chr.getOnlineID() : -1;
    ...
    public boolean isLockedToCharacter(IsoGameCharacter chr) {
        if (GameClient.client
            && chr instanceof IsoPlayer isoPlayer
            && isoPlayer.getRole() != null
            && isoPlayer.getRole().hasCapability(Capability.CanOpenLockedDoors)) {
            return false;
        } else {
    ...
    private ParameterMeleeHitSurface.Material getMeleeHitSurface() {
        if (!this.isDoor() && this.hasProperty(IsoPropertyType.THUMP_SOUND)) {
            String soundName = this.getProperty(IsoPropertyType.THUMP_SOUND);
            if (!StringUtils.isNullOrWhitespace(soundName)) {
                return switch (soundName) {
                    case "ZombieThumpGarageDoor" -> ParameterMeleeHitSurface.Material.GarageDoor;
                    case "ZombieThumpGeneric" -> ParameterMeleeHitSurface.Material.Default;
                    case "ZombieThumpMetalPoleGate" -> ParameterMeleeHitSurface.Material.MetalGate;
                    case "ZombieThumpMetal" -> ParameterMeleeHitSurface.Material.Metal;
                    case "ZombieThumpChainlinkFence" -> ParameterMeleeHitSurface.Material.MetalGate;
                    case "ZombieThumpWindow", "ZombieThumpWindowExtra" -> ParameterMeleeHitSurface.Material.Glass;
                    case "ZombieThumpWood" -> ParameterMeleeHitSurface.Material.Wood;
                    default -> ParameterMeleeHitSurface.Material.Default;
                };
            }
        }

        return ParameterMeleeHitSurface.getMaterialFromString(this.getSoundPrefix());
    }

    public static String GetBreakFurnitureSound(IsoSprite sprite) {
        if (sprite == null) {
            return SoundKey.BREAK_FURNITURE.getSoundName();
        }

        if (sprite.getProperties().propertyEquals(IsoPropertyType.MOVE_TYPE, "Vegitation")) {
            return SoundKey.BREAK_PLANT.getSoundName();
        }

        String sinkTypeStr = sprite.getProperty(IsoPropertyType.SINK_TYPE);
        if (sinkTypeStr != null) {
            ParameterSinkType.SinkType sinkType = ParameterSinkType.SinkType.fromString(sinkTypeStr, ParameterSinkType.SinkType.GENERIC);

            return switch (sinkType) {
                case CERAMIC -> SoundKey.BREAK_FURNITURE_CERAMIC.getSoundName();
                case METAL -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
                default -> SoundKey.BREAK_FURNITURE.getSoundName();
            };
        } else {
            String fenceTypeLow = sprite.getProperties().get(IsoPropertyType.FENCE_TYPE_LOW);
            if (fenceTypeLow != null) {
                ParameterFenceTypeLow.FenceType fenceType = ParameterFenceTypeLow.FenceType.fromString(fenceTypeLow, ParameterFenceTypeLow.FenceType.WOOD);

                return switch (fenceType) {
                    case BARBWIRE, METAL, METAL_GATE -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
                    default -> SoundKey.BREAK_FURNITURE.getSoundName();
                };
            } else {
                String fenceTypeHigh = sprite.getProperties().get(IsoPropertyType.FENCE_TYPE_HIGH);
                if (fenceTypeHigh != null) {
                    ParameterFenceTypeHigh.FenceType fenceType = ParameterFenceTypeHigh.FenceType.fromString(
                        fenceTypeHigh, ParameterFenceTypeHigh.FenceType.WOOD
                    );

                    return switch (fenceType) {
                        case METAL, METAL_GATE -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
                        default -> SoundKey.BREAK_FURNITURE.getSoundName();
                    };
                } else {
                    String materialTypeStr = sprite.getProperty(IsoPropertyType.MATERIAL_TYPE);
                    MaterialType materialType = StringUtils.tryParseEnum(MaterialType.class, materialTypeStr, MaterialType.Default);

                    return switch (materialType) {
                        case Ceramic -> SoundKey.BREAK_FURNITURE_CERAMIC.getSoundName();
                        case Metal, Metal_Large, Metal_Light, Metal_Solid -> SoundKey.BREAK_FURNITURE_METAL.getSoundName();
                        default -> SoundKey.BREAK_FURNITURE.getSoundName();
                    };
                }
            }
    ...
    public void animalHit(IsoAnimal animal) {
        ...
        if (this.health <= 0) {
            animal.getEmitter().playSound(this.getBreakSound(), this);
            WorldSoundManager.instance.addSound(null, this.square.getX(), this.square.getY(), this.square.getZ(), 10, 20, true, 4.0F, 15.0F);
            if (GameServer.server) {
                GameServer.PlayWorldSoundServer(animal, this.getBreakSound(), false, animal.getCurrentSquare(), 0.2F, 20.0F, 1.1F, true);
            }
