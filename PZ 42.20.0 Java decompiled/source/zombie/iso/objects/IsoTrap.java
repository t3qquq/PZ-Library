...
public class IsoTrap extends IsoObject implements IItemProvider {
            ...
            if (this.getSmokeRange() > 0 && this.getObjectIndex() != -1) {
                ...
                for (int x = this.getSquare().getX() - radius; x <= this.getX() + radius; x++) {
                    for (int y = this.getSquare().getY() - radius; y <= this.getSquare().getY() + radius; y++) {
                        IsoGridSquare explodedSquare = this.getCell().getGridSquare(x, y, this.getZ());
                            ...
                            if (this.timerBeforeExplosion <= 0) {
                                for (IsoMovingObject movingObject : IsoWorld.instance.currentCell.getObjectList()) {
                                    ...
                                    if ((character == null || !character.isInvisible())
                                        && movingObject.getZi() == this.getSquare().getZ()
                                        && !(
                                            IsoUtils.DistanceToSquared(movingObject.getX(), movingObject.getY(), this.getX() + 0.5F, this.getY() + 0.5F)
                                                > this.getSensorRange() * this.getSensorRange()
                                        )) {
                                        LosUtil.TestResults testResults = LosUtil.lineClear(
                                            this.getSquare().getCell(),
                                            this.getSquare().getX(),
                                            this.getSquare().getY(),
                                            this.getSquare().getZ(),
                                            movingObject.getXi(),
                                        ...
                                        if (testResults != LosUtil.TestResults.Blocked && testResults != LosUtil.TestResults.ClearThroughClosedDoor) {
                                            if (GameServer.server) {
                                                UdpConnection excluded = null;
                                                HandWeapon TRIGGER_EXPLOSION = null;
                                                INetworkPacket.sendToAll(
                                                    PacketTypes.PacketType.AddExplosiveTrap, excluded, TRIGGER_EXPLOSION, this.getAttacker(), this.getSquare()
                                                );
                                            }
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoChild, boolean bWallLightingPass, Shader shader) {
        ...
        if (!this.sprite.hasNoTextures()) {
            Texture tex = this.sprite.getTextureForCurrentFrame(this.getForwardIsoDirection(), this);
            if (tex != null) {
                if (tex.getName().startsWith("Item_")) {
                    ...
                    this.offsetY = 0.0F;
                    this.sx = 0.0F;
                    this.sprite.render(this, x + 0.5F, y + 0.5F, z, this.getForwardIsoDirection(), this.offsetX + dx, this.offsetY + dy, col, true);
                } else {
    ...
    public void place() {
        if (this.getSquare() != null && this.getHandWeapon() != null) {
            if (!GameClient.client) {
                ...
                if (GameServer.server) {
                    INetworkPacket.sendToAll(PacketTypes.PacketType.AddExplosiveTrap, null, this.getHandWeapon(), this.getAttacker(), this.getSquare());
                }
    ...
    public static void triggerRemote(IsoPlayer player, int remoteID, int range) {
        ...
        int py = player.getYi();
        int pz = player.getZi();
        int minZ = Math.max(pz - range / 2, -32);
        int maxZ = Math.min(pz + range / 2, 32);
    ...
    public boolean shouldPlaceInWorldAfterThrowing() {
        return !this.isInstantExplosion() || this.getHandWeapon() != null && this.getHandWeapon().canBeReused() || this.getExplosionDuration() > 0;
    }
    ...
    private boolean shouldProcess(IsoGameCharacter target) {
        if (!GameClient.client && !GameServer.server) {
            return true;
        } else if (GameClient.client) {
            return target instanceof IsoZombie zombie && zombie.isLocal();
        } else if (target instanceof IsoAnimal || target instanceof IsoZombie) {
            return true;
        } else if (target instanceof IsoPlayer player) {
            IsoGameCharacter attacker = this.getAttacker();
            return attacker != null && attacker == target
                ? true
                : ServerOptions.getInstance().pvp.getValue()
                    && (!player.getSafety().isEnabled() || attacker instanceof IsoPlayer playerAttacker && !playerAttacker.getSafety().isEnabled());
        } else {
            return false;
        }
    }
    ...
    private void explosion(IsoGridSquare isoGridSquare) {
        ...
        for (int i = 0; i < isoGridSquare.getMovingObjects().size(); i++) {
            ...
            if (moving instanceof IsoGameCharacter isoGameCharacter && this.shouldProcess(isoGameCharacter)) {
