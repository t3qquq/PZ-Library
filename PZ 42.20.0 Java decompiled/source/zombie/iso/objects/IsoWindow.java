...
public class IsoWindow extends IsoObject implements BarricadeAble, Thumpable {
    ...
    public static final int SMASH_SOUND_RADIUS = 70;
    ...
    public void WeaponHit(IsoGameCharacter owner, HandWeapon weapon) {
        ...
        Thumpable thumpable = this.getThumpableFor(owner, weapon);
            ...
            if (thumpable instanceof IsoBarricade) {
                ...
                if (weapon == ((IsoLivingCharacter)owner).bareHands) {
                    ...
                    if (weapon != null && weapon.getDoorHitSound() != null) {
                        ParameterMeleeHitSurface.Material material = ParameterMeleeHitSurface.Material.Glass;
    ...
    public void smashWindow(boolean bRemote, boolean doAlarm) {
        if (!this.destroyed) {
            ...
            if (!bRemote) {
                if (GameServer.server) {
                    GameServer.PlayWorldSoundServer("SmashWindow", false, this.square, 0.2F, 70.0F, 1.1F, true);
                } else {
                    SoundManager.instance.PlayWorldSound("SmashWindow", this.square, 0.2F, 70.0F, 1.0F, true);
                }
            ...
            this.getSquare().removeGlassAttachments(this);
            if (doAlarm) {
                this.handleAlarm();
            }
    ...
    public void Thump(IsoMovingObject thumper, int thumpEventCount) {
        ...
        if (thumper instanceof IsoZombie isoZombie) {
            ...
            this.damage(isoZombie.strength * thumpEventCount, thumper);
    ...
    public Thumpable getThumpableFor(IsoGameCharacter chr) {
        ...
        if (barricade != null) {
            ...
            return !this.isDestroyed() && !this.IsOpen() ? this : this.getBarricadeOppositeCharacter(chr);
        }
    }
        ...
        if (chr == IsoPlayer.getInstance()) {
            ...
            if (s != null) {
                for (int n = 0; n < s.getSpecialObjects().size(); n++) {
                    if (s.getSpecialObjects().get(n) instanceof IsoCurtain) {
        ...
        while (sq != null) {
            ...
            for (int i = 0; i < sq.getObjects().size(); i++) {
                ...
                if (o.getProperties() != null && (o.getProperties().has(type1) || o.getProperties().has(type2))) {
                    sq.transmitRemoveItemFromSquare(o);
                    if (player != null) {
                        InventoryItem item = player.getInventory().AddItem(o.getName());
                        if (GameServer.server) {
    ...
    public boolean canAttackBypassIsoBarricade(IsoGameCharacter isoGameCharacter, HandWeapon handWeapon) {
        ...
        if (isoBarricade != null && !isoBarricade.canAttackBypassIsoBarricade(isoGameCharacter, handWeapon)) {
            return false;
        }

        isoBarricade = this.getBarricadeOppositeCharacter(isoGameCharacter);
        return isoBarricade == null || isoBarricade.canAttackBypassIsoBarricade(isoGameCharacter, handWeapon);
    }
    ...
    private enum LockedHouseFrequency {
        Never(0, 0),
        ExtremelyRare(1, 5),
        Rare(2, 10),
        Sometimes(3, 50),
        ...
        LockedHouseFrequency(final int value, final int lockChance) {
            this.value = value;
