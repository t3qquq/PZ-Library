...
public class IsoFireplace extends IsoObject {
    ...
    public IsoFireplace(IsoCell cell, IsoGridSquare sq, IsoSprite gid) {
        super(cell, sq, gid);
        String containerType = gid != null && gid.getProperties().has(IsoFlagType.container) ? gid.getProperties().get(IsoPropertyType.CONTAINER) : "fireplace";
    ...
    private void updateFuelSprite() {
        if (!this.getSprite().getProperties().has(IsoPropertyType.CUSTOM_NAME)
            || !this.getSprite().getProperties().get(IsoPropertyType.CUSTOM_NAME).equals("Metal Drum")) {
    ...
    private void updateFireSprite() {
        if (this.isLit()) {
            if (this.fireSpriteIndex == -1) {
                ...
                float scale = 0.5F * (Core.tileScale / 2.0F);
                if (Objects.equals(this.getSprite().getName(), "camping_03_16")) {
                    ...
                    String sprite = split[0] + "_" + split[1] + "_" + split[2] + "_on_" + split[3];
                    this.setOverlaySprite(sprite);
                } else if (this.getSprite().getProperties().has(IsoPropertyType.CUSTOM_NAME)
                    && this.getSprite().getProperties().get(IsoPropertyType.CUSTOM_NAME).equals("Metal Drum")) {
                ...
                if (this.attachedAnimSprite != null && this.fireSpriteIndex < this.attachedAnimSprite.size()) {
                    ...
                    if (sprite1.hasAnimation()) {
                        spriteInstance.frame = Rand.Next(0.0F, sprite1.currentAnim.frames.size());
                    }

                    spriteInstance.setScale(scale, scale);
                    Texture tex = sprite1.getTextureForCurrentFrame(this.getDir(), this);
                    if (tex != null) {
                        sprite1.soffX = (short)(sprite1.soffX + (short)(this.offsetX - tex.getWidthOrig() * scale / 2.0F));
                        sprite1.soffY = (short)(sprite1.soffY + (short)(this.offsetY - tex.getHeightOrig() * scale));
                    }
                }
            }
    ...
    private int calcLightRadius() {
        return (int)GameTime.instance.Lerp(1.0F, 8.0F, Math.min(this.getFuelAmount(), 60.0F) / 60.0F);
    }
        ...
        if (this.attachedAnimSprite != null) {
            for (int i = 0; i < this.attachedAnimSprite.size(); i++) {
                ...
                if (!PerformanceSettings.fboRenderChunk || !spriteInstance.getParentSprite().animate) {
                    ...
                    spriteInstance.getParentSprite().render(spriteInstance, this, x, y, z, this.getForwardIsoDirection(), offsetX, offsetY, col, true);
                    this.sx = 0.0F;
                }
            }
        }
