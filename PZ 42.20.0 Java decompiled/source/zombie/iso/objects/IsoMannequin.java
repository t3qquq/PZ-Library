...
public class IsoMannequin extends IsoObject implements IHumanVisual {
    ...
    public void rotate(IsoDirections newDir) {
        if (newDir != null) {
            this.setForwardIsoDirection(newDir);
    ...
    public void saveChange(IsoObjectChange change, KahluaTable tbl, ByteBufferWriter bb) {
        if (change == IsoObjectChange.ROTATE) {
            bb.putEnum(this.getForwardIsoDirection());
        } else {
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        this.setForwardIsoDirection(IsoDirections.fromIndex(input.get()));
    ...
    public void save(ByteBuffer output, boolean isDebugSave) throws IOException {
        ...
        output.put((byte)this.getForwardIsoDirection().ordinal());
    ...
    private void validatePose() {
        ...
        if (animSet == null) {
            DebugType.General.warn("ERROR: mannequin AnimSet \"%s\" doesn't exist", this.animSet);
            this.pose = "Invalid";
        } else {
            AnimState state = animSet.GetState(this.animState);
            if (state == null) {
                DebugType.General.warn("ERROR: mannequin AnimSet \"%s\" state \"%s\" doesn't exist", this.animSet, this.animState);
                this.pose = "Invalid";
            } else {
                ...
                if (state.nodes == null) {
                    DebugType.General.warn("ERROR: mannequin AnimSet \"%s\" state \"%s\" node \"%s\" doesn't exist", this.animSet, this.animState, this.pose);
                    this.pose = "Invalid";
                } else {
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoChild, boolean bWallLightingPass, Shader shader) {
        ...
        if (this.animate) {
            ...
            if (FBORenderChunkManager.instance.isCaching()) {
                ...
                IsoDirections dirOld = this.getForwardIsoDirection();

                try {
                    IsoMannequin.PerPlayer perPlayer = this.perPlayer[playerIndex];
                    if (perPlayer.renderDirection != null) {
                        this.setForwardIsoDirection(perPlayer.renderDirection);
                    ...
                    if (alpha != 0.0F) {
                        ...
                        perPlayer.lastRenderDirection = this.getForwardIsoDirection();
                        return;
                    }
                } finally {
                    this.setForwardIsoDirection(dirOld);
                }
                ...
                perPlayer.lastRenderDirection = dir;
    ...
    public DeadBodyAtlas.BodyTexture getAtlasTexture() {
        ...
        IsoMannequin.PerPlayer perPlayer = this.perPlayer[playerIndex];
        IsoDirections dirOld = this.getForwardIsoDirection();
        if (perPlayer.lastRenderDirection != null) {
            this.setForwardIsoDirection(perPlayer.lastRenderDirection);
        }
    ...
    private void getPropertiesFromSprite() {
        switch (this.sprite.name) {
            case "location_shop_mall_01_65":
                ...
                this.setForwardIsoDirection(IsoDirections.SE);
                break;
            case "location_shop_mall_01_66":
                this.mannequinScriptName = "FemaleWhite02";
                this.setForwardIsoDirection(IsoDirections.S);
                break;
        ...
        if (this.getObjectIndex() != -1) {
            ...
            if (metaCell != null && metaCell.mannequinZones != null) {
                ...
                if (zone != null) {
                    ...
                    if (zone.dir != null) {
                        this.setForwardIsoDirection(zone.dir);
                    }
    ...
    private void syncModel() {
        ...
        if (this.animate) {
            ...
            this.animatedModel.setVariable("Female", this.female);
            this.animatedModel.setVariable("Pose", this.getPose());
            this.animatedModel.setAngle(this.getForwardIsoDirection().ToVector());
            this.animatedModel.setModelData(this.humanVisual, this.itemVisuals);
        }
    }
    ...
    public static boolean isMannequinSprite(IsoSprite sprite) {
        return sprite != null && "Mannequin".equals(sprite.getProperties().get(IsoPropertyType.CUSTOM_NAME));
    }
