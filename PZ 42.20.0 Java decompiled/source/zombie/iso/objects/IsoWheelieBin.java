...
public class IsoWheelieBin extends IsoPushableObject {
    ...
    public IsoWheelieBin(IsoCell cell) {
        ...
        this.width = 0.3F;
        this.setForwardIsoDirection(IsoDirections.E);
    ...
    public void update() {
        ...
        if (IsoPlayer.getInstance() != null && IsoPlayer.getInstance().getDragObject() != this) {
            if (this.velx != 0.0F
                && this.vely == 0.0F
                && (this.getForwardIsoDirection() == IsoDirections.E || this.getForwardIsoDirection() == IsoDirections.W)) {
                this.setNextX(this.getNextX() + this.velx * 0.65F * capacityDelta);
            }

            if (this.vely != 0.0F
                && this.velx == 0.0F
                && (this.getForwardIsoDirection() == IsoDirections.N || this.getForwardIsoDirection() == IsoDirections.S)) {
    ...
    public float getWeight(float x, float y) {
        ...
        if (capacityDelta > 1.0F) {
            ...
            if (this.getForwardIsoDirection() != IsoDirections.W && (this.getForwardIsoDirection() != IsoDirections.E || y != 0.0F)) {
                return this.getForwardIsoDirection() != IsoDirections.N && (this.getForwardIsoDirection() != IsoDirections.S || x != 0.0F)
                    ? weight * 3.0F
                    : weight / 2.0F;
            } else {
                return weight / 2.0F;
