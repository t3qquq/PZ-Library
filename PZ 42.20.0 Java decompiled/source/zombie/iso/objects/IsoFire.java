...
public class IsoFire extends IsoObject {
    public static final int LIGHT_RADIUS_MINIMUM = 1;
    public static final int LIGHT_RADIUS_LOW = 5;
    public static final int LIGHT_RADIUS_MEDIUM = 8;
    public static final int LIGHT_RADIUS_HIGH = 12;
    ...
    public static final float LIGHT_R = 0.61F;
    public static final float LIGHT_G = 0.175F;
    public static final float LIGHT_B = 0.0F;
    private float accum;
    ...
    public IsoFire(IsoCell cell, IsoGridSquare gridSquare, boolean canBurnAnywhere, int startingEnergy, int setLife) {
        ...
        if (setLife == 0 && this.square.getProperties() != null && !this.square.getProperties().has(IsoFlagType.vegitation) && this.square.getFloor() != null) {
    ...
    public void render(float x, float y, float z, ColorInfo col, boolean bDoChild, boolean bWallLightingPass, Shader shader) {
        ...
        for (int i = 0; i < this.attachedAnimSprite.size(); i++) {
            ...
            if (sprite != null && sprite.currentAnim != null && sprite.def != null) {
                Texture tex = sprite.getTextureForCurrentFrame(this.getDir(), this);
                if (tex != null) {
                    ...
                    sprite.soffX = (short)(sprite.soffX + (short)(this.offsetX - tex.getWidthOrig() / 2.0F * scale));
                    sprite.soffY = (short)(sprite.soffY + (short)(this.offsetY - tex.getHeightOrig() * scale));
                    this.attachedAnimSprite.get(i).setScale(scale, scale);
                }
            }
        }
        ...
        if (this.attachedAnimSprite != null) {
            ...
            for (int i = 0; i < this.attachedAnimSprite.size(); i++) {
                ...
                if (sprite != null && sprite.def != null) {
                    Texture tex = sprite.getTextureForCurrentFrame(this.getForwardIsoDirection(), this);
                    if (tex != null) {
                        ...
                        sprite.soffX = (short)(soffX + 32 * Core.tileScale - tex.getWidthOrig() / 2.0F * scale);
                        sprite.soffY = (short)(soffY + 128 * Core.tileScale - tex.getHeightOrig() * scale);
                        spriteInstance.setScale(scale, scale);
                        spriteInstance.getParentSprite()
                            .render(spriteInstance, this, x + 0.0F, y + 0.0F, z, this.getForwardIsoDirection(), offsetX, offsetY, tempColorInfo, true);
                        sprite.soffX = soffX;
                        sprite.soffY = soffY;
                    }
                }
