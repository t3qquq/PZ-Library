...
public class IsoBarricade extends IsoObject implements Thumpable, IHasHealth {
    ...
    private void chooseSprite() {
        ...
        if (this.metalHealth > 0) {
            ...
            switch (this.getForwardIsoDirection()) {
                case W:
    ...
    public IsoObject.VisionResult TestVision(IsoGridSquare from, IsoGridSquare to) {
        ...
        if (from == this.square) {
            if (this.getForwardIsoDirection() == IsoDirections.N && to.getY() < from.getY()) {
                return IsoObject.VisionResult.Blocked;
            }

            if (this.getForwardIsoDirection() == IsoDirections.S && to.getY() > from.getY()) {
                return IsoObject.VisionResult.Blocked;
            }

            if (this.getForwardIsoDirection() == IsoDirections.W && to.getX() < from.getX()) {
                return IsoObject.VisionResult.Blocked;
            }

            if (this.getForwardIsoDirection() == IsoDirections.E && to.getX() > from.getX()) {
                return IsoObject.VisionResult.Blocked;
            }
    ...
    public void Thump(IsoMovingObject thumper, int thumpEventCount) {
        if (!this.isDestroyed()) {
            if (thumper instanceof IsoZombie isoZombie) {
                ...
                this.Damage(isoZombie.strength * thumpEventCount);
    ...
    public Vector2 getFacingPosition(Vector2 pos) {
        if (this.square == null) {
        ...
        } else if (this.getForwardIsoDirection() == IsoDirections.N) {
            ...
            return this.getForwardIsoDirection() == IsoDirections.E
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        this.setForwardIsoDirection(IsoDirections.fromIndex(dirIndex));
    ...
    public boolean canAttackBypassIsoBarricade(IsoGameCharacter isoGameCharacter, HandWeapon handWeapon) {
        if (handWeapon == null) {
            ...
            return true;
        } else if (this.isBlockVision()) {
            return false;
        } else {
            return handWeapon.isAimedFirearm() ? true : handWeapon.canAttackPierceTransparentWall(isoGameCharacter, handWeapon);
        }
    }
