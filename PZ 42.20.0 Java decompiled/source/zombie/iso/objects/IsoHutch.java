...
public class IsoHutch extends IsoObject {
    ...
    public void transmitCompleteItemToClients() {
        if (GameServer.server) {
            ...
            if (extraSprites != null) {
                Set<IsoGridSquare> checkedSquares = new HashSet<>();
                KahluaTableIterator iterator = extraSprites.iterator();
                ...
                while (iterator.advance()) {
                    ...
                    if (!checkedSquares.contains(sq2)) {
                        checkedSquares.add(sq2);
    ...
    public void syncIsoObjectReceive(ByteBufferReader bb) {
        ...
        byte nestBoxes = bb.getByte();

        for (int i = 0; i < nestBoxes; i++) {
            byte eggs = bb.getByte();
            IsoHutch.NestBox nestBox = this.getNestBox(i);
            nestBox.eggs.clear();

            for (int j = 0; j < eggs; j++) {
                try {
                    if (InventoryItem.loadItem(bb.bb, IsoWorld.getWorldVersion()) instanceof Food food) {
    ...
    private void sendAnimalUpdate(IsoAnimal animal) {
        if (GameServer.server) {
            ...
            byte lastLocation = packet.location;
            animal.getNetworkCharacterAI().setAnimalPacket(null);
            if (lastLocation != packet.location) {
    ...
    private KahluaTableImpl getDefFromSprite() {
        ...
        while (iterator.advance()) {
            ...
            while (iterator2.advance()) {
                ...
                if (spriteName1.equals(spriteDef.rawgetStr("sprite")) || spriteName1.equals(spriteDef.rawgetStr("spriteOpen"))) {
                    return value;
                }
            }

            KahluaTableImpl eggHatchDoors = (KahluaTableImpl)value.rawget("eggHatchDoors");
            if (eggHatchDoors != null) {
                KahluaTableIterator iterator3 = eggHatchDoors.iterator();

                KahluaTableImpl spriteDef;
                do {
                    if (!iterator3.advance()) {
                        continue label57;
                    }

                    spriteDef = (KahluaTableImpl)iterator3.getValue();
                } while (!spriteName1.equals(spriteDef.rawgetStr("sprite")) && !spriteName1.equals(spriteDef.rawgetStr("closedSprite")));
    ...
    public boolean addAnimalInside(IsoAnimal animal, boolean bSync) {
        if (this.animalInside.containsValue(animal)) {
            DebugType.Animal.warn("animal already exists in animalInside");
            return false;
        }
    ...
    public void load(ByteBuffer input, int worldVersion, boolean isDebugSave) throws IOException {
        ...
        if (!this.isSlave()) {
            ...
            for (int i = 0; i < loadedAnimals.size(); i++) {
                IsoAnimal animal = loadedAnimals.get(i);
    ...
    public void removeHutch() {
        for (IsoHutch hutch : this.getAllHutchObjects()) {
            this.removeHutchObject(hutch);
        }
    }
    ...
    public void removeHutchObject(IsoHutch hutch) {
        ...
        hutch.dropAllEggs();
        hutch.removeFromWorld();
        hutch.getSquare().transmitRemoveItemFromSquare(hutch);
    }

    public List<IsoHutch> getAllHutchObjects() {
        List<IsoHutch> hutchObjects = new ArrayList<>();
        ...
        for (int x = this.square.x - 2; x < this.square.x + 3; x++) {
            for (int y = this.square.y - 2; y < this.square.y + 3; y++) {
                ...
                if (sq != null) {
                    List<IsoHutch> hutches = sq.getHutchTiles(this);
                    hutchObjects.addAll(hutches);
                }
            }
        }
    ...
    public void handleBurning() {
        Set<IsoGridSquare> burnSquares = new HashSet<>();
        burnSquares.add(this.getSquare());

        for (IsoHutch hutch : this.getAllHutchObjects()) {
            if (hutch.getSquare() != null && !burnSquares.contains(hutch.getSquare())) {
                burnSquares.add(hutch.getSquare());
                this.removeHutchObject(hutch);
            }
        }

        for (IsoGridSquare square : burnSquares) {
            square.addAshes();
