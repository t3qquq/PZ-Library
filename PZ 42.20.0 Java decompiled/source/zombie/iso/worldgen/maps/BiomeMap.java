...
public class BiomeMap {
    ...
    private BiomeRaster getRaster(CellCoord coord) {
        ...
        if (filename != null) {
            try {
                ...
                DebugType.WorldGen.error(String.format("Loading error of BiomeMap at (%d, %d)", coord.x(), coord.y()));
                raster = null;
            }
        } else {
            DebugType.WorldGen.debugln(String.format("BiomeMap could not find any biome image file at (%d, %d)", coord.x(), coord.y()));
            raster = null;
        }
    ...
    public int[] getCell(int cellX, int cellY, BiomeMap.Type type) {
        BiomeRaster raster = this.getRaster(new CellCoord(cellX, cellY));
        return raster == null ? null : raster.getSamples(0, 0, 256, 256, type.getId(), null);
    }
