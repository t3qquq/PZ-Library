                    ...
                    while (!Arrays.stream(hasForaging).allMatch(b -> b)) {
                        ...
                        if (!found) {
                            ...
                            for (int x = minX; x <= 32; x++) {
                                if (x == 32 || hasForaging[yoffset * 32 + x]) {
                                    maxX = x - 1;
                                    break;
                                }
                            }
                            ...
                            IsoWorld.instance
                                .getMetaGrid()
                                .registerZone(
                                    "", this.map.getZoneName(key), (minX + minChunkX) * 8, (minY + minChunkY) * 8, 0, (maxX + 1 - minX) * 8, (maxY - minY) * 8
                                );
                            ...
                            for (int x = 0; x < 32; x++) {
                                for (int y = 0; y < 32; y++) {
                                    ...
                                    hasForaging[var24] = hasForaging[var24]
                                        | metaGrid.getChunkData(x + minChunkX, y + minChunkY).doesHaveZone(this.map.getZoneName(key));
                                }
                            }
                            ...
                            for (int c = 0; c < config.count(); c++) {
                                if (!(rnd.nextFloat() > chance)) {
                                    ...
                                    for (int i = 0; i < pointsCount; i++) {
                                        ...
                                        int x = (int)Math.min(
                                            Math.max(radius * Math.cos(angle) + centerX + cellSquareX, cellSquareX + 1), cellSquareX + 256 - 2
                                        );
                                        int y = (int)Math.min(
                                            Math.max(radius * Math.sin(angle) + centerY + cellSquareY, cellSquareY + 1), cellSquareY + 256 - 2
                                        );
                                        points.add(x);
                                        points.add(y);
                        ...
                        if (offsetInData != -1) {
                            ...
                            if (numInts > 0) {
                                ...
                                for (int n = 0; n < numInts; n++) {
                                    ...
                                    if (spr != null && spr.getTileType() == IsoObjectType.tree) {
