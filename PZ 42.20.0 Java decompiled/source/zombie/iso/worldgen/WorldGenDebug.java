        ...
        for (int nGen = 0; nGen < IsoWorld.instance.getWgChunk().getRoadGenerators().size(); nGen++) {
            ...
            for (RoadNexus nexus : generator.getRoadNexus()) {
                ...
                for (Vector2i remote : remotes) {
                    float uiX1 = PZMath.floor(ui.getAPI().worldToUIX(remote.x, remote.y));
                    float uiY1 = PZMath.floor(ui.getAPI().worldToUIY(remote.x, remote.y));
                    float uiX2 = PZMath.floor(ui.getAPI().worldToUIX(origin.x, origin.y));
                    float uiY2 = PZMath.floor(ui.getAPI().worldToUIY(origin.x, origin.y));
                ...
                for (RoadEdge edge : edges) {
                    ...
                    float uiX1 = PZMath.floor(ui.getAPI().worldToUIX(edge.a.x, edge.a.y));
                    float uiY1 = PZMath.floor(ui.getAPI().worldToUIY(edge.a.x, edge.a.y));
                    float uiX2 = PZMath.floor(ui.getAPI().worldToUIX(edge.subnexus.x, edge.subnexus.y));
                    float uiY2 = PZMath.floor(ui.getAPI().worldToUIY(edge.subnexus.x, edge.subnexus.y));
                    float uiX3 = PZMath.floor(ui.getAPI().worldToUIX(edge.b.x, edge.b.y));
                    float uiY3 = PZMath.floor(ui.getAPI().worldToUIY(edge.b.x, edge.b.y));
                    ui.DrawLine(null, uiX1, uiY1, uiX2, uiY2, 0.5F, b, 0.0, 1.0, 1.0);
                    ui.DrawLine(null, uiX2, uiY2, uiX3, uiY3, 0.5F, b, 0.0, 1.0, 1.0);
                }
            }
        }
    }
