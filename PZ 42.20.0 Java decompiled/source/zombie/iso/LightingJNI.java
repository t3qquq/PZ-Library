            ...
            for (BaseVehicle vehicle : IsoWorld.instance.currentCell.getVehicles()) {
                if (vehicle.vehicleId != -1) {
                    for (int j = 0; j < vehicle.getLightCount(); j++) {
                        ...
                        checkTorch(part, 1024 + vehicle.vehicleId * 10 + j);
                    }
                }
            }
    ...
    public static void update() {
        if (IsoWorld.instance != null && IsoWorld.instance.currentCell != null) {
            ...
            if (checkLightsFuture != null) {
                try (GameProfiler.ProfileArea gameTime = profiler.profile("checkLights")) {
                    checkLightsFuture.join();
                }
            } else {
                try (GameProfiler.ProfileArea var17 = profiler.profile("checkLights")) {
                    checkLights();
                }
            }
            ...
            try (GameProfiler.ProfileArea var19 = profiler.profile("DeadBodyAtlas")) {
