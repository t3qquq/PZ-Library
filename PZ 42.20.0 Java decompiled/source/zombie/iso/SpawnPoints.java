...
public final class SpawnPoints {
    ...
    private void initSpawnRegions() {
        ...
        if (spawnRegionMgr == null) {
            DebugType.General.error("SpawnRegionMgr is undefined");
        } else {
    ...
    private boolean parseServerSpawnPoint() {
        ...
        if (spawnPoint.length == 3) {
            try {
                ...
                DebugType.General.error("SpawnPoint must be x,y,z, got \"" + ServerOptions.instance.spawnPoint.getValue() + "\"");
            }
        } else {
            DebugType.General.error("SpawnPoint must be x,y,z, got \"" + ServerOptions.instance.spawnPoint.getValue() + "\"");
        }
    ...
    private void initSpawnBuildings() {
        for (int i = 0; i < this.spawnPoints.size(); i++) {
            ...
            if (roomDef != null && roomDef.getBuilding() != null) {
                ...
                DebugType.General.warn("initSpawnBuildings: no room or building at %d,%d,%d", location.x, location.y, location.z);
            }
        }
    }
