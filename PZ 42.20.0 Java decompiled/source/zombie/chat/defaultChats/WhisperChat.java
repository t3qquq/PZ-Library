...
public class WhisperChat extends ChatBase {
    ...
    public WhisperChat(ByteBufferReader bb, ChatTab tab, IsoPlayer owner) {
        super(bb, ChatType.whisper, tab, owner);
        if (!this.isCustomSettings()) {
            this.setSettings(getDefaultSettings());
        }

        this.player1 = bb.getUTF();
        this.player2 = bb.getUTF();
    }
    ...
    public void init() {
        if (this.player1.equals(IsoPlayer.getInstance().getUsername())) {
            ...
            if (!this.player2.equals(IsoPlayer.getInstance().getUsername())) {
                ...
                DebugType.DetailedInfo.trace("Wrong id in whisper chat. Whisper chat not inited for players: " + this.player1 + " " + this.player2);
                return;
            }
