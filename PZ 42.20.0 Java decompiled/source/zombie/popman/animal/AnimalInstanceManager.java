...
public class AnimalInstanceManager {
    ...
    public short allocateID() {
        for (int attempts = 0; attempts < 32767; attempts++) {
            short id = AnimalMap.allocateID();
            if (AnimalMap.get(id) == null) {
                return id;
            }
        }

        throw new IllegalStateException("Failed to allocate unique animal ID");
    }

    public void add(IsoAnimal animal, short onlineID) {
        DebugType.Animal.noise("Animal add id=%d", onlineID);
    ...
    public void remove(IsoAnimal animal) {
        DebugType.Animal.noise("Animal remove id=%d", animal.getOnlineID());
                        ...
                        for (int x1 = x - radius; x1 <= x + radius; x1++) {
                            ...
                            if (square != null) {
                                ...
                                for (int i = square.getMovingObjects().size() - 1; i >= 0; i--) {
                                    if (square.getMovingObjects().get(i) instanceof IsoAnimal animal) {
                                        animal.remove();
                                    }
                                }
                            }
