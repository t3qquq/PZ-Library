...
public class WorldDictionary {
    ...
    private static final Map<String, ItemInfo> itemLoadListMain = new HashMap<>();
    private static final Map<String, EntityInfo> entityLoadListMain = new HashMap<>();
    private static final Map<String, ItemInfo> itemLoadListMP = new HashMap<>();
    private static final Map<String, EntityInfo> entityLoadListMP = new HashMap<>();
    ...
    public static void StartScriptLoading() {
        ...
        getItemLoadList().clear();
        getEntityLoadList().clear();
    ...
    private static void onLoadItem(Item item) {
        if (!GameClient.client) {
            if (!allowScriptItemLoading) {
                ...
                ItemInfo info = getItemLoadList().get(item.getFullName());
                if (info == null) {
                    info = new ItemInfo();
                    info.name = item.getName();
                    info.moduleName = item.getModuleName();
                    info.fullType = item.getFullName();
                    getItemLoadList().put(item.getFullName(), info);
                }
    ...
    public static void onLoadEntity(GameEntityScript entityScript) {
        if (!GameClient.client) {
            if (!allowScriptItemLoading) {
                ...
                EntityInfo info = getEntityLoadList().get(entityScript.getFullName());
                if (info == null) {
                    info = new EntityInfo();
                    info.name = entityScript.getName();
                    info.moduleName = entityScript.getModuleName();
                    info.fullType = entityScript.getFullName();
                    getEntityLoadList().put(entityScript.getFullName(), info);
                }
    ...
    public static void saveDataForClient(ByteBufferWriter bb) throws IOException {
        if (GameServer.server) {
            ...
            bb.putUTF("DICTIONARY_PACKET_END");
    ...
    public static void init() throws WorldDictionaryException {
        ...
        try {
            ...
            logInfo = new Log.Info(dateFormat.format(new Date()), Core.gameSaveWorld, 249, modIDs);
            WorldDictionaryLogger.log(logInfo);
            data.parseInfoLoadList(getItemLoadList());
            data.parseInfoLoadList(getEntityLoadList());
    ...
    private static Map<String, ItemInfo> getItemLoadList() {
        return !GameClient.client && !GameServer.server ? itemLoadListMain : itemLoadListMP;
    }

    private static Map<String, EntityInfo> getEntityLoadList() {
        return !GameClient.client && !GameServer.server ? entityLoadListMain : entityLoadListMP;
    }
}
