...
public final class GameSounds {
            ...
            for (GameSoundClip clip : sound.clips) {
                if (clip.event != null && clip.eventDescription == null) {
                    ...
                    if (clip.eventDescription == null) {
                        DebugType.Sound.println("No such FMOD event \"%s\" for GameSound \"%s\"", clip.event, sound.getName());
                        missingEventCount++;
                    }

                    clip.eventDescriptionMp = FMODManager.instance.getEventDescription("event:/Remote/" + clip.event);
                    if (clip.eventDescriptionMp != null) {
                        DebugType.Sound.println("MP event %s", clip.eventDescriptionMp.path);
                    }
                }
            }
        }
    ...
    public static GameSound getOrCreateSound(String name) {
        ...
        if (gameSound == null) {
            DebugType.Sound.warn("no GameSound called \"" + name + "\", adding a new one");
            ...
            if (BaseSoundBank.instance instanceof FMODSoundBank) {
                ...
                if (clip.event == null && clip.file == null) {
                    DebugType.Sound.warn("couldn't find an FMOD event or .ogg or .wav file for sound \"" + name + "\"");
                }
            }
        }
        ...
        if (Core.debug && BaseSoundBank.instance instanceof FMODSoundBank bank) {
            ...
            for (int i = 0; i < bankCount; i++) {
                ...
                for (int j = 0; j < eventDescCount; j++) {
                    try {
                        ...
                        DebugType.Sound.warn("FMOD cannot get path for " + eventDescList[j] + " event");
                    }
                }
            }
            ...
            if (DebugLog.isEnabled(DebugType.Sound)) {
                for (String event : unusedEvents) {
                    DebugType.Sound.warn("FMOD event \"%s\" not used by any GameSound", event);
                }
            } else {
                DebugType.Sound.warn("FMOD %s missing events", missingEventCount);
                DebugType.Sound.warn("FMOD %s events not used by any GameSound", unusedEvents.size());
                DebugType.Sound.warn("FMOD [Turn on DebugType.Sound for detailed lists of missing and unused events]", unusedEvents.size());
            }
        }
    }
    ...
    public static void previewSound(String name) {
        if (Core.soundDisabled) {
            DebugType.Sound.printf("sound is disabled, not playing " + name);
        } else if (!isKnownSound(name)) {
            DebugType.Sound.warn("sound is not known, not playing " + name);
        } else {
            GameSound gameSound = getSound(name);
            if (gameSound == null) {
                DebugType.Sound.warn("no such GameSound " + name);
            } else {
                GameSoundClip clip = gameSound.getRandomClip();
                if (clip == null) {
                    DebugType.Sound.warn("GameSound.clips is empty");
                } else {
                    if (soundIsPaused) {
    ...
    private static final class BankPreviewSound implements GameSounds.IPreviewSound {
        ...
        public boolean play(GameSoundClip clip) {
            if (clip.eventDescription == null) {
                DebugType.Sound.error("failed to get event " + clip.getEvent());
                return false;
            }

            this.instance = javafmod.FMOD_Studio_System_CreateEventInstance(clip.eventDescription.address);
            if (this.instance < 0L) {
                DebugType.Sound.error("failed to create EventInstance: error=" + this.instance);
                this.instance = 0L;
                return false;
            }
