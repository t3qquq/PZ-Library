...
public class WorldMapSymbols {
    ...
    public boolean isEditingSharedSymbolSettings(UIWorldMap ui) {
        return this.isUserEditing() && ui.getAPI().getRenderer().isDimUnsharedSymbols();
    }
        ...
        try {
            for (int i = 0; i < this.symbols.size(); i++) {
                ...
                if (this.isSymbolVisible(ui, symbol) && (symbol.isUserDefined() || ui.isMapEditor())) {
                    ...
                    if (layout.collided) {
                        float x1 = layout.x + (symbol.widthScaled(ui) / 2.0F - 1.5F);
                        float y1 = layout.y + (symbol.heightScaled(ui) / 2.0F - 1.5F);
                        float x2 = x1 + 6.0F;
                        float y2 = y1 + 6.0F;
