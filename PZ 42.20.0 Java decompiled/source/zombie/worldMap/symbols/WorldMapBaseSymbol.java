...
public abstract class WorldMapBaseSymbol {
    ...
    public boolean isVisible(UIWorldMap ui) {
        ...
        if (this.isShared() && !this.isAuthorLocalPlayer()) {
            if (!ui.getAPI().getRenderer().getBoolean("RemoteSymbols")) {
                return false;
            }

            if (!this.owner.isEditingSharedSymbolSettings(ui) && WorldMapClient.getInstance().isAuthorHidden(this.getNetworkInfo().getAuthor())) {
                return false;
            }

            if (!this.getNetworkInfo().isVisibleToEveryone()) {
    ...
    public void load(ByteBuffer input, SymbolSaveData saveData) throws IOException {
        ...
        this.r = (input.get() & 0xFF) / 255.0F;
        this.g = (input.get() & 0xFF) / 255.0F;
        this.b = (input.get() & 0xFF) / 255.0F;
        this.a = (input.get() & 0xFF) / 255.0F;
        this.collide = input.get() != 0;
        if (saveData.symbolsVersion >= 2) {
