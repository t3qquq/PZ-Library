...
public final class WorldMapTextSymbol extends WorldMapBaseSymbol {
    static final ObjectPool<WorldMapTextSymbol> s_pool = new ObjectPool<>(WorldMapTextSymbol::new, "WorldMapTextSymbol.s_pool");
    ...
    private void DrawTextRotated(
        ...
        if (sdfShader.getShaderProgram().isCompiled()) {
            ...
            if (this.isUserDefined() && this.isShared() && drawer.renderer.isDimUnsharedSymbols() && layout.isAuthorHidden) {
                VBORenderer vbor = VBORenderer.getInstance();
                vbor.startRun(vbor.formatPositionColor);
                vbor.setMode(1);
                this.getOutlinePoints(drawer, leftTop, rightTop, rightBottom, leftBottom);
                vbor.addLine((float)rightTop.x, (float)rightTop.y, 0.0F, (float)leftBottom.x, (float)leftBottom.y, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F);
                vbor.addLine((float)leftTop.x, (float)leftTop.y, 0.0F, (float)rightBottom.x, (float)rightBottom.y, 0.0F, 1.0F, 0.0F, 0.0F, 1.0F);
                vbor.endRun();
            }
