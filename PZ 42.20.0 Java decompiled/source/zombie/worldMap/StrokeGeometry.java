...
public class StrokeGeometry {
    ...
    static ArrayList<StrokeGeometry.Point> getStrokeGeometry(StrokeGeometry.Point[] points, StrokeGeometry.Attrs attrs) {
        ...
        if (points.length == 2) {
            ...
            for (int i = 1; i < middlePoints.size(); i++) {
                createTriangles(middlePoints.get(i - 1), points[i], middlePoints.get(i), vertices, lineWidth, join, miterLimit);
            }
        }
    ...
    static void createRoundCap(
        ...
        double radius = StrokeGeometry.Point.Sub(center, p0).length();
    ...
    static void createTriangles(
        ...
        if (!(anchorLength > p0p1Length) && !(anchorLength > p1p2Length)) {
            ...
            if (join.equals("round")) {
                StrokeGeometry.Point newP0 = StrokeGeometry.Point.Add(p1, t0);
                StrokeGeometry.Point newP1 = StrokeGeometry.Point.Add(p1, t2);
                StrokeGeometry.Point newP2 = StrokeGeometry.Point.Sub(p1, anchor);
                verts.add(newP0);
                verts.add(p1);
                verts.add(newP2);
                createRoundCap(p1, newP0, newP1, newP2, verts);
                verts.add(p1);
                verts.add(newP1);
                verts.add(newP2);
            } else {
