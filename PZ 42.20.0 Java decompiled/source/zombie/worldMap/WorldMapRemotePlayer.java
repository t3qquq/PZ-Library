...
public final class WorldMapRemotePlayer {
    ...
    private boolean seesInvisiblePlayers;
    ...
    public void setPlayer(IsoPlayer player) {
        ...
        if (!this.accessLevel.equals(player.getRole().getName())) {
            ...
            this.seesInvisiblePlayers = player.getRole().hasCapability(Capability.SeesInvisiblePlayers);
            changed = true;
        }
    ...
    public void setFullData(
        ...
        this.seesInvisiblePlayers = seesInvisiblePlayers;
    ...
    public boolean canSeeInvisiblePlayer() {
        return this.seesInvisiblePlayers;
    }

    public String getUsername(Boolean canShowFirstname) {
        String nameStr = this.username;
        if (GameClient.client) {
            boolean bViewerIsAdmin = IsoPlayer.getInstance().getRole() != null
                    && IsoPlayer.getInstance().getRole().hasCapability(Capability.CanSeePlayersStats)
                || IsoPlayer.getInstance().canSeeAll();
            if (!bViewerIsAdmin && this.disguised) {
                nameStr = Translator.getText("IGUI_Disguised_Player_Name");
            } else if (canShowFirstname && ServerOptions.instance.showFirstAndLastName.getValue() && !bViewerIsAdmin) {
