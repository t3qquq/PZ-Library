...
public class RandomizedRanchBase extends RandomizedWorldBase {
    ...
    public static RanchZoneDefinitions getRandomDef(String animalType) {
        ...
        if (!StringUtils.isNullOrEmpty(animalType)) {
            ...
            if (possibleDefs == null) {
                DebugType.Animal.debugln(animalType + " wasn't found in the RanchZoneDefinitions");
                return null;
            }
    ...
    public static void randomizeRanch(Zone zone, DesignationZoneAnimal newDZone) {
        ...
        if (choosenDef == null) {
            DebugType.Animal.debugln("No def was found for this ranch " + animalType + " was found in the RanchZoneDefinitions");
        } else {
            ...
            if (femaleDef == null) {
                DebugType.Animal.debugln("No female def was found for " + choosenDef.femaleType);
            } else if (maleDef == null) {
                DebugType.Animal.debugln("No male def was found for " + choosenDef.maleType);
            } else {
                ...
                if (!StringUtils.isNullOrEmpty(choosenDef.forcedBreed)) {
                    ...
                    randomBreed = false;
                    if (breed == null) {
                        DebugType.Animal.debugln("No breed def was found for " + choosenDef.forcedBreed + " taking random one");
                        randomBreed = true;
                    }
                }
                ...
                for (int i = 0; i < femaleNb; i++) {
                    ...
                    if (sq == null) {
                        DebugType.Animal.debugln("No free square was found in the zone.");
                        return;
                    }
