        ...
        if (body != null) {
            if (Rand.NextBool(2)) {
                ...
                for (int b = 0; b < bleach; b++) {
                    ...
                    trySpawnStoryItem(emptyBleach, body.getSquare(), Rand.Next(0.5F, 1.0F), Rand.Next(0.5F, 1.0F), 0.0F);
                }
                ...
                trySpawnStoryItem(poison, body.getSquare(), Rand.Next(0.5F, 1.0F), Rand.Next(0.5F, 1.0F), 0.0F);
            }

            if (Rand.Next(2) == 0) {
                InventoryItem note = InventoryItemFactory.CreateItem(ItemKey.Literature.NOTE);
                if (Rand.Next(2) == 0) {
                    trySpawnStoryItem(note, body.getSquare(), Rand.Next(0.5F, 1.0F), Rand.Next(0.5F, 1.0F), 0.0F);
                } else {
