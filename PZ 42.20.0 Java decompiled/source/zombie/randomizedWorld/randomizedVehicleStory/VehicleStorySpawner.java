...
public class VehicleStorySpawner {
    ...
    private static final ObjectPool<VehicleStorySpawner.Element> s_elementPool = new ObjectPool<>(
        VehicleStorySpawner.Element::new, "VehicleStorySpawner.s_elementPool"
    );
    ...
    public void render(float centerX, float centerY, float z, float width, float height, float angleRadians) {
        ...
        for (VehicleStorySpawner.Element element : this.elements) {
            ...
            minX = PZMath.min(minX, s_AABB[0]);
            minY = PZMath.min(minY, s_AABB[1]);
            maxX = PZMath.max(maxX, s_AABB[2]);
            maxY = PZMath.max(maxY, s_AABB[3]);
        }
    }
