...
public final class Faction extends Invite {
    ...
    public static boolean canCreateFaction(IsoPlayer player) {
        if (!ServerOptions.instance.faction.getValue()) {
            return false;
        }

        if (player.getRole().hasCapability(Capability.FactionCheat)) {
            return true;
        }

        int daysSurvived = ServerOptions.instance.factionDaySurvivedToCreate.getValue();
        return daysSurvived == 0 ? true : player.getHoursSurvived() >= daysSurvived * 24;
    }
    ...
    public static boolean isInSameFaction(IsoPlayer player, IsoPlayer other) {
        return isInSameFaction(player, other.getUsername());
    }

    public static boolean isInSameFaction(IsoPlayer player, String username) {
        Faction faction = getPlayerFaction(player);
        return faction != null && (faction.isMember(username) || faction.isOwner(username));
    }
