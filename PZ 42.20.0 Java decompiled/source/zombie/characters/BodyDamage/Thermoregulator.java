...
public final class Thermoregulator {
    ...
    public static final float THERMAL_COLD_DAMAGE_MOD = 1.2F;
    ...
    private final float TWO_HOUR_LETHAL_COLD_DURATION = 7200.0F;
    ...
    public void save(ByteBuffer output) throws IOException {
        ...
        output.putFloat(this.thermalDuration);
    ...
    public void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        if (worldVersion >= 249) {
            this.thermalDuration = input.getFloat();
        }
    ...
    private void updateThermalDamage(float airTemperature) {
        this.damageCounter = this.damageCounter + GameTime.instance.getGameWorldSecondsSinceLastUpdate();
        if (this.damageCounter > 1.0F) {
            if (this.player.getMoodles().getMoodleLevel(MoodleType.HYPOTHERMIA) == 4
                ...
                this.thermalDamage *= 1.2F;
                this.thermalDuration = this.thermalDuration + this.damageCounter;
                this.thermalDamage = Math.min(this.thermalDamage, this.thermalDuration / 7200.0F);
    ...
    private void updateMetabolicRate() {
        ...
        if (this.player != null) {
            ...
            if (this.player.getFitness().getCurrentExe() != null) {
                this.setMetabolicTarget(Metabolics.Fitness);
            }
        }
    ...
    private void updateNodesHeatDelta() {
        ...
        for (int i = 0; i < this.nodes.length; i++) {
            ...
            if (node.secondaryDelta > 0.0F) {
                ...
                float var27 = Metabolics.MAX.getMet() * 0.75F * node.secondaryDelta * node.skinSurface / (1.0F + node.insulation);
                var27 *= fluids;
                var27 *= 0.85F + 0.15F * humidityMod;
                var27 *= 1.0F - 0.2F * weightDelta;
                var27 *= 1.0F + 0.2F * fitnessDelta;
                node.heatDelta -= var27;
            } else {
                float energy = 0.1F + 0.9F * this.getEnergy();
                float var32 = Metabolics.Default.getMet() * PZMath.abs(node.secondaryDelta) * node.skinSurface;
                var32 *= energy;
                var32 *= 1.0F + 0.2F * weightDelta;
                var32 *= 1.0F + 0.2F * fitnessDelta;
                node.heatDelta += var32;
            }
        ...
        nodesTotalHeat += this.metabolicRateReal;
        this.totalHeat = nodesTotalHeat;
    }
    ...
    public class ThermalNode {
        ...
        private final ArrayList<Clothing> clothing = new ArrayList<>();

        public ThermalNode(final float initTemperature, final BodyPart bodyPart, final float insulationMultiplier) {
            this(false, initTemperature, bodyPart, insulationMultiplier);
        }

        public ThermalNode(final boolean isCore, final float initTemperature, final BodyPart bodyPart, final float insulationMultiplier) {
            this.isCore = isCore;
            ...
            if (layers > 0) {
                for (int i = 0; i < layers; i++) {
                    ...
                    if (!hasHole) {
                        ...
                        itemInsulation *= (1.0F - itemWetness * 0.75F) * itemCondition;
                        itemWindResist *= (1.0F - itemWetness * 0.45F) * itemCondition;
                        this.insulation += itemInsulation;
                        this.windresist += itemWindResist;
                    }
                }
