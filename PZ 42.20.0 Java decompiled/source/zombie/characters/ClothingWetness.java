...
public final class ClothingWetness {
    ...
    public void updateWetness(float outerWetnessInc, float outerWetnessDec) {
        ...
        for (int i = 0; i < this.clothingList.length; i++) {
            ...
            BodyPartType bodyPartType = BodyPartType.FromIndex(i);
            if (bodyPartType != BodyPartType.MAX) {
                ...
                if (bodyPart != null && thermalNode != null) {
                    ...
                    this.perspiringParts[i] = baseDelta > 0.0F ? 1 : 0;
                    if (baseDelta != 0.0F) {
                        if (baseDelta > 0.0F) {
                        ...
                        if ((!(baseDelta > 0.0F) || !(bodyPart.getWetness() < 25.0F)) && (!(baseDelta < 0.0F) || !(bodyPart.getWetness() > 50.0F))) {
                            ...
                            int j = this.clothingList[i].size() - 1;
                            ...
                            while (true) {
                                ...
                                if (baseDelta > 0.0F) {
                                    this.perspiringParts[i]++;
                                }

                                item = this.clothingList[i].get(j);
                                if (item instanceof Clothing clothing) {
                                    increaseMod = 1.0F;
        ...
        for (int i = 0; i < this.clothingList.length; i++) {
            ...
            if (bodyPartType != BodyPartType.MAX) {
                ...
                if (bodyPart != null && thermalNode != null) {
                    ...
                    float baseDelta = 0.0F;
                    if (outerWetnessInc > 0.0F) {
                        baseDelta = outerWetnessInc * baseIncrease;
                    } else {
                        baseDelta -= outerWetnessDec * baseDecrease;
                    }
                    ...
                    for (int j = 0; j < this.clothingList[i].size(); j++) {
                        ...
                        float increaseMod = 1.0F;
                        InventoryItem item = this.clothingList[i].get(j);
                        if (item instanceof Clothing clothing) {
                            ItemVisual itemVisual = clothing.getVisual();
                            if (itemVisual != null) {
                                if (itemVisual.getHole(part) > 0.0F) {
                    ...
                    if (!this.clothingList[i].isEmpty()) {
                        InventoryItem bottomItem = this.clothingList[i].get(this.clothingList[i].size() - 1);
