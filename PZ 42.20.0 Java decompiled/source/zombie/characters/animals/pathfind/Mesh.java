...
public final class Mesh {
    ...
    public Zone zone;
    static final ObjectPool<Mesh> pool = new ObjectPool<Mesh>(Mesh::new, "Mesh.pool") {
    ...
    public void renderOutline(IPathRenderer renderer, float r, float g, float b, float a) {
        for (int i = 0; i < this.polygon.size(); i++) {
            float polyX1 = this.polygon.get(i).x();
            float polyY1 = this.polygon.get(i).y();
            float polyX2 = this.polygon.get((i + 1) % this.polygon.size()).x();
            float polyY2 = this.polygon.get((i + 1) % this.polygon.size()).y();
            renderer.drawLine(polyX1, polyY1, polyX2, polyY2, r, g, b, a);
        }
    }
    ...
    public void renderPoints(IPathRenderer renderer, float r, float g, float b, float a) {
        for (int i = 0; i < this.polygon.size(); i++) {
            float polyX1 = this.polygon.get(i).x();
            float polyY1 = this.polygon.get(i).y();
            renderer.drawRect(polyX1 - 0.5F, polyY1 - 0.5F, 1.0F, 1.0F, r, g, b, a);
        }
    }

    public void renderTriangles(IPathRenderer renderer, float r, float g, float b, float a) {
        for (int i = 0; i < this.triangles.size(); i += 3) {
            float triangleX1 = this.triangles.get(i).x();
            float triangleY1 = this.triangles.get(i).y();
            float triangleX2 = this.triangles.get(i + 1).x();
            float triangleY2 = this.triangles.get(i + 1).y();
            float triangleX3 = this.triangles.get(i + 2).x();
            float triangleY3 = this.triangles.get(i + 2).y();
            renderer.drawLine(triangleX1, triangleY1, triangleX2, triangleY2, r, g, b, a);
            renderer.drawLine(triangleX2, triangleY2, triangleX3, triangleY3, r, g, b, a);
            renderer.drawLine(triangleX1, triangleY1, triangleX3, triangleY3, r, g, b, a);
        }
    }
