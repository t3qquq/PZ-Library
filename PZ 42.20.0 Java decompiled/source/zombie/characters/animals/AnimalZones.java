...
public final class AnimalZones {
    ...
    private final Set<AnimalChunk> chunksWithTracksSet = new THashSet<>();
    private final ArrayList<VirtualAnimal> tempAnimalList = new ArrayList<>();
    private final Set<VirtualAnimal> doneVirtualAnimals = new THashSet<>();
    private static final Set<AnimalChunk> chunksWithTracks = new HashSet<>();
    public static final List<AnimalTracks> clientTracks = new ArrayList<>();
    ...
    public void render(UIWorldMap ui, boolean bAnimals, boolean bTracks) {
        if (GameClient.client) {
            this.renderClient(ui, bTracks);
        } else {
            this.renderServer(ui, bAnimals, bTracks);
        }
    ...
    private void renderClient(UIWorldMap ui, boolean bTracks) {
        if (bTracks) {
            this.renderTracks(ui, clientTracks);
        }
    }

    private void renderServer(UIWorldMap ui, boolean bAnimals, boolean bTracks) {
        this.renderAnimalCells(ui, bAnimals, bTracks);
    }

    public static void updateVirtualAnimals() {
        AnimalZones instance = getInstance();
        synchronized (chunksWithTracks) {
        ...
        for (AnimalChunk chunk : instance.chunksWithTracksSet) {
            ...
            for (int i = 0; i < instance.tempAnimalList.size(); i++) {
                ...
                } else if (instance.doneVirtualAnimals.add(animal)) {
                    animal.update();
                }
            }
        }
    ...
    private void renderTracks(UIWorldMap ui, List<AnimalTracks> tracks) {
        double r = 0.33;
        double g = 0.33;
        double b = 0.33;
        double a = 1.0;

        for (AnimalTracks track : tracks) {
            float uiX = PZMath.floor(ui.getAPI().worldToUIX(track.x, track.y));
            float uiY = PZMath.floor(ui.getAPI().worldToUIY(track.x, track.y));
            ui.DrawTextureScaledColor(null, uiX - 3.0, uiY - 3.0, 6.0, 6.0, 0.33, 0.33, 0.33, 1.0);
            String txt = Translator.getText("IGUI_MigrationGroup_" + track.animalType) + " " + AnimalTracks.getTrackStr(track.trackType);
            if (track.dir != null) {
                txt = txt + " " + track.dir;
            }

            txt = txt + "\n" + PZMath.fastfloor((float)((GameTime.getInstance().getCalender().getTimeInMillis() - track.addedTime) / 60000L)) + " mins ago";
            ui.DrawTextCentre(txt, uiX, uiY + 4.0, 0.33, 0.33, 0.33, 1.0);
        }
    }
                ...
                if (cell != null && cell.loaded) {
                    ...
                    for (int chy = 0; chy < 32; chy++) {
                        for (int chx = 0; chx < 32; chx++) {
                            ...
                            if (bTracks) {
                                this.renderTracks(ui, chunk.animalTracks);
                            }
                        }
                    }
                }
    ...
    private void renderIsoAnimals(UIWorldMap ui) {
        for (IsoMovingObject movingObject : IsoWorld.instance.currentCell.getObjectList()) {
            IsoAnimal animal = Type.tryCastTo(movingObject, IsoAnimal.class);
            if (animal != null && !animal.isOnHook()) {
                ...
                if (animal.isCurrentState(instance)) {
                    Object subState = animal.get(AnimalZoneState.STATE);
