...
public class NetworkZombieAI extends NetworkCharacterAI {
    ...
    public NetworkZombieAI(IsoGameCharacter character) {
        ...
        this.ulBeatenVehicle.Reset(400L);
    ...
    private void setUsingThump(ZombiePacket packet) {
        ...
        packet.z = (byte)PZMath.fastfloor(((IsoObject)this.zombie.getThumpTarget()).getZ());
    ...
    private void setUsingClimb(ZombiePacket packet) {
        ...
        packet.z = (byte)PZMath.fastfloor(this.zombie.getTarget().getZ());
    ...
    private void setUsingLungeState(ZombiePacket packet, long t) {
        if (this.zombie.target == null) {
            ...
            if (length > 5.0F) {
                ...
                packet.z = (byte)PZMath.fastfloor(this.zombie.target.getZ());
    ...
    private void setUsingWalkTowardState(ZombiePacket packet) {
        if (this.zombie.getPath2() == null) {
            ...
            if (length > 5.0F) {
                ...
                packet.x = this.getPfb2().getTargetX();
                packet.y = this.getPfb2().getTargetY();
                packet.z = (byte)PZMath.fastfloor(this.getPfb2().getTargetZ());
                packet.predictionType = 7;
            }
        } else {
            packet.x = this.getPfb2().pathNextX;
            packet.y = this.getPfb2().pathNextY;
            packet.z = (byte)PZMath.fastfloor(this.zombie.getZ());
            packet.predictionType = 7;
        }
    }
    ...
    private void setUsingPathFindState(ZombiePacket packet) {
        ...
        packet.y = this.getPfb2().pathNextY;
        packet.z = (byte)PZMath.fastfloor(this.zombie.getZ());
        float time = IsoUtils.DistanceTo(this.zombie.getX(), this.zombie.getY(), this.getPfb2().pathNextX, this.getPfb2().pathNextY)
            / 5.0E-4F
            * this.zombie.speedMod;
    ...
    public void set(ZombiePacket packet) {
        ...
        if (this.zombie.getCurrentState() == ThumpState.instance() && this.zombie.getThumpTarget() != null && !this.zombie.getThumpTarget().isDestroyed()) {
            if (this.zombie.getThumpTarget() instanceof IsoObject isoObject && isoObject.getSquare() != null) {
                ...
                DebugType.Multiplayer.error("Unexpected thump target %s", this.zombie.getThumpTarget().getClass().getSimpleName());
            }
    ...
    private boolean checkZTeleport(IsoZombie zombie) {
        if (PZMath.fastfloor(zombie.getZ()) == zombie.realz) {
            return false;
        }

        IsoGridSquare square = zombie.getCell().getGridSquare(zombie.realx, zombie.realy, zombie.realz);
        return square == null ? true : !square.HasStairs() && !square.hasSlopedSurface();
    }

    public void parse(ZombiePacket packet) {
        if (this.usePathFind) {
            this.getPfb2().pathToLocationF(packet.realX, packet.realY, packet.realZ);
            this.getPfb2().walkingOnTheSpot.reset(this.zombie.getX(), this.zombie.getY());
        }
        ...
        if (this.zombie.isRemoteZombie()) {
            ...
            this.zombie.set(ZombieTurnAlerted.TARGET_ANGLE, packet.smParamTargetAngle / 1000.0F);
            ...
            this.zombie.setSpeedTypeFromWalkType();
        ...
        this.zombie.setDirectionAngle(packet.dirAngleRads * (180.0F / (float)Math.PI));
        if ((
                IsoUtils.DistanceToSquared(this.zombie.getX(), this.zombie.getY(), this.zombie.realx, this.zombie.realy) > 9.0F
                    || this.checkZTeleport(this.zombie)
            )
            && (
            ...
            this.zombie.setCurrentSquare(null);
    ...
    public void preupdate() {
        if (GameClient.client) {
            ...
            byte direction = (byte)this.zombie.getForwardIsoDirection().ordinal();
    ...
    private PathFindBehavior2 getPfb2() {
        return this.zombie.getPathFindBehavior2();
    }
}
