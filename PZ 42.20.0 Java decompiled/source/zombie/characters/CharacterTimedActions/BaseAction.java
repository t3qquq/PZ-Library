...
public class BaseAction {
    ...
    public void update() {
        ...
        if (this.maxTime == -1) {
            if (bUseProgressBar) {
                UIManager.getProgressBar(((IsoPlayer)this.chr).getIndex()).setValue(Float.POSITIVE_INFINITY);
            }
        } else {
            if (this.maxTime == 0) {
                this.delta = 0.0F;
            ...
            if (bUseProgressBar) {
                UIManager.getProgressBar(((IsoPlayer)this.chr).getIndex()).setValue(this.delta);
            }
        }
    }
    ...
    public void stop() {
        UIManager.getProgressBar(((IsoPlayer)this.chr).getIndex()).setValue(0.0F);
    ...
    public void perform() {
        UIManager.getProgressBar(((IsoPlayer)this.chr).getIndex()).setValue(1.0F);
        if (!this.loopAction) {
            this.stopTimedActionAnim();
        }
    }
    ...
    public void setActionAnim(String animNode) {
        ...
        if (Core.debug) {
            this.chr.getAdvancedAnimator().printDebugCharacterActions(animNode);
        }
