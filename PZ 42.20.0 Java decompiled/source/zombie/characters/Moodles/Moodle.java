...
public final class Moodle {
    ...
    public boolean Update() {
        ...
        if (this.moodleType == MoodleType.FOOD_EATEN && this.isoGameCharacter.getBodyDamage().getHealth() != 0.0F) {
            ...
            if (this.isoGameCharacter.getBodyDamage().getHealthFromFoodTimer() > this.isoGameCharacter.getBodyDamage().getStandardHealthFromFoodTime() * 2.0F) {
                moodleLevelToSet = Moodle.MoodleLevel.HighMoodleLevel.ordinal();
            }

            if (this.isoGameCharacter.getBodyDamage().getHealthFromFoodTimer() > this.isoGameCharacter.getBodyDamage().getStandardHealthFromFoodTime() * 3.0F) {
                moodleLevelToSet = Moodle.MoodleLevel.MaxMoodleLevel.ordinal();
            }
        }
