...
public class NetObject implements INetworkPacketField {
    ...
    public NetObject setObject(IsoObject value) {
        if (value instanceof IsoMovingObject) {
            DebugType.General.printStackTrace("The NetObject can't process IsoMovingObjects");
        }
    ...
    public IsoObject getObject() {
        if (!this.isProcessed) {
            ...
            if (this.objectType == 1) {
                ...
                if (square == null) {
                    this.object = null;
                } else if (this.objectId >= 0 && this.objectId < square.getObjects().size()) {
                    this.object = square.getObjects().get(this.objectId);
                } else {
                    this.object = null;
                    DebugType.Multiplayer
                        .warn(
                            "Can't find object index [%d] for square [%d] [%d] with size [%d]",
                            this.objectId,
                            this.squareX,
                            this.squareY,
