...
import zombie.network.packets.HaloTextPacket;
...
import zombie.network.packets.ManageAnimalTrackOnWorldMapPacket;
...
import zombie.network.packets.SyncEntityResourcePacket;
...
import zombie.network.packets.SyncPuddlesPacket;
...
import zombie.network.packets.character.ClothingWetnessPacket;
...
import zombie.network.packets.faction.FactionSyncPacket;
import zombie.network.packets.foraging.ForagePoolPacket;
...
import zombie.vehicleNetworkSound.AddVehiclePacket;
import zombie.vehicleNetworkSound.RemoveVehiclePacket;
import zombie.vehicleNetworkSound.UpdateVehiclePacket;
...
public class PacketTypes {
    ...
    static {
        for (PacketTypes.PacketType packetType : PacketTypes.PacketType.values()) {
            ...
            if (previous != null) {
                DebugType.Multiplayer.error(String.format("PacketType: duplicate \"%s\" \"%s\" id=%d", previous.name(), packetType.name(), packetType.getId()));
            }
        }
    }

    public interface CallbackClientProcess {
        void call(ByteBufferReader var1, short var2) throws IOException;
    }

    public interface CallbackServerProcess {
        void call(ByteBufferReader var1, UdpConnection var2, short var3) throws Exception;
    }
    ...
    public static class PacketAuthorization {
        ...
        public static void onUnauthorized(UdpConnection connection, PacketTypes.PacketType type) {
            AntiCheat.Capability.act(connection, type.name() + ": no capability");
        }
    }
    ...
    public enum PacketType {
        ...
        ForageRequestZone(ForageRequestZonePacket.class),
        ForagePool(ForagePoolPacket.class),
        ForageSpot(ForageSpotPacket.class),
        PlayerDropHeldItems(PlayerDropHeldItemsPacket.class),
        ClothingWetness(ClothingWetnessPacket.class),
        PlayerVisitedPacket(PlayerVisitedPacket.class),
        ...
        ManageAnimalTrackOnWorldMap(ManageAnimalTrackOnWorldMapPacket.class),
        ...
        VehicleSoundAddVehicle(AddVehiclePacket.class),
        VehicleSoundRemoveVehicle(RemoveVehiclePacket.class),
        VehicleSoundUpdateVehicle(UpdateVehiclePacket.class),
        ...
        WorldMapPlayerPosition(1, 1, Capability.LoginOnServer, GameServer::receiveWorldMapPlayerPosition, GameClient::receiveWorldMapPlayerPosition, null),
        ...
        ReceiveContainerModData(ReceiveContainerModDataPacket.class),
        ...
        SyncPuddles(SyncPuddlesPacket.class),
        ...
        ChunkObjectStateResponse(1, 2, 3, Capability.LoginOnServer, null, GameClient::receiveChunkObjectState, null),
        ChunkObjectStateRequest(ChunkObjectStateRequestPacket.class),
        ...
        ChangePlayerStats(1, 2, Capability.CanSeePlayersStats, GameServer::receiveChangePlayerStats, GameClient::receiveChangePlayerStats, null),
        ...
        FactionInvite(FactionInvitePacket.class),
        FactionAccept(FactionAcceptPacket.class),
        FactionRemoveMember(FactionRemoveMemberPacket.class),
        FactionChangeOwner(FactionChangeOwnerPacket.class),
        FactionChangeTag(FactionChangeTagPacket.class),
        FactionChangeTitle(FactionChangeTitlePacket.class),
        FactionDisband(FactionDisbandPacket.class),
        FactionCreate(FactionCreatePacket.class),
        FactionSync(FactionSyncPacket.class),
        FactionStats(FactionStatsPacket.class),
        ...
        SyncEntityResource(SyncEntityResourcePacket.class);
        ...
        PacketType(final Class<? extends INetworkPacket> handler) {
        ...
        PacketType(
            ...
            if (handler != null) {
                ...
                if (setting == null) {
                    DebugType.Multiplayer.error("The %s class doesn't have PacketSetting attributes", handler.getSimpleName());
                } else {
                    ...
                    this.isCached = setting.isCached();
                }
            }
        }
        ...
        public void onServerPacket(ByteBufferReader bb, UdpConnection connection) throws Exception {
            ...
            if (PacketTypes.PacketAuthorization.isAuthorized(connection, this)) {
                if (connection.isLimitExceeded(this)) {
                    DebugType.Multiplayer.debugln("Packets limit detected for connection %s", connection.toString());
                }

                if ((this.handlingType & 1) != 0) {
                    if (this.handler != null) {
                        INetworkPacket packet = connection.getPacket(this);
                        if (packet.isPostponed() || packet.shouldInstantiate()) {
                            packet = this.handler.getDeclaredConstructor().newInstance();
                        }

                        packet.parseServer(bb, connection);
                        if (!packet.isConsistent(connection)) {
                            packet.logInconsistentPacket(connection, this);
                        ...
                        if (this.anticheats != null) {
                            for (AntiCheat anticheat : this.anticheats) {
                                if (AntiCheat.None != anticheat && anticheat.isEnabled() && !anticheat.isValid(connection, packet)) {
                                    DebugType.Multiplayer.warn("The packet %s is not valid", this.name());
                                    packet.sync(this, connection);
                                    return;
                                }
                            }
        ...
        public void onClientPacket(ByteBufferReader bb) throws Exception {
            ...
            if ((this.handlingType & 2) != 0) {
                if (this.handler != null) {
                    ...
                    if (this.isLogEnabled()) {
                        this.logPacket(GameClient.connection);
                    }
        ...
        public boolean onClientLoadingPacket(ByteBufferReader bb) throws Exception {
        ...
        private void logPacket(IConnection connection) {
            try {
                if (connection.getPacket(this) == null) {
                    DebugType.Packet.debugln("%s %s", connection.getDescription(), this.name());
                } else {
                    DebugType.Packet.debugln("%s %s", connection.getDescription(), connection.getPacket(this).getDescription());
                }
            } catch (Exception e) {
