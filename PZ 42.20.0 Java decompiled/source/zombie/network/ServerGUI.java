...
public class ServerGUI {
    ...
    private static void renderWorld() {
        ...
        if (player != null) {
            ...
            for (IsoMovingObject obj : cellx.getObjectList()) {
                obj.renderlast();
            }
    ...
    private static void RenderTiles() {
        ...
        for (int zza = 0; zza <= maxZ; zza++) {
            ...
            for (int i = 0; i < VegetationCorpses.size(); i++) {
                ...
                renderMinusFloor(square, false, true);
                renderCharacters(square, true);
            }

            for (int i = 0; i < MinusFloorCharacters.size(); i++) {
                IsoGridSquare square = MinusFloorCharacters.get(i);
                boolean hasSE = renderMinusFloor(square, false, false);
                renderCharacters(square, false);
                if (hasSE) {
                    renderMinusFloor(square, true, false);
                }
            }
        }
    ...
    private static boolean renderMinusFloor(IsoGridSquare square, boolean doSE, boolean vegitationRender) {
        ...
        for (int n = start; doSE ? n >= end : n <= end; n += doSE ? -1 : 1) {
            ...
            if ((
                ...
                if (obj.sprite != null
                    && (
                        obj.sprite.getTileType() == IsoObjectType.WestRoofB
                            || obj.sprite.getTileType() == IsoObjectType.WestRoofM
                            || obj.sprite.getTileType() == IsoObjectType.WestRoofT
                    )
                    && square.z == maxZ
                ...
                if (bDoIt) {
                    ...
                    if (obj.sprite == null
                        ...
                        || obj.sprite.getTileType() != IsoObjectType.doorFrW
                            && obj.sprite.getTileType() != IsoObjectType.doorFrN
                            && obj.sprite.getTileType() != IsoObjectType.doorW
                            && obj.sprite.getTileType() != IsoObjectType.doorN
                        ...
                        if (obj.getTargetAlpha(playerIndex) < 1.0F) {
                            ...
                            if (bForceNoStencil) {
                                ...
                                } else if (obj.sprite.getTileType() != IsoObjectType.doorFrW
                                    && obj.sprite.getTileType() != IsoObjectType.doorW
                                    && !obj.sprite.getProperties().has(IsoFlagType.cutW)) {
                                    if (obj.sprite.getTileType() == IsoObjectType.doorFrN
                                        || obj.sprite.getTileType() == IsoObjectType.doorN
                                        || obj.sprite.getProperties().has(IsoFlagType.cutN)) {
                        ...
                        } else if (obj.sprite.getTileType() == IsoObjectType.doorFrN
                            || obj.sprite.getTileType() == IsoObjectType.doorN
                            || obj.sprite.getProperties().has(IsoFlagType.cutN)) {
    ...
    private static void renderCharacters(IsoGridSquare square, boolean deadRender) {
        ...
        for (int n = 0; n < size; n++) {
            IsoMovingObject mov = square.getMovingObjects().get(n);
