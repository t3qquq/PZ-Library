...
public final class PlayerDownloadServer {
    ...
    public final class WorkerThread extends Thread {
        ...
        final HashSet<Integer> cancelled = new HashSet<>();
        ...
        private void sendArray(ClientChunkRequest ccr) throws IOException {
            ClientChunkRequest ccrForRetries = PlayerDownloadServer.this.getClientChunkRequest(false);
            PlayerDownloadServer.this.connection.getPlayerDownloadServer().ccrWaiting.add(ccrForRetries);
                ...
                if (!this.isRequestCancelled(reqChunk)) {
                    ...
                    if (reqChunk.bb != null) {
                        ...
                        if (inFile.exists()) {
                            ...
                            if (ccrForRetries.isChunksFilled()) {
                                ccrForRetries = PlayerDownloadServer.this.connection.getPlayerDownloadServer().getClientChunkRequest(false);
                                PlayerDownloadServer.this.connection.getPlayerDownloadServer().ccrWaiting.add(ccrForRetries);
                            }

                            ClientChunkRequest.Chunk chunkForRetry = ccrForRetries.getRetryChunk(reqChunk);
                            if (chunkForRetry == null) {
                                this.sendNotRequired(reqChunk, crc == 0L);
                            } else {
                                ccrForRetries.chunks.add(chunkForRetry);
                            }
                        }
                    }
                }
        ...
        private boolean isRequestCancelled(ClientChunkRequest.Chunk reqChunk) {
            ...
            if (this.cancelled.remove(reqChunk.requestNumber)) {
                if (PlayerDownloadServer.this.networkFileDebug) {
                    DebugType.NetworkFileDebug.debugln("cancelled request #" + reqChunk.requestNumber);
                }
