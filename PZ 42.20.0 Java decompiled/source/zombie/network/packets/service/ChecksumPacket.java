...
public class ChecksumPacket implements INetworkPacket {
    ...
    public void write(ByteBufferWriter b) {
        if (GameServer.server) {
            ...
            if (this.pkt == 1) {
                ...
                b.putUTF(AdvancedAnimator.getChecksum());
            }
        ...
        switch (this.pkt) {
            case 1:
                if (NetChecksum.comparer.state != NetChecksum.Comparer.State.SentTotalChecksum) {
                    ...
                    this.okAnim = b.getBoolean();
                    DebugType.Multiplayer.noise("total checksum lua=" + this.okLua + " script=" + this.okScript + " anim=" + this.okAnim);
                    if (this.okLua && this.okScript && this.okAnim) {
            ...
            case 2:
                if (NetChecksum.comparer.state != NetChecksum.Comparer.State.SentGroupChecksum) {
                    ...
                    if (index >= NetChecksum.comparer.currentIndex && index < NetChecksum.comparer.currentIndex + 10) {
                        DebugType.Multiplayer.noise("group checksum " + index + " match=" + this.match);
                        if (this.match) {
    ...
    public void parseServer(ByteBufferReader b, UdpConnection connection) {
        ...
        switch (this.pkt) {
            case 1:
                ...
                String animChecksum = b.getUTF();
                this.okLua = luaChecksum.equals(GameServer.checksum);
                this.okScript = scriptChecksum.equals(ScriptManager.instance.getChecksum());
                this.okAnim = animChecksum.equals(AdvancedAnimator.getChecksum());
                DebugType.Multiplayer.noise("PacketTotalChecksum lua=" + this.okLua + " script=" + this.okScript);
                if (connection.getRole().hasCapability(Capability.BypassLuaChecksum)) {
                    this.okLua = this.okScript = this.okAnim = true;
                }

                connection.checksumState = this.okLua && this.okScript && this.okAnim
                    ? UdpConnection.ChecksumState.Done
                    : UdpConnection.ChecksumState.Different;
                connection.checksumTime = System.currentTimeMillis();
                if (!this.okLua || !this.okScript || !this.okAnim) {
                    DebugType.Multiplayer.warn("user " + connection.getUserName() + " will be kicked in 8000ms because Lua/script checksums do not match");
                    String text = "";
                    if (!this.okLua) {
                        text = text + "Lua";
                    }
                    ...
                    AntiCheat.log(connection, AntiCheat.ChecksumUpdate, 1, text + " incorrect checksum");
                    ServerWorldDatabase.instance
                        .addUserlog(connection.getUserName(), Userlog.UserlogType.LuaChecksum, text, this.getClass().getSimpleName(), 1);
                }
    ...
    public static void sendTotalChecksum() {
        if (GameClient.client) {
            DebugType.Multiplayer.noise("send total checksum");
    ...
    private void sendGroupChecksum() {
        if (GameClient.client) {
            if (NetChecksum.comparer.currentIndex >= NetChecksum.GroupOfFiles.groups.size()) {
                ...
                DebugType.Multiplayer.noise("send group checksums " + NetChecksum.comparer.currentIndex + "-" + lastIndex);
    ...
    private void sendFileChecksums() {
        if (GameClient.client) {
            DebugType.Multiplayer.noise("send file checksums " + NetChecksum.comparer.currentIndex);
