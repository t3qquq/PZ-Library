...
public class ZombieHitPlayerPacket extends ZombieHit implements AntiCheatHitShortDistance.IAntiCheat, AntiCheatTarget.IAntiCheat {
    ...
    public void set(IsoZombie wielder, IsoPlayer target, boolean didDamage, String hitReaction, int bodyPart) {
        this.set(wielder);
        this.target.set(target, false);
        this.bite.set(wielder, didDamage, hitReaction, bodyPart);
    }
    ...
    public boolean isConsistent(IConnection connection) {
        return GameClient.client
            ? this.target.isConsistent(connection) && super.isConsistent(connection) && this.wielder.isConsistent(connection)
            : this.target.isConsistent(connection) && super.isConsistent(connection) && this.wielder.getZombie().getOwner() == connection;
    }
    ...
    public short getPlayerId() {
        return this.target.getPlayer().getOnlineID();
    }
}
