...
public class VehicleUpdatePacket extends VehiclePacket implements INetworkPacket {
    ...
    public void parse(ByteBufferReader bb, IConnection connection) {
        ...
        try {
            ...
            } else if (this.doRemove(connection)) {
                bb.position(expectedPositionOfEnd);
            ...
            if (positionOfEnd != expectedPositionOfEnd) {
                throw new IOException(
                    String.format(
                        "Unexpected buffer position. Read bytes %d, expected: %d.", positionOfEnd - positionOfStart, expectedPositionOfEnd - positionOfStart
                    )
                );
            }
