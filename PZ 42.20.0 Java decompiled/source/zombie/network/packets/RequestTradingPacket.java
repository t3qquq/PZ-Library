...
public class RequestTradingPacket implements INetworkPacket {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        this.type = b.getEnum(RequestTradingPacket.RequestType.class);
    ...
    public void processClient(UdpConnection connection) {
        if (this.type == RequestTradingPacket.RequestType.Ask) {
            LuaEventManager.triggerEvent("RequestTrade", this.playerA.getPlayer(), this.playerB.getPlayer());
        } else {
            LuaEventManager.triggerEvent(
                "AcceptedTrade", this.playerA.getPlayer(), this.playerB.getPlayer(), this.type == RequestTradingPacket.RequestType.Accept
            );
        }
    }
