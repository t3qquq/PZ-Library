...
public class SyncPlayerAlarmClockPacket implements INetworkPacket {
    ...
    private <T extends IAlarmClock> void updateAlarmClock(T ac) {
        if (ac == null) {
            DebugType.Multiplayer.noise("[SyncAlarmClock] skipping update of null AlarmClock");
        } else {
            if (this.stopRinging) {
                ac.stopRinging();
            } else {
    ...
    private void findAlarmClockPlayer(IsoPlayer player) {
        if (player == null) {
            DebugType.Multiplayer.warn("[SyncAlarmClock] unable to find clock for null player");
        } else {
    ...
    public boolean isRelevant(UdpConnection connection) {
        if (this.playerId.getPlayer() == null) {
            DebugType.Multiplayer.warn("[SyncAlarmClock] not relevant null isoPlayer");
            return false;
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        if (this.isRelevant(connection)) {
            ...
            if (this.playerId.getPlayer() == null) {
                DebugType.Multiplayer.warn("[SyncAlarmClock] unable to process on server null isoPlayer");
            } else {
