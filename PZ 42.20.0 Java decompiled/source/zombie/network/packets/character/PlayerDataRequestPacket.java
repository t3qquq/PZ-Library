...
public class PlayerDataRequestPacket extends IDShort implements INetworkPacket {
    ...
    public void processServer(PacketTypes.PacketType packetType, UdpConnection connection) {
        ...
        if (player != null
            && (
                connection.getRole().hasCapability(Capability.SeesInvisiblePlayers)
                    || !player.isInvisible() && (connection.isRelevantTo(player.getX(), player.getY()) || connection.isRelevantTo(player.realx, player.realy))
            )) {
