...
@PacketSetting(ordering = 7, priority = 2, reliability = 2, requiredCapability = Capability.LoginOnServer, handlingType = 3)
public class AnimalEventPacket implements INetworkPacket {
    ...
    public void setData(Object... values) {
        if (values.length == 3 && values[0] instanceof IsoAnimal && values[1] instanceof Boolean) {
            ...
            DebugType.Multiplayer.warn("%s.set get invalid arguments", this.getClass().getSimpleName());
        }
    }
    ...
    public boolean isConsistent(IConnection connection) {
        ...
        if (!result) {
            DebugType.Multiplayer.debugln("[Event] is not consistent %s", this.getDescription());
        }
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        ...
        if (this.eventId >= 0 && this.eventId < AnimalEventPacket.EventType.values().length) {
            ...
            DebugType.Multiplayer.warn("Unknown event=" + this.eventId);
            this.event = null;
        }
    }
    ...
    public void process(UdpConnection connection) {
        if (this.isConsistent(connection)) {
            DebugType.Multiplayer.debugln(this.getDescription());
            switch (this.event) {
                case EventAlerted:
