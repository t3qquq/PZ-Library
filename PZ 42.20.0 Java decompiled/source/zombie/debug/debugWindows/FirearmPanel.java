            ...
            if (this.doTreeNode("gunplayFirearm", "Firearm", false, true)) {
                ...
                if (this.doTreeNode("gunplayAttach", "Attachments", false, false)) {
                    ...
                    if (ImGui.beginCombo("Add attachment", this.selectedAttachment)) {
                        for (WeaponPart i : this.attachments) {
                            if (i.canAttach(player, weapon)) {
                                Wrappers.valueBoolean.set(this.selectedAttachment.equals(i.getFullType()));
                                ImGui.selectable(i.getFullType(), Wrappers.valueBoolean);
                                if (Wrappers.valueBoolean.get()) {
                                    this.selectedAttachment = i.getFullType();
                ...
                if (ImGui.beginTable("gunplayTargetTable", 10, 1984)) {
                    ...
                    for (int i = 0; i < hitInfoList.size(); i++) {
                        ...
                        if (hitInfo.getObject() instanceof IsoGameCharacter obj) {
                            ...
                            ImGui.tableSetColumnIndex(columnIndex++);
                            ImGui.text(Float.toString(PZMath.roundFloat(obj.getCurrentSquare().getLightLevel(player.getIndex()), 3)));
                            ImGui.tableSetColumnIndex(columnIndex++);
                            ImGui.text(
                                Float.toString(
                                    PZMath.max(
                                        ...
                                        this.combatConfig.get(CombatConfigKey.LOW_LIGHT_TO_HIT_MAXIMUM_PENALTY)
                                            * (
                                                1.0F
                                                    - obj.getCurrentSquare().getLightLevel(player.getIndex())
                            ...
                            if (ballisticsController != null) {
                                ...
                                if (isCameraTarget) {
                                    int targetedBodyPart = ballisticsController.getCachedTargetedBodyPart(zombie.getID());
