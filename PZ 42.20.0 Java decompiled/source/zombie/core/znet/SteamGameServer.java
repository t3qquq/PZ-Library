...
public class SteamGameServer {
    public static final int STEAM_SERVERS_DISCONNECTED = 0;
    public static final int STEAM_SERVERS_CONNECTED = 1;
    public static final int STEAM_SERVERS_CONNECTFAILURE = 2;
    ...
    public static void AddPlayer(IsoPlayer player) {
        if (GameServer.server && SteamUtils.isSteamModeEnabled() && player != null) {
            AddPlayer(player.getOnlineID(), player.getUsername(), player.getZombieKills());
        }
    }

    public static void RemovePlayer(IsoPlayer player) {
        if (GameServer.server && SteamUtils.isSteamModeEnabled() && player != null) {
            RemovePlayer(player.getOnlineID());
        }
    }

    public static void UpdatePlayer(IsoPlayer player) {
        if (GameServer.server && SteamUtils.isSteamModeEnabled() && player != null) {
            UpdatePlayer(player.getOnlineID(), player.getZombieKills());
        }
    }
}
