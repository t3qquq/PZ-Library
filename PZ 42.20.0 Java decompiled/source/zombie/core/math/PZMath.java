...
public final class PZMath {
    ...
    private static final double SEGMENT_LENGTH_SQUARED_EPSILON = 1.0E-12;
    ...
    public static Quaternion setFromAxisAngle(float ax, float ay, float az, float angleRadians, Quaternion result) {
        ...
        result.y *= s;
        result.z *= s;
        result.w = (float)Math.cos(0.5 * angleRadians);
        return result;
    }
    ...
    public static float getClosestAngle(float radsA, float radsB) {
        float angleA = wrap(radsA, (float) (Math.PI * 2));
        float angleB = wrap(radsB, (float) (Math.PI * 2));
        float diff = angleB - angleA;
        return wrap(diff, (float) -Math.PI, (float) Math.PI);
    }

    public static float getClosestAngleDegrees(float degsA, float degsB) {
        float radsA = degToRad(degsA);
        float radsB = degToRad(degsB);
        float closestAngleRads = getClosestAngle(radsA, radsB);
    ...
    public static float wrap(float val, float min, float max) {
        float realMax = max(max, min);
        float realMin = min(max, min);
    ...
    public static float angleBetween(Vector2 va, Vector2 vb) {
        ...
        float vbx = vb.x;
        float vby = vb.y;
        return angleBetween(vax, vay, vbx, vby);
    }

    public static float angleBetween(float ax, float ay, float bx, float by) {
        float vaLength = sqrt(ax * ax + ay * ay);
        if (vaLength == 0.0F) {
            return 0.0F;
        }

        float vbLength = sqrt(bx * bx + by * by);
        if (vbLength == 0.0F) {
            return 0.0F;
        }
    ...
    public static float calculateBearing(Vector3 fromPosition, Vector2 fromForward, Vector3 toPosition) {
        ...
        return angleBetween(fromForward.x, fromForward.y, toTargetX, toTargetY) * (180.0F / (float)Math.PI);
    }
    ...
    public static Vector3f rotateVector(Vector3f vector, Quaternion quaternion, Vector3f result) {
        ...
        float vy = vector.y;
        float vz = vector.z;
        return rotateVector(vx, vy, vz, qx, qy, qz, qw, result);
    }
    ...
    public static Vector3 cross(Vector3 a, Vector3 b, Vector3 out) {
        return cross(a.x, a.y, a.z, b.x, b.y, b.z, out);
    }
    ...
    public static boolean isNullOrZero(Vector2 vec) {
        return vec == null || isZero(vec);
    }

    private static boolean isZero(Vector2 vec) {
    ...
    public static double closestPointOnLineSegment(float x1, float y1, float x2, float y2, float px, float py, double endpointSnapEpsilon, Vector2f out) {
        double dx = x2 - x1;
        double dy = y2 - y1;
        ...
        if (segmentLengthSquared < 1.0E-12) {
            ...
            double u = ((px - x1) * dx + (py - y1) * dy) / segmentLengthSquared;
            if (u <= endpointSnapEpsilon) {
                if (out != null) {
                    out.set(x1, y1);
                }
                ...
                double ddy = py - y1;
                return ddx * ddx + ddy * ddy;
            } else if (u >= 1.0 - endpointSnapEpsilon) {
                if (out != null) {
                    out.set(x2, y2);
                }
    ...
    public static double closestPointsOnLineSegments(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, Vector2f p1, Vector2f p2) {
        ...
        float newS = clamp_01((t * uv + ru) / uu);
        float newT = clamp_01((s * uv - rv) / vv);
