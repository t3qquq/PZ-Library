...
public class PZGLUtil {
    ...
    public static void checkGLErrorThrow(String format, Object... args) throws OpenGLException {
        checkGLErrorThrowInternal(false, format, args);
    }

    public static void checkGLErrorThrowTexture(String format, Object... args) throws OpenGLException {
        checkGLErrorThrowInternal(false, format, args);
    }

    private static void checkGLErrorThrowInternal(boolean glErrorCheckEnabled, String format, Object... args) throws OpenGLException {
        if (glErrorCheckEnabled) {
    ...
    private static String createErrorMessage(int glErrorCode) {
        String errorString = Util.translateGLErrorString(glErrorCode);
        return errorString + " (" + glErrorCode + ")";
    }
    ...
    private static void glDebugOutput(int source, int type, int id, int severity, int length, long message, long userParam) {
        if (severity <= 37191 && (severity != 33387 || 37191 == severity)) {
            ...
            Consumer<String> log = switch (severity) {
                case 37190 -> DebugType.General::error;
                case 37191, 37192 -> DebugType.General::warn;
                default -> DebugType.General::print;
            };
