...
public final class CarController {
    ...
    public void update() {
        if (this.vehicleObject.getVehicleTowedBy() == null) {
            ...
            if (bDrunkDriver && this.vehicleObject.getEngineState() != BaseVehicle.engineStateTypes.Idle) {
            ...
            if (!GameServer.server) {
                ...
                if (this.engineForce > 0.0F && this.vehicleObject.getEngineState() == BaseVehicle.engineStateTypes.Idle && !this.engineStartingFromKeyboard) {
            ...
            if (this.vehicleObject.getEngineState() != BaseVehicle.engineStateTypes.Running) {
        ...
        if (vehicleTowedBy != null) {
            if (GameServer.server) {
                ...
                if (vehicleTowedBy.getDriver() == null && this.vehicleObject.getDriver() != null && !GameClient.client) {
                    ...
                    this.checkShouldBeActive();
                    float towedByEngineForce = vehicleTowedBy.getController() != null ? vehicleTowedBy.getController().engineForce : 0.0F;
                    float nonZeroEngineForceToDisableStoppingMovementForce = 1.0E-4F;
                    this.engineForce = towedByEngineForce > 0.0F ? 1.0E-4F : 0.0F;
                    this.brakingForce = 0.0F;
                    this.vehicleSteering = 0.0F;
                    if (!this.vehicleObject.isBurnt() && this.vehicleObject.getPartById(VehiclePart.ENGINE.toString()) != null) {
                        this.steerInDirectionOfTowing();
                    }
    ...
    private void steerInDirectionOfTowing() {
        BaseVehicle vehicleTowedBy = this.vehicleObject.getVehicleTowedBy();
        Vector3f attachPointSelfPos = this.vehicleObject.getAttachmentWorldPos(this.vehicleObject.getTowAttachmentSelf(), BaseVehicle.allocVector3f());
        Vector2 attachPointSelfToTowedByDirection = BaseVehicle.allocVector2()
            .set(vehicleTowedBy.getX() - attachPointSelfPos.x, vehicleTowedBy.getY() - attachPointSelfPos.y);
        attachPointSelfToTowedByDirection.normalize();
        Vector3f forwardSelf = this.vehicleObject.getForwardVector(BaseVehicle.allocVector3f());
        Vector2 forwardAdjusted = BaseVehicle.allocVector2().set(forwardSelf.x, forwardSelf.z);
        if (this.vehicleObject.isBeingTowedBackwards()) {
            forwardAdjusted.mul(-1.0F);
        }

        float angle = attachPointSelfToTowedByDirection.angleBetween(forwardAdjusted) * (180.0F / (float)Math.PI);
        angle = PZMath.clamp(angle / 2.0F, 0.0F, 45.0F) / 45.0F;
        float p0x = this.vehicleObject.getX();
        float p0y = this.vehicleObject.getY();
        float p1x = p0x + forwardSelf.x;
        float p1y = p0y + forwardSelf.z;
        if (PZMath.isLeft(p0x, p0y, p1x, p1y, vehicleTowedBy.getX(), vehicleTowedBy.getY()) > 0.0F) {
            angle *= -1.0F;
        }

        this.vehicleSteering = angle;
        BaseVehicle.releaseVector2(forwardAdjusted);
        BaseVehicle.releaseVector3f(forwardSelf);
        BaseVehicle.releaseVector2(attachPointSelfToTowedByDirection);
        BaseVehicle.releaseVector3f(attachPointSelfPos);
    }
        ...
        if (this.vehicleObject.getEngine() == null) {
            ...
            if (!this.vehicleObject.isEngineRunning()) {
                if (this.vehicleObject.getEngineSpeed() > 0.0) {
                    this.vehicleObject.setEngineSpeed(Math.max(this.vehicleObject.getEngineSpeed() - 50.0F * fpsMult, 0.0));
                }
                ...
                if (!this.vehicleObject.isRegulator()) {
                    this.vehicleObject.addEngineSpeed(-20.0F * fpsMult);
                }
            } else {
                this.vehicleObject.addEngineSpeed(20.0F * fpsMult);
            }
    ...
    private void control_Braking() {
        float fpsMult = GameTime.getInstance().getMultiplier() / 0.8F;
        if (this.vehicleObject.getEngineSpeed() > this.vehicleObject.getScript().getEngineIdleSpeed()) {
            this.vehicleObject.addEngineSpeed(-Rand.Next(10, 30) * fpsMult);
        } else {
            this.vehicleObject.addEngineSpeed(Rand.Next(20) * fpsMult);
        }
    ...
    private void control_Forward(float speed) {
        ...
        if (this.vehicleObject.getEngineSpeed() > 3000.0 && this.vehicleObject.transmissionChangeTime.Check()) {
            this.vehicleObject.changeTransmission(this.vehicleObject.transmissionNumber.getNext(gearRatioCount));
        }
        ...
        this.vehicleObject.addEngineSpeed(-Math.min(0.5 * (this.vehicleObject.getEngineSpeed() - engineSpeedCalc), 100.0) * fpsMult);
        if (trainFastDriver) {
            if (speed < 50.0F) {
                this.vehicleObject.addEngineSpeed(-Math.min(0.06 * (this.vehicleObject.getEngineSpeed() - 7000.0), 30.0F - speed) * fpsMult);
            }
        } else if (speed < 30.0F) {
            this.vehicleObject.addEngineSpeed(-Math.min(0.02 * (this.vehicleObject.getEngineSpeed() - 7000.0), 30.0F - speed) * fpsMult);
        }

        this.engineForce = (float)(this.vehicleObject.getEnginePower() * (0.5 + this.vehicleObject.getEngineSpeed() / 24000.0));
        ...
        if (this.vehicleObject.getEngineSpeed() > 6000.0) {
            this.engineForce = (float)(this.engineForce * ((7000.0 - this.vehicleObject.getEngineSpeed()) / 1000.0));
        }
    ...
    private void control_ForwardNew(float speed) {
        ...
        if (this.vehicleObject.transmissionNumber == TransmissionNumber.N) {
            ...
            this.vehicleObject.transmissionNumber = targetTransmissionNumber;
            this.vehicleObject.setEngineSpeed(carRpmData[this.vehicleObject.transmissionNumber.getIndex() - 1].afterGearChange);
        }

        if (this.vehicleObject.transmissionNumber.getIndex() < gearRatioCount && this.vehicleObject.transmissionNumber.getIndex() - 1 >= 0) {
            this.vehicleObject
                .setEngineSpeed(
                    Math.min(this.vehicleObject.getEngineSpeed(), carRpmData[this.vehicleObject.transmissionNumber.getIndex() - 1].gearChange + 100.0F)
                );
        }

        if (this.vehicleObject.getEngineSpeed() > engineSpeedCalc) {
            this.vehicleObject.addEngineSpeed(-Math.min(0.5 * (this.vehicleObject.getEngineSpeed() - engineSpeedCalc), 10.0) * fpsMult);
        } else {
            ...
            this.vehicleObject.addEngineSpeed(rpmIncrease * fpsMult);
        }
        ...
        this.engineForce = (float)(enginePower * (0.3F + this.vehicleObject.getEngineSpeed() / 30000.0));
        this.engineForce = this.engineForce - this.engineForce * (speed / 200.0F);
        if (speed < 15.0F && this.vehicleObject.getVehicleTowing() != null && this.vehicleObject.getVehicleTowing().isBurnt()) {
            this.engineForce = this.engineForce * Math.min(1.2F, this.vehicleObject.getVehicleTowing().getMass() / 500.0F);
            this.engineForce *= 2.0F;
        }

        if (this.vehicleObject.getEngineSpeed() > 6000.0) {
            this.engineForce = this.engineForce * ((7000.0F - (float)this.vehicleObject.getEngineSpeed()) / 1000.0F);
        }
    ...
    private void control_Reverse(float speed) {
        ...
        this.engineForce = (float)(-1.0F * this.vehicleObject.getEnginePower() * (0.75 + this.vehicleObject.getEngineSpeed() / 24000.0));
        if (this.vehicleObject.getEngineSpeed() > 6000.0) {
            this.engineForce = (float)(this.engineForce * ((7000.0 - this.vehicleObject.getEngineSpeed()) / 1000.0));
        }
            ...
            for (int dx = -1; dx <= 1; dx++) {
                ...
                if (chunk != null) {
                    for (BaseVehicle vehicle : chunk.vehicles) {
                        if (vehicle != this.vehicleObject && vehicle.getDriver() != null) {
                            float distSq = this.vehicleObject.getClosestPointOnPoly(vehicle, p1, p2);
    ...
    public void debug() {
        if (Core.debug && DebugOptions.instance.vehicleRenderOutline.getValue()) {
            ...
            int joypad = this.vehicleObject.getJoypad();
            if (joypad != -1) {
                Vector2 vecAim = JoypadManager.instance.getMovementAxis(joypad, this.tempVec2);
                if (vecAim.getLengthSquared() > 1.0E-4F) {
    ...
    private void renderClosestPointToOtherVehicle() {
        ...
        for (BaseVehicle other : IsoWorld.instance.currentCell.getVehicles()) {
