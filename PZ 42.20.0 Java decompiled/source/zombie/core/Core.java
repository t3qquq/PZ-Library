...
import zombie.input.ControllerButtonStyle;
...
public final class Core {
    ...
    public static final float characterRangedAimPointHeight = 0.468F;
    public static final float characterMeleeAimPointHeight = 0.57F;
    ...
    private final int VERSION_BUILD_42 = 8;
    ...
    private final BooleanConfigOption optionDisableLightningDuringStorms = this.newOption("disableLightningDuringStorms", false);
    ...
    private final BooleanConfigOption optionMapViewPause = this.newOption("mapViewPause", true);
    ...
    private final BooleanConfigOption optionTextureCompression = this.newOption("textureCompression", false);
    ...
    private final BooleanConfigOption optionDoWindSpriteEffects = this.newOption("doWindSpriteEffects", false);
    ...
    private final IntegerConfigOption optionControllerButtonStyle = this.newOption("controllerButtonStyle", 1, 3, 1, this::onOptionControllerButtonStyleChanged);
    private final StringConfigOption optionGamepadBindingPreset = this.newOption("gamepadBindingPreset", "Original", this::onOptionGamepadBindingPresetChanged);
    ...
    private final BooleanConfigOption populateServerListOnStart = this.newOption("populateServerListOnStart", false);
    ...
    private IntegerConfigOption newOption(String name, int minValue, int maxValue, int defaultValue) {
        return this.newOption(name, minValue, maxValue, defaultValue, null);
    }

    private IntegerConfigOption newOption(String name, int minValue, int maxValue, int defaultValue, ConfigOption.ConfigOptionOnChangeCallback onChange) {
        IntegerConfigOption option = new IntegerConfigOption(name, minValue, maxValue, defaultValue, onChange);
        this.options.add(option);
        this.optionByName.put(name, option);
        return option;
    }
    ...
    private StringConfigOption newOption(String name, String defaultValue, ConfigOption.ConfigOptionOnChangeCallback onChange) {
        StringConfigOption option = new StringConfigOption(name, defaultValue, onChange);
        this.options.add(option);
        this.optionByName.put(name, option);
        return option;
    }
    ...
    public void TakeFullScreenshot(String filename) {
        ...
        RenderThread.invokeOnRenderContext(filename, lFilename -> {
            ...
            File file = new File(ZomboidFileSystem.instance.getScreenshotDir() + File.separator + lFilename);
    ...
    public boolean loadOptions_OLD() throws IOException {
        ...
        try {
            ...
            while ((line = br.readLine()) != null) {
                if (line.startsWith("version=")) {
                ...
                } else if (line.startsWith("mapViewPause=")) {
                    this.optionMapViewPause.parse(line.replaceFirst("mapViewPause=", ""));
                ...
                } else if (line.startsWith("populateServerListOnStart=")) {
                    this.populateServerListOnStart.parse(line.replaceFirst("populateServerListOnStart=", ""));
                    ...
                    } else if (line.startsWith("gamepadBindingPreset=")) {
                        this.optionGamepadBindingPreset.parse(line.replaceFirst("gamepadBindingPreset=", ""));
    ...
    private String upgradeOptionValue(String optionName, String optionValue, int version) {
        return version >= 8 || !Objects.equals(optionName, "textureCompression") && !Objects.equals(optionName, "doWindSpriteEffects") ? optionValue : "false";
    }
    ...
    public void saveOptions_OLD() throws IOException {
        ...
        try {
            ...
            fw.write("mapViewPause=" + this.optionMapViewPause.getValueAsString() + "\r\n");
            ...
            fw.write("populateServerListOnStart=" + this.populateServerListOnStart.getValueAsString() + "\r\n");
        ...
        try {
            if (!newFile.exists()) {
                ...
                for (int n = 0; n < modes.length; n++) {
                    ...
                    if (!test.contains(resolution)) {
                        t.rawset(c, resolution);
                        fw.write(resolution + " \r\n");
                        test.add(resolution);
                        c++;
                    }
                ...
                BufferedReader br;
                String line;
                for (br = new BufferedReader(new FileReader(newFile)); (line = br.readLine()) != null; c++) {
    ...
    public void init(int width, int height) throws LWJGLException {
        ...
        DebugType.General.println("VSync: %s", this.getOptionVSync() ? "ON" : "OFF");
    ...
    public boolean invalidBindingShiftCtrl(Core.KeyBinding keyB) {
        ...
        if (keyB.altKey() >= 10000 && Mouse.isButtonDownUICheck(keyB.altKey() - 10000)) {
            return false;
        }

        if (keyB.keyValue() >= 10000 && Mouse.isButtonDownUICheck(keyB.keyValue() - 10000)) {
            return false;
        }

        if (keyB.keyValue() == 0 && keyB.altKey == 0) {
            return true;
        }
    ...
    public String getVersion() {
        return gameVersion + ".0 " + getGitRevisionString();
    }
    ...
    public String getGitSha() {
        if (this.gitSha == null) {
            this.gitSha = IS_DEV ? DevMainScreenState.getDevSha() : "a2947723ca";
        }
    ...
    public String getGitRevision() {
        if (this.gitRevisionString == null) {
            if (IS_DEV) {
                ...
                this.gitRevisionString = "a2947723ca".substring(0, Math.min("a2947723ca".length(), 20));
            }
        }
    ...
    public static String getGitRevisionString() {
        return IS_DEV ? core.getGitRevision() : "a2947723ca";
    }
    ...
    public void zoomOptionChanged(boolean inGame) {
        if (inGame) {
            RenderThread.invokeOnRenderContext(() -> {
                this.offscreenBuffer.zoomEnabled = this.getOptionZoom();
                if (safeModeForced) {
                    this.offscreenBuffer.destroy();
                    safeMode = true;
                    this.supportsFbo = false;
    ...
    public int getOptionFontSizeReal() {
        ...
        if (size == 6) {
            ...
            int maxHeight = 2400;
            int divisions = 132;
            int scale = (int)PZMath.floor((getInstance().getScreenHeight() - 1080) / 132.0F) + 1;
    ...
    public boolean getOptionMapViewPause() {
        return this.optionMapViewPause.getValue();
    }

    public void setOptionMapViewPause(boolean pause) {
        this.optionMapViewPause.setValue(pause);
    }
    ...
    public void setOptionTexture2x(boolean b) {
        ...
        DebugType.General.warn("1x textures are disabled.");
    }
    ...
    public void onOptionControllerButtonStyleChanged(ConfigOption sender) {
        LuaEventManager.triggerEvent("OptionControllerButtonStyleChanged", this.getOptionControllerButtonStyleString());
    }

    @UsedFromLua
    public String getOptionControllerButtonStyleString() {
        return ControllerButtonStyle.fromOptionCode(this.getOptionControllerButtonStyle()).getLabel();
    }

    @UsedFromLua
    public void setOptionGamepadBindingPreset(String newValue) {
        this.optionGamepadBindingPreset.setValue(newValue);
    }

    public void onOptionGamepadBindingPresetChanged(ConfigOption sender) {
        DebugType.General.debugln("Triggering Lua Event: %s(%s)", "OptionGamepadBindingPresetChanged", this.getOptionGamepadBindingPreset());
        LuaEventManager.triggerEvent("OptionGamepadBindingPresetChanged", this.getOptionGamepadBindingPreset());
    }

    @UsedFromLua
    public String getOptionGamepadBindingPreset() {
        return this.optionGamepadBindingPreset.getValue();
    }
    ...
    public boolean isPopulateServerListOnStart() {
        return this.populateServerListOnStart.getValue();
    }

    public void setPopulateServerListOnStart(boolean populateServerListOnStart) {
        this.populateServerListOnStart.setValue(populateServerListOnStart);
    }
    ...
    public boolean getOptionDisableLightningDuringStorms() {
        return this.optionDisableLightningDuringStorms.getValue();
    }

    public void setOptionDisableLightningDuringStorms(boolean enable) {
        this.optionDisableLightningDuringStorms.setValue(enable);
    }
    ...
    public String debugOutputMissingSpawn(String directory, String category) throws Exception {
        ...
        for (int i = 0; i < missingClothing.size(); i++) {
            String itemName = missingClothing.get(i);
