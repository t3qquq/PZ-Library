...
public class ShaderPropertyBlock {
    ...
    public void SetShader(Shader shader) {
        if (shader != this.shader) {
            ...
            if (shader != null && shader.isInstanced()) {
                if (this.shader != null) {
                    MemoryUtil.memRealloc(this.data, shader.instancedData.GetBufferData().GetSize());
                } else {
                    this.data = MemoryUtil.memAlloc(shader.instancedData.GetBufferData().GetSize());
                }

                ShaderBufferData bufferData = shader.instancedData.GetBufferData();
    ...
    public Matrix4f SetMatrix4(String name, org.joml.Matrix4f value) {
        ...
        ShaderParameter param = param1;
        return param.GetMatrix4();
    }
