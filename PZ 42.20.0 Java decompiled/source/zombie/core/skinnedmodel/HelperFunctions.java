...
public final class HelperFunctions {
    ...
    private static final ObjectPool<Vector3f> VectorPool = new ObjectPool<>(Vector3f::new, "HelperFunctions.VectorPool");
    private static final ObjectPool<Quaternion> QuaternionPool = new ObjectPool<>(Quaternion::new, "HelperFunctions.QuaternionPool");
    ...
    private static void CreateFromQuaternionPositionScale(
        Vector3f position, Quaternion rotation, Vector3f scale, Matrix4f result, Matrix4f reusableRotation, Matrix4f reusableScale
    ) {
        reusableScale.setIdentity();
        reusableScale.scale(scale);
        CreateFromQuaternion(rotation, reusableRotation);
    ...
    public static float getRotationY(Quaternion rotation) {
        ...
        vCrossPx = 0.0F;
        vCrossPz = 1.0F;
        vDotP = vY * 1.0F - vZ * 0.0F;
        float vCrossPzx = vX * 0.0F - vY * 0.0F;
        float vDotPx = 0.0F * vX + 0.0F * vY + 1.0F * vZ;
        xaxisX = (s2 - v2) * 0.0F + 2.0F * qs * vDotP + 2.0F * vX * vDotPx;
        xaxisY = (s2 - v2) * 1.0F + 2.0F * qs * vCrossPzx + 2.0F * vZ * vDotPx;
        xaxisZ = (rxaxisX + xaxisY) / 2.0F;
        vCrossPx = (rxaxisZ - xaxisX) / 2.0F;
        vCrossPz = PZMath.sqrt(xaxisZ * xaxisZ + vCrossPx * vCrossPx);
        vDotP = xaxisZ / vCrossPz;
        vCrossPzx = vCrossPx / vCrossPz;
        return (float)Math.atan2(-vCrossPzx, vDotP);
    }
    ...
    public static Vector3f ToEulerAngles(Quaternion rot, Vector3f angles) {
        double sinrCosp = 2.0 * (rot.w * rot.x + rot.y * rot.z);
        double cosrCosp = 1.0 - 2.0 * (rot.x * rot.x + rot.y * rot.y);
        angles.x = (float)Math.atan2(sinrCosp, cosrCosp);
        double sinp = 2.0 * (rot.w * rot.y - rot.z * rot.x);
        if (Math.abs(sinp) >= 1.0) {
            angles.y = (float)Math.copySign((float) (Math.PI / 2), sinp);
        } else {
            angles.y = (float)Math.asin(sinp);
        }

        double sinyCosp = 2.0 * (rot.w * rot.z + rot.x * rot.y);
        double cosyCosp = 1.0 - 2.0 * (rot.y * rot.y + rot.z * rot.z);
    ...
    public static Quaternion setFromAxisAngle(float axisX, float axisY, float axisZ, float angleRads, Quaternion result) {
        ...
        result.y *= s;
        result.z *= s;
        result.w = (float)Math.cos(0.5 * angleRads);
        return result;
    }
