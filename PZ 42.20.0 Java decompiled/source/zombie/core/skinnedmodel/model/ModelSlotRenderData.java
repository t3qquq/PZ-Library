...
public final class ModelSlotRenderData extends TextureDraw.GenericDrawer {
    private static final ObjectPool<ModelSlotRenderData> pool = new ObjectPool<>(ModelSlotRenderData::new, "ModelSlotRenderData.pool");
    ...
    public void initModel(ModelManager.ModelSlot modelSlot) {
        ...
        if (this.character != null && (this.textureCreator != null || bHasTextureCreators)) {
            ...
            for (int i = 0; i < this.character.getReadyModelData().size(); i++) {
                ModelInstance modelInstance = this.character.getReadyModelData().get(i);
    ...
    public synchronized ModelSlotRenderData init(ModelManager.ModelSlot modelSlot) {
        ...
        if (this.object instanceof BaseVehicle vehicle) {
            ...
            for (int i = 0; i < this.effectLights.length; i++) {
                if (this.debugChunkState) {
                    ...
                    ModelInstance.EffectLight el = playerData.effectLightsMain[i];
                    this.effectLights[i].set(el.x, el.y, el.z, el.r, el.g, el.b, el.radius);
                }
            }
        ...
        try (GameProfiler.ProfileArea var18 = profiler.profile("Init Inst")) {
            this.initRenderData();
        }

        try (GameProfiler.ProfileArea var19 = profiler.profile("Set Lights")) {
    ...
    private void renderCharacter() {
        ...
        if (this.ready || !this.readyModelData.isEmpty()) {
            ...
            if (this.characterOutline) {
                ...
                try (GameProfiler.ProfileArea var10 = profiler.profile("performRenderCharacterOutline")) {
            ...
            try (GameProfiler.ProfileArea var11 = profiler.profile("renderCharacter")) {
    ...
    private void performRenderCharacter() {
        ...
        if (Core.debug
            ...
            && this.character instanceof IsoZombie zombie
            && zombie.imposter.cardRendered) {
            this.renderCard(zombie.imposter);
            usingCard = true;
        }
