...
public final class HeightTerrain {
    ...
    public HeightTerrain(int widthTiles, int heightTiles) {
        ...
        for (int x = 0; x < xVerts; x++) {
            for (int y = 0; y < yVerts; y++) {
                ...
                noise *= 1.0F;
                noise++;
                VertexPositionNormalTangentTextureSkin vert = new VertexPositionNormalTangentTextureSkin();
        ...
        for (int x = 0; x < xVerts; x++) {
            for (int y = 0; y < yVerts; y++) {
                VertexPositionNormalTangentTextureSkin vert = verts.get(n);
    ...
    public void pushView(int ox, int oy, int oz) {
        ...
        GL11.glPushMatrix();
        GL11.glLoadIdentity();
        double screenWidth = Math.abs(Core.getInstance().getOffscreenWidth(0)) / 1920.0F;
        double screenHeight = Math.abs(Core.getInstance().getOffscreenHeight(0)) / 1080.0F;
        GL11.glLoadIdentity();
