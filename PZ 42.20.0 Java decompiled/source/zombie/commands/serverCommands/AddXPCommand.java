...
public class AddXPCommand extends CommandBase {
    ...
    protected String Command() {
        ...
        if (PerkFactory.Perks.MAX == perk) {
            ...
            return "List of available perks :" + nl + sb;
        } else {
            float xp;
            try {
            ...
            p.getXp().AddXP(perk, xp, false, isMultiplierEnabled != null && isMultiplierEnabled.equals("-true"), false, false);
            if (PerkFactory.Perks.Fitness == perk) {
                p.getStats().set(CharacterStat.FITNESS, p.getPerkLevel(PerkFactory.Perks.Fitness) / 5.0F - 1.0F);
            }
