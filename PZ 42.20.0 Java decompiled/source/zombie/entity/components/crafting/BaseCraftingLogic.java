...
public abstract class BaseCraftingLogic {
    ...
    public void addEventListener(String event, Object function, Object targetTable) {
        if (this.events.containsKey(event)) {
            ...
            DebugType.General.warn("Event '" + event + "' is unknown.");
        }
    }
    ...
    public void filterRecipeList(String filter, String categoryFilter, boolean force, IsoPlayer player) {
        List<CraftRecipe> recipesKnown = new ArrayList<>();
        if (player == null) {
    ...
    public static CraftRecipeListNodeCollection filterAndSortRecipeList(
        ...
        if (!StringUtils.isNullOrWhitespace(filterString)) {
            ...
            List<CraftRecipe> resultsExact = new ArrayList<>();
            List<CraftRecipe> resultsStartsWith = new ArrayList<>();
            List<CraftRecipe> resultsAllPartExact = new ArrayList<>();
            List<CraftRecipe> resultsAllPartStartsWith = new ArrayList<>();
            List<CraftRecipe> resultsContains = new ArrayList<>();
            if (filterMode == CraftRecipeManager.FilterMode.Tags) {
                ...
                for (int i = 0; i < CraftRecipeManager.getAllRecipeTags().size(); i++) {
                    ...
                    BaseCraftingLogic.FilterStringMatchType matchType = getFilterStringMatchType(
                        filterString, CraftRecipeManager.getAllRecipeTags().get(i), filterStringParts, false
                    );
                ...
                for (int i = 0; i < categoryFilteredSourceList.size(); i++) {
                    CraftRecipe recipe = categoryFilteredSourceList.get(i);
                        ...
                        case InputName:
                            ...
                            if (fluidSearch != null) {
                                for (CraftRecipe.IOScript ioLine : recipe.getIoLines()) {
                                    if (ioLine instanceof InputScript inputScript && inputScript.getResourceType() == ResourceType.Fluid) {
                                ...
                                for (int x = 0; x < recipe.getInputs().size(); x++) {
                                    InputScript input = recipe.getInputs().get(x);

                                    for (int j = 0; j < input.getPossibleInputItems().size(); j++) {
                                        Item item = input.getPossibleInputItems().get(j);
                                        BaseCraftingLogic.FilterStringMatchType matchType = getFilterStringMatchType(
                                            filterString,
                            ...
                            for (int x = 0; x < recipe.getOutputs().size(); x++) {
                                ...
                                if (fluidSearch == null) {
                                    if (input.getOutputMapper() != null && input.getOutputMapper().getResultItems() != null) {
                                        for (int j = 0; j < input.getOutputMapper().getResultItems().size(); j++) {
                                            Item item = input.getOutputMapper().getResultItems().get(j);
                                            BaseCraftingLogic.FilterStringMatchType matchType = getFilterStringMatchType(
                                                filterString,
                                                fullName ? item.getFullName().toLowerCase() : item.getDisplayName().toLowerCase(),
                                                filterStringParts,
    ...
    public boolean setContainers(ArrayList<ItemContainer> containersToUse) {
        Set<InventoryItem> oldItems = new HashSet<>(this.allItems);
        ...
        if (hasChanged) {
            List<InventoryItem> currentRecipeInputs = this.recipeData.getAllInputItems();
            this.recipeData.canConsumeInputs(this.sourceResources, this.allItems, true, true);
            ...
            this.populateInputs(this.player, currentRecipeInputs, this.sourceResources, true);
    ...
    public void refresh() {
        if (this.getRecipe() != null) {
            ...
            this.recipeData.canConsumeInputs(this.sourceResources, this.allItems, true, true);
            this.populateInputs(this.player, currentRecipeInputs, this.sourceResources, true);
            Set<String> oldManualInputs = new HashSet<>(this.manualInputAllowedItemTypes);
            this.manualInputAllowedItemTypes.clear();
            this.autoPopulateInputs();
            this.manualInputAllowedItemTypes.addAll(oldManualInputs);
        }
    }
    ...
    public boolean isContainersAccessible(List<ItemContainer> containers) {
        for (int i = 0; i < containers.size(); i++) {
            ...
            if (outer.getVehiclePart() != null
                ...
                DebugType.CraftLogic.debugln("Can't craft: can't access vehicle container");
                return false;
            }
            ...
            if (outer.getParent() != null && outer.getParent() instanceof IsoThumpable thumpable && thumpable.isLockedToCharacter(this.player)) {
                DebugType.CraftLogic.debugln("Can't craft: thumpable container is locked to the character");
                return false;
            }
        }
    ...
    public void populateInputs(IsoGameCharacter player, List<InventoryItem> inputItems, List<Resource> resources, boolean clearExisting) {
        if (this.recipeData != null) {
            ...
            this.recipeData.populateInputs(player, inputItems, resources, clearExisting);
            this.cachedCanPerformDirty = true;
        }
    }

    public void autoPopulateInputs() {
        this.populateInputs(this.player, this.getAllViableInputInventoryItems(), this.getAllViableInputResources(), false);
        this.recipeData.canConsumeInputs(this.sourceResources, null, true, false);
        this.cachedPossibleCraftCount = this.getPossibleCraftCount(true);
    ...
    public ArrayList<InventoryItem> getMulticraftConsumedItemsFor(InputScript inputScript, ArrayList<InventoryItem> list) {
        List<Item> appliedItemTypes = new ArrayList<>();
        ...
        for (int i = 0; i < this.multicraftConsumedItems.size(); i++) {
            ...
            if (!list.contains(itemToAssess) && inputScript.canUseItem(itemToAssess, this.player) && appliedItemTypes.contains(itemToAssess.getScriptItem())) {
