...
public class CraftRecipeManager {
    ...
    public static List<CraftRecipe> queryRecipes(CraftRecipeTag... craftRecipeTag) {
        ...
        return queryRecipes(query.toString());
    }
    ...
    public static List<CraftRecipe> filterRecipeList(String filterString, List<CraftRecipe> listToPopulate, List<CraftRecipe> sourceList) {
        ...
        if (sourceList == null || listToPopulate == null) {
            DebugType.General.error("one of list parameters is null.");
    ...
    public static ArrayList<InventoryItem> getAllValidItemsForRecipe(
        CraftRecipe recipe, ArrayList<InventoryItem> sourceItems, ArrayList<InventoryItem> filteredItems, IsoGameCharacter character
    ) {
        for (int i = 0; i < sourceItems.size(); i++) {
            InventoryItem inventoryItem = sourceItems.get(i);
            if (isItemValidForRecipe(recipe, inventoryItem, character)) {
    ...
    public static InputScript getValidInputScriptForItem(CraftRecipe recipe, InventoryItem inventoryItem, IsoGameCharacter character) {
        for (int i = 0; i < recipe.getInputs().size(); i++) {
            ...
            if (input.getResourceType() == ResourceType.Item && isItemValidForInputScript(input, inventoryItem, character)) {
                return input;
            }
        }
    ...
    public static boolean isItemToolForRecipe(CraftRecipe recipe, InventoryItem inventoryItem, IsoGameCharacter character) {
        InputScript input = getValidInputScriptForItem(recipe, inventoryItem, character);
    ...
    public static boolean isItemValidForInputScript(InputScript input, InventoryItem inventoryItem, IsoGameCharacter character) {
        return input.getResourceType() == ResourceType.Item ? consumeInputItem(input, inventoryItem, true, null, character) : false;
    }

    public static boolean isValidRecipeForCharacter(
        CraftRecipe recipe, IsoGameCharacter character, CraftRecipeMonitor monitor, ArrayList<ItemContainer> containers
    ) {
        if (recipe == null) {
            DebugType.CraftLogic.debugln("Recipe is null");
            return false;
        }

        if (character != null && !validateHasRequiredSkill(recipe, character, containers)) {
            if (monitor != null) {
                monitor.log("Player doesn't have required skill for " + recipe.getScriptObjectFullType());
            }

            DebugType.CraftLogic.debugln("Player doesn't have required skill for " + recipe.getScriptObjectFullType());
            return false;
            ...
            DebugType.CraftLogic.debugln("Player doesn't know recipe " + recipe.getScriptObjectFullType());
            return false;
        } else {
            return true;
        }
    ...
    private static boolean isCurrentCraftActionPending(IsoGameCharacter player, KahluaTable action) {
        LuaTimedActionNew currentAction = player.getCharacterActions().isEmpty() ? null : (LuaTimedActionNew)player.getCharacterActions().get(0);
        Boolean craftStarted = (Boolean)action.rawget("craftStarted");
        return currentAction != null && currentAction.getTable() == action && (craftStarted == null || !craftStarted);
    }

    private static boolean consumesEntireItem(InputScript inputScript, InventoryItem item) {
        return inputScript.isDestroy()
            || inputScript.hasFlag(InputFlag.DontReplace) && (item.getReplaceOnUse() != null || item.getScriptItem().getReplaceOnDeplete() != null)
            || inputScript.isItemCount();
    }

    public static float itemGetQueuedAmount(IsoGameCharacter player, InventoryItem item, CraftRecipeData.CacheData cacheData) {
        if (!GameServer.server && player != null && item != null) {
            ...
            KahluaTable actions = (KahluaTable)queue.rawget("queue");
            if (actions == null) {
                return 0.0F;
            }
            ...
            while (actionsIt.advance()) {
                ...
                if ("ISHandcraftAction".equals(action.getMetatable().getString("Type")) && !isCurrentCraftActionPending(player, action)) {
                    KahluaTable manualInputs = (KahluaTable)action.rawget("manualInputs");
                    if (manualInputs != null) {
                        CraftRecipe recipe = (CraftRecipe)action.rawget("craftRecipe");
                        KahluaTableIterator inputsIt = manualInputs.iterator();

                        while (inputsIt.advance()) {
                            List<?> manualInputItems = (List<?>)inputsIt.getValue();
                            if (manualInputItems != null && manualInputItems.size() == 1 && manualInputItems.contains(item)) {
                                Double index = (Double)inputsIt.getKey();
                                if (index != null) {
                                    InputScript inputScript = (InputScript)recipe.getIOForIndex((int)index.doubleValue());
                                    if (inputScript != null) {
                                        if (inputScript.hasConsumeFromItem()) {
                                            switch (inputScript.getConsumeFromItemScript().getResourceType()) {
                                                case Item:
                                                case Energy:
                                                    amount += (int)inputScript.getAmount();
                                                    break;
                                                case Fluid:
                                                    amount += inputScript.getAmount();
                                            }
                                        } else {
                                            if (consumesEntireItem(inputScript, item)) {
                                                return Float.MAX_VALUE;
                                            }

                                            if (!inputScript.isKeep()) {
                                                float variableInputRatio = cacheData != null && cacheData.getRecipeData() != null
                                                    ? cacheData.getRecipeData().getVariableInputRatio()
                                                    : 1.0F;
                                                float variableAmount = inputScript.isVariableAmount()
                                                    ? inputScript.getIntAmount() * variableInputRatio
                                                    : inputScript.getIntAmount();
                                                float amountTruncated = variableAmount > 1.0E-5F ? (float)Math.ceil(variableAmount) : 0.0F;
                                                float itemScale = inputScript.getRelativeScale(item.getScriptItem().getFullName());
                                                amount += (int)(amountTruncated * itemScale);
                                            }
                                        }
                                    }
                                }
    ...
    protected static boolean consumeInputFromResources(
        InputScript input,
        ...
        if (input != null && resources != null) {
            switch (input.getResourceType()) {
                case Item:
                    return consumeInputItem(input, resources, testOnly, cacheData, character, usedResources);
                case Fluid:
                    return consumeInputFluid(input, resources, testOnly, cacheData, 0.0F);
                case Energy:
                    return consumeInputEnergy(input, resources, testOnly, cacheData);
                default:
                    return false;
            }
        } else {
    ...
    protected static boolean consumeInputItem(
        ...
        if (resources != null && !resources.isEmpty()) {
            ...
            HashSet<Resource> tempUsedResources = new HashSet<>();
            HashMap<InventoryItem, Resource> tempConsumedUsedItems = new HashMap<>();
            ...
            for (int j = 0; j < inputItems.size(); j++) {
                for (int idx = 0; idx < resources.size(); idx++) {
                    ...
                    if (resource != null
                        && !resource.isEmpty()
                        && input.getResourceType() == resource.getType()
                        && !usedResources.contains(resource)
                        && !tempUsedResources.contains(resource)) {
                ...
                if (requiredAmount <= 1.0E-5F) {
                    ...
                    if (!testOnly && (!input.isKeep() || canMoveItemsToOutput)) {
                        for (Entry<InventoryItem, Resource> entry : tempConsumedUsedItems.entrySet()) {
    ...
    protected static boolean consumeInputItem(
        ...
        float useQueued = success && testOnly ? itemGetQueuedAmount(character, inventoryItem, cacheData) : 0.0F;
        if (testOnly && success && (useQueued == Float.MAX_VALUE || !input.hasConsumeFromItem() && inventoryItem.getCurrentUses() < (int)useQueued)) {
            success = false;
        }
        ...
        if (success && input.hasConsumeFromItem()) {
            ...
            switch (script.getResourceType()) {
                case Item:
                    success = consumeInputItemUsesInternal(script, inventoryItem, testOnly, cacheData, (int)useQueued);
                    if (success && !input.isDestroy()) {
                        moveToOutput = inventoryItem.getCurrentUses() < input.getAmount() + useQueued;
                    }
                    break;
                case Fluid:
                    success = consumeInputFluidInternal(script, inventoryItem.getFluidContainer(), testOnly, cacheData, useQueued);
                    if (success) {
                        moveToOutput = inventoryItem.getFluidContainer().getAmount() < input.getAmount() + useQueued;
                    }
                    break;
                case Energy:
                    success = consumeInputEnergyFromItemInternal(script, inventoryItem, testOnly, cacheData, (int)useQueued);
                    if (success) {
                        moveToOutput = inventoryItem.getCurrentUses() < input.getAmount() + useQueued;
                    }
            }
        ...
        if (success && input.hasCreateToItem()) {
            ...
            switch (script.getResourceType()) {
                case Item:
                    success = createOutputItemUsesInternal(script, inventoryItem, testOnly, cacheData, (int)useQueued);
                    break;
                case Fluid:
                    success = createOutputFluidInternal(script, inventoryItem.getFluidContainer(), testOnly, cacheData, useQueued);
                    break;
                case Energy:
                    success = createOutputEnergyToItemInternal(script, inventoryItem, testOnly, cacheData, (int)useQueued);
            }
        }
    ...
    private static boolean consumeInputItemUsesInternal(
        InputScript input, InventoryItem inventoryItem, boolean testOnly, CraftRecipeData.CacheData cacheData, int useQueued
    ) {
        if (inventoryItem instanceof DrainableComboItem comboItem) {
            ...
            if (comboItem.getCurrentUses() >= useAmount + useQueued) {
    ...
    private static boolean consumeInputFluid(
        InputScript input, List<Resource> resources, boolean testOnly, CraftRecipeData.CacheData cacheData, float useQueued
    ) {
        ...
        for (Resource resource : resources) {
            if (resource != null
                && resource.getType() == ResourceType.Fluid
                && !resource.isEmpty()
                && consumeInputFluidInternal(input, ((ResourceFluid)resource).getFluidContainer(), testOnly, cacheData, useQueued)) {
                return true;
            }
        }
    ...
    private static boolean consumeInputFluidInternal(
        InputScript input, FluidContainer fc, boolean testOnly, CraftRecipeData.CacheData cacheData, float useQueued
    ) {
        if (fc != null && !fc.isEmpty()) {
            ...
            if (parentScript.isItemCount() && fc.getAmount() < requiredFluid + useQueued) {
                return false;
            }
            ...
            if (input.isFluidMatch(fc)) {
                float fluidToTake = Math.min(requiredFluid, Math.max(fc.getAmount() - useQueued, 0.0F));
                if (fluidToTake < 1.0E-5F) {
                    return false;
                }
    ...
    private static boolean consumeInputEnergy(InputScript input, List<Resource> resources, boolean testOnly, CraftRecipeData.CacheData cacheData) {
        ...
        for (Resource resource : resources) {
            if (resource != null && resource.getType() == ResourceType.Energy && !resource.isEmpty()) {
                ResourceEnergy resourceEnergy = (ResourceEnergy)resource;
                if ((!input.hasFlag(InputFlag.IsFull) || resource.isFull())
                    && resourceEnergy.getEnergyAmount() >= input.getAmount()
                    && input.isEnergyMatch(resourceEnergy.getEnergy())) {
    ...
    private static boolean consumeInputEnergyFromItemInternal(
        ...
        if (inventoryItem instanceof DrainableComboItem comboItem) {
            ...
            if (comboItem.getCurrentUses() >= useAmount + useQueued && input.isEnergyMatch(comboItem)) {
                if (!testOnly && !input.isKeep()) {
                    comboItem.setCurrentUses(comboItem.getCurrentUses() - useAmount);
                }
    ...
    protected static boolean createOutputItem(OutputScript output, Item item, boolean testOnly, CraftRecipeData.CacheData cacheData, IsoGameCharacter character) {
        ...
        if (success && !testOnly && output.hasCreateToItem() && cacheData.getMostRecentItem() != null) {
            ...
            switch (script.getResourceType()) {
                case Item:
                    b = createOutputItemUsesInternal(script, inventoryItem, testOnly, cacheData, 0);
                    break;
                case Fluid:
                    b = createOutputFluidInternal(script, inventoryItem.getFluidContainer(), testOnly, cacheData, 0.0F);
                    break;
                case Energy:
                    b = createOutputEnergyToItemInternal(script, inventoryItem, testOnly, cacheData, 0);
            }

            if (!b) {
                DebugType.General.warn("unable to create uses/fluid/energy to item: " + (item != null ? item.getFullName() : "unknown"));
            }
        }
        ...
        if (item != null && item.getFullName() != null) {
            if (!testOnly) {
                ...
                if (inventoryItem != null) {
                    ...
                    DebugType.General.warn("Failed to create item: " + item.getFullName());
                }
            }
    ...
    private static boolean createOutputItemUsesInternal(
        OutputScript output, InventoryItem targetItem, boolean testOnly, CraftRecipeData.CacheData cacheData, int useQueued
    ) {
        if (targetItem instanceof DrainableComboItem comboItem) {
            int stored = Math.max(targetItem.getCurrentUses() - useQueued, 0);
    ...
    private static boolean createOutputFluid(OutputScript output, Resource resource, boolean testOnly, CraftRecipeData.CacheData cacheData) {
        if (!validateOutputScript(output, ResourceType.Fluid, testOnly)) {
            ...
            return resource != null && resource.getType() == ResourceType.Fluid && !resource.isFull()
                ? createOutputFluidInternal(output, ((ResourceFluid)resource).getFluidContainer(), testOnly, cacheData, 0.0F)
                : false;
        }
    }

    private static boolean createOutputFluidInternal(
        OutputScript output, FluidContainer fc, boolean testOnly, CraftRecipeData.CacheData cacheData, float useQueued
    ) {
        ...
        if (fc != null && !fc.isFull()) {
            float stored = Math.max(fc.getAmount() - useQueued, 0.0F);
    ...
    private static boolean createOutputEnergyToItemInternal(
        ...
        if (targetItem instanceof DrainableComboItem comboItem) {
            if (comboItem.isEnergy() && comboItem.getEnergy() != null && comboItem.getEnergy().equals(output.getEnergy())) {
                int stored = Math.max(comboItem.getCurrentUses() - useQueued, 0);
                if (stored > 0 && output.hasFlag(OutputFlag.ForceEmpty)) {
                    stored = 0;
                    if (!testOnly) {
                        comboItem.setCurrentUses(0);
    ...
    public static ArrayList<CraftRecipe> getUniqueRecipeItems(InventoryItem item, IsoGameCharacter chr, ArrayList<ItemContainer> containers) {
        RecipeList.clear();
        List<CraftRecipe> allRecipes = queryRecipes(CraftRecipeTag.IN_HAND_CRAFT, CraftRecipeTag.ANY_SURFACE_CRAFT);
        ...
        for (int i = 0; i < allRecipes.size(); i++) {
            ...
            if (isValidRecipeForCharacter(recipe, chr, null, containers)
                && getValidInputScriptForItem(recipe, item, chr) != null
                && recipe.OnTestItem(item, chr)) {
