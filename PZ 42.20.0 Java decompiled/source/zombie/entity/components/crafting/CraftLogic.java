...
public class CraftLogic extends Component {
    ...
    public CraftRecipeMonitor debugCanStart(IsoPlayer player) {
        try {
            DebugType.General.println("=== Starting debug canStart test ===");
            ...
            return CraftUtil.debugCanStart(player, this.craftTestData, this.recipes, inputs, outputs, monitor);
        } catch (Exception e) {
    ...
    protected boolean canStart(StartMode startMode, IsoPlayer player) {
        ...
        List<Resource> inputs = this.getInputResources();
        return inputs != null && !inputs.isEmpty() && this.willOutputsAccommodate(this.craftTestData.getToOutputItems())
            ? CraftUtil.canPerformRecipe(this.craftTestData.getRecipe(), this.craftTestData, inputs, this.getOutputResources())
            : false;
    }

    private boolean isInvalidStartMode(StartMode startMode, IsoPlayer player) {
        return !this.isValid()
            || this.startMode == StartMode.Passive
            || this.startMode != startMode
            || this.startMode == StartMode.Manual && !this.owner.isUsingPlayer(player);
    }
    ...
    public boolean canStartWithInventoryItems(IsoPlayer player, List<InventoryItem> selectedInventoryItems) {
        ...
        List<Resource> inputs = new ArrayList<>(this.getInputResources());
        List<Resource> outputs = this.getOutputResources();
        if (!inputs.isEmpty() || selectedInventoryItems != null && !selectedInventoryItems.isEmpty()) {
            ...
            if (selectedInventoryItems != null && !selectedInventoryItems.isEmpty() && this.willInputsAccommodate(selectedInventoryItems)) {
                ResourceBlueprint temporaryResourceBlueprint = ResourceBlueprint.alloc(
                    "temp",
                    ResourceType.Item,
                    ResourceIO.Input,
                    selectedInventoryItems.size(),
                    null,
                    ResourceChannel.NO_CHANNEL,
                    EnumBitStore.of(ResourceFlag.Temporary)
                );
    ...
    public boolean willInputsAccommodate(List<InventoryItem> inventoryItems) {
        if (inventoryItems != null && !inventoryItems.isEmpty()) {
            List<Resource> inputs = this.getInputResources();
            Map<ResourceItem, Integer> resourceCapacityMap = new HashMap<>();
            Map<ResourceItem, Item> resourceItemMap = new HashMap<>();

            for (Resource input : inputs) {
                if (input instanceof ResourceItem resourceItem) {
            ...
            for (InventoryItem inventoryItem : inventoryItems) {
                for (Entry<ResourceItem, Integer> entry : resourceCapacityMap.entrySet()) {
                    ResourceItem resource = entry.getKey();
                    int resourceFreeItemCapacity = entry.getValue();
                    Item resourceFirstItem = resourceItemMap.get(resource);
                    if (resourceFreeItemCapacity > 0 && this.canStackItem(resource, resourceFirstItem, inventoryItem.getScriptItem())) {
                        allocatedItems++;
    ...
    private String getBestStyleName(List<String> availableStyles) {
        if (this.isCraftingMixedRecipes()) {
            for (String style : availableStyles) {
                if (style.startsWith(SpriteOverlayConfigKey.DEFAULT.toString())) {
    ...
    private String getBestStyle(int percentageComplete, List<String> availableStyles) {
        ...
        if (styleName != null) {
            ...
            for (String style : viableStyles) {
                ...
                int diff = percentageComplete - progressComponent;
