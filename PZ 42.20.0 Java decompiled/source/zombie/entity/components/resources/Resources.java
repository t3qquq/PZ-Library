...
public class Resources extends Component {
    ...
    public void createResourceFromSerial(String groupName, String blueprintSerial) {
        try {
            ...
            DebugType.General.printException(e, LogSeverity.Error, "ResourceBlueprint serial: " + blueprintSerial);
        }
    }
    ...
    private void addResourceInternal(String groupName, Resource resource) {
        if (resource == null) {
            DebugType.General.warn("unable to add resource 'null'");
        } else if (this.idToResourceMap.containsKey(resource.getId())) {
            DebugType.General.warn("unable to add resource, duplicate ID '" + resource.getId() + "'");
        } else {
    ...
    public Resource getResource(String nameId) {
        if (nameId != null) {
            for (int i = 0; i < this.resources.size(); i++) {
                ...
                if (resource != null && resource.getId() != null && resource.getId().equalsIgnoreCase(nameId)) {
                    return resource;
                }
            }
        }
    ...
    private List<Resource> getResourcesFromGroup(String group, List<Resource> list, ResourceIO io, ResourceType type, ResourceChannel channel, boolean clear) {
        ...
        if (group != null) {
            ...
            if (resourceGroup == null) {
                DebugType.General.warn("Group '" + group + "' does not exist.");
                return list;
            }
