...
public class FluidContainer extends Component {
    ...
    protected void readFromScript(ComponentScript componentScript) {
        ...
        if (script.getInitialFluids() != null && !script.getInitialFluids().isEmpty() && script.getInitialAmount() > 0.0F) {
            ...
            if (script.getInitialFluids().size() == 1) {
                this.addInitialFluid(initialAmount, script.getInitialFluids().getFirst(), script.getName());
        ...
        if (layout != null) {
            ...
            if (!this.isEmpty()) {
                if (this.fluids.size() == 1) {
                    FluidInstance fi = this.fluids.getFirst();
                    item = layout.addItem();
                    ...
                    float fluidRatio = this.getAmount() / this.getCapacity();
                    item.setProgress(fluidRatio, this.getColor().r, this.getColor().g, this.getColor().b, 1.0F);
                    item = layout.addItem();
                    item.setLabel(Translator.getFluidText("Fluid_Fluids") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
                    tempFluidUI.clear();
                    tempFluidUI.addAll(this.fluids);
                    tempFluidUI.sort(Comparator.comparing(FluidInstance::getTranslatedName));

                    for (int i = 0; i < tempFluidUI.size(); i++) {
                        FluidInstance fluidInstance = tempFluidUI.get(i);
                        float combinedAmount = fluidInstance.getAmount();

                        while (
                            i + 1 < tempFluidUI.size()
                                && (
                                    SandboxOptions.instance.enableTaintedWaterText.getValue()
                                        ? tempFluidUI.get(i + 1).getFluid() == fluidInstance.getFluid()
                                        : tempFluidUI.get(i + 1).getFluid().getTranslatedName().equals(fluidInstance.getFluid().getTranslatedName())
                                )
                        ) {
                            combinedAmount += tempFluidUI.get(++i).getAmount();
                        }

                        item = layout.addItem();
                        item.setLabel(fluidInstance.getFluid().getTranslatedName() + ":", 0.7F, 0.7F, 0.4F, 1.0F);
                        fluidRatio = combinedAmount / this.getAmount();
                        item.setProgress(fluidRatio, fluidInstance.getColor().r, fluidInstance.getColor().g, fluidInstance.getColor().b, 1.0F);
                    }
                }

                boolean hasOtherPoison = false;

                for (FluidInstance fluidInstance : this.fluids) {
                    Fluid fluid = fluidInstance.getFluid();
                    if (fluid.getFluidType() != FluidType.TaintedWater && fluid.isPoisonous()) {
                        hasOtherPoison = true;
                        break;
                    }
                }
                ...
                if (this.isPoisonous()
                    && this.getPoisonEffect() != PoisonEffect.None
                    && (hasOtherPoison || !this.isTainted() || SandboxOptions.instance.enableTaintedWaterText.getValue())) {
                ...
                if (Core.debug) {
                    ...
                    item = layout.addItem();
                    item.setLabel(Translator.getText("IGUI_Fluids_Categories") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
                    Set<String> cats = new TreeSet<>();

                    for (FluidInstance fluid : this.fluids) {
                        for (FluidCategory cat : fluid.getFluid().getCategories()) {
    ...
    public void setContainerName(String name) {
        if (StringUtils.containsWhitespace(name)) {
            DebugType.General.error("Sanitizing container name '" + name + "', name may not contain whitespaces.");
    ...
    public String getUiName() {
        if (this.cacheInvalidated || this.nameCache == null) {
            ...
            List<FluidInstance> visibleFluids = new ArrayList<>(this.fluids);
            boolean uiIsPoisonous = this.isPoisonous();
            if (!SandboxOptions.instance.enableTaintedWaterText.getValue()) {
                FluidInstance tainted = null;
                boolean hasRealPoison = false;

                for (FluidInstance fluidinstance : visibleFluids) {
                    Fluid fluid = fluidinstance.getFluid();
                    if (fluid == Fluid.TaintedWater) {
                        tainted = fluidinstance;
                    } else if (fluid.isCategory(FluidCategory.Poisons) || fluid.isPoisonous()) {
                        hasRealPoison = true;
                    }
                }
                ...
                if (tainted != null) {
                    for (FluidInstance fluidInstance : visibleFluids) {
                        if (fluidInstance != tainted && fluidInstance.getTranslatedName().equals(tainted.getTranslatedName())) {
                            visibleFluids.remove(tainted);
                            break;
                        }
                    }
                }
            ...
            if (visibleFluids.isEmpty()) {
            ...
            } else if (visibleFluids.size() == 1) {
                this.nameCache = Translator.getText("Fluid_HoldingOneType", containerName, visibleFluids.getFirst().getFluid().getTranslatedName());
            ...
            } else if (visibleFluids.size() == 2 && uiIsPoisonous) {
                this.nameCache = containerName + " " + Translator.getFluidText("Fluid_Of") + " ";
                FluidInstance fluidInstanceA = visibleFluids.get(0);
                FluidInstance fluidInstanceB = visibleFluids.get(1);
                if (!fluidInstanceA.getFluid().isCategory(FluidCategory.Poisons)) {
                    this.nameCache = Translator.getText("Fluid_HoldingOneType", containerName, fluidInstanceA.getFluid().getTranslatedName());
                } else {
                    this.nameCache = Translator.getText("Fluid_HoldingOneType", containerName, fluidInstanceB.getFluid().getTranslatedName());
                }

                if (this.contains(Fluid.TaintedWater)
                    && this.getPoisonEffect() != PoisonEffect.None
                    && SandboxOptions.instance.enableTaintedWaterText.getValue()) {
                    this.nameCache = this.nameCache + " " + Translator.getFluidText("Fluid_Tainted");
                }
            } else if (visibleFluids.size() == 2) {
                FluidInstance fluidInstanceA = visibleFluids.get(0);
                FluidInstance fluidInstanceB = visibleFluids.get(1);
                FluidInstance fluid = fluidInstanceA.getAmount() < fluidInstanceB.getAmount() ? fluidInstanceA : fluidInstanceB;
                this.nameCache = Translator.getText("Fluid_HoldingOneType", containerName, fluid.getFluid().getTranslatedName());
                if (!fluidInstanceA.getTranslatedName().equals(fluidInstanceB.getTranslatedName())) {
                    FluidInstance fluidB = fluidInstanceA.getAmount() < fluidInstanceB.getAmount() ? fluidInstanceB : fluidInstanceA;
                ...
                for (FluidInstance fluidInstance : visibleFluids) {
                    if (!fluidInstance.getFluid().isCategory(FluidCategory.Beverage)) {
                        allBeverages = fluidInstance.getFluid().isCategory(FluidCategory.Poisons);
                    }

                    if (fluidInstance.getFluid().isCategory(FluidCategory.Beverage) && fluidInstance.getFluid().isCategory(FluidCategory.Alcoholic)) {
                        alcoholics++;
                    }
                }
    ...
    private void recalculateCaches(boolean force, boolean removeInvalid) {
        if (this.cacheInvalidated || force) {
            ...
            if (this.amountCache > 0.0F) {
                for (FluidInstance fluidInstance : this.fluids) {
                    FluidInstance fluid = fluidInstance;
    ...
    public boolean isTainted() {
        for (FluidInstance fluid : this.fluids) {
            if (fluid.getFluid().getFluidType() == FluidType.TaintedWater) {
                return true;
            }
        }
    ...
    public boolean isPureFluid(Fluid fluid) {
        return this.fluids.size() == 1 && this.fluids.getFirst().getFluid() == fluid;
    }
    ...
    public Fluid getPrimaryFluid() {
        ...
        if (this.fluids.size() == 1) {
            return this.fluids.getFirst().getFluid();
        }

        FluidInstance primary = this.fluids.getFirst();

        for (FluidInstance test : this.fluids) {
    ...
    public float getPrimaryFluidAmount() {
        ...
        if (this.fluids.size() == 1) {
            return this.fluids.getFirst().getAmount();
        }
    ...
    public boolean isWaterOnlySource() {
        ...
        for (FluidInstance instance : this.fluids) {
            Fluid fluid = instance.getFluid();
            if (fluid == null || !fluid.isCategory(FluidCategory.Water)) {
                return false;
            }
        }
    ...
    private FluidInstance getFluidInstance(Fluid fluid) {
        ...
        for (FluidInstance instance : this.fluids) {
            FluidInstance fluidInstance = instance;
            if (fluidInstance.getFluid() == fluid) {
    ...
    public float getRatioForFluid(Fluid fluid) {
        for (FluidInstance fluidInstance : this.fluids) {
            if (fluidInstance.getFluid() == fluid) {
                return fluidInstance.getPercentage();
            }
        }
    ...
    public static boolean CanTransfer(FluidContainer source, FluidContainer target) {
        if (source == null || target == null) {
            ...
            return false;
        } else if (target.getOwner() instanceof InventoryItem item
    ...
    public void save(ByteBuffer output) throws IOException {
        ...
        if (!this.fluids.isEmpty()) {
            ...
            if (this.fluids.size() == 1) {
                header.addFlags(4);
                FluidInstance.save(this.fluids.getFirst(), output);
            } else {
    ...
    public boolean isMultiTileMoveable() {
        return this.getOwner() instanceof IsoWorldInventoryObject obj && obj.getItem() instanceof Moveable mov && mov.getSpriteGrid() != null
            || this.getOwner() instanceof InventoryItem item && item instanceof Moveable mov2 && mov2.getSpriteGrid() != null;
    }

    public float getTransferRate() {
        if (this.getOwner() instanceof InventoryItem item) {
            Item itemScript = item.getScriptItem();
            FluidContainerScript componentScript = itemScript.getComponentScriptFor(ComponentType.FluidContainer);
            if (componentScript != null) {
                return componentScript.getTransferRate();
            }
        }
