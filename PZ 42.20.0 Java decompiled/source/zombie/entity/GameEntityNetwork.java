...
public class GameEntityNetwork implements INetworkPacket {
    ...
    public static void sendPacketData(EntityPacketData data, GameEntity entity, Component component, IConnection connection, boolean isIgnoreConnection) {
        if (entity == null) {
            DebugType.General.warn("Packet should have entity set.");
        } else if (entity.getGameEntityType() != GameEntityType.Template) {
            if (GameClient.client || GameServer.server) {
                if (data.getEntityPacketType().isEntityPacket() && component != null) {
                    DebugType.General.warn("Entity Packet should not have component set.");
                } else if (data.getEntityPacketType().isComponentPacket() && component == null) {
                    DebugType.General.warn("Component Packet requires to have component set.");
                } else {
    ...
    public void parse(ByteBufferReader b, IConnection connection) {
        if (GameClient.client || GameServer.server) {
            ...
            if (gameEntity == null) {
                DebugType.Objects.debugln("Cannot find registered game entity for id = %d", entityNetId);
                ...
                if (gameEntity == null) {
                    DebugType.Objects.debugln("Cannot find inventory item for id = %d", entityNetId);
                    return;
                }
            }
            ...
            try {
                if (packetType.getGroup() != PacketGroup.GameEntity) {
                    ...
                    if (!success && Core.debug) {
                        DebugType.Entity.error("ReadPacketContents returned failure. PacketType = " + packetType);
                    }
                }
            } catch (Exception e) {
