...
public class ItemConfigurator {
    ...
    public static void Preprocess() {
        ...
        for (String vehicleContainer : vehicle_containers) {
            if (registerString(vehicleContainer)) {
            }
        }
        ...
        for (Entry<String, IsoSprite> entry : IsoSpriteManager.instance.namedMap.entrySet()) {
            if (entry.getValue().getProperties() != null
                ...
                && registerString(entry.getValue().getProperties().get(IsoPropertyType.CONTAINER))) {
            }
        }
