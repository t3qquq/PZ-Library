...
public final class Food extends InventoryItem {
    ...
    public void load(ByteBuffer input, int worldVersion) throws IOException {
        ...
        if (this.is(ItemKey.Food.CORPSE_ANIMAL)) {
        ...
        if (outer != null) {
            if (outer.getParent() instanceof IsoPlayer player) {
                if (GameClient.client) {
                    ...
                    INetworkPacket.send(player, PacketTypes.PacketType.ItemStats, this.getContainer(), this);
                }
            } else if (this.getContainer().getSquare() != null) {
                IsoGridSquare sq = this.getContainer().getSquare();
                INetworkPacket.sendToRelative(PacketTypes.PacketType.ItemStats, sq.getX(), sq.getY(), this.getContainer(), this);
            }
        } else {
            if (GameClient.client) {
                INetworkPacket.send(PacketTypes.PacketType.ItemStats, this.getContainer(), this);
    ...
    private void syncItemFieldForWorldItem() {
        ...
        if (GameServer.server && this.getWorldItem() != null) {
            ...
            INetworkPacket.sendToRelative(PacketTypes.PacketType.ItemStats, this.getWorldItem().getX(), this.getWorldItem().getY(), floorContainer, this);
        }
    }
