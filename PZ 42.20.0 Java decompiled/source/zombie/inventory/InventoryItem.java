...
public class InventoryItem extends GameEntity {
    ...
    private static final long SERVER_MAX_UPDATE_DELTA_MS = 6000L;
    private static final EnumMap<IsoDirections, String> COMPASS_TOOLTIP = new EnumMap<>(
        Map.of(
            IsoDirections.N,
            "Tooltip_compass_9",
            IsoDirections.NW,
    ...
    private final List<String> gunTypeDisplayName = new ArrayList<>();
    ...
    public boolean isInsideBagOnSquare(IsoGridSquare square) {
        return square != null
            ...
            && this.getOutermostContainer().hasWorldItem()
            && this.getOutermostContainer().getWorldItem().getSquare() == square;
    }
    ...
    public boolean isForceDropHeavyItem() {
        return this.isHumanCorpse()
            ...
            || "CorpseAnimal".equalsIgnoreCase(this.getType());
    }
    ...
    private IsoDeadBody createDefaultDeadBody(IsoGridSquare square) throws Throwable {
        if (this.isHumanCorpse()) {
            ...
            zombie.setForwardDirection(zombie.getForwardIsoDirection().ToVector());
    ...
    public void DoTooltipEmbedded(ObjectTooltip tooltipUI, ObjectTooltip.Layout layoutOverride, int offsetY) {
        ...
        if (SandboxOptions.instance.isUnstableScriptNameSpam()) {
            ...
            item.setLabel(Translator.getText("Item Report") + ":", 1.0F, 0.4F, 0.7F, 1.0F);
            item.setValue(this.getFullType(), 1.0F, 1.0F, 0.8F, 1.0F);
        }
        ...
        if (player != null && this.scriptItem != null && !this.scriptItem.getResearchableRecipes(player).isEmpty()) {
            ...
            if (this.scriptItem.getResearchableRecipes(player).size() == 1) {
                item.setLabel(Translator.getText("Tooltip_item_CanResearch"), color.getR(), color.getG(), color.getB(), 1.0F);
            } else {
                item.setLabel(Translator.getText("Tooltip_item_CanResearchPlural"), color.getR(), color.getG(), color.getB(), 1.0F);
            }
        }
        ...
        if (player != null && this.scriptItem != null && this.scriptItem.isFavouriteRecipeInput(player)) {
            ColorInfo color = Core.getInstance().getGoodHighlitedColor();
            ObjectTooltip.LayoutItem item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_item_IsFavouriteInput"), color.getR(), color.getG(), color.getB(), 1.0F);
        }

        if (player != null && this.isNoRecipes(player)) {
            ColorInfo color = Core.getInstance().getBadHighlitedColor();
            ObjectTooltip.LayoutItem item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_NoRecipes_More"), color.getR(), color.getG(), color.getB(), 1.0F);
        }

        if (player != null && this.isUnwanted(player)) {
            ColorInfo color = Core.getInstance().getBadHighlitedColor();
            ObjectTooltip.LayoutItem item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_Unwanted_More"), color.getR(), color.getG(), color.getB(), 1.0F);
        }

        ObjectTooltip.LayoutItem item = layout.addItem();
        item.setLabel(Translator.getText("Tooltip_item_Weight") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
        ...
        if (!(this instanceof HandWeapon)
            ...
            if (this instanceof AnimalInventoryItem) {
                item.setValueRightNoPlus(this.getWeight());
            } else {
                ...
                if (this.getAttachedSlot() > -1) {
                    ...
                    item.setValueRightNoPlus(weight);
                }
            }
        ...
        if (tooltipUI.getWeightOfStack() > 0.0F) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_item_StackWeight") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
        ...
        if (this.getMaxAmmo() > 0 && !(this instanceof HandWeapon)) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_weapon_AmmoCount") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValue(this.getCurrentAmmoCount() + " / " + this.getMaxAmmo(), 1.0F, 1.0F, 1.0F, 1.0F);
        }

        if (!(this instanceof HandWeapon) && this.getAmmoType() != null) {
            item = layout.addItem();
            item.setLabel(Translator.getText("ContextMenu_AmmoType") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            ...
            item.setValue(Translator.getText(displayName), 1.0F, 1.0F, 1.0F, 1.0F);
        }

        if (this.gunType != null) {
            item = layout.addItem();
            item.setLabel(Translator.getText("ContextMenu_GunType") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValue(this.getGunTypeString(), 1.0F, 1.0F, 1.0F, 1.0F);
        }

        if (Core.debug && DebugOptions.instance.tooltipInfo.getValue()) {
            item = layout.addItem();
            item.setLabel("getActualWeight()", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.getActualWeight());
            item = layout.addItem();
            item.setLabel("getWeight()", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.getWeight());
            item = layout.addItem();
            item.setLabel("getEquippedWeight()", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.getEquippedWeight());
            item = layout.addItem();
            item.setLabel("getUnequippedWeight()", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.getUnequippedWeight());
            item = layout.addItem();
            item.setLabel("getContentsWeight()", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.getContentsWeight());
            if (this instanceof Key || "Doorknob".equals(this.type)) {
                item = layout.addItem();
                item.setLabel("DBG: keyId", 1.0F, 1.0F, 0.8F, 1.0F);
                item.setValueRightNoPlus(this.getKeyId());
            }

            item = layout.addItem();
            item.setLabel("ID", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.id);
            item = layout.addItem();
            item.setLabel("DictionaryID", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValueRightNoPlus(this.getRegistry_id());
            ClothingItem clothingItem = this.getClothingItem();
            if (clothingItem != null) {
                item = layout.addItem();
                item.setLabel("ClothingItem", 1.0F, 1.0F, 1.0F, 1.0F);
                item.setValue(this.getClothingItem().mame, 1.0F, 1.0F, 1.0F, 1.0F);
            }
        }
        ...
        if (Core.debug && DebugOptions.instance.tooltipInfo.getValue() || LuaManager.GlobalObject.isAdmin()) {
            ...
            item.setLabel("Loot Category:", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValue(category, 1.0F, 1.0F, 1.0F, 1.0F);
        }
        ...
        if (this instanceof Food && ((Food)this).isTainted() && SandboxOptions.instance.enableTaintedWaterText.getValue()) {
            item = layout.addItem();
            if (!this.hasMetal()) {
                item.setLabel(Translator.getText("Tooltip_item_TaintedWater"), 1.0F, 0.5F, 0.5F, 1.0F);
            } else {
                item.setLabel(Translator.getText("Tooltip_item_TaintedWater_Plastic"), 1.0F, 0.5F, 0.5F, 1.0F);
            }
        }
        ...
        if (!this.scriptItem.getForageFocusCategories().isEmpty()) {
            for (String categoryName : this.scriptItem.getForageFocusCategories()) {
                item = layout.addItem();
                item.setLabel(Translator.getText("UI_search_mode_item_focus") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
                item.setValue(Translator.getText("IGUI_SearchMode_Categories_" + categoryName), 1.0F, 1.0F, 1.0F, 1.0F);
            }
        }

        if (this.getFatigueChange() != 0.0F) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_item_Fatigue") + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
            if (this.getFatigueChange() < 0.0F) {
                item.setProgress(this.getFatigueChange() * -1.0F, goodR, goodG, goodB, 1.0F);
            } else {
                item.setProgress(this.getFatigueChange(), badR, badG, badB, 1.0F);
            }
        }
        ...
        if (this.getRemoteControlID() != -1) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_TrapControllerID"), 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValue(Integer.toString(this.getRemoteControlID()), 1.0F, 1.0F, 0.8F, 1.0F);
        }

        if (this.getHaveBeenRepaired() > 0) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_weapon_Repaired") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            if (this.hasTimesHeadRepaired()) {
                item.setLabel(Translator.getText("Tooltip_handle_Repaired") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            }

            item.setValue(this.getHaveBeenRepaired() + "x", 1.0F, 1.0F, 1.0F, 1.0F);
        }

        if (this.hasTimesHeadRepaired() && this.getTimesHeadRepaired() > 0) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_head_Repaired") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            item.setValue(this.getTimesHeadRepaired() + "x", 1.0F, 1.0F, 1.0F, 1.0F);
        }

        if (this.isEquippedNoSprint()) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_CantSprintEquipped"), 1.0F, 0.1F, 0.1F, 1.0F);
        }

        if (this.isWet()) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_Wetness") + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
        ...
        if (this.getMaxCapacity() > 0) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_container_Capacity") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
            ...
            if (this.getItemCapacity() > -1.0F) {
                item.setValue(this.getItemCapacity() + " / " + capacity, 1.0F, 1.0F, 0.8F, 1.0F);
            } else {
                item.setValue("0 / " + capacity, 1.0F, 1.0F, 0.8F, 1.0F);
            }
        }
        ...
        if (this.isRecordedMedia()) {
            ...
            if (data != null) {
                if (data.getTranslatedTitle() != null) {
                    item = layout.addItem();
                    item.setLabel(Translator.getText("Tooltip_media_title") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
                    item.setValue(data.getTranslatedTitle(), 1.0F, 1.0F, 1.0F, 1.0F);
                    if (data.getTranslatedSubTitle() != null) {
                        item = layout.addItem();
                        item.setLabel("", 1.0F, 1.0F, 0.8F, 1.0F);
                        item.setValue(data.getTranslatedSubTitle(), 1.0F, 1.0F, 1.0F, 1.0F);
                    }
                }

                if (data.getTranslatedAuthor() != null) {
                    item = layout.addItem();
                    item.setLabel(Translator.getText("Tooltip_media_author") + ":", 1.0F, 1.0F, 0.8F, 1.0F);
                    item.setValue(data.getTranslatedAuthor(), 1.0F, 1.0F, 1.0F, 1.0F);
                }
            }
        ...
        if (this.hasTag(ItemTag.COMPASS) && this.isInPlayerInventory()) {
            ...
            item = layout.addItem();
            item.setLabel(Translator.getText(COMPASS_TOOLTIP.get(dir)), 1.0F, 1.0F, 0.8F, 1.0F);
        }

        if (this.isFishingLure()) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_IsFishingLure"), 1.0F, 1.0F, 0.8F, 1.0F);
        }
        ...
        if (this.getVisionModifier() != 1.0F) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_item_VisionImpariment") + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
            if (this.getVisionModifier() < 1.0F) {
                item.setProgress(1.0F - this.getVisionModifier(), badR, badG, badB, 1.0F);
            } else {
                item.setProgress(this.getVisionModifier() - 1.0F, goodR, goodG, goodB, 1.0F);
            }
        }

        if (this.getHearingModifier() != 1.0F) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_item_HearingImpariment") + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
            if (this.getHearingModifier() < 1.0F) {
                item.setProgress(1.0F - this.getHearingModifier(), badR, badG, badB, 1.0F);
            } else {
                item.setProgress(this.getHearingModifier() - 1.0F, goodR, goodG, goodB, 1.0F);
            }
        }

        if (this.getDiscomfortModifier() != 0.0F) {
            item = layout.addItem();
            item.setLabel(Translator.getText("Tooltip_item_Discomfort") + ": ", 1.0F, 1.0F, 0.8F, 1.0F);
            if (this.getDiscomfortModifier() > 0.0F) {
                item.setProgress(this.getDiscomfortModifier(), badR, badG, badB, 1.0F);
            } else {
                item.setProgress(this.getDiscomfortModifier() * -1.0F, goodR, goodG, goodB, 1.0F);
            }
        }
        ...
        if (Core.getInstance().getOptionShowItemModInfo() && !this.isVanilla()) {
            ...
            item.setLabel("Mod: " + this.getModName(), c.r, c.g, c.b, 1.0F);
            ...
            if (info != null && info.getModOverrides() != null) {
                ...
                if (info.getModOverrides().size() == 1) {
                    item.setLabel("This item overrides: " + WorldDictionary.getModNameFromID(info.getModOverrides().get(0)), 0.5F, 0.5F, 0.5F, 1.0F);
                } else {
                    item.setLabel("This item overrides:", 0.5F, 0.5F, 0.5F, 1.0F);

                    for (int i = 0; i < info.getModOverrides().size(); i++) {
                        item = layout.addItem();
                        item.setLabel(" - " + WorldDictionary.getModNameFromID(info.getModOverrides().get(i)), 0.5F, 0.5F, 0.5F, 1.0F);
                    }
                }
            }
        }
        ...
        if (this.getTooltip() != null) {
            item = layout.addItem();
            item.setLabel(Translator.getText(this.getTooltip()), 1.0F, 1.0F, 0.8F, 1.0F);
        }
    ...
    private boolean shouldUpdateInRain() {
        if (GameClient.client) {
            ...
            if (worldItem != null && worldItem.getObjectIndex() != -1 && worldItem.getSquare().isOutside()) {
                boolean validFood = this instanceof Food food && !food.isTainted();
                boolean validWetItem = this.is(ItemKey.Normal.BATH_TOWEL_WET, ItemKey.Normal.DISH_CLOTH_WET);
                return validFood || validWetItem;
            } else {
                return false;
            }
        }
    ...
    public boolean shouldUpdateInWorld() {
        if (!GameServer.server && this.itemHeat != 1.0F) {
            ...
            return this.shouldUpdateInRain();
        }
    }

    private void handleRainTaint(Food food) {
        if (!(LuaManager.GlobalObject.ZombRandFloat(0.0F, 1.0F) >= RainManager.getRainIntensity())) {
            food.setTainted(true);
    ...
    private void soakTowel() {
        this.setWetCooldown(this.getWetCooldown() + RainManager.getRainIntensity() * (4.0F * this.timeMultiplier));
        if (GameServer.server) {
            GameServer.sendItemStats(this);
        }
    }
    ...
    protected void calculateTimeMultiplier() {
        if (GameServer.server) {
            ...
            long delta = this.lastUpdateMs == -1L ? 0L : Math.clamp(nowMs - this.lastUpdateMs, 0L, 6000L);
            this.timeMultiplier = GameTime.getInstance().getMultiplierFromTimeDelta((float)delta * 0.001F);
            this.lastUpdateMs = nowMs;
        } else {
            this.timeMultiplier = GameTime.getInstance().getMultiplier();
        }
    }
    ...
    public void update() {
        ...
        if (this.isWet()) {
            this.wetCooldown = this.wetCooldown - 1.0F * this.timeMultiplier;
            if (this.wetCooldown <= 0.0F) {
                ...
                if (worldItem != null) {
                    worldItem.swapItem(dryItem);
                } else if (container != null) {
                    container.addItem(dryItem);
                    container.Remove(this);
                    if (GameServer.server) {
                        GameServer.sendReplaceItemInContainer(container, this, dryItem);
                    }
                }
        ...
        if (this.hasComponent(ComponentType.FluidContainer)) {
            ...
            if (outermostContainer != null) {
                ...
                if (this.itemHeat < temp
                    ...
                    this.itemHeat = this.itemHeat + temp / (GameServer.server ? 16 : 1000) * this.timeMultiplier;
                ...
                if (this.itemHeat > 1.6F && !cont.isEmpty()) {
                    if (cont.contains(Fluid.TaintedWater)) {
                        ...
                        float remove = PZMath.min(oldAmount, (GameServer.server ? 0.6F : 0.01F) * this.timeMultiplier);
        ...
        if ((this.container == null || this.getWorldItem() != null) && this.itemHeat != 1.0F) {
            ...
            if (this.itemHeat > 1.0F) {
                this.itemHeat = this.itemHeat - 0.001F * this.timeMultiplier;
                if (this.itemHeat < 1.0F) {
                    this.itemHeat = 1.0F;
                }
            }
        ...
        this.handleWorldItemInRain();
    }

    public boolean finishupdate() {
        if (this.shouldUpdateInRain()) {
            return false;
        }
        ...
        if (!header.equals(0)) {
            ...
            if (header.hasFlags(16)) {
                ...
                if (this.getVisual() != null) {
                    this.getVisual().setTint(new ImmutableColor(this.colorRed, this.colorGreen, this.colorBlue));
                }
            }
    ...
    public ClothingItem getClothingItem() {
        String alternateModelName1 = this.getAlternateModelName();
        if ("RightHand".equalsIgnoreCase(alternateModelName1)) {
            return this.getItemReplacementPrimaryHand().clothingItem;
        }

        if ("LeftHand".equalsIgnoreCase(alternateModelName1)) {
            return this.getItemReplacementSecondHand().clothingItem;
        }

        if ("WithoutDrainable".equalsIgnoreCase(alternateModelName1)) {
            ...
            if (withoutDrainableItem != null && withoutDrainableItem.getClothingItemAsset() != null) {
                return withoutDrainableItem.getClothingItemAsset();
            }
        }
    ...
    public String getName(IsoPlayer player) {
        ...
        String prefix = "";
        if (this.isBloody()) {
            prefix = prefix + Translator.getText("IGUI_ClothingName_Bloody") + ", ";
        }

        if (this.isBroken()) {
            prefix = prefix + Translator.getText("Tooltip_broken") + ", ";
        } else if (this.getCondition() < this.getConditionMax() / 3.0F) {
            prefix = prefix + Translator.getText("IGUI_Name_Worn") + ", ";
        }

        if (!this.isBroken() && this.hasSharpness() && this.getSharpness() < 0.33333334F) {
            if (this.getSharpness() <= 0.0F) {
                prefix = prefix + Translator.getText("Tooltip_blunt") + ", ";
            } else {
                prefix = prefix + Translator.getText("Tooltip_dull") + ", ";
            }
        }
        ...
        if (this.canBeActivated() && this.isActivated()) {
            prefix = prefix + Translator.getText("Tooltip_activated") + ", ";
        }
    ...
    public Texture getTexture() {
        ...
        Object customIconName = this.getModData().rawget("customInventoryIcon");
        if (!(customIconName instanceof String)) {
            return this.texture;
        }

        Texture customTexture = Texture.getSharedTexture((String)customIconName);
        return customTexture != null ? customTexture : this.texture;
    }
    ...
    public float getActualWeight() {
        return this.getActualWeightUnmodded();
    }
    ...
    public void doBreakSound() {
        ...
        if (player != null) {
            if (!StringUtils.isNullOrWhitespace(this.getBreakSound())) {
                if (GameServer.server) {
                    INetworkPacket.sendToRelative(PacketTypes.PacketType.PlaySound, player.getXi(), player.getYi(), this.getBreakSound(), (byte)0, player);
                    return;
                }
    ...
    public void doDamagedSound() {
        ...
        if (player != null) {
            if (!StringUtils.isNullOrWhitespace(this.getDamagedSound())) {
                if (GameServer.server) {
                    INetworkPacket.sendToRelative(PacketTypes.PacketType.PlaySound, player.getXi(), player.getYi(), this.getDamagedSound(), (byte)0, player);
                    return;
                }
    ...
    public boolean isWaterOnlySource() {
        return this.hasComponent(ComponentType.FluidContainer)
            && this.getFluidContainer().isWaterOnlySource()
            && !this.getFluidContainer().isMultiTileMoveable();
    }
    ...
    public void setCustomName(boolean customName) {
        ...
        this.getModData().rawset("customName", String.valueOf(this.name));
    }
    ...
    public float getContentsWeight() {
        if (this.ammoType != null) {
            Item scriptAmmo = ScriptManager.instance.FindItem(this.ammoType.getItemKey());
            if (scriptAmmo != null) {
    ...
    public float getHotbarEquippedWeight() {
        return this.hasTag(ItemTag.LIGHT_WHEN_ATTACHED)
            ? (this.getActualWeight() + this.getContentsWeight()) * (float)ZomboidGlobals.equippedOrWornEncumbranceMultiplier
    ...
    public String getStringItemType() {
        ...
        if (!item.isItemType(ItemType.WEAPON_PART)
    ...
    public void syncItemFields() {
        ...
        if (GameServer.server && this.getWorldItem() != null) {
            ItemContainer container = new ItemContainer("floor", this.getWorldItem().square, null);
            INetworkPacket.sendToRelative(
                PacketTypes.PacketType.ItemStats, this.getWorldItem().getSquare().x, this.getWorldItem().getSquare().y, container, this
            );
        }
    }
    ...
    public int getWeaponLevel() {
        if (this.isEquipped() && this instanceof HandWeapon weapon) {
            WeaponType var4 = WeaponType.getWeaponType((HandWeapon)this);
            int level = -1;
            if (var4 != null && var4 != WeaponType.UNARMED) {
