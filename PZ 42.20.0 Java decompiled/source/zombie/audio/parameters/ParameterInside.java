...
public final class ParameterInside extends FMODGlobalParameter {
    ...
    public static void calculateFloodFill() {
        if (AmbientStreamManager.instance.isParameterInsideTrue()
            && FMODParameterUtils.getFirstListener() instanceof IsoPlayer player
            && player.getCurrentSquare() instanceof IsoGridSquare square) {
            floodFill.calculate(player.getIndex(), square);
        } else {
            floodFill.reset();
        }
    }
