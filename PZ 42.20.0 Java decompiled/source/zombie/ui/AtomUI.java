...
public class AtomUI implements UIElementInterface {
    ...
    public Boolean onConsumeMouseMove(double dx, double dy, double x, double y) {
        ...
        if (!consumed && lx >= this.leftSide && ly >= this.topSide && lx < this.rightSide && ly < this.downSide && this.luaMouseMove != null) {
            Boolean res = LuaManager.caller
                .protectedCallBoolean(
                    UIManager.getDefaultThread(), this.luaMouseMove, new Object[]{this.table, BoxedStaticValues.toDouble(dx), BoxedStaticValues.toDouble(dy)}
                );
            return res == Boolean.TRUE;
        } else {
            return consumed;
    ...
    public boolean onConsumeKeyPress(int key) {
        ...
        if (this.luaKeyPress != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaKeyPress, this.table, BoxedStaticValues.toDouble(key));
            return res == Boolean.TRUE;
        } else {
            return false;
        }
    ...
    public boolean onConsumeKeyRepeat(int key) {
        ...
        if (this.luaKeyRepeat != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaKeyRepeat, this.table, BoxedStaticValues.toDouble(key));
            return res == Boolean.TRUE;
        } else {
            return false;
        }
    ...
    public boolean onConsumeKeyRelease(int key) {
        ...
        if (this.luaKeyRelease != null) {
            Boolean res = LuaManager.caller.protectedCallBoolean(UIManager.getDefaultThread(), this.luaKeyRelease, this.table, BoxedStaticValues.toDouble(key));
            return res == Boolean.TRUE;
        } else {
            return false;
        }
    ...
    boolean isOverElementLocal(double x, double y) {
        ...
        if (!this.stencil || !(localX < this.leftSide) && !(localY < this.topSide) && !(localX >= this.rightSide) && !(localY >= this.downSide)) {
            for (int i = 0; i < this.nodes.size(); i++) {
                AtomUI ui = this.nodes.get(i);
