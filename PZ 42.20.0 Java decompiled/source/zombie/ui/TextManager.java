...
public final class TextManager {
    ...
    public void DrawStringCentre(AngelCodeFont font, double x, double y, String str, double r, double g, double b, double a) {
        x -= font.getWidth(str) / 2;
        font.drawString((float)x, (float)y, str, (float)r, (float)g, (float)b, (float)a);
    }
    ...
    public void Init() throws FileNotFoundException {
        ...
        for (AngelCodeFont acFont : this.normal) {
            if (acFont != null) {
                acFont.destroy();
            }
        }
    ...
    public boolean isAnyFontLoading() {
        for (AngelCodeFont font : this.enumToFont) {
            if (font != null && font.isLoading()) {
                return true;
            }
        }

        for (AngelCodeFont font : this.normal) {
            if (font != null && font.isLoading()) {
                return true;
            }
        }
