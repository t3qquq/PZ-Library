...
public final class UIManager {
    ...
    private static final IsoObjectPicker.ClickObject pickedCopy = new IsoObjectPicker.ClickObject();
    ...
    public static void render() {
        if (!useUiFbo || Core.getInstance().uiRenderThisFrame) {
            if (!suspend) {
                ...
                try (GameProfiler.ProfileArea playerIndex = profiler.profile("UITransition.UpdateAll")) {
                        ...
                        try {
                            if (element.isDefaultDraw()) {
                                if (GameProfiler.isRunning()) {
                                    try (GameProfiler.ProfileArea fontHeight = profiler.profile("Render " + element)) {
                                        element.render();
                                    }
                                } else {
                                    element.render();
    ...
    public static void update() {
        if (!suspend) {
            ...
            int consumed;
            try (GameProfiler.ProfileArea var34 = profiler.profile("updateMouseButtons")) {
            ...
            try (GameProfiler.ProfileArea var38 = profiler.profile("updateMouseMove")) {
            ...
            try (GameProfiler.ProfileArea var39 = profiler.profile("updateUIElements")) {
                updateUIElements();
            }

            try (GameProfiler.ProfileArea var40 = profiler.profile("updateTooltip")) {
                updateTooltip(mx, my);
            }
