        ...
        if (this.isAlarmRinging) {
            ...
            for (int i = 0; i < IsoPlayer.numPlayers; i++) {
                ...
                if (player != null && !player.isDead()) {
                    ...
                    for (int j = 0; j < player.getInventory().getItems().size(); j++) {
                        InventoryItem inventoryItem = player.getInventory().getItems().get(j);
                        if (inventoryItem instanceof AlarmClock alarmClock && alarmClock.isDigital() && !alarmClock.isAlarmSet()) {
                            alarmClock.setAlarmSet(true);
                            if (this.isAlarmSet) {
                                return true;
                            }
                        ...
                        if (inventoryItem instanceof AlarmClockClothing alarmClockClothing
                            && alarmClockClothing.isDigital()
                            && !alarmClockClothing.isAlarmSet()) {
                            alarmClockClothing.setAlarmSet(true);
                            if (this.isAlarmSet) {
                                return true;
                            }
