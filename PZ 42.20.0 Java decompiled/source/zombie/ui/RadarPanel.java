...
public final class RadarPanel extends UIElement {
    ...
    private void renderZombies() {
        ...
        for (int i = 0; i < size; i++) {
            RadarPanel.ZombiePos zpos = this.zombiePos.get(i);
            this.renderRect(zpos.x - 0.5F, zpos.y - 0.5F, 1.0F, 1.0F, 1.0F, 0.0F, 0.0F, 1.0F);
        }

        for (int i = 0; i < this.zombiePosOccupied.length; i++) {
            if (this.zombiePosOccupied[i]) {
                double radians = Math.toRadians((float)i / this.zombiePosOccupied.length * 360.0F);
                SpriteRenderer.instance
                    .render(
                        null,
