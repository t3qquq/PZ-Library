...
function ISMoveableSpriteProps.new( _sprite )
	...
	if sprite then
		...
		if props then
			...
			s.fitsBeneathCountertop		= props:has(IsoPropertyType.FITS_BENEATH_COUNTERTOP)
    ...
    if self.isMoveable and self.isMultiSprite then
        ...
        for x=0,spriteGrid:getWidth()-1 do
            for y=0,spriteGrid:getHeight()-1 do
                ...
                if spriteForPos then
                    spriteName = spriteForPos:getName();
                end
                ...
                if _getWorldObjects then
                    obj, sprInstance = self:findOnSquare( square, spriteName );
                    if not obj then
                        return false;
                    end
                end
...
function ISMoveableSpriteProps:canPickUpMoveableInternal( _character, _square, _object, _isMulti )
    ...
    if self.isMoveable and instanceof(_square,"IsoGridSquare") then
        ...
        if (not _isMulti or self.isForceSingleItem) and canPickUp then
...
function ISMoveableSpriteProps:saveThumpableParameters(_table, _object)
    ...
    if _object:canBeLockByPadlock() then
        _table.canBeLockedByPadlock = true
    end
    if _object:isLockedByPadlock() and _object:getKeyId() ~= -1 then
        _table.lockedByKeyId = _object:getKeyId()
    end
    if _object:getLockedByCode() > 0 then
        _table.lockedByCode = _object:getLockedByCode()
    end
...
function ISMoveableSpriteProps:restoreThumpableParameters(modData, obj)
    ...
    if modData.canBeLockedByPadlock then
        obj:setCanBeLockByPadlock(true)
    end
    if tonumber(modData.lockedByKeyId) ~= nil then
        obj:setLockedByPadlock(true)
        obj:setKeyId(tonumber(modData.lockedByKeyId))
    end
    if tonumber(modData.lockedByCode) ~= nil then
        obj:setLockedByCode(tonumber(modData.lockedByCode))
    end
end
...
function ISMoveableSpriteProps:canPlaceMoveableInternal( _character, _square, _item, _forceTypeObject )
    ...
    if self.isMoveable then
        ...
        if self.type == "Object" or _forceTypeObject then
            if self.fitsBeneathCountertop and _square:has(IsoPropertyType.COUNTERTOP) and not _square:has(IsoPropertyType.FITS_BENEATH_COUNTERTOP) then
                canPlace = true
                self.yOffsetCursor = 0
            elseif self.isTableTop then
...
function ISMoveableSpriteProps:placeMoveableInternal( _square, _item, _spriteName )
    ...
    elseif self.type=="WindowObject" then
        ...
        local currentSurface = self:getTotalTableHeight(_square);
        if (self.fitsBeneathCountertop and not _square:has(IsoPropertyType.FITS_BENEATH_COUNTERTOP)) or (not self.isTableTop and not self.isStackable) then
            currentSurface = 0
            local countertop = _square:getCountertopObject()
            if countertop ~= nil then
...
--                     print("ISMoveableSpriteProps:placeMoveableInternal 8 ", obj)
                    ...
                    if _item:hasModData() then
                        self:restoreThumpableParameters(_item:getModData(), obj)
                    end
...
function ISMoveableSpriteProps:scrapObjectInternal( _character, _scrapDef, _square, _object, _scrapResult, _chance, _perkName )
    ...
    if scrapDef and object and square then
        ...
        if instanceof(object, "IsoThumpable") and object:hasBuildMaterials() then
            --scrapDef = copyTable(_scrapDef)
            scrapDef.returnItems = {}
            scrapDef.returnItemsStatic = {}
            for type,v in pairs(self.object:getBuildMaterials()) do
